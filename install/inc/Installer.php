...
class Installer
{
    ...
    public function parseProfile(string $directory, string $profile, ?array $options=null): ?array
    {
        ...
        if (!($parser = self::profileParser($path))) {
            return null;
        }
     ...
     */
    public static function validateProfile(string $directory, string $profile, ?string $format=null)
    {
        if (!($path = \caGetProfilePath($directory, $profile))) {
            return null;
        }
        if (!($parser = self::profileParser($path, $format))) {
            return null;
        }
     ...
     * @return array
     */
    public static function getProfileInfo(string $directory, string $profile): ?array
    {
        if (!($path = \caGetProfilePath($directory, $profile))) {
            return null;
        }
     ...
     * @return array
     */
    public static function profileParser(string $profile_path, ?string $format=null): ?\Installer\Parsers\BaseProfileParser
    {
     ...
     *
     */
    protected function addNotice(string $stage, string $notice): void
    {
        $this->logStatus(_t("[Notice:%1] %2", $stage, $notice));
        $this->notices[] = ['stage' => $stage, 'message' => $notice];
    }
    # --------------------------------------------------
    /**
     *
     */
    protected function addWarning(string $stage, string $warning): void
    {
        $this->logStatus(_t("[Warning:%1] %2", $stage, $warning));
        $this->warnings[] = ['stage' => $stage, 'message' => $warning];
    }
    # --------------------------------------------------
    /**
     *
     */
    protected function addError(string $stage, string $error): void
    {
        $this->logStatus(_t("[Error:%1] %2", $stage, $error));
        $this->errors[] = ['stage' => $stage, 'message' => $error];
    }
    # --------------------------------------------------
    /**
     * Returns number of notices that occurred while processing
     *
     * @return int number of notices
     */
    public function numNotices(): int
    {
        return is_array($this->notices) ? sizeof($this->notices) : 0;
    }
    # --------------------------------------------------
    /**
     * Returns array of notice messages
     *
     * @return array errors
     */
    ...
    # --------------------------------------------------
    /**
     * Returns number of warnings that occurred while processing
     *
     * @return int number of warnings
     */
    public function numWarnings(): int
    {
        return is_array($this->warnings) ? sizeof($this->warnings) : 0;
    }
    # --------------------------------------------------
    /**
    ...
    public function performPostInstallTasks()
    {
        ...
        if (sizeof($this->metadata_element_deferred_settings_processing)) {
            foreach ($this->metadata_element_deferred_settings_processing as $element_code => $settings) {
                if (!($t_element = \ca_metadata_elements::getInstance($element_code))) {
                    continue;
                }
                $available_settings = $t_element->getAvailableSettings();
                foreach ($settings as $setting_name => $setting_values) {
                    if (!isset($available_settings[$setting_name])) {
                        continue;
                    }
    ...
    public function processLists($f_callback=null)
    {
        ...
        foreach ($lists as $list) {
            ...
            if ((int)$list['defaultSort'] >= 0) {
                $t_list->set("default_sort", (int)$list['defaultSort']);
            }
            if ($t_list->getPrimaryKey()) {
                $t_list->update();
            } else {
    ...
    public function processUserInterfaces()
    {
        ...
        foreach ($uis as $ui_code => $ui) {
            ...
            foreach ($ui['screens'] as $screen) {
                ...
                if ($color = $screen["color"]) {
                    $t_ui_screens->set('color', $color);
                }
                ...
                if ($screen['roleAccess']) {
                    ...
                    if (sizeof($ui_screen_roles)>0) {
                        ...
                        foreach ($all_roles as $role_id => $role_info) {
                            if (!isset($ui_screen_roles[$role_id])) {
                                $ui_screen_roles[$role_id] = 0;
                            }
                        }
                ...
                foreach ($screen['bundles'] as $placement) {
                    ...
                    if (is_array($bundle_type_restrictions) && sizeof($bundle_type_restrictions)) {
                        $bundle_type_restrictions_types = array_map(function ($v) {
                            return $v['type'];
                        }, $bundle_type_restrictions);
                        $bundle_type_restrictions_include_sub_types = (bool)array_reduce($bundle_type_restrictions, function ($c, $i) {
                            return $c + ((bool)$i['includeSubtypes'] ? 1 : 0);
                        }, 0);
    ...
    public function processDisplays()
    {
        ...
        foreach ($displays as $display) {
            ...
            if ($this->config->get("{$table}_disable")) {
                continue;
            }
            ...
            if ($display['roleAccess']) {
                ...
                if (sizeof($display_roles)>0) {
                    $all_roles = $t_role->getRoleList();
                    foreach ($all_roles as $role_id => $role_info) {
                        if (!isset($display_roles[$role_id])) {
                            $display_roles[$role_id] = 0;
                        }
                    }
        ...
        foreach ($forms as $form) {
            ...
            if ($form['roleAccess']) {
                ...
                if (sizeof($form_roles)>0) {
                    $all_roles = $t_role->getRoleList();
                    foreach ($all_roles as $role_id => $role_info) {
                        if (!isset($form_roles[$role_id])) {
                            $form_roles[$role_id] = 0;
                        }
                    }
                                ...
                                case 'restrict_to_relationship_types':
                                    if ($rel_table = \Datamodel::getLinkingTableName($table, $right_table)) {
                                        if (is_array($ret = caValidateRelationshipTypeList($rel_table, $setting_value))) {
                                            foreach (array_keys(array_filter($ret, function ($v) {
                                                return !$v;
                                            })) as $bad_type) {
                                ...
                                if ($t_authority_instance instanceof \BaseModelWithAttributes && is_string($setting_value)) {
                                    ...
                                    if ($type_id) {
                                        ...
                                        $this->addError(
                                            'processSettings',
                                            _t(
                                                'Failed to lookup type id for type restriction %1 in element %2 as could not retrieve type record. ',
                                                $setting_value,
