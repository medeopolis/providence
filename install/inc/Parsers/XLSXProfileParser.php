...
class XLSXProfileParser extends BaseProfileParser
{
    ...
    public function parse(string $directory, string $profile): array
    {
        ...
        // Parse sections
        if (isset($sheet_map['locales'])) {
            $this->processLocales($sheet_map['locales']);
        }
        if (isset($sheet_map['lists'])) {
            $this->processLists($sheet_map['lists']);
        }
        if (isset($sheet_map['relationshipTypes'])) {
            $this->processRelationshipTypes($sheet_map['relationshipTypes']);
        }
        if (isset($sheet_map['metadataElements'])) {
            $this->processMetadataElementSets($sheet_map['metadataElements']);
        }
        if (isset($sheet_map['uis'])) {
            $this->processUIs($sheet_map['uis']);
        }
        if (isset($sheet_map['logins'])) {
            $this->processLogins($sheet_map['logins']);
        }
        ...
        foreach ($sheet_nums as $ui_spec => $sheet_num) {
            ...
            if ($sheet) {
                ...
                for ($r=3; $r <= $hrow; $r++) {
                    ...
                    if (!($label = trim($sheet->getCellByColumnAndRow(2, $r)->getValue()))) {
                        continue;
                    }
    ...
    protected function processUIBundles(string $table, array $bundles)
    {
        ...
        foreach ($bundles as $bundle) {
            ...
            $settings = [];
            if ($type_res) {
                $settings['restrict_to_types'][] = array_map(function ($v) {
                    return caTextToSnake($v);
                }, preg_split("![;,\n]+!", $type_res));
            }
            if ($relationship && ($rel_table = self::tableNameFromString($relationship)) && $relationship_type) {
                $settings['restrict_to_relationship_types'][] = array_map(function ($v) {
                    return caTextToSnake($v);
                }, preg_split("![;,\n]+!", $relationship_type));
            }
            if ($label) {
    ...
    {
        if (\Datamodel::tableExists($string)) {
            return $string;
        }
        switch(strtolower($string)) {
            case 'entity':
