...
class EADCollectionHierarchyBuilderRefinery extends BaseRefinery
{
     ...
     *
     */
    public static $mapping_options = ['skipGroupIfEmpty'];
    ...
    private function _mapLevels($data, &$l, $level_mappings, $refinery_data)
    {
        if (isset($level_mappings[(string)$data["level"]]) && is_array($mapping = $level_mappings[(string)$data["level"]])) {
                    ...
                    if (sizeof(array_intersect(array_merge(['source', 'delimiter', 'useAsSingleValue', 'skipGroupIfEmpty'], self::$mapping_options), $mapping_keys)) !== sizeof($mapping_keys)) {
                        ...
                        foreach ($t as $sf => $st) {
                            ...
                            if ($r = ltrim($xpath_roots[$index], '/')) {
                                ...
                                $values = [];
                                foreach ($xpaths[$sf] as $i => $x) {
                                    $values[$i] = [];
                                }
                                foreach ($subdata as $si => $sd) {
                                    $svalues = array_map(function ($v) use ($sd) {
                                        $v = ltrim($v, '/');
                                        return $v ? $sd->xpath("{$v}") : '';
                                    }, array_map(function ($x) use ($r) {
                                        $x = str_replace($r, '', $x);
                                        return ($x && ($x !== '/')) ? $x : "./text()";
                                    }, $xpaths[$sf]));
                                ...
                                $values = array_map(function ($v) use ($data) {
                                    $v = ltrim($v, '/');
                                    return $data->xpath("{$v}");
                                }, $xpaths[$sf]);	// get values for each xpath tag
                            }
                            ...
                            if (is_array($xpaths[$sf]) && (sizeof($xpaths[$sf]) > 0)) {
                                foreach ($xpaths[$sf] as $i => $p) {	// replace tags with values
                                    foreach ($values[$i] as $vindex => $v) {
                                        ...
                                        if (!trim($tv) && isset($st['skipGroupIfEmpty']) && $st['skipGroupIfEmpty']) {
                                            unset($mapped_values[$f][$mapping_index + $vindex]);
                                            break;
                                        }
                                        $mapped_values[$f][$mapping_index + $vindex][$sf] = $tv;
                                        if ($tv) {
                                            $values_set[$mapping_index + $vindex] = true;
                                        }
                                    }
                                }
                            } else {
                                for ($z=0; $z<sizeof($xpaths); $z++) {
                                    if (!isset($mapped_values[$f][$mapping_index + $z])|| !is_array($mapped_values[$f][$mapping_index + $z]) || !sizeof($mapped_values[$f][$mapping_index + $z])) {
                                        continue;
                                    }
                        ...
                        foreach ($mapped_values[$f] as $i => $x) {
                            if (!isset($values_set[$i]) || !$values_set[$i]) {
                                print "UNSET $f/$i\n";
                        ...
                        // Remove empty arrays
                        $mapped_values[$f] = array_filter($mapped_values[$f], function ($x) {
                            return (is_array($x) && sizeof($x));
                        });
                    } else {
                        ...
                        $values = array_map(function ($v) use ($data) {
                            $v = ltrim($v, '/');
                            return $data->xpath("{$v}");
                        }, $xpaths); // get values for each xpath tag
            ...
            if (isset($mapping['relationships']) && is_array($mapping['relationships'])) {
                if (!caIsIndexedArray($mapping['relationships'])) {
