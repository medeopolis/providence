...
class recordsByStatusWidget extends BaseWidget implements IWidget
{
    ...
    public static $s_widget_settings = array(	);
        ...
        if ($t_table = Datamodel::getInstanceByTableName($pa_settings['display_type'], true)) {
            ...
            $vs_sql = "
					...
					WHERE
						...
						{$vs_deleted_sql}
				";
            if (!in_array($pa_settings["display_status"], $this->opa_status_values)) {
                return;
            }
            ...
            while ($qr_records->nextRow() && sizeof($va_item_list)<intval($pa_settings["display_limit"])) {
                ...
                if (!($vs_label = $qr_records->get($t_table->getLabelTableName().".".$t_table->getLabelDisplayField()))) {
                    $vs_label = '???';
                }
