...
class prepopulatePlugin extends BaseApplicationPlugin
{
    ...
    public function prepopulateFields(&$t_instance, $pa_options=null)
    {
        ...
        } elseif ($pa_options['excludeRules']) {
        ...
        foreach ($va_rules as $vs_rule_key => $va_rule) {
            ...
            if ($useFor && !is_array($useFor)) {
                $useFor = [$useFor];
            }
            $useFor = is_array($useFor) ? array_map(function ($v) {
                return strtolower($v);
            }, $useFor) : null;
            ...
            if (is_array($useFor) && !is_null($hook) && !in_array($hook, $useFor, true)) {
                ...
                if (($hook === 'edit') && !$default_prepop_on_edit) {
                    continue;
                }
                if (($hook === 'save') && !$default_prepop_on_save) {
                    continue;
                }
            }
            ...
            if (!$vb_is_relationship_rule) {
                ...
                if ((strlen($vs_template) < 1) && (strlen($vs_source = caGetOption('source', $va_rule, null)) < 1)) {
            ...
            if ((sizeof($va_parts) == 1) && $vb_is_relationship_rule) {    // clone relationships
                ...
                if (is_array($va_rels)) {
                    foreach ($va_rels as $va_rel) {
                        if (is_array($va_restrict_to_relationship_types) && sizeof($va_restrict_to_relationship_types) && !in_array($va_rel['relationship_type_code'], $va_restrict_to_relationship_types)) {
                            continue;
                        }
                        if (is_array($va_exclude_relationship_types) && sizeof($va_exclude_relationship_types) && in_array($va_rel['relationship_type_code'], $va_exclude_relationship_types)) {
                            continue;
                        }

                        $va_related_types = caMakeTypeList($vs_target, [$va_rel['item_type_id']]);
                        if (is_array($va_restrict_to_related_types) && sizeof($va_restrict_to_related_types) && sizeof(array_intersect($va_related_types, $va_restrict_to_related_types))) {
                            continue;
                        }
                        if (is_array($va_exclude_related_types) && sizeof($va_exclude_related_types) && !sizeof(array_intersect($va_related_types, $va_exclude_related_types))) {
                            continue;
                        }
                        ...
                        if (is_array($source_values = $t_instance->get($vs_source, ['returnWithStructure' => true]))) {
                            ...
                            foreach ($va_attributes as $attr) {
                                foreach ($attr->getValues() as $v) {
                                    if (is_array($va_source_map) && sizeof($va_source_map) && !array_key_exists($v->getElementCode(), $va_source_map)) {
                                        continue;
                                    }

                                    if (strlen($v->getDisplayValue()) > 0) {
                                        $isset = true;
                                        break(2);
                                    }
                                }
                            }
                            if (($vs_mode == 'addifempty') && $isset) {
                                continue;
                            }
                            if (($vs_mode == 'overwriteifset') && !$isset) {
                                continue;
                            }
            ...
            // "container"
                ...
                // labels
                    ...
                    if (!($t_label = Datamodel::getInstanceByTableName($t_instance->getLabelTableName(), true))) {
                        continue;
                    }
        ...
        if ($t_instance->attributesChanged() || (count($t_instance->getChangedFieldValuesArray()) > 0)) {
            if (isset($_REQUEST['form_timestamp']) && ($_REQUEST['form_timestamp'] > 0)) {
