...
class ExportController extends ActionController
{
    ...
    public function RunExport()
    {
        ...
        if (!is_array($items = $t_set->getItems(['returnRowIdsOnly' => true]))) {
            $items = [];
        }
        if (sizeof($items) == 0) {
        ...
        $headers = array_merge(array_values(array_map(function ($v) {
            if (is_array($v['settings']) && is_array($v['settings']['label']) && sizeof($v['settings']['label'])) {
                if (isset($v['settings']['label'][__CA_DEFAULT_LOCALE__])) {
                    return $v['settings']['label'][__CA_DEFAULT_LOCALE__];
                }
        ...
        $va_links = [];
        if (!is_array($job_info['data']['created'])) {
            $job_info['data']['created'] = [];
        }
        if (!is_array($job_info['data']['updated'])) {
            $job_info['data']['updated'] = [];
        }
