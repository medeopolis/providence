<?php
/* ----------------------------------------------------------------------
 * app/plugins/ResourceSpacePlugin.php :
 * ----------------------------------------------------------------------
 * CollectiveAccess
...
class ResourceSpacePlugin extends BaseApplicationPlugin
{
    ...
    public function __construct($ps_plugin_path)
    {
        ...
        $this->description = _t('Imports records from ResourceSpace');

        parent::__construct();

        $this->opo_config = Configuration::load($ps_plugin_path.'/conf/resourcespace.conf');
    }
    # -------------------------------------------------------
    /**
    ...
    public function hookRenderMenuBar($pa_menu_bar)
    {
        if ($o_req = $this->getRequest()) {
            #if (!$o_req->user->canDoAction('can_import_resourcespace')) { return $pa_menu_bar; }
            ...
            $va_menu_items['ResourceSpace'] = array(
                'displayName' => _t('ResourceSpace'),
                "default" => array(
    ...
    {
        return array(
            'can_import_resourcespace' => array(
                    'label' => _t('Can use ResourceSpace import functions'),
                    'description' => _t('User can import ResourceSpace records and media.')
                )
        );
    }
    # -------------------------------------------------------
