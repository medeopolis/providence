...
class AppNavigation extends BaseObject
{
    ...
    public function &getNavInfo($pn_level=0)
    {
        ...
        while (sizeof($va_path) && ($vn_i < $pn_level)) {
            ...
            if (!$vs_path_element) {
                return $n;
            }							// don't try to return menu if none exists
        ...
        $n = null;
        if ((!is_array($va_nav_info)) || (!sizeof($va_nav_info))) {
            return $n;
        }
    ...
    public function getHTMLMenuBar($ps_css_id)
    {
        ...
        if (((time() - Session::getVar('ca_nav_menubar_cache_lasttime')) < 600) && (intval($this->opo_config->get('do_menu_bar_caching')) > 0) && ($vs_menu_cache = Session::getVar('ca_nav_menubar_cache'))) {
            return $vs_menu_cache;
        }
    ...
    public function getHTMLMenuBarAsLinkArray()
    {
        ...
        if ((intval($this->opo_config->get('do_menu_bar_caching')) > 0) && ($va_menu_cache = Session::getVar('ca_nav_menubar_link_cache'))) {
    ...
    public function getHTMLSideNav($ps_css_id)
    {
        ...
        if (intval($this->opo_config->get('do_menu_bar_caching')) > 0) {
            ...
            if (isset($va_sidebar_cache[$vs_dest])) {
                return $va_sidebar_cache[$vs_dest];
            }
        }
    ...
    private function &_genMenu(&$pa_navinfo, $ps_base_path, $ps_cur_selection)
    {
        ...
        foreach ($pa_navinfo as $vs_nav => $va_nav_info) {
            if (isset($va_nav_info['hide']) && $va_nav_info['hide']) {
                continue;
            }
            ...
            if (!isset($pa_navinfo[$vs_nav]['displayName']) || (!$vs_display_name = $pa_navinfo[$vs_nav]['displayName'])) {
                $vs_display_name = $vs_nav;
            }
            ...
            if (isset($pa_navinfo[$vs_nav]['remember_last_used_navigation']) && $pa_navinfo[$vs_nav]['remember_last_used_navigation']) {
                ...
                $navs = [];
                if ($pa_navinfo[$vs_nav]['altLabel']) {
                    $navs[] = $pa_navinfo[$vs_nav]['altLabel'];
                }
                $navs[] = $vs_nav;
                foreach ($navs as $n) {
                    ...
                    if ($pa_navinfo[$vs_nav]['submenu']['type'] == 'dynamic') {
                        ...
                        if (sizeof($va_submenu_nav)) {
                            ...
                            if (is_array($pa_navinfo[$vs_nav]['requires'])) {
                                $table = array_shift(array_filter(array_values(array_map(function ($v) {
                                    return preg_match("!^action:can_search_([a-z_]+)$!", $v, $m) ? $m[1] : null;
                                }, array_keys($pa_navinfo[$vs_nav]['requires']))), function ($v) {
                                    return $v;
                                }));
    ...
    private function _genSubMenu($pa_submenu_nav, $ps_cur_selection, $pa_additional_params, $ps_base_path, $pa_defaults)
    {
        ...
        foreach ($pa_submenu_nav as $va_submenu_item) {
            if (is_array($va_requirements = $va_submenu_item['requires'])) {
                // DOES THIS USER HAVE PRIVS FOR THIS MENU ITEM?
                if (!$this->_evaluateRequirements($va_requirements, $va_submenu_item['parameters'])) {
                    continue;
                }
            }
            $vs_buf .= "<li class=\"sf-menu\">";
            if (isset($va_submenu_item) && isset($va_submenu_item['default']) && is_array($va_submenu_item['default'])) {
                $pa_defaults = (isset($va_submenu_item['default']) ? $va_submenu_item['default'] : null);
            }
            if (!isset($va_submenu_item['parameters']) || !is_array($va_submenu_item['parameters'])) {
                $va_submenu_item['parameters'] = array();
            }
    ...
    {
        if (!is_array($pa_defaults) || (!sizeof($pa_defaults))) {
            return array();
        }
    ...
    {
        if (!is_array($pa_requirements) || (is_array($pa_requirements) && (sizeof($pa_requirements) == 0))) {
            return true;
    ...
    # Caching
    # -------------------------------------------------------
    public static function clearMenuBarCache($po_request)
    {
