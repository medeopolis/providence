...
class Visualizer
{
    ...
    public static function getAvailableVisualizationsAsHTMLFormElement($ps_table, $ps_name, $pa_attributes=null, $pa_options=null)
    {
        ...
        if (!is_array($va_viz) || !sizeof($va_viz)) {
            return null;
        }
        ...
        foreach ($va_viz as $vs_viz_code => $va_viz_opt) {
            if (is_array($va_valid_viz_codes) && !in_array($vs_viz_code, $va_valid_viz_codes)) {
                continue;
            }
    ...
    {
        if (is_array(Visualizer::$s_plugin_names)) {
        ...
        while (($vs_plugin = readdir($r_dir)) !== false) {
            if ($vs_plugin == "BaseVisualizerPlugin.php") {
                continue;
            }
    ...
    {
        if (preg_match('![^A-Za-z0-9_\-]+!', $ps_plugin_name)) {
            return null;
        }
        if (!file_exists(__CA_LIB_DIR__.'/Plugins/Visualizer/'.$ps_plugin_name.'.php')) {
            return null;
        }
