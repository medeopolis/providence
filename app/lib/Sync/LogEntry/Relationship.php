...
class Relationship extends Base
{
    ...
    public function setIntrinsicsFromSnapshotInModelInstance()
    {
        ...
        foreach ($va_snapshot as $k => $v) {
            if (in_array($k, [$vs_type_field, $vs_left_field, $vs_right_field, $pk], true)) {
                continue;
            }
    ...
    private function setLeftOrRightFieldNameFromSnapshot($ps_field, $pb_left=true)
    {
        ...
        if (isset($va_snapshot[$ps_field . '_guid']) && ($vs_reference_guid = $va_snapshot[$ps_field . '_guid'])) {
            ...
            if (!$t_instance) {
                throw new InvalidLogEntryException('Could not get left or right table instance for relationship log entry');
            }
