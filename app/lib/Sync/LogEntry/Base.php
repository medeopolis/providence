...
abstract class Base
{
    ...
    public function isRelevant()
    {
        ...
        if (!method_exists($this->getModelInstance(), "loadByGUID")) {
            return false;
        }
        ...
        if (isset($this->opa_log['snapshot']) && is_array($this->opa_log['snapshot'])) {
            ...
            if (
                ...
                foreach (['item_id', 'value_longtext1', 'value_longtext2', 'value_blob', 'value_decimal1', 'value_decimal2', 'value_integer1', 'value_source'] as $vs_f) {
                    if (!isset($this->opa_log['snapshot'][$vs_f])) {
                        $this->opa_log['snapshot'][$vs_f] = null;
                    }
                }
            }
    ...
    public function sanityCheck()
    {
        ...
        foreach ($va_snapshot as $vs_field => $vm_val) {
            ...
            // skip primary key
            if ($this->getModelInstance()->primaryKey() == $vs_field) {
                continue;
            }

            // don't try to build hierarchy indexes by hand
            if ($vs_field == $this->getModelInstance()->getProperty('HIERARCHY_LEFT_INDEX_FLD')) {
                continue;
            }
            if ($vs_field == $this->getModelInstance()->getProperty('HIERARCHY_RIGHT_INDEX_FLD')) {
                continue;
            }
        ...
        foreach ($va_snapshot as $vs_field => $vm_val) {
            ...
            if ($va_fld_info = $this->getModelInstance()->getFieldInfo($vs_field)) {
                ...
                if (isset($va_many_to_one_rels[$vs_field]) && ($t_rel_item = \Datamodel::getInstanceByTableName($va_many_to_one_rels[$vs_field]['one_table'], true)) && ($t_rel_item instanceof \BundlableLabelableBaseModelWithAttributes)) {
                    ...
                    if (in_array($vs_field, ['home_location_id'])) {
                        continue;
                    }
    ...
    public function checkModelInstanceForErrors()
    {
        ...
        if ($this->getModelInstance()->numErrors() > 0) { // is this critical or not? hmm
            ...
            throw new InvalidLogEntryException(
                _t(
                    "There were errors processing record from log entry %1: %2",
                    $this->getLogId(),
                    join(' ', $this->getModelInstance()->getErrors())
                )
            );
        }
    }
