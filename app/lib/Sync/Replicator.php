...
class Replicator
{
        ...
        foreach ($this->getSourcesAsServiceClients() as $vs_source_key => $o_source) {
            ...
            foreach ($this->getTargetsAsServiceClients() as $vs_target_key => $o_target) {
                ...
                if ($replicated_log_id > 0) {
                    ...
                    $this->log(_t(
                        "Couldn't get last replicated log id for source %1 and target %2. Starting at the beginning.",
                        $vs_source_key,
                        $vs_target_key
                    ), Zend_Log::WARN);
                    $replicated_log_id = 1;
                ...
                $this->log(_t(
                    "Starting replication for source %1 and target %2, log id is %3.",
                    $vs_source_key,
                    $vs_target_key,
                    $replicated_log_id
                ), Zend_Log::INFO);
                ...
                if (($chunk_size = (int)$this->opo_replication_conf->get('chunk_size')) <= 0) {
                    $chunk_size = 100;
                }
                ...
                while (true) { // use chunks of 10 entries until something happens (success/err)
                    ...
                    if (!is_array($va_source_log_entries) || !sizeof($va_source_log_entries)) {
                        $this->log(_t(
                            "No new log entries found for source %1 and target %2. Skipping this combination now.",
                            $vs_source_key,
                            $vs_target_key
                        ), Zend_Log::INFO);
                        break;
                    ...
                    if (
                        ...
                        foreach ($va_source_log_entries as $vn_log_id => $va_source_log_entry) {
                            ...
                            if ($this->sent_log_ids[$vn_log_id]) {
                                continue;
                            }	// Don't send a source entry more than once (should never happen)
                                    ...
                                    if (!$vb_have_access_to_subject && $vb_subject_exists_on_target) {
                                        ...
                                        while ((sizeof($missing_guids) > 0)) {
                                            ...
                                            if (is_array($va_log_for_missing_guid)) {
                                                ...
                                                if (sizeof($va_filtered_log_for_missing_guid) == 0) {
                                                    continue;
                                                }
                                                ...
                                                if (sizeof($va_dependent_guids) > 0) {
                                                    $tries = 0;
                                                    $va_guids_exist_for_dependencies = null;
                                                    ...
                                                    $va_dependent_guids = array_keys(array_filter($va_guids_exist_for_dependencies, function ($v) {
                                                        return !is_array($v);
                                                    }));
                                                    ...
                                                    if (is_array($pa_filter_on_access_settings)) {
                                                        ...
                                                        if (is_array($va_access_for_missing)) {
                                                            $missing_guids = array_filter(array_keys(array_filter($va_access_for_missing, function ($v) use ($pa_filter_on_access_settings) {
                                                                return (($v == '?') || (in_array((int)$v, $pa_filter_on_access_settings, true)));
                                                            })), 'strlen');
                                                            $this->guids_to_skip = array_filter(array_unique(array_merge($this->guids_to_skip, array_keys(array_filter($va_access_for_missing, function ($v) use ($pa_filter_on_access_settings) {
                                                                return !in_array((int)$v, $pa_filter_on_access_settings, true);
                                                            })))), 'strlen');
                                                        } else {
                                                ...
                                                if (sizeof($va_dependent_guids) == 0) {
                                                    ...
                                                    while (sizeof($va_filtered_log_for_missing_guid) > 0) {
                                                        ...
                                                        while (sizeof($va_filtered_log_for_missing_guid) > 0) {
                                                            ...
                                                            if (!$vn_first_missing_log_id) {
                                                                $vn_first_missing_log_id = $vn_mlog_id;
                                                            }
                                                            ...
                                                            if ((is_array($va_entries) && (sizeof($va_entries) >= 10)) || (is_array($va_source_log_entries_for_missing_guid) && (sizeof($va_source_log_entries_for_missing_guid) == 0))) {
                                                                break;
                                                            }
                                                        }
                    ...
                    if (isset($va_response_data['warnings']) && is_array($va_response_data['warnings']) && sizeof($va_response_data['warnings'])) {
                        foreach ($va_response_data['warnings'] as $vn_log_id => $va_warns) {
                            $this->log(_t(
                                "There were warnings while processing sync for source %1, target %2, log id %3: %4.",
                                $vs_source_key,
                                $vs_target_key,
                                $vn_log_id,
                                join(' ', $va_warns)
    ...
    private function _analyzeDependencies($source_log_entries_for_missing_guids)
    {
        ...
        foreach ($source_log_entries_for_missing_guids as $vs_missing_guid => $va_source_log_entries_for_missing_guid) {
            if (!is_array($va_source_log_entries_for_missing_guid)) {
                continue;
            }
            ...
            foreach ($va_source_log_entries_for_missing_guid as $entry) {
                if (is_array($entry['subjects'])) {
                    foreach ($entry['subjects'] as $dep_subject) {
                        ...
                        if ($dep_subject['subject_table_num'] == 4) {
                            continue;
                        }		// attributes should not be dependencies

                        if ($entry['guid'] === $dep_subject['guid']) {
                            continue;
                        }		// don't make item dependency of itself
                    ...
                    foreach ($entry['snapshot'] as $snk => $snv) {
                        if (preg_match("!([A-Za-z0-9_]+)_guid$!", $snk, $matches)) {
                            if (
                                ...
                                if (!isset($source_log_entries_for_missing_guids[$snv])) {
                                    continue;
                                }
                ...
                while (sizeof($va_source_log_entries_for_missing_guid) > 0) {
                    ...
                    if (($va_log_entry['logged_table_num'] == 3) && ($vs_attr_guid = caGetOption(['attribute_guid', 'attribute_id_guid'], $va_log_entry['snapshot'], null))) {
                        ...
                        if ($va_chk_attr_existence[$vs_attr_guid] == '???') {
                            ...
                            $va_attr_log = is_array($va_attr_log) ? array_filter($va_attr_log, function ($v) {
                                return !$v['SKIP'];
                            }) : null;
                    ...
                    if ((sizeof($va_entries) >= 200) || (sizeof($va_source_log_entries_for_missing_guid) == 0)) {
                        break;
                    }
                }
                ksort($va_entries, SORT_NUMERIC);
