...
class BaseSearchBuilderController extends BaseRefineableSearchController
{
        ...
        switch($pa_options['output_format']) {
            ...
            default:
                ...
                if (isset($pa_options['view']) && $pa_options['view']) {
                    ...
                    $this->render('SearchBuilder/'.$this->ops_tablename.'_search_builder_html.php');
                }
                break;
                # ------------------------------------
        }
    ...
    private function _getSubTypes($pa_subtypes, $pa_types_by_parent_id, $pa_restrict_to_types=null)
    {
        ...
        foreach ($pa_subtypes as $vn_i => $va_type) {
            ...
            if (is_array($limit_to_types) && sizeof($limit_to_types) && !in_array($va_type['idno'], $limit_to_types)) {
                continue;
            }
            if (is_array($exclude_types) && sizeof($exclude_types) && in_array($va_type['idno'], $exclude_types)) {
                continue;
            }
