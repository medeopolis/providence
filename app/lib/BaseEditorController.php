...
class BaseEditorController extends ActionController
{
    ...
    public function Edit($pa_values=null, $pa_options=null)
    {
        ...
        $va_nav = $t_ui->getScreensAsNavConfigFragment(
            $this->request,
            $vn_type_id,
    ...
    public function Save($pa_options=null)
    {
        ...
        if ($vs_idno_context_field = $t_subject->getProperty('ID_NUMBERING_CONTEXT_FIELD')) {
            ...
            if ($vn_context_id && !$t_subject->get($vs_idno_context_field)) {
    ...
    public function Summary($pa_options=null)
    {
        ...
        if (!isset($va_displays[$vn_display_id]) || (is_array($va_displays[$vn_display_id]['settings']['show_only_in']) && sizeof($va_displays[$vn_display_id]['settings']['show_only_in']) && !in_array('editor_summary', $va_displays[$vn_display_id]['settings']['show_only_in']))) {
            $va_tmp = array_filter($va_displays, function ($v) {
                return !isset($v['settings']['show_only_in']) || !is_array($v['settings']['show_only_in']) || in_array('editor_summary', $v['settings']['show_only_in']);
            });
    ...
    public function PrintSummary($pa_options=null)
    {
        ...
        if (!is_array($last_settings = Session::getVar($t_subject->tableName().'_summary_last_settings'))) {
            $last_settings = [];
        }
    ...
    public function _genDynamicNav($pa_params, $pa_options=null)
    {
        ...
        $va_nav = $t_ui->getScreensAsNavConfigFragment(
            $this->request,
            $vn_type_id,
            $pa_params['default']['module'],
            $pa_params['default']['controller'],
            $pa_params['default']['action'],
    ...
    public function _genTypeNav($pa_params)
    {
        ...
        if (is_array($va_hier)) {
            ...
            foreach ($va_hier as $vn_item_id => $va_item) {
                if (is_array($limit_to_types) && sizeof($limit_to_types) && !in_array($va_item['idno'], $limit_to_types)) {
                    continue;
                }
                if (is_array($exclude_types) && sizeof($exclude_types) && in_array($va_item['idno'], $exclude_types)) {
                    continue;
                }
    ...
    private function _getSubTypes($pa_subtypes, $pa_types_by_parent_id, $pn_sort_type, $pa_restrict_to_types=null, $options=null)
    {
        ...
        foreach ($pa_subtypes as $vn_i => $va_type) {
            if (is_array($pa_restrict_to_types) && !in_array($va_type['item_id'], $pa_restrict_to_types)) {
                continue;
            }
        ...
        foreach ($va_subtypes as $vs_sort_key => $va_type) {
            foreach ($va_type as $vn_item_id => $va_item) {
                if (is_array($pa_restrict_to_types) && !in_array($vn_item_id, $pa_restrict_to_types)) {
                    continue;
                }
                if (is_array($limit_to_types) && sizeof($limit_to_types) && !in_array($va_item['parameters']['type_id'], $limit_to_types)) {
                    continue;
                }
                if (is_array($exclude_types) && sizeof($exclude_types) && in_array($va_item['parameters']['type_id'], $limit_to_types)) {
                    continue;
                }
    ...
    public function loadBundleValues()
    {
        ...
        if ($this->request->user->getBundleAccessLevel($t_subject->tableName(), $ps_bundle_name) < __CA_BUNDLE_ACCESS_READONLY__) {
            return false;
        }
    ...
    public function DownloadMedia($pa_options=null)
    {
        ...
        if (!$ps_version) {
            $ps_version = caGetOption('download_version', $di, 'original');
        }
        ...
        foreach ($va_child_ids as $vn_child_id) {
            ...
            switch($t_subject->tableName()) {
                ...
                default:
                    if (!is_a($t_subject, 'RepresentableBaseModel')) {
                        throw new ApplicationException(_t('No media to download for this type of record'));
                    }
            ...
            foreach ($va_reps as $vn_representation_id => $va_rep) {
                ...
                if (isset($va_rep['representation_id']) && ($va_rep['representation_id'] > 0)) {
                    ...
                    if (!($vs_path = caEmbedMediaMetadataIntoFile(
                        $t_rep->getMediaPath('media', $ps_version),
                        $t_subject->tableName(),
                        $t_subject->getPrimaryKey(),
                        $t_subject->getTypeCode(), // subject table info
                        $t_rep->getPrimaryKey(),
                        $t_rep->getTypeCode() // rep info
                    ))) {
    ...
    public function UploadFiles($options=null)
    {
        ...
        if (is_array($_FILES['files'])) {
            ...
            foreach ($_FILES['files']['tmp_name'] as $i => $f) {
                ...
                if (!@copy($f, $dest_path = "{$user_dir}/{$dest_filename}")) {
                    continue;
                }
            ...
            foreach ($_FILES as $k => $info) {
                if (!is_array($info) || !array_key_exists('tmp_name', $info) || !strlen($info['tmp_name'])) {
                    continue;
                }

                $dest_filename = isset($info['name']) ? $info['name'] : pathinfo($info['tmp_name'], PATHINFO_FILENAME);
                if (!@copy($info['tmp_name'], $dest_path = "{$user_dir}/{$dest_filename}")) {
                    continue;
                }
            ...
            if (!$this->request->user->canDoAction("can_edit_{$target}")) {
                ...
                if (is_array($policies) && sizeof($policies)) {
                    ...
                    foreach ($policies as $n => $p) {
                        if (!isset($p['elements']) || !isset($p['elements']['ca_storage_locations'])) {
                            continue;
                        }
                        ...
                        if ($is_fk) {
                            ...
                            if (!($qr_res = $primary->getContents($n))) {
                                continue;
                            }
                        }
                        while ($qr_res->nextHit()) {
                            ...
                            if (!($location_id = $t_instance->get('home_location_id'))) {
                                continue;
                            }
                            ...
                            if (!$is_fk && (!is_array($d) || !isset($d['trackingRelationshipType']))) {
                                $errors[] = $t_id;
                                continue;
                            }
    ...
    public function Sort()
    {
        ...
        if (!is_array($va_sort_keys) || !sizeof($va_sort_keys)) {
            return;
        }
