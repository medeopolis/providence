...
class BaseMediaViewer
{
     ...
     *
     */
    public static function prepareViewerHTML($request, $o_view, $data=null, $options=null)
    {
        ...
        if ($t_subject && $t_instance && ($request->user->canDoAction('can_download_media') || $request->user->canDoAction('can_download_ca_object_representations'))) {
            if (is_array($versions = $request->config->getList('ca_object_representation_download_versions'))) {
                ...
                if (is_a($t_instance, 'ca_object_representations')) {
                    $controls .= caHTMLHiddenInput("representation_id", array('value' => $t_instance->getPrimaryKey()));
                }
                if (is_a($t_instance, 'ca_site_page_media')) {
                    $controls .= caHTMLHiddenInput("media_id", array('value' => $t_instance->getPrimaryKey()));
                }
                if (is_a($t_instance, 'ca_attribute_values')) {
                    $controls .= caHTMLHiddenInput("value_id", array('value' => $t_instance->getPrimaryKey()));
                }
