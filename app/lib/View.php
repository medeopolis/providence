...
class View extends BaseObject
{
    ...
    public function setViewPath($pm_path, $pa_options=null)
    {
        ...
        foreach ($pm_path as $ps_path) {
            ...
            if ($vs_suffix = join("/", array_reverse($va_suffix_bits))) {
                $vs_suffix = '/'.$vs_suffix;
                break;
            }
        }
    ...
    public function isCompiled($ps_filepath)
    {
        ...
        if (!file_exists($vs_compiled_path)) {
            return false;
        }
        if (filesize($vs_compiled_path) === 0) {
            return false;
        }
        ...
        if ($va_view_stat['mtime'] > $va_compiled_stat['mtime']) {
            return false;
        }
    ...
    public function compile($ps_filepath, $pb_force_recompile=false, $pa_options=null)
    {
        ...
        foreach ($va_matches[1] as $vn_i => $vs_potential_tag) {
            if (strpos($vs_potential_tag, "<?php") !== false) {
                unset($va_matches[1][$vn_i]);
            }
        }
    ...
    {
        if ($this->ops_last_render) {
            return $this->ops_last_render;
        }
