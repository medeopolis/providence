...
class ResultContext
{
    ...
    public function getSearchExpression($pb_from_context_only=false)
    {
        if (!$pb_from_context_only && ($ps_search = urldecode((strip_tags(html_entity_decode($this->opo_request->getParameter('search', pString, ['forcePurify' => true])))))) != '') {
            ...
            if ($ps_search != $this->getContextValue('expression')) {
                ...
                if (is_null($this->opb_search_expression_has_changed)) {
    ...
    {
        if (!is_null($this->opb_search_expression_has_changed)) {
            return $this->opb_search_expression_has_changed;
        }
    ...
    public function getSearchExpressionForDisplay($ps_search_expression=null)
    {
        ...
        if ($ps_search_expression && isset($va_expressions_for_display[$ps_search_expression])) {
            return $va_expressions_for_display[$ps_search_expression];
        }	// return display expression for specified search expression if defined
        if ($ps_search_expression) {
            return $ps_search_expression;
        }				// return specified search expression if passed and no display expression is available
    ...
    {
        if (!is_null($pb_is_new_search)) {
            $this->opb_is_new_search = $pb_is_new_search;
        }
                    ...
                    if ($ps_show_in) {
                        foreach ($displays as $id => $d) {
                            ...
                            if (($show_in_setting = caGetOption('show_only_in', $d['settings'], null, ['castTo' => 'array'])) && (sizeof($show_in_setting) > 0)) {
                                if (sizeof(array_filter($show_in_setting, function ($v) use ($ps_show_in) {
                                    return preg_match("!{$ps_show_in}!", $v);
                                })) > 0) {
                                    $pn_display_id = $id;
                                    break;
                                }
    ...
    public static function getLastFind($po_request, $pm_table_name_or_num, $pa_options=null)
    {
        ...
        if (!ResultContextStorage::$storageLoaded) {
            ResultContextStorage::init($po_request);
        }
    ...
    public static function getResultsUrl($request, $table_name_or_num, $find_type, $options=null)
    {
        ...
        if (is_null($nav = caGetOption($find_type, $find_nav, null))) {
            return null;
        }
    ...
    public function saveContext($ps_find_type=null, $pa_context=null, $ps_find_subtype=null)
    {
        ...
        if (!is_array($va_used_find_types = ResultContextStorage::getVar('used_find_types'))) {
            $va_used_find_types = array();
        }
    ...
    {
        if (!is_array($va_findtypes = ResultContextStorage::getVar('used_find_types'))) {
            $va_findtypes = array();
        }
...
class ResultContextStorage
{
     ...
     *
     */
    public static function init($po_request)
    {
     ...
     *
     */
    public static function setVar($key, $value, $options=null)
    {
