...
if (!defined('__CA_BUNDLE_ACCESS_NONE__')) {
    define('__CA_BUNDLE_ACCESS_NONE__', 0);
}
if (!defined('__CA_BUNDLE_ACCESS_READONLY__')) {
    define('__CA_BUNDLE_ACCESS_READONLY__', 1);
}
if (!defined('__CA_BUNDLE_ACCESS_READONLY__')) {
    define('__CA_BUNDLE_ACCESS_READONLY__', 2);
}
...
class BundlableLabelableBaseModelWithAttributes extends LabelableBaseModelWithAttributes implements IBundleProvider
{
     ...
     * ca_locales model
     */
    public static $s_locales = null;

    /**
     * TimeExpressionParser
     */
    public static $s_tep = null;
    ...
    public function insert($pa_options=null)
    {
        if (!is_a($this, "BaseRelationshipModel")) {
            ...
            if (!$this->_validateIncomingAdminIDNo(true, !caGetOption('validateAllIdnos', $pa_options, false))) {
                $vb_error =  true;
            }
    ...
    public function duplicate($pa_options=null)
    {
        ...
        foreach ($va_field_list as $vn_i => $vs_field) {
            if (in_array($vs_field, array($vs_idno_fld, $vs_idno_sort_fld, $vs_pk))) {
                continue;
            }		// skip idno fields
        ...
        if ($vs_idno_fld && (!isset($va_duplicate_element_settings[$vs_idno_fld]) || (isset($va_duplicate_element_settings[$vs_idno_fld]) && ($va_duplicate_element_settings[$vs_idno_fld] == 1)))) {
            ...
            if (method_exists($this, "getIDNoPlugInInstance") && ($o_numbering_plugin = $this->getIDNoPlugInInstance())) {
                ...
                if (!preg_match("!%$!", $vs_idno_template)) {
                    $vb_needs_suffix_generated = true;
                }

                if (!is_array($va_idno_values = $o_numbering_plugin->htmlFormValuesAsArray($vs_idno_fld, $vs_idno_template, false, false, false))) {
                    $va_idno_values = [];
                }
            ...
            if ($vs_idno == $this->get($vs_idno_fld)) {
                $vs_idno .= " ["._t('DUP')."]";
            }
            $t_dupe->set($vs_idno_fld, $vs_idno);
        }

        if (isset($pa_options['user_id']) && $pa_options['user_id'] && $t_dupe->hasField('user_id')) {
            $t_dupe->set('user_id', $pa_options['user_id']);
        }
        ...
        foreach ($va_labels as $vn_label_id => $va_labels_by_locale) {
            foreach ($va_labels_by_locale as $vn_locale_id => $va_label_list) {
                foreach ($va_label_list as $vn_i => $va_label_info) {
                    ...
                    if (!$vb_duplicate_nonpreferred_labels && key_exists('is_preferred', $va_label_info) && !$va_label_info['is_preferred']) {
                        continue;
                    }
                    if (!$this->getAppConfig()->get('dont_mark_duplicated_records_in_preferred_label')) {
                        $va_label_info[$vs_label_display_field] .= " ["._t('Duplicate')."]";
                    }
                    $t_dupe->addLabel(
                        $va_label_info,
                        $va_label_info['locale_id'],
                        $va_label_info['type_id'],
                        isset($va_label_info['is_preferred']) ? (bool)$va_label_info['is_preferred'] : false
                    );
                    if ($t_dupe->numErrors()) {
                        $this->errors = $t_dupe->errors;
                        if ($vb_we_set_transaction) {
        ...
        foreach (array(
            ...
            if (!in_array($vs_rel_table, $va_duplicate_relationships)) {
                continue;
            }
            ...
            if ($vb_duplicate_current_relationships_only) {
                if (is_array($history_tracking_policies) && sizeof($history_tracking_policies)) {
                    ...
                    foreach ($history_tracking_policies as $policy) {
                        if (!$table::historyTrackingPolicyUses($policy, $vs_rel_table)) {
                            continue;
                        }
                        if (!is_array($h = $this->getHistory(['currentOnly' => true, 'limit' => 1, 'policy' => $policy])) || !sizeof($h)) {
                            continue;
                        }
    ...
    private function _validateIncomingAdminIDNo($pb_post_errors=true, $pb_dont_validate_idnos_for_parents_in_mono_hierarchies=false)
    {
        ...
        if ($pb_dont_validate_idnos_for_parents_in_mono_hierarchies && in_array($vn_hier_type, array(__CA_HIER_TYPE_SIMPLE_MONO__, __CA_HIER_TYPE_MULTI_MONO__)) && ($this->get('parent_id') == null)) {
            return true;
        }
    ...
    public function checkForDupeAdminIdnos($ps_idno=null, $pb_dont_remove_self=false)
    {
        if ($vs_idno_field = $this->getProperty('ID_NUMBERING_ID_FIELD')) {
            ...
            if (!$ps_idno) {
                $ps_idno = $this->get($vs_idno_field);
            }
            ...
            if ($o_idno = $this->getIDNoPlugInInstance()) {
                if (is_array($elements = $o_idno->getElements($this->tableName(), ($t = $this->getTypeName()) ? $t : '__default__'))) {
                    $seq_by_type = array_filter($elements, function ($v) {
                        return isset($v['sequence_by_type']) && (bool)$v['sequence_by_type'];
                    });
            ...
            if ($o_idno && ($additional_tables = $o_idno->getFormatProperty('compare_against_values_in'))) {
                ...
                if (is_array($additional_tables)) {
                    foreach ($additional_tables as $additional_table) {
                        if ($additional_table === $this->tableName()) {
                            continue;
                        }
                        if (!($t = Datamodel::getInstance($additional_table, true))) {
                            continue;
                        }
                        if (!($idno_field = $t->getProperty('ID_NUMBERING_ID_FIELD'))) {
                            continue;
                        }
        ...
        if (($vs_idno_field = $this->getProperty('ID_NUMBERING_ID_FIELD')) && ($vs_idno_sort_field = $this->getProperty('ID_NUMBERING_SORT_FIELD'))) {
            if (($o_idno = $this->getIDNoPlugInInstance()) && (method_exists($o_idno, 'getSortableValue'))) {	// try to use plug-in's sort key generator if defined
                ...
                if ($this->hasField("{$vs_idno_sort_field}_num") && (method_exists($o_idno, 'getSortableNumericValue'))) {
                    if (($n = $o_idno->getSortableNumericValue($this->get($vs_idno_field))) < 0) {
                        return;
                    }
    ...
    public function checkForDupeLabel($pn_locale_id, $pa_label_values, $pb_preferred_only=true)
    {
        ...
        if (!empty($pa_label_values['surname']) || !empty($pa_label_values['forename'])) {
        ...
        $va_values = array_map(function ($v) {
            return is_null($v) ? '' : $v;
        }, array_values($pa_label_values));
     ...
     */
    public function isReadable($po_request, $ps_bundle_name=null)
    {
        ...
        if ($ps_bundle_name) {
            if ($t_user->getBundleAccessLevel($this->tableName(), $ps_bundle_name) < __CA_BUNDLE_ACCESS_READONLY__) {
                return false;
            }
        }
        ...
        if ((defined("__CA_APP_TYPE__") && (__CA_APP_TYPE__ == "PAWTUCKET") && ($this->hasField('access')))) {
            ...
            if (is_array($va_access) && sizeof($va_access) && !in_array($this->get('access'), $va_access)) {
                return false;
            }
        }
    ...
    public function isSaveable($po_request, $ps_bundle_name=null)
    {
        ...
        if ($ps_bundle_name) {
            if ($t_user->getBundleAccessLevel($this->tableName(), $ps_bundle_name) < __CA_BUNDLE_ACCESS_EDIT__) {
                return false;
            }
        }
    ...
    public function isDeletable($po_request)
    {
        ...
        if ($config->get("{$table}_disable_delete")) {
            return false;
        }
        if ($config->get("{$table}_".$this->getTypeCode()."_disable_delete")) {
            return false;
        }
    ...
    public function getBundleFormHTML($ps_bundle_name, $ps_placement_code, $pa_bundle_settings, $pa_options=null, &$ps_bundle_label=null)
    {
        ...
        switch($vs_type) {
            ...
            case 'intrinsic':
                ...
                if ($this->getFieldInfo($ps_bundle_name, 'IDENTITY')) {
                    ...
                    $o_view->setVar('form_element', $this->htmlFormElement(
                        $ps_bundle_name,
                        ($this->getProperty('ID_NUMBERING_ID_FIELD') == $ps_bundle_name) ? $o_config->get('idno_element_display_format_without_label') : $o_config->get('bundle_element_display_format_without_label'),
                        array_merge(
                            [
                    ...
                    if ($custom_view_exists) {
                        $o_view->setVar('form_element_raw', $this->htmlFormElement(
                            $ps_bundle_name,
                            '^ELEMENT',
                            array_merge(
                                array(
            ...
            case 'related_table':
                ...
                switch($ps_bundle_name) {
                    ...
                    case 'ca_tour_stops':
                    case 'ca_sets':
                        if (($this->_CONFIG->get($ps_bundle_name.'_disable')) && ($ps_bundle_name !== 'ca_object_representations')) {
                            return '';
                        }		// don't display if master "disable" switch is set
                        $pa_options['start'] = 0;
                        $pa_options['limit'] = caGetOption('numPerPage', $pa_bundle_settings, 10);
            ...
            case 'special':
                ...
                switch($ps_bundle_name) {
                    ...
                    case 'ca_object_representation_captions':
                        ...
                        if (!$this->representationIsOfClass("video") && !$this->representationIsOfClass("audio")) {
                            return '';
                        }
                    ...
                    case 'ca_metadata_dictionary_rules':
                        ...
                        if (!($this instanceof ca_metadata_dictionary_entries)) {
                            return null;
                        }
        ...
        if (is_array($violations_to_prompt) && sizeof($violations_to_prompt)) {
            ...
            $violations_text = array_map(function ($v) {
                return caExtractSettingsValueByUserLocale('violationMessage', $v);
            }, $violations_to_prompt);
    ...
    {
        if (isset($this->BUNDLES[$ps_bundle_name])) {
            return $this->BUNDLES[$ps_bundle_name];
        }
        if (isset($this->BUNDLES["ca_attribute_{$ps_bundle_name}"])) {
            return $this->BUNDLES["ca_attribute_{$ps_bundle_name}"];
        }
    ...
    public function getDisplayLabel($ps_field, $options=null)
    {
        ...
        switch(sizeof($va_tmp)) {
            ...
            case 3:		// table_name.field_name.sub_element
                ...
                $vs_prefix = $vs_suffix = '';
                if (caGetOption('includeSourceSuffix', $options, true)) {
    ...
    public function htmlFormElementForSearch($po_request, $ps_field, $pa_options=null)
    {
        ...
        switch($va_tmp[0]) {
            case '_fulltext':
                ...
                if (!isset($pa_options['id'])) {
                    $pa_options['id'] = str_replace('.', '_', $ps_field);
                }
        ...
        if ($vs_rel_types = join(";", caGetOption('restrictToRelationshipTypes', $pa_options, []))) {
    ...
    public function htmlFormElementForSimpleForm($po_request, $ps_field, $pa_options=null)
    {
        ...
        if (!isset($pa_options['values'][$ps_field])) {
            $pa_options['values'][$ps_field] = null;
        }
    ...
    public function getBundleFormHTMLForScreen($pm_screen, $pa_options, &$pa_placements=null)
    {
        ...
        if (is_array($va_bundles)) {
            ...
            foreach ($va_bundles as $va_bundle) {
                ...
                if (!is_array($va_bundle['settings'])) {
                    $va_bundle['settings'] = [];
                }
    ...
    private function _getHierarchyLocationHTMLFormBundleInfo($po_request, $ps_form_name, $ps_placement_code, $pa_options=null, $pa_bundle_settings=null)
    {
        ...
        $o_view->setVar('objectTypeList', trim($t_object->getTypeListAsHTMLFormElement(
            "{$ps_placement_code}{$ps_form_name}object_type_id",
...
//
    ...
    public function getRelatedHTMLFormBundle($po_request, $ps_form_name, $ps_related_table, $ps_placement_code=null, $pa_bundle_settings=null, $pa_options=null)
    {
        ...
        if (!is_array($pa_bundle_settings['prepopulateQuickaddFields'])) {
            $pa_bundle_settings['prepopulateQuickaddFields'] = [];
        }
        ...
        if ($pa_bundle_settings['disableQuickadd']) {
            $vb_quickadd_enabled = false;
        }
    ...
    protected function getBundleListsForScreen($pm_screen, $po_request, $t_ui, $pa_options=null)
    {
        ...
        if (is_array($va_bundles)) {
            foreach ($va_bundles as $vn_i => $va_tmp) {
                if (isset($va_tmp['settings']['readonly']) && (bool)$va_tmp['settings']['readonly']) {
                    continue;
                }			// don't attempt to save "read-only" bundles
        ...
        if (is_array($va_bundle_lists['fields_by_type']['attribute'])) {
            foreach ($va_bundle_lists['fields_by_type']['attribute'] as $vs_placement_code => $vs_f) {
                ...
                foreach ($_REQUEST as $vs_key => $vs_val) {
                    ...
                    if (!($vn_element_id = ca_metadata_elements::getElementID($vs_element_set_code))) {
                        continue;
                    }
    ...
    public function saveBundlesForScreenWillChangeParent($pm_screen, $po_request, &$pa_options)
    {
        ...
        foreach ($_REQUEST as $vs_k => $vs_v) {
            if (!preg_match("!{$vs_form_prefix}_new_parent_id$!", $vs_k)) {
                continue;
            }
    ...
    */
    public function saveBundlesForScreen($pm_screen, $po_request, &$pa_options)
    {
                    ...
                    if (is_array($va_attrs = $this->getAttributesByElement($vn_element_id))) {
                        ...
                        //
                            ...
                            foreach ($va_sub_elements as $vn_i => $va_element_info) {
                                ...
                                if (is_null($po_request->parameterExists($vs_k)) && !isset($_FILES[$vs_k])) {
                                    continue;
                                }
        ...
        if (is_array($va_fields_by_type['special'])) {
            foreach ($va_fields_by_type['special'] as $vs_placement_code => $vs_bundle) {
                ...
                if ($vb_batch && ($po_request->getParameter($vs_placement_code.$vs_form_prefix.'_batch_mode', pString) !== '_replace_')) {
                    continue;
                }
                    ...
                    if ($parent_table == $target_table) {
                        ...
                        if (sizeof($multiple_move_selection) > 0) {
                            if ($qr = $table::findAsSearchResult([$this->primaryKey() => ['IN', $multiple_move_selection]])) {
                                if ($qr->getPrimaryKey() ==  $this->getPrimaryKey()) {
                                    continue;
                                }
                                while ($qr->nextHit()) {
                                    $t = $qr->getInstance();
                                    if (!$t->isSaveable($po_request)) {
                                        continue;
                                    }
                        ...
                        if (sizeof($multiple_move_selection) > 0) {
                            $object_selection = array_map(
                                function ($v) {
                                    return (int)substr($v, 11);
                                },
                                array_filter($multiple_move_selection, function ($v) {
                                    return preg_match("!^ca_objects\-!", $v);
                                })
                            );
                                    ...
                                    if ($vn_label_locale_id = $po_request->getParameter($vs_placement_code.$vs_form_prefix.'_Pref'.'locale_id_'.$va_label['label_id'], pString)) {
                                        ...
                                        if (is_array($va_label_values = $this->getLabelUIValuesFromRequest($po_request, $vs_placement_code.$vs_form_prefix, $va_label['label_id'], true))) {
                                            if ($vb_check_for_dupe_labels && $this->checkForDupeLabel($vn_label_locale_id, $va_label_values)) {
                                                $this->postError(1125, _t('Value <em>%1</em> is already used and duplicates are not allowed', join('', array_map(function ($v) {
                                                    return "<code>{$v}</code>";
                                                }, array_filter($va_label_values, function ($v) {
                                                    return (bool)strlen($v);
                                                })))), "BundlableLabelableBaseModelWithAttributes->saveBundlesForScreen()", $this->tableName().'.preferred_labels');
                ...
                switch($vs_f) {
                    ...
                    case 'ca_object_representations':
                        ...
                        if (!$vb_batch && is_array($va_reps)) {
                            ...
                            if (!in_array('is_primary', $bundles_to_save)) {
                                $bundles_to_save[] = 'is_primary';
                            }
                            if (!in_array('rep_type_id', $bundles_to_save)) {
                                $bundles_to_save[] = 'rep_type_id';
                            }

                            if (in_array('ca_object_representations.preferred_labels.name', $bundles_to_save)) {
                                $bundles_to_save[] = 'rep_label';
                            }
                            $bundles_on_screen_proc = array_map(function ($v) {
                                return array_pop(explode('.', $v));
                            }, $bundles_to_save);
                        ...
                        if ($vb_batch) {
                            ...
                            $file_index = 0;
                            $va_file_list = array_map(function ($v) {
                                $v['is_form_upload'] = 1;
                                return $v;
                            }, $_FILES);
                            foreach ($_REQUEST as $vs_key => $vs_value) {
                                if (preg_match('/^'.$vs_prefix_stub.'media_url_new_([\d]+)$/', $vs_key, $va_matches)) {
                                    if (!isURL($vs_value, ['strict' => true, 'schemes' => ['http', 'https']])) {
                                        continue;
                                    }
                            ...
                            if (isset($va_file_list['empty'])) {
                                ...
                                $va_file_list = array_filter($va_file_list, function ($v) use ($index) {
                                    return (($v['index'] !== $index) || (sizeof($v) === 1));
                                });
                            }
            ...
            foreach ($va_fields_by_type['special'] as $vs_placement_code => $vs_f) {
                ...
                switch($vs_f) {
                            ...
                            foreach ($refs as $t => $r) {
                                if (is_array($rp = $po_request->getParameter("{$vs_placement_code}{$vs_form_prefix}_delete_{$t}", pArray))) {
                                    if (is_array($rows_to_delete = array_intersect($r, $rp))) {	// only attempt to delete values passed that are actually in the history
                                        if (!($t_instance = Datamodel::getInstance($t, true))) {
                                            continue;
                                        }
                                        if ($this->inTransaction()) {
                            ...
                            if ($vn_location_id) {
                                ...
                                if ($t_loc->load($vn_location_id)) {
                                    ...
                                    if ($vn_relationship_type_id) {
                                        ...
                                        if ($t_item_rel) {
                                            SearchResult::clearResultCacheForRow($t_item_rel->tableName(), $t_item_rel->getPrimaryKey());
                                        }
                                    }
                                }
                            }
                    ...
                    case 'ca_metadata_alert_triggers':
                        ...
                        if (!$po_request->user->canDoAction('can_use_metadata_alerts')) {
                            break;
                        }
                    ...
                    case 'ca_metadata_dictionary_rules':
                        ...
                        if (!$po_request->user->canDoAction('can_configure_data_dictionary')) {
                            break;
                        }
                    ...
                    case 'ca_site_pages_content':
                        if (is_array($va_field_list = $this->getHTMLFormElements())) {
                            if (!is_array($va_content = $this->get('content'))) {
                                $va_content = [];
                            }
                    ...
                    case 'ca_item_tags':
                        ...
                        if (!$vb_batch && is_array($ids_sorted = $va_rel_sort_order = explode(';', $po_request->getParameter("{$vs_placement_code}{$vs_form_prefix}BundleList", pString)))) {
                            $tags = $this->getTags();
                            $tag_ids = array_map(function ($v) {
                                return $v['relation_id'];
                            }, $tags);
                            $current_tag_ranks = array_map(function ($v) {
                                return $v['rank'];
                            }, $tags);
    ...
    private function _processRelated($po_request, $ps_bundle_name, $ps_form_prefix, $ps_placement_code, $pa_options=null)
    {
        ...
        if ($bundle_type_restrictions = caGetOption('bundleTypeRestrictions', $pa_settings, null)) {
            if (!is_array($bundle_type_restrictions)) {
                $bundle_type_restrictions = [$bundle_type_restrictions];
            }
            if (!in_array($this->getTypeID(), $bundle_type_restrictions)) {
                return;
            }
        }
        ...
        if ($vn_max_relationships == 0) {
            $vn_max_relationships = 65535;
        }
    ...
    public function getRelatedItems($pm_rel_table_name_or_num, $options=null, &$pn_count=null)
    {
        ...
        if (!isset($options['dontIncludeSubtypesInTypeRestriction']) && (isset($options['dont_include_subtypes_in_type_restriction']) && $options['dont_include_subtypes_in_type_restriction'])) {
            $options['dontIncludeSubtypesInTypeRestriction'] = $options['dont_include_subtypes_in_type_restriction'];
        }
        if (!isset($options['returnNonPreferredLabels']) && (isset($options['restrict_to_type']) && $options['restrict_to_type'])) {
            $options['returnNonPreferredLabels'] = $options['restrict_to_type'];
        }
        if (!isset($options['returnLabelsAsArray']) && (isset($options['return_labels_as_array']) && $options['return_labels_as_array'])) {
            $options['returnLabelsAsArray'] = $options['return_labels_as_array'];
        }
        if (!isset($options['restrictToLists']) && (isset($options['restrict_to_lists']) && $options['restrict_to_lists'])) {
            $options['restrictToLists'] = $options['restrict_to_lists'];
        }
        ...
        if (!isset($options['useLocaleCodes']) && (isset($options['returnLocaleCodes']) && $options['returnLocaleCodes'])) {
            $options['useLocaleCodes'] = $options['returnLocaleCodes'];
        }
        ...
        if (is_numeric($pm_rel_table_name_or_num)) {
            if (!($vs_related_table_name = Datamodel::getTableName($pm_rel_table_name_or_num))) {
                return null;
            }
        } else {
            ...
            if (!($o_instance = Datamodel::getInstanceByTableName($pm_rel_table_name_or_num, true))) {
                return null;
            }
        ...
        switch(sizeof($va_path = array_keys(Datamodel::getPath($vs_subject_table_name, $vs_related_table_name)))) {
            ...
            case 2:
                ...
                if (!($t_rel_item = Datamodel::getInstance($va_path[1], true))) {
                    return null;
                }
                    ...
                    if ($pb_group_fields) {
                        ...
                        if ($t_rel_item_label) {
                            foreach ($t_rel_item_label->getFormFields() as $vs_field => $va_field_info) {
                                if (!isset($va_rels[$vs_sort_key][$vn_id][$vs_field]) || ($vs_field == $vs_rel_pk)) {
                                    continue;
                                }
        ...
        // END - traverse self relation
            //
        } elseif (method_exists($this, 'isSelfRelationship') && $this->isSelfRelationship()) {
            //
        ...
        // END - from self relation itself
            ...
            while ($qr_res->nextRow()) {
                ...
                if ($pb_group_fields) {
                    ...
                    if ($t_rel_item_label) {
                        foreach ($t_rel_item_label->getFormFields() as $vs_field => $va_field_info) {
                            if (!isset($va_rels_for_id[$vs_v][$vs_field]) || ($vs_field == $vs_rel_pk)) {
                                continue;
                            }
        ...
        if (
            ...
            if (($this->tableName() == 'ca_objects') && $this->getAppConfig()->get('ca_objects_x_collections_hierarchy_enabled') && !$this->getAppConfig()->get('ca_objects_x_collections_hierarchy_disable_object_collection_idno_inheritance') && $pa_options['request'] && ($vn_collection_id = $pa_options['request']->getParameter('collection_id', pInteger))) {
                ...
                if ($t_coll->getPrimaryKey()) {
                    ...
                    if (!$this->opo_idno_plugin_instance->formatHas('PARENT') && !$this->opo_idno_plugin_instance->getFormatProperty('dont_inherit_from_parent_collection')) {
    ...
    public function makeSearchResult($pm_rel_table_name_or_num, $pa_ids, $pa_options=null)
    {
        ...
        if (!isset($pa_options['instance']) || !($t_instance = $pa_options['instance'])) {
            if (!($t_instance = Datamodel::getInstance($pm_rel_table_name_or_num, true))) {
                return null;
            }
        }
        $va_ids = [];
        ...
        if (!($vs_search_result_class = $t_instance->getProperty('SEARCH_RESULT_CLASSNAME'))) {
            return null;
        }
     ...
     * @return SearchResult A search result of for the specified table
     */
    public static function createResultSet($pa_ids)
    {
        if (!is_array($pa_ids) || !sizeof($pa_ids)) {
            return null;
        }
        $pn_table_num = Datamodel::getTableNum(get_called_class());
        if (!($t_instance = Datamodel::getInstanceByTableNum($pn_table_num))) {
            return null;
        }
    ...
    public function setACLUsers($pa_user_ids)
    {
        ...
        if (!$this->addACLUsers($pa_user_ids)) {
            return false;
        }
    ...
    public function removeACLUsers($pa_user_ids)
    {
        ...
        foreach ($pa_user_ids as $vn_user_id) {
            if (!isset($va_current_users[$vn_user_id]) && $va_current_users[$vn_user_id]) {
                continue;
            }
    ...
    *
    ...
    public function setACLUserGroups($pa_group_ids, $pa_options=null)
    {
        if (is_array($va_groups = $this->getACLUserGroups())) {
            ...
            if (!$this->addACLUserGroups($pa_group_ids, $pa_options)) {
                return false;
            }
    ...
    public function supportsACL()
    {
        ...
        if ($this->getAppConfig()->get($this->tableName().'_dont_do_item_level_access_control')) {
            return false;
        }
...
side. For many self-relations the direction determines the nature and display text for the relationship.
    ...
    public function addRelationship($pm_rel_table_name_or_num, $pn_rel_id, $pm_type_id=null, $ps_effective_date=null, $ps_source_info=null, $ps_direction=null, $pn_rank=null, $pa_options=null)
    {
        ...
        if (is_null($ps_effective_date) && is_array($pa_options) && is_array($pa_options['interstitialValues'])) {
            ...
            if (is_array($ps_effective_date)) {
                $ps_effective_date = $ps_effective_date['effective_date'] ?? null;
            }
            ...
            foreach ($pa_options['interstitialValues'] as $vs_element => $va_value) {
                if ($t_rel->hasField($vs_element)) {
                    if (is_array($va_value)) {
                        $va_value = $va_value[$vs_element] ?? null;
                    }
    ...
    public function getMetadataDictionaryRuleViolations($ps_bundle_name=null, $options=null)
    {
        ...
        if (($screen_id = caGetOption('screen_id', $options, null)) || ($placement_id = caGetOption('placement_id', $options, null))) {
            ...
            if (is_array($screen_placements = $t_screen->getPlacements($placement_id ? ['placement_id' => $placement_id] : ['screen_id' => $screen_id]))) {
                $bundles_on_screen = array_map(function ($v) {
                    return preg_replace("!^ca_attribute_!", "", $v['bundle_name']);
                }, $screen_placements);
            }
        }
        $o_db = $this->getDb();
        ...
        while ($qr_res->nextRow()) {
            ...
            if ($limit_to_show_as_prompt && !$show_as_prompt) {
                continue;
            }

            if (is_array($bundles_on_screen)) {
                if (!in_array(join(".", $bundle_elements), $bundles_on_screen)) {
                    if (sizeof($bundle_elements) > 1) {
                        array_shift($bundle_elements);
                    }
                ...
                $rule_settings_restrict_to_types = array_filter(caGetOption(['restrict_to_types', 'restrictToTypes'], $rule_settings, []), function ($v) {
                    return strlen($v);
                });
                $rule_settings_restrict_to_relationship_types = array_filter(caGetOption(['restrict_to_relationship_types', 'restrictToRelationshipTypes'], $rule_settings, []), function ($v) {
                    return strlen($v);
                });
                ...
                foreach ($bundles_on_screen as $placement_id => $placement_bundle_name) {
                    if ($placement_bundle_name == $p) {
                        ...
                        $placement_restrict_to_types = array_filter(caGetOption(['restrict_to_types', 'restrictToTypes'], $screen_placements[$placement_id]['settings'], []), function ($v) {
                            return strlen($v);
                        });
                        $placement_restrict_to_relationship_types = array_filter(caGetOption(['restrict_to_relationship_types', 'restrictToRelationshipTypes'], $screen_placements[$placement_id]['settings'], []), function ($v) {
                            return strlen($v);
                        });
    ...
    */
     public function validateUsingMetadataDictionaryRules($pa_options=null)
     {
         ...
         if ($this->inTransaction()) {
             $t_violation->setTransaction($this->getTransaction());
         }
     ...
     */
    public static function merge(array $ids, ?array $options=null): ?BundlableLabelableBaseModelWithAttributes
    {
        ...
        if (is_array($merge)) {
            $merge = array_filter(array_map(function ($v) {
                return strtolower($v);
            }, $merge), function ($v) use ($merge_opts) {
                return in_array($v, $merge_opts);
            });
        }
        if (!is_array($merge) || !sizeof($merge)) {
            $merge = $merge_opts;
        }
        ...
        foreach ($instances as $id => $t_instance) {
            ...
            foreach ($rel_tables as $rel_table) {
                ...
                if (!is_array($rel_ids) || !sizeof($rel_ids)) {
                    continue;
                }
        ...
        try {
            ...
            if (in_array('preferred_labels', $merge, true)) {
                self::_mergePrefLabels($t_base, $base_content['pref_labels'], array_map(function ($v) {
                    return $v['pref_labels'];
                }, $content), $options);
            }
            // 2. Nonpreferred labels?
            if (in_array('nonpreferred_labels', $merge, true)) {
                self::_mergeNPrefLabels($t_base, $base_content['npref_labels'], array_map(function ($v) {
                    return $v['npref_labels'];
                }, $content), $options);
            }

            // 3. Intrinsics
            if (in_array('intrinsics', $merge, true)) {
                self::_mergeIntrinsics($t_base, $base_content['intrinsics'], array_map(function ($v) {
                    return $v['intrinsics'];
                }, $content), $options);
            }
            ...
            if (in_array('attributes', $merge, true)) {
                self::_mergeAttributes($t_base, $base_content['attributes'], array_map(function ($v) {
                    return $v['attributes'];
                }, $content), $options);
            }
     ...
     * @throws MergeException
     */
    private static function _mergePrefLabels($t_base, $base, $label_list, $options=null): void
    {
        $notification = caGetOption('notification', $options, null);
                    ...
                    switch(strtolower($mode)) {
                        case 'merged':
                            ...
                            if (!in_array($label_id, array_keys($base))) {
                                $use_label_by_locale_id[$locale_id] = array_filter($label, function ($k) use ($label_fields) {
                                    return in_array($k, $label_fields);
                        ...
                        case 'base':
                            ...
                            foreach (reset($base) as $blabel) {
                                if ($label['locale_id'] == $blabel['locale_id']) {
                                    $use_label_by_locale_id[$locale_id] = array_filter($blabel, function ($k) use ($label_fields) {
                                        return in_array($k, $label_fields);
                                    }, ARRAY_FILTER_USE_KEY);
                                    break(2);
                                }
     ...
     * @throws MergeException
     */
    private static function _mergeNPrefLabels($t_base, $base, $label_list, $options=null): void
    {
        $notification = caGetOption('notification', $options, null);
        $table = $t_base->tableName();
        $label_fields = $t_base->getLabelUIFields();
        ...
        foreach (array_merge([$base], $label_list) as $labels_by_row) {
            foreach ($labels_by_row as $row_id => $labels) {
                foreach ($labels as $label_id => $label) {
                    ...
                    if (!$label_exists) {
                        $labels_by_locale_id[$locale_id][] = array_filter($label, function ($k) use ($label_fields) {
                            return (in_array($k, $label_fields) || ($k === 'type_id'));
                        }, ARRAY_FILTER_USE_KEY);
                    }
                }
            }
     ...
     * @throws MergeException
     */
    private static function _mergeIntrinsics($t_base, $base, $rows, $options=null): void
    {
        $notification = caGetOption('notification', $options, null);
        $ret = $base;
        ...
        foreach ($rows as $id => $ivalues) {
            ...
            foreach ($ivalues as $f => $v) {
                if (in_array($f, [$pk, 'parent_id', 'type_id', 'source_id', 'locale_id', 'rank', 'access', 'status'])) {
                    continue;
                }
                if (preg_match("!^hier_!", $f)) {
                    continue;
                }
     ...
     * @throws MergeException
     */
    private static function _mergeAttributes($t_base, $base, $rows, $options=null): void
    {
        $notification = caGetOption('notification', $options, null);
        ...
        foreach ($rows as $row_id => $attrs) {
            foreach ($attrs as $attrs_for_element) {
                foreach ($attrs_for_element as $attr) {
                    ...
                    if (!$t_restriction) {
                        return;
                    }		// attribute not bound to this type
     ...
     * @throws MergeException
     */
    private static function _mergeRelationships(BundlableLabelableBaseModelWithAttributes $t_base, array $row_ids, ?array $options=null): void
    {
     ...
     */
    private static function _relationshipTablesToMerge(?array $options=null): array
    {
        ...
        if (is_array($skip_relationships)) {
            $rel_tables = array_filter($rel_tables, function ($v) use ($skip_relationships) {
                return !in_array($v, $skip_relationships);
            });
        }
        return $rel_tables;
    }
