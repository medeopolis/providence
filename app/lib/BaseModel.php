...
class BaseModel extends BaseObject
{
     ...
     */
    public static $s_ca_models_definitions;

    /**
     * Array of cached relationship info arrays
     */
    public static $s_relationship_info_cache = array();
     ...
     *
     */
    public static $s_field_value_arrays_for_IDs_cache = array();
    ...
    public static $field_list_for_load = [];
    ...
    public function __construct($id=null, ?array $options=null)
    {
        ...
        if (!is_array(self::$field_list_for_load)) {
            self::$field_list_for_load = [];
        }
        ...
        if ($id) {
            $this->load($id, caGetOption('useCache', $options, true));
        }
    }
    ...
    {
        if (!isset($this->_FIELD_VALUES[$this->PRIMARY_KEY])) {
            return null;
        }
    ...
    public function get($ps_field, $pa_options=null)
    {
        ...
        if (
            ...
            if ($va_tmp[0] !== $this->tableName()) {
                array_unshift($va_tmp, $this->tableName());
            }
    ...
    {
        if ((!is_array($pa_ids) && (int)$pa_ids > 0)) {
            $pa_ids = array($pa_ids);
        }
        ...
        if (!is_array($pa_fields)) {
            $pa_fields = $this->getFormFields(true, true, true);
        }
        ...
        if (!sizeof($va_ids)) {
            return BaseModel::$s_field_value_arrays_for_IDs_cache[$vn_table_num][$vs_cache_key] = null;
        }
        ...
        $vs_fld_list = (sizeof($va_fld_list)) ? join(", ", array_map(function ($v) {
     ...
     */
    public static function getIDsForIdnos($idnos, $options=null)
    {
        if (!is_array($idnos) && strlen($idnos)) {
            $idnos = [$idnos];
        }
        ...
        if (!($t_instance = Datamodel::getInstanceByTableName($table_name, true))) {
            return null;
        }

        $idnos = array_map(function ($v) {
            return (string)$v;
        }, $idnos);
     ...
     * @return array Array with keys set to idnos and values set to row_ids. Returns null on error.
     */
    public static function getIdnosForIDs(array $ids, ?array $options=null)
    {
     ...
     */
    public static function getFieldValueArraysForIDs($pa_ids, $ps_table_name=null)
    {
        ...
        if (!($t_instance = Datamodel::getInstanceByTableName($vs_table_name, true))) {
            return false;
        }
     ...
     */
     private function _calcHierarchicalIndexing($pa_parent_info)
     {
		...
		", $pa_parent_info[$vs_hier_left_fld], $pa_parent_info[$vs_hier_right_fld], $pa_parent_info[$this->primaryKey()]);
         ...
         if ($qr_up->nextRow()) {
             ...
             if (($vn_scale = strlen(floor($vn_gap_size/10000))) < 1) {
                 $vn_scale = 1;
             }
     ...
     {
         if (!isset(BaseModel::$s_instance_cache[$this->tableName()][(int)$pn_id])) {
             return false;
         }
    ...
    public function insert($pa_options=null)
    {
        ...
        foreach ($this->FIELDS as $vs_field => $va_attr) {
            ...
            if (isset($va_attr['DONT_PROCESS_DURING_INSERT_UPDATE']) && (bool)$va_attr['DONT_PROCESS_DURING_INSERT_UPDATE']) {
                continue;
            }
            ...
            if ($vs_field_value_is_null && !in_array($vs_field_type, array(FT_MEDIA, FT_FILE))) {
                ...
                switch($vs_field_type) {
                    ...
                    case (FT_HISTORIC_DATERANGE):
                        ...
                        if (is_null($this->_FIELD_VALUES[$start_field_name])) {
                            $vm_start_val = 'null';
                        } else {
                            $vm_start_val = $this->_FIELD_VALUES[$start_field_name];
                        }
                        ...
                        if (is_null($this->_FIELD_VALUES[$end_field_name])) {
                            $vm_end_val = 'null';
                        } else {
                            $vm_end_val = $this->_FIELD_VALUES[$end_field_name];
                        }
                    ...
                    case (FT_TIMERANGE):
                        ...
                        if (is_null($this->_FIELD_VALUES[$end_field_name])) {
                            ...
                            $end_field_name = $this->_FIELD_VALUES[$end_field_name];
                        }
        ...
        if ($this->numErrors() == 0) {
            ...
            if ($this->numErrors() == 0) {
                ...
                if (method_exists($this, "deriveHistoryTrackingCurrentValue")) {
                    ...
                    if ($table::isHistoryTrackingCriterion($table)) {
                        $this->updateDependentHistoryTrackingCurrentValues();
                    }
                }
                ...
                if (($vn_fields_that_have_been_set > 0) && !caGetOption('dontLogChange', $pa_options, false)) {
                    $this->logChange("I", null, ['log_id' => $vn_log_id = caGetOption('log_id', $pa_options, null)]);
                }
    ...
    public function update($pa_options=null)
    {
        ...
        foreach ($this->FIELDS as $vs_field => $va_attr) {
            ...
            if (($vs_field_value_is_null) && (!(isset($va_attr["UPDATE_ON_UPDATE"]) && $va_attr["UPDATE_ON_UPDATE"]))) {
                ...
                if (($vs_field_type != FT_TIMESTAMP) && !$this->changed($vs_field)) {
                    continue;
                }		// don't try to update fields that haven't changed -- saves time, especially for large fields like FT_VARS and FT_TEXT when text is long
                switch($vs_field_type) {
                    # -----------------------------
                    case (FT_NUMBER):
                    case (FT_BIT):
            ...
            if ($vn_fields_that_have_been_set > 0) {
                ...
                if (is_array($va_rebuild_hierarchical_index) && (!isset($pa_options['dontSetHierarchicalIndexing']) || !$pa_options['dontSetHierarchicalIndexing'])) {
                    ...
                    foreach ($va_rebuild_hierarchical_index as $vn_child_id) {
                        if ($vn_child_id == $this->getPrimaryKey()) {
                            continue;
                        }
                        if ($t_instance->load($vn_child_id)) {
                ...
                if (($vn_fields_that_have_been_set > 0) && !caGetOption('dontLogChange', $pa_options, false)) {
                    $this->logChange("U", null, ['log_id' => caGetOption('log_id', $pa_options, null)]);
                }
    ...
    {
        if (defined("__CA_DONT_DO_SEARCH_INDEXING__") || defined('__CA_IS_SIMPLE_SERVICE_CALL__')) {
            return;
        }
        ...
        return $o_indexer->indexRow(
            $this->tableNum(),
            $this->getPrimaryKey(), // identify record
    ...
    public function delete($pb_delete_related=false, $pa_options=null, $pa_fields=null, $pa_table_list=null)
    {
        ...
        if (method_exists($this, "deriveHistoryTrackingCurrentValue")) {
            ...
            if ($table::isHistoryTrackingCriterion($table)) {
                $this->updateDependentHistoryTrackingCurrentValues(['row_id' => $vn_id, 'mode' => 'delete']);
            }
        }
        ...
        if ($this->hasField('deleted') && (!isset($pa_options['hard']) || !$pa_options['hard'])) {
            ...
            if (!caGetOption('dontLogChange', $pa_options, false)) {
                $this->logChange("D", null, ['log_id' => caGetOption('log_id', $pa_options, null)]);
            }
            ...
            if (isset($this->_SET_FILES[$ps_field]['tmp_name']) && (isUrl($this->_SET_FILES[$ps_field]['tmp_name']) || is_readable($this->_SET_FILES[$ps_field]['tmp_name']))) {
                ...
                if (isset($this->_SET_FILES[$ps_field]['tmp_name']) && (file_exists($this->_SET_FILES[$ps_field]['tmp_name']))) {
                    ...
                    if (is_array($va_tmp = $this->getMediaInfo($ps_field))) {
                        $va_center = caGetOption('_CENTER', $va_tmp, []);
                    }
                    $media_desc = [
                    ...
                    foreach ($va_versions as $v) {
                        ...
                        if (!is_array($rules) || (sizeof($rules) == 0)) {
                            ...
                            if (
                                ...
                                $alt_version_list = array_filter(array_keys($media_desc), function ($x) use ($v) {
                                    return ($x !== $v);
                                });
                                $replace_with_version = $replace_with_version_opt ? $replace_with_version_opt : array_pop($alt_version_list);
                                if (!in_array($replace_with_version, $va_versions)) {
                                    $replace_with_version = null;
                                }
                                $media_desc[$v]['REPLACE_WITH_VERSION'] = $replace_with_version ? $replace_with_version : $va_versions[0];
                            } elseif ((bool)$version_info[$v]["USE_EXTERNAL_URL_WHEN_AVAILABLE"]) {
    ...
    public function getMediaCenter($ps_field, $pa_options=null)
    {
        ...
        if (($vn_current_center_x < 0) || ($vn_current_center_x > 1)) {
            $vn_current_center_x = 0.5;
        }

        $vn_current_center_y = caGetOption('y', $va_media_info['_CENTER'], 0.5);
        if (($vn_current_center_y < 0) || ($vn_current_center_y > 1)) {
            $vn_current_center_y = 0.5;
        }
    ...
    public function setMediaCenter($ps_field, $pn_center_x, $pn_center_y, $pa_options=null)
    {
        ...
        // Is center different?
        if (($vn_current_center_x == $pn_center_x) && ($vn_current_center_y == $pn_center_y)) {
            return true;
        }
    ...
    {
        if ($pn_percent_of_image_width > 1) {
            $pn_percent_of_image_width /= 100;
        }
        if ($pn_percent_of_image_width <= 0) {
            $pn_percent_of_image_width = 1;
        }
        $va_media_info = $this->getMediaInfo($ps_field);
        if (!is_array($va_media_info)) {
            return null;
    ...
    public function &getFileInfo($ps_field, $ps_property=null)
    {
        ...
        if ($ps_property) {
            return isset($va_file_info[$ps_property]) ? $va_file_info[$ps_property] : null;
        }
        return $va_file_info;
    }
    # --------------------------------------------------------------------------------
    ...
    public function logChange($ps_change_type, $pn_user_id=null, $pa_options=null)
    {
        ...
        if (!($vn_row_id = caGetOption('row_id', $pa_options, null)) && !($vn_row_id = $this->getPrimaryKey())) {
            return false;
        ...
        if ($vb_is_metadata) {
            ...
            if (($ps_change_type !== 'D') && !caGetOption('forceLogChange', $pa_options, false) && !sizeof($this->getChangedFieldValuesArray())) {
                return null;
            }	// don't log if nothing has changed
        ...
        if (!($va_snapshot = caGetOption('snapshot', $pa_options, null))) {
        ...
        if (!(($ps_change_type == 'U') && (!sizeof($va_snapshot)))) {
            ...
            $this->opqs_change_log->execute(
                ...
                $this->tableNum(),
                $vn_row_id,
                ((int)$g_change_log_batch_id ? (int)$g_change_log_batch_id : null)
            );
     ...
     */
    public static function getCreatedOnTimestampsForIDs(array $row_ids, array $options=null): array
    {
     ...
     * @return array An array of dates, one per row id.
     */
    public static function getLastModifiedTimestampsForIDs(array $row_ids, array $options=null): array
    {
     ...
     * @return array An array of dates, one per row id.
     */
    private static function _getTimestampsForIDs(string $mode, array $row_ids, array $options=null): array
    {
        $o_db = new Db();
        $t_instance = Datamodel::getInstance(get_called_class(), true);
        ...
        $row_ids = array_filter(array_map('intval', $row_ids), function ($v) {
            return ($v > 0);
        });
        $qr_res = $o_db->query(
            "
    ...
    public function &getHierarchy($pn_id=null, $pa_options=null)
    {
        ...
        if ($this->isHierarchical()) {
            ...
            if (!($vs_rank_fld = caGetOption('sort', $pa_options, $this->getProperty('RANK'))) || !$this->hasField($vs_rank_fld)) {
                $vs_rank_fld = $vs_hier_left_fld;
            }
            ...
            if ($o_db->numErrors()) {
                ...
                if ($qr_root->nextRow()) {
                    ...
                    $vs_sql = "
						SELECT ".join(',', array_map(function ($v) {
                        return "{$v}.*";
                    }, $select_tables))."
						FROM {$vs_table_name}
						{$vs_sql_joins}
                ...
                if ($qr_root->numRows()) {
                    ...
                    if ($vn_parent_id) {
                        do {
                            ...
                            while ($qr_hier->nextRow()) {
                                if (!$vn_parent_id) {
                                    $vn_parent_id = $qr_hier->get($vs_hier_parent_id_fld);
                                }
                                if (!$vn_parent_id && $pb_omit_root) {
                                    continue;
                                }
    ...
    {
        if (!is_array($pa_hierarchy_data[$pn_id])) {
            return array();
        }
     ...
     * @return mixed
     */
    public static function getHierarchyAncestorsForIDs($pa_row_ids, $pa_options=null)
    {
        if (!is_array($pa_row_ids) || (sizeof($pa_row_ids) == 0)) {
            return null;
        }
     ...
     * @return mixed
     */
    public static function getHierarchyChildrenForIDs($pa_row_ids, $pa_options=null)
    {
        if (!is_array($pa_row_ids) || (sizeof($pa_row_ids) == 0)) {
            return null;
        }
                    ...
                    if (($vn_display_width > 0) && (in_array($va_attr["DISPLAY_TYPE"], array(DT_SELECT, DT_LIST, DT_LIST_MULTIPLE)))) {
                        ...
                        if ($vs_list_code) {
                            ...
                            $vs_element = $t_list->getListAsHTMLFormElement(
                                $vs_list_code,
                                $pa_options["name"].$vs_multiple_name_extension,
                                $va_list_attrs,
                                array(
    ...
    public function getMandatoryFields()
    {
        ...
        foreach ($va_fields as $vs_field => $va_info) {
            if (isset($va_info['IDENTITY']) && $va_info['IDENTITY']) {
                continue;
            }
    ...
    public function addRelationship($pm_rel_table_name_or_num, $pn_rel_id, $pm_type_id=null, $ps_effective_date=null, $ps_source_info=null, $ps_direction=null, $pn_rank=null, $pa_options=null)
    {
        ...
        if ($this->inTransaction()) {
            $o_trans = $this->getTransaction();
            $t_item_rel->setTransaction($o_trans);
        }
        ...
        if ($va_rel_info['related_table_name'] == $this->tableName()) {
            ...
            if ($t_item_rel->hasField($f = $t_item_rel->getTypeFieldName())) {
                $t_item_rel->set($f, $pn_type_id);
            }		// TODO: verify type_id based upon type_id's of each end of the relationship
            if (!is_null($ps_effective_date)) {
                $t_item_rel->set('effective_date', $ps_effective_date);
            }
            if (!is_null($ps_source_info)) {
    ...
    *		useAsRelatedIdOnly =
    ...
    public function editRelationship($pm_rel_table_name_or_num, $pn_relation_id, $pn_rel_id, $pm_type_id=null, $ps_effective_date=null, $pa_source_info=null, $ps_direction=null, $pn_rank=null, $pa_options=null)
    {
        ...
        if ($va_rel_info['related_table_name'] == $this->tableName()) {
            ...
            if ($t_item_rel->load($pn_relation_id)) {
                ...
                if (!is_null($pn_type_id) && $t_item_rel->hasField($f = $t_item_rel->getTypeFieldName())) {
                    $t_item_rel->set($f, $pn_type_id);
                }	// TODO: verify type_id based upon type_id's of each end of the relationship
                if (!is_null($ps_effective_date)) {
                    $t_item_rel->set('effective_date', $ps_effective_date);
                }
                if (!is_null($pa_source_info)) {
                    $t_item_rel->set("source_info", $pa_source_info);
                }
            ...
            switch(sizeof($va_rel_info['path'])) {
                case 3:		// many-to-many relationship
                    if ($t_item_rel->load($pn_relation_id)) {
                        ...
                        if (!is_null($pn_type_id) && ($f = $t_item_rel->getTypeFieldName())) {
                            $t_item_rel->set($f, $pn_type_id);
                        }		// TODO: verify type_id based upon type_id's of each end of the relationship
                        if (!is_null($ps_effective_date)) {
                            $t_item_rel->set('effective_date', $ps_effective_date);
    ...
    public function removeRelationships($pm_rel_table_name_or_num, $pm_relationship_type_id=null, $pa_options=null)
    {
        ...
        $t_item_rel = $va_rel_info['t_item_rel'];
        if (!method_exists($t_item_rel, "isRelationship") || !$t_item_rel->isRelationship()) {
            return false;
        }
        $va_sql_params = array();
    ...
    {
        if ($va_rel_info = $this->_getRelationshipInfo($pm_rel_table_name_or_num)) {
            if ($va_rel_info['t_item_rel']) {
                return $va_rel_info['t_item_rel']->tableName();
            }
        }
        return null;
    }
    ...
    {
        if (!($row_id = $this->getPrimaryKey())) {
            return null;
        }
        if (!($rel_info = $this->_getRelationshipInfo($rel_table_name_or_num))) {
            return null;
        }
        ...
        if ($rt == $this->getSelfRelationTableName()) {
            ...
            if (!sizeof($to_reindex_relations)) {
                return 0;
            }
    ...
    public function copyRelationships($pm_rel_table_name_or_num, $pn_to_id, $pa_options=null)
    {
        ...
        if (is_numeric($relation_ids) && !is_array($relation_ids)) {
        ...
        if ($t_item_rel->tableName() == $this->getSelfRelationTableName()) {
            ...
            if (!sizeof($va_to_reindex_relations)) {
                return 0;
            }
            ...
            foreach ($va_to_reindex_relations as $vn_relation_id => $va_row) {
                ...
                if (isset($va_row['source_info'])) {
                    $va_row['source_info'] = caUnserializeForDatabase($va_row['source_info']);
                }
                ...
                if (array_key_exists('sdatetime', $va_row)) {
                    ...
                    unset($va_row['sdatetime']);
                    unset($va_row['edatetime']);
                }

                $t_item_rel->set($va_row);
                $t_item_rel->insert();
                if ($t_item_rel->numErrors()) {
                    if (($t_item_rel->numErrors() == 1) && ($t_item_rel->errors()[0]->getErrorNumber() == 251)) {
                        continue;
                    }  // don't worry about failed duplicates
                    $this->errors = $t_item_rel->errors;
                    return null;
                }
            ...
            foreach ($va_one_to_many_relations as $vs_many_table => $va_info) {
                foreach ($va_info as $va_relationship) {
                    ...
                    $qr_record_check = $o_db->query("
						...
						FROM {$vs_many_table}
						WHERE
							({$va_relationship['many_table_field']} = ?)", array((int)$vn_id));
    ...
    public function addTag($ps_tag, $pn_user_id=null, $pn_locale_id=null, $pn_access=0, $pn_moderator=null, $pa_options=null)
    {
        ...
        $criteria = ['tag' => $ps_tag];
        if ($pn_locale_id > 0) {
            $criteria['locale_id'] = $pn_locale_id;
        }
    ...
    public function getTags($pn_user_id=null, $pb_moderation_status=null, $pn_row_id=null)
    {
        ...
        return array_map(function ($v) {
            $v['moderation_message'] = $v['access'] ? '' : _t('Needs moderation');
            return $v;
        }, $qr_comments->getAllRows());
    }
    # --------------------------------------------------------------------------------------------
    /**
     ...
     * @return array
     */
    public static function suggestTags(string $text, array $options=null): array
    {
    ...
    public function removeComment($pn_comment_id, $pn_user_id=null, $options=null)
    {
        ...
        if (!($vn_row_id = $this->getPrimaryKey()) && !$force) {
            return null;
        }
    ...
    public function getComments($pn_user_id=null, $pb_moderation_status=null, $pa_options=null)
    {
        ...
        $va_comments = array_map(function ($v) {
            $v['moderation_message'] = $v['access'] ? '' : _t('Needs moderation');
            return $v;
        }, $va_comments);
        ksort($va_comments);
     ...
     */
    public static function getNumCommentsForIDs($pa_ids, $pb_moderation_status=true, $pa_options=null)
    {
        if (!is_array($pa_ids) || !sizeof($pa_ids)) {
            return null;
        }
        ...
        if (!($vn_table_num = Datamodel::getTableNum(get_called_class()))) {
            return null;
        }
     ...
     *
     */
    public static function getLoggingUnitID()
    {
     ...
     *
     */
    public static function getCurrentLoggingUnitID()
    {
        global $g_change_log_unit_id;
     ...
     *
     */
    public static function setChangeLogUnitID()
    {
        global $g_change_log_unit_id;
     ...
     *
     */
    public static function unsetChangeLogUnitID()
    {
        global $g_change_log_unit_id;
    ...
    public static function getIdnoForID($pn_id, $pa_options=null)
    {
        ...
        if (($t_instance = Datamodel::getInstance(static::class, true)) && ($vs_idno_fld = $t_instance->getProperty('ID_NUMBERING_ID_FIELD'))) {
            ...
            if ($qr_res->nextRow()) {
                if ($has_access && (is_array($pa_check_access) && (sizeof($pa_check_access) > 0)) && $t_instance->hasField('access') &&  !in_array($qr_res->get('access'), $pa_check_access)) {
                    return false;
                }
    ...
    public static function getIDForIdno($ps_idno, $pa_options=null)
    {
        ...
        if (($t_instance = Datamodel::getInstance(static::class, true)) && ($vs_idno_fld = $t_instance->getProperty('ID_NUMBERING_ID_FIELD'))) {
            ...
            $pa_check_access = caGetOption('checkAccess', $pa_options, null);
            if ($qr_res->nextRow()) {
                if ((is_array($pa_check_access) && (sizeof($pa_check_access) > 0)) && $t_instance->hasField('access') &&  !in_array($qr_res->get('access'), $pa_check_access)) {
                    return false;
                }
    ...
    public static function find($pa_values, $pa_options=null)
    {
        ...
        if ($pa_values === '*') {
            $pa_values = [$t_instance->primaryKey() => '*'];
        }
        ...
        if (method_exists($t_instance, "getTypeFieldName")) {
            if (is_array($va_field_values = $pa_values[$vs_type_field_name = $t_instance->getTypeFieldName()])) {
                foreach ($va_field_values as $vn_i => $va_field_value) {
                    ...
                    if (!caIsValidSqlOperator($vs_op, ['type' => 'numeric', 'nullable' => false, 'isList' => is_array($vm_value)])) {
                        throw new ApplicationException(_t('Invalid numeric operator: %1', $vs_op));
                    }
        ...
        foreach ($pa_values as $vs_field => $va_field_values) {
            foreach ($va_field_values as $vn_i => $va_field_value) {
                ...
                if ($vs_list_code = $t_instance->getFieldInfo($vs_field, 'LIST_CODE')) {
                    if (!caIsValidSqlOperator($vs_op, ['type' => 'numeric', 'nullable' => $t_instance->getFieldInfo($vs_field, 'IS_NULL'), 'isList' => is_array($vm_value)])) {
                        throw new ApplicationException(_t('Invalid numeric operator: %1', $vs_op));
                    }

                    if (is_array($vm_value)) {
                        $pa_values[$vs_field][$vn_i] = [$vs_op, $z=array_map(function ($v) use ($vs_list_code) {
                            return is_numeric($v) ? $v : ca_lists::getItemID($vs_list_code, $v);
                        }, $vm_value)];
            ...
            foreach ($pa_values as $vs_field => $va_field_values) {
                foreach ($va_field_values as $va_field_value) {
                    ...
                    if ($t_instance->_getFieldTypeType($vs_field) == 0) {
                        if (!caIsValidSqlOperator($vs_op, ['type' => 'numeric', 'nullable' => true, 'isList' => is_array($vm_value)])) {
                            throw new ApplicationException(_t('Invalid numeric operator: %1', $vs_op));
                        }

                        if (is_array($vm_value)) {
                            $vm_value = array_map(function ($v) {
                                return (int)$v;
                            }, $vm_value);
                        ...
                        if (!caIsValidSqlOperator($vs_op, ['type' => 'string', 'nullable' => true, 'isList' => is_array($vm_value)])) {
                            throw new ApplicationException(_t('Invalid string operator: %1', $vs_op));
                        }
                        ...
                        if (is_null($vm_value) && !$t_instance->getFieldInfo($vs_field, 'IS_NULL')) {
                            $vs_op = '=';
                        }
                    }
                    ...
                    if (is_null($vm_value) || ($vs_op === 'IS')) {
                        ...
                        if (!in_array(strtoupper($vs_op), ['=', 'NOT IN', 'BETWEEN'])) {
                            $vs_op = 'IN';
                        }
                        ...
                        if (($vs_op === 'IN') && (!is_array($vm_value) || !sizeof($vm_value))) {
                            continue;
                        }
        ...
        $va_sql = [];
        if ($vs_wheres = join(" {$ps_boolean} ", $va_sql_wheres)) {
            $va_sql[] = $vs_wheres;
        }
        if ($vs_type_restriction_sql) {
            $va_sql[] = $vs_type_restriction_sql;
        }
        if ($vs_deleted_sql) {
            $va_sql[] = $vs_deleted_sql;
        }
        $va_sql = array_filter($va_sql, function ($v) {
            return strlen($v) > 0;
        });
        ...
        if ($limit > 0) {
            $limit_sql .= $limit_sql ? ", {$limit}" : "{$limit}";
        }
    ...
    public function addNotification($pn_type, $ps_message, $pb_system=false, array $pa_options=[])
    {
        ...
        if (is_array($va_additional_subjects)) {
            foreach ($va_additional_subjects as $va_subject) {
                ...
                if (!($t_instance = Datamodel::getInstanceByTableNum($va_subject['table_num'], true))) {
                    continue;
                }
     ...
     * @return array Array of notifications indexed by notification subject_id. Each value is an array of notification content.
     */
    public static function getQueuedEmailNotifications(array $pa_options = [])
    {
        if (!($t_instance = Datamodel::getInstanceByTableName($vs_table_name = get_called_class()))) {
            return null;
        }
        ...
        $va_fields = array_map(function ($v) {
            return "t.{$v}";
        }, array_keys(array_filter($t_instance->getFieldsArray(), function ($v) {
            if (!in_array($v['FIELD_TYPE'], [FT_VARS]) && !in_array($v['DISPLAY_TYPE'], [DT_OMIT])) {
                return $v;
            }
        })));
