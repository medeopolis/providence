...
class BaseLookupController extends ActionController
{
    ...
    public function __construct(&$po_request, &$po_response, $pa_view_paths=null)
    {
        if ($this->ops_search_class) {
            require_once(__CA_LIB_DIR__."/Search/".$this->ops_search_class.".php");
        }
    ...
    public function SetSortOrder()
    {
        ...
        try {
            ...
            $t_item = new $this->ops_table_name($pn_id);
            if (!$t_item->isLoaded()) {
                throw new ApplicationException(_t('Could not load %1', $t_item->getProperty('NAME_PLURAL')));
            }
            ...
            switch($t_item->getProperty('HIERARCHY_TYPE')) {
                case __CA_HIER_TYPE_SIMPLE_MONO__:
                    ...
                    if (!$this->request->user->canDoAction("can_edit_{$this->ops_table_name}")) {
                ...
                case __CA_HIER_TYPE_MULTI_MONO__:
                    ...
                    if (!$t_def->isLoaded()) {
                        throw new ApplicationException(_t('Could not load %1', $t_def->getProperty('NAME_PLURAL')));
                    }
                    $vs_def_pk = $t_def->primaryKey();

                    if (!$this->request->user->canDoAction("can_edit_{$this->ops_table_name}") && !$this->request->user->canDoAction("can_edit_{$vs_def_table_name}")) {
                        throw new ApplicationException(_t('Access denied'));
                    }
                    break;
                default:
            ...
            if ($vs_def_table_name && $t_def->hasField('default_sort')) {
                if ((int)$t_def->get('default_sort') !== __CA_LISTS_SORT_BY_RANK__) {
                    throw new ApplicationException(_t('%1 must have default sort set to rank', $t_def->getProperty('NAME_SINGULAR')));
                }
            ...
            if ($t_item->numErrors()) {
                throw new ApplicationException(_t('Could not update rank: %1', join('; ', $t_item->getErrors())));
            }
            $vn_return = 1;
        } catch (Exception $e) {
