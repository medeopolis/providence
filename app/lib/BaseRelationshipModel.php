...
class BaseRelationshipModel extends BundlableLabelableBaseModelWithAttributes implements IRelationshipModel
{
    ...
    {
        if ($pa_fields && $pm_value && !is_array($pa_fields)) {
            $pa_fields = [$pa_fields => $pm_value];
        }
    ...
    {
        if (!in_array($ps_tablename, array($this->RELATIONSHIP_LEFT_TABLENAME, $this->RELATIONSHIP_RIGHT_TABLENAME))) {
            return null;
        }
    ...
    public function getRelationshipTypes($pn_sub_type_left_id=null, $pn_sub_type_right_id=null, $pa_options=null)
    {
        ...
        if ($pn_sub_type_left_id) {
            ...
            $va_params[] = $pn_sub_type_left_id;
            $va_params[] = $va_ancestor_ids;
        ...
        if ($pn_sub_type_right_id) {
            ...
            $va_params[] = $pn_sub_type_right_id;
            $va_params[] = $va_ancestor_ids;
    ...
    public function getRelationshipTypesAsHTMLSelect($ps_orientation, $pn_sub_type_left_id=null, $pn_sub_type_right_id=null, $pa_attributes=null, $pa_options=null)
    {
        ...
        if (!in_array($ps_orientation, array($vs_left_table_name, $vs_right_table_name))) {
            $ps_orientation = $vs_left_table_name;
        }
        ...
        if ($vs_left_table_name === $vs_right_table_name) {
            ...
            while ($qr_res->nextRow()) {
                ...
                if ($ps_orientation == $vs_left_table_name) {
                    ...
                    if ($pn_type_id && ($va_subtypes_to_check && !sizeof(array_intersect($va_subtypes_to_check, $va_ancestor_ids)))) {
                        continue;
                    }
                    $vs_subtype = $va_row['sub_type_right_id'];
                ...
                foreach ($va_types_by_subtype as $vs_key => $va_types_by_key) {
                    foreach ($va_types_by_key as $vn_locale_id => $va_t) {
                        if (!is_array($va_types_by_locale[$vn_locale_id])) {
                            $va_types_by_locale[$vn_locale_id] = [];
                        }
    ...
    {
        $va_types_to_return = [];
        if (!is_array($pa_hier[$pn_id])) {
            return [];
        }
        if (!is_array($pa_types[$pn_id])) {
            return [];
        }
    ...
    public function getRelatedIDsForSelfRelationship($pa_primary_ids=null, $pa_relationship_ids=null)
    {
        ...
        while ($qr_res->nextRow()) {
            if (!in_array($vn_id = $qr_res->get($vs_left_fld), $pa_primary_ids)) {
                $va_ids[$vn_id] = 1;
            }
            if (!in_array($vn_id = $qr_res->get($vs_right_fld), $pa_primary_ids)) {
                $va_ids[$vn_id] = 1;
            }
        }
