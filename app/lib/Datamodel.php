...
class Datamodel
{
    ...
    {
        if (Datamodel::$opo_graph) {
            return;
        }	// datamodel graph is already loaded
     ...
     */
    public static function getTableNum($ps_table)
    {
        if (!$ps_table) {
            return null;
        }
     ...
     * @return null|string
     */
    public static function getTableName($pn_tablenum)
    {
        if (!$pn_tablenum) {
            return null;
        }
        if (!is_numeric($pn_tablenum)) {
            return $pn_tablenum;
        }
     ...
     */
    public static function getFieldNum($ps_table, $ps_field)
    {
        if (is_numeric($ps_table)) {
            $ps_table = Datamodel::getTableName($ps_table);
        }
        if (!$ps_table || !$ps_field) {
            return null;
        }
     ...
     */
    public static function getFieldName($ps_table, $pn_field_num)
    {
        if (is_numeric($ps_table)) {
            $ps_table = Datamodel::getTableName($ps_table);
        }
        if (!$ps_table || !is_int($pn_field_num)) {
            return null;
        }
     ...
     */
    public static function getFieldInfo($ps_table, $ps_field, $ps_key=null)
    {
        if (is_numeric($ps_table)) {
            $ps_table = Datamodel::getTableName($ps_table);
        }
     ...
     */
    public static function tableExists($ps_table)
    {
        if (is_numeric($ps_table)) {
            $ps_table = Datamodel::getTableName($ps_table);
        }
     ...
     */
    public static function getInstance($table_name_or_num, ?bool $use_cache=false, ?int $id=null, ?array $options=null)
    {
     ...
     * @return null|BaseModel
     */
    public static function getInstanceByTableName($table, $use_cache=false, ?array $options=null)
    {
        if (!$table) {
            return null;
        }
        if ($use_cache && isset(Datamodel::$s_instance_cache[$table])) {
            return Datamodel::$s_instance_cache[$table];
        }		// keep instances in statics for speed

        if (Datamodel::$opo_graph->hasNode($table)) {
            if (!MemoryCache::contains($table, 'DatamodelModelInstance')) {
                if (!file_exists(__CA_MODELS_DIR__.'/'.$table.'.php')) {
                    return null;
                }
            ...
            if ($use_cache) {
                Datamodel::$s_instance_cache[$t_instance->tableNum()] = Datamodel::$s_instance_cache[$table] = $t_instance;
            }
            return $t_instance;
        } else {
     ...
     * @return null|BaseModel
     */
    public static function getInstanceByTableNum($tablenum, $use_cache=false, ?array $options=null)
    {
        if ($use_cache && isset(Datamodel::$s_instance_cache[$tablenum])) {
            return Datamodel::$s_instance_cache[$tablenum];
        }		// keep instances in statics for speed
        if ($table = Datamodel::getTableName($tablenum)) {
            ...
            $t_instance = new $table(null, $options);
            if ($use_cache) {
                MemoryCache::save($table, $t_instance, 'DatamodelModelInstance');
                Datamodel::$s_instance_cache[$tablenum] = Datamodel::$s_instance_cache[$table] = $t_instance;
            }
            return $t_instance;
        } else {
            return null;
     ...
     */
    public static function primaryKey($pn_tablenum, $pb_include_tablename=false)
    {
     ...
     */
    public static function getTableProperty($pn_tablenum, $ps_property)
    {
     ...
     * @return string Value of property or null if $pn_tablenum is invalid
     */
    public static function isRelationship($pn_tablenum)
    {
        if ($t_instance = Datamodel::getInstanceByTableNum($pn_tablenum, true)) {
     ...
     * @return string Value of property or null if $pn_tablenum is invalid
     */
    public static function isSelfRelationship($pn_tablenum)
    {
        if ($t_instance = Datamodel::getInstanceByTableNum($pn_tablenum, true)) {
     ...
     * @return string Value of property or null if $pn_tablenum is invalid
     */
    public static function isLabel($pn_tablenum)
    {
        if ($t_instance = Datamodel::getInstanceByTableNum($pn_tablenum, true)) {
     ...
     */
    public static function getManyToOneRelations($ps_table, $ps_field=null)
    {
        if (!$ps_table) {
            return null;
        }
     ...
     *
     */
    public static function getOneToManyRelations($ps_table, $ps_many_table=null)
    {
        if (!$ps_table) {
            return null;
        }
     ...
     */
    public static function getManyToManyRelations($ps_table, $ps_table2=null)
    {
     ...
     *
     */
    public static function getPath($ps_left_table, $ps_right_table)
    {
        if (!$ps_left_table || !$ps_right_table) {
            return [];
        }
        if (is_numeric($ps_left_table)) {
            $ps_left_table = Datamodel::getTableName($ps_left_table);
        }
        if (is_numeric($ps_right_table)) {
            $ps_right_table = Datamodel::getTableName($ps_right_table);
        }
     ...
     */
    public static function getLinkingTableName($ps_left_table, $ps_right_table)
    {
     ...
     *
     */
    public static function getRelationships($ps_left_table, $ps_right_table)
    {
