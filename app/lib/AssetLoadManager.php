...
class AssetLoadManager
{
     ...
     */
    public static $s_use_minified = true;
    # --------------------------------------------------------------------------------
     ...
     *
     */
    public static function useMinified($pb_use_minified=null)
    {
     ...
     */
    public static function register($ps_package, $ps_library=null, $pn_priority=10, $output_target='header')
    {
        global $g_asset_config, $g_asset_load_list;
        ...
        if (!is_array($va_packages)) {
            $va_packages = array();
        }
        if (!is_array($va_theme_packages)) {
            $va_theme_packages = array();
        }
        ...
        if ($ps_library) {
            ...
            if (isset($va_list[$ps_library]) && $va_list[$ps_library]) {
                ...
                if (!$vb_is_theme_specific && isset($va_exclude_packages[$ps_package][$ps_library])) {
                    return true;
                }
            ...
            // register loadset
            if (!is_array($va_theme_loadsets = $g_asset_config->getAssoc('themeLoadSets'))) {
                $va_theme_loadsets = array();
            }
            if (!is_array($va_loadsets = $g_asset_config->getAssoc('loadSets'))) {
                $va_loadsets = array();
            }
     ...
     */
    public static function addComplementaryScript($ps_content=null)
    {
     ...
     */
    public static function getLoadHTML($po_request, $pa_options=null)
    {
        global $g_asset_config, $g_asset_load_list, $g_asset_complementary;
