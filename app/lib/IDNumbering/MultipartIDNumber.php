...
class MultipartIDNumber extends IDNumber
{
    ...
    public function isSerialFormat($format=null, $type=null)
    {
        ...
        if (!$this->isValidType($type, $format)) {
            $type = '__default__';
        }
    ...
    public function formatIsExtensionOf($format, $type='__default__')
    {
        ...
        for ($i=0; $i < $num_elements; $i++) {
            ...
            if ($base_element['type'] != $ext_element['type']) {
                return false;
            }
            if ($base_element['width'] > $ext_element['width']) {
                return false;
            }

            switch($base_element['type']) {
                case 'LIST':
                    if (!is_array($base_element['values']) || !is_array($ext_element['values'])) {
                        return false;
                    }
                    if (sizeof($base_element['values']) != sizeof($ext_element['values'])) {
                        return false;
                    }
                    for ($j=0; $j < sizeof($base_element['values']); $j++) {
                        if ($base_element['values'][$j] != $ext_element['values'][$j]) {
                            return false;
                        }
                    }
                    break;
                case 'CONSTANT':
                if ($base_element['value'] != $ext_element['value']) {
                    return false;
                }
                break;
                case 'NUMERIC':
                    if ($base_element['minimum_length'] < $ext_element['minimum_length']) {
                        return false;
                    }
                    if ($base_element['maximum_length'] > $ext_element['maximum_length']) {
                        return false;
                    }
                    if ($base_element['minimum_value'] < $ext_element['minimum_value']) {
                        return false;
                    }
                    if ($base_element['maximum_value'] > $ext_element['maximum_value']) {
                        return false;
                    }
                    break;
                case 'ALPHANUMERIC':
            ...
            foreach ($this->getElements() as $element_info) {
                switch ($element_info['type']) {
                    case 'PARENT':
                        ...
                        break;
                    default:
                        if (!array_key_exists($i, $element_vals)) {
                            $element_vals[$i] = null;
                        }
                        break;
                }
            ...
            foreach ($elements as $ename => $element_info) {
                ...
                switch($element_info['type']) {
                    ...
                    case 'DAY':
                        ...
                        if ($element_info['type'] == 'MONTH') {
                            $element_vals[]  = $date['mon'];
                        }
                        if ($element_info['type'] == 'DAY') {
                            $element_vals[]  = $date['mday'];
                        }
                        break;
                    case 'LIST':
    ...
    {
        $separator = $this->getSeparator();
        if (!is_array($elements_normal_order = $this->getElements())) {
            $elements_normal_order = [];
        }
        ...
        foreach ($elements as $element) {
            ...
            switch($element_info['type']) {
                ...
                case 'SERIAL':
                case 'NUMERIC':
                    if ($padding < $element_info['width']) {
                        $padding = $element_info['width'];
                    }
    ...
    public function getIndexValues($value=null, $options=null)
    {
        ...
        foreach ($elements as $element) {
            ...
            if ($max_value_count < sizeof($output[$i])) {
                $max_value_count = sizeof($output[$i]);
            }
        }
        ...
        if (preg_match_all("![^A-Za-z0-9]+!", $value, $delimiters)) {
            ...
            foreach ($element_values as $element_value) {
                ...
                if (sizeof($delimiters[0]) > 0) {
                    $acc[] = array_shift($delimiters[0]);
                }
            }
        }
        ...
        switch($element_info['type']) {
            ...
            case 'LIST':
                ...
                if (!$element_value || $element_info['editable'] || $generate_for_search_form) {
                    if (!$element_value && !$generate_for_search_form) {
                        $element_value = $element_info['default'];
                    }
            ...
            case 'CONSTANT':
                ...
                if (!$element_value) {
                    $element_value = $element_info['value'];
                }
            ...
            case 'DAY':
                ...
                if ($element_value == '') {
                    ...
                    if (!$generate_for_search_form) {
                        if ($element_info['type'] == 'YEAR') {
                            $v = ($element_info['width'] == 2) ? substr($date['year'], 2, 2) : $date['year'];
                        }
                        if ($element_info['type'] == 'MONTH') {
                            $v = $date['mon'];
                        }
