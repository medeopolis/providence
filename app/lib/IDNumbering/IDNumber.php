...
abstract class IDNumber implements IIDNumbering
{
    ...
    public function formatHas($element_type, $index=null, $format=null, $type=null, $options=null)
    {
        ...
        if (!is_null($index) && isset($va_elements[$index])) {
        ...
        if ($format && $type) {
            if (is_array($this->formats[$format][$type]['elements'])) {
                ...
                foreach ($this->formats[$format][$type]['elements'] as $k => $element_info) {
                    if (!$is_child && isset($element_info['child_only']) && (bool)$element_info['child_only']) {
                        continue;
                    }
                    if ($is_child && $element_info['root_only']) {
                        continue;
                    }
    ...
    public function isFormatEditable($format, $options=null)
    {
        ...
        foreach ($types as $type) {
            if (!is_array($elements = $this->getElements($format))) {
                continue;
            }

            foreach ($elements as $ename => $info) {
                if (isset($info['editable']) && (bool)$info['editable']) {
                    return true;
                }
                if ($single_elements_only) {
                    break;
                }
            }
        }
