...
class OktaAuthAdapter extends BaseAuthAdapter implements IAuthAdapter
{
        ...
        if ($this->isAuthenticated()) {
            ...
            if (is_array($user_info)) {
                ...
                $va_groups = array_merge(
                    ...
                    }, $group_list), function ($v) {
                        return strtolower($v) !== 'everyone';
                    })
                );
                return [
                    'user_name' => $va_attrs['sub'],
    ...
    public function callback($options=null)
    {
        ...
        if (array_key_exists('code', $_REQUEST)) {
            ...
            if (!($redirect_url = Session::getVar('pawtucket2_last_page')) && ($redirect_url !== '/')) {
                $redirect_url = $this->config->get('default_action');
            }
    ...
    private static function oktaRequest($url, $headers, $options=null)
    {
        ...
        if (curl_error($ch)) {
            throw new OktaException(_t("Could not connect to Okta."));
        }
        curl_close($ch);
