...
abstract class AbstractLDAPAuthAdapter extends BaseAuthAdapter
{
    ...
    public function getUserInfo($ps_username, $ps_password, $pa_options=null)
    {
        foreach ($this->getLinkIdentifiers() as $vs_key => $r_ldap) {
            ...
            $vs_search_filter = $this->getProcessedConfigValue(
                $this->opa_auth_config_fragments[$vs_key],
                "ldap_user_search_filter_format",
                $ps_username,
                $vs_user_ou,
                $vs_base_dn
            );
            ...
            return array(
                ...
                'roles' => array_merge(
                    $va_default_roles,
                    $this->getRolesToAddFromDirectory(
                    $r_ldap,
                    $ps_username,
                    $this->opa_auth_config_fragments[$vs_key]
                )
                ...
                'groups' => array_merge(
                    $va_default_groups,
                    $this->getGroupsToAddFromDirectory(
                    $r_ldap,
                    $ps_username,
                    $this->opa_auth_config_fragments[$vs_key]
                )
     ...
     * @return array
     */
    abstract protected function getLDAPOptions();
    # --------------------------------------------------------------------------------
    /**
     ...
     */
    abstract protected function isUserInAnyGroup($pr_ldap, $ps_username, $pa_group_cn_list, $pa_config);
    # --------------------------------------------------------------------------------
    /**
     ...
     * @return array
     */
    abstract protected function getRolesToAddFromDirectory($pr_ldap, $ps_username, $pa_config);
    # --------------------------------------------------------------------------------
    /**
     ...
     * @return array
     */
    abstract protected function getGroupsToAddFromDirectory($pr_ldap, $ps_username, $pa_config);
    # --------------------------------------------------------------------------------
    /**
     * Bind to directory
