...
class AuthenticationManager
{
    ...
    public static function init($ps_adapter=null)
    {
        if (!is_null($ps_adapter) || (self::$g_authentication_adapter === null)) {
            ...
            if ($is_local = (isset($_REQUEST['local']) && $_REQUEST['local'])) {
                $vs_auth_adapter = 'CaUsers';
            }
    ...
    {
        self::init();
        if (AuthenticationManager::isFree()) {
            return null;
        }
    ...
    private static function isFree()
    {
        ...
        if ($g_request) {
            ...
            if (in_array(strtolower($g_request->getController()), $free_controllers)) {
                return true;
            }
        }
        return false;
