...
class BaseFindEngine extends BaseObject
{
    ...
    public function filterHitsByACL($pa_hits, $pn_table_num, $pn_user_id, $pn_access=__CA_ACL_READONLY_ACCESS__)
    {
        ...
        if (!($t_table = Datamodel::getInstanceByTableNum($pn_table_num, true))) {
            return $pa_hits;
        }
    ...
    public function sortHits(&$hits, $table, $sort_list, $sort_directions='asc', $options=null)
    {
        ...
        $embedded_sort_directions = array_map(function ($v) {
            $t = explode(':', $v);
            return (sizeof($t) == 2) ? strtolower(trim(array_pop($t))) : null;
        }, $sort_fields);
        ...
        $sort_directions = array_map(function ($v) {
            return strtolower($v);
        }, $sort_directions);
        ...
        foreach ($sort_fields as $i => $sort_field) {
            ...
            if (!($set_id = preg_match("!^ca_sets.set_id:([\d]+)$!", $sort_field, $va_matches) ? $va_matches[1] : null)) {	// sort on set_id (Eg. ca_sets.set_id:23)
                if (($sort_table == 'ca_set_items') && ($sort_field == 'rank') && ((int)$sort_rel_type > 0)) {
                    $set_id = (int)$sort_rel_type;
                }	// sort of ca_set_items.rank with set_id expressed as relationship type (Eg. ca_set_items.rank/23)
            }
            ...
            if ($set_id) {	// sort by set ranks
                ...
                if (sizeof($path) > 1) {
                    ...
                    if ($t_sort_field_table->hasField($sort_field)) { // sorting on intrinsic in related table
                        ...
                        if (!($qr_keys = caMakeSearchResult($sort_table, $rel_ids))) {
                            return $hits;
                        }
    ...
    private function _doSort(&$hits, $sortable_values, $sort_directions, $options=null)
    {
        ...
        if (!($max_hits_for_in_memory_sort = $o_conf->get('max_hits_for_in_memory_sort'))) {
            $max_hits_for_in_memory_sort = 1000000;
        }
        if (sizeof($hits) < $max_hits_for_in_memory_sort) {
            $sort_mode = SORT_FLAG_CASE;
            if (!$o_conf->get('dont_use_natural_sort')) {
                $sort_mode |= SORT_NATURAL;
            } else {
                $sort_mode |= SORT_STRING;
            }
            ...
            foreach ($hits as $idx => $hit) {
                ...
                foreach ($sortable_values as $i => $sortable_values_level) {
                    if (!sizeof($sortable_values_level)) {
                        continue;
                    }
     ...
     *
     */
    private static function _sortKeys($sort_buffer, $sort_mode, $sort_directions, $level=0)
    {
        ksort($sort_buffer, $sort_mode);
        if (strtolower($sort_directions[$level]) === 'desc') {
            $sort_buffer = array_reverse($sort_buffer);
        }
    ...
    private function _getSortKeysForElement($ps_element_code, $pn_table_num, $pa_hits)
    {
        ...
        foreach ($pa_hits as $id) {
            if (!isset($va_sort_keys[$id])) {
                $va_sort_keys[$id] = [$pad_keys ? '000000000' : ''];
            }
        }
        foreach ($va_sort_keys as $row_id => $keys) {
            if (sizeof(array_filter($keys, function ($v) {
                return is_numeric($v);
            })) > 0) {
    ...
    {
        if (!($t_original_table = Datamodel::getInstanceByTableNum($pn_original_table_num, true))) {
            return false;
        }
        if (!($t_target_table = Datamodel::getInstanceByTableNum($pn_target_table, true))) {
            return false;
        }
