<?php
/** ---------------------------------------------------------------------
 * app/lib/ApplicationPluginManager.php :
 * ----------------------------------------------------------------------
 * CollectiveAccess
...
class ApplicationPluginManager
{
     ...
     */
    public static $s_application_plugin_manager_did_do_plugin_init = false;

    /**
      *
      */
    public static $s_application_plugin_instances = array();

    /**
    *
    */
    public static $s_application_plugin_hooks = array();
    ...
    public static function initPlugins()
    {
        ...
        if (is_dir(__CA_THEME_DIR__."/plugins")) {
            array_unshift($va_plugin_locations, __CA_THEME_DIR__."/plugins");
        }
        ...
        foreach ($va_plugin_locations as $vs_app_plugin_dir) {
            foreach ($va_app_plugin_dirs as $vs_plugin_dir) {
                if (ApplicationPluginManager::$s_application_plugin_instances[$vs_plugin_dir]) {
                    continue;
                }
    ...
    public static function getPluginNames($pa_plugin_locations=null)
    {
        ...
        if (!is_array($pa_plugin_locations)) {
            if (!($pa_plugin_locations[] = $o_config->get('application_plugins'))) {
                return array();
            }
        }
