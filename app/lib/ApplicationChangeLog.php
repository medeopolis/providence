...
/* ]]> */
                    ...
                    foreach ($va_log_entries as $va_log_entry) {
                        ...
                        if (($pn_table_num == $va_log_entry['logged_table_num'])) {
                            foreach ($va_log_entry['snapshot'] as $vs_field => $vs_value) {
                                ...
                                if ($t_user && !$t_user->getBundleAccessLevel($t_item->tableName(), $vs_field)) {
                                    continue;
                                }	// does user have access to this bundle?
                                ...
                                if ((isset($va_field_info['DISPLAY_FIELD'])) && (is_array($va_field_info['DISPLAY_FIELD'])) && ($va_disp_fields = $va_field_info['DISPLAY_FIELD'])) {
                                    ...
                                    if (sizeof($va_keys)) {
                                        ...
                                        if ($t_user && !$t_user->getBundleAccessLevel($t_item->tableName(), $va_keys['one_table'])) {
                                            continue;
                                        }	// does user have access to this bundle?
                        ...
                        if ($va_log_entry['logged_table_num'] == 3) {	// attribute_values
                            if ($t_element = ca_attributes::getElementInstance($va_log_entry['snapshot']['element_id'])) {
                                ...
                                if ($t_user && !$t_user->getBundleAccessLevel($t_item->tableName(), $vs_element_code)) {
                                    continue;
                                }	// does user have access to this bundle?
                        ...
                        if (sizeof($va_keys) > 0) {
                            if (method_exists($t_obj, 'getLeftTableNum')) {
                                ...
                                if ($t_user && !$t_user->getBundleAccessLevel($t_item->tableName(), $t_related_table->tableName())) {
                                    continue;
                                }	// does user have access to this bundle?
    ...
    public function getDeletions($pm_table_name_or_num, $options=null)
    {
        ...
        if ($qr_res = $t_subject->makeSearchResult($vs_table_name, array_keys($va_log))) {
            ...
            while ($qr_res->nextHit()) {
                if (!($vn_row_id = $qr_res->get("{$vs_table_name}.{$vs_pk}"))) {
                    continue;
                }
     ...
     */
    public static function getChangeLogUsers($options=null)
    {
        ...
        if (is_array($daterange_values)) {
            ...
            $params[] = $daterange_values['start'];
            $params[] = $daterange_values['end'];
        }
        ...
        while ($qr->nextRow()) {
            $row = $qr->getRow();
            if (!$qr->get('user_name') || !($qr->get('lname') || $qr->get('fname'))) {
                continue;
            }
            $email = $qr->get('email');
     ...
     */
    public static function getChangeLogUsersForSelect($options=null)
    {
     ...
     */
    public static function getChangeLog($options=null)
    {
     ...
     */
    public static function getChangeData($options=null)
    {
        $o_db = ($trans = caGetOption('transaction', $options, null)) ? $trans->getDb() : new Db();
        $limit_by_unit = caGetOption('limitByUnit', $options, false);

        $table_nums = [];
        if (($tables = caGetOption('tables', $options, null)) && !is_array($tables)) {
            $tables = [$tables];
        }
        if ($table = caGetOption('table', $options, null)) {
            if ($table && !($table_name = Datamodel::getTableName($table))) {
                return null;
            }
            ...
            $table_nums = array_map(function ($v) {
                return Datamodel::getTableNum($v);
            }, $tables);
        }
        $params = [];
        ...
        if ($user_id) {
            ...
            if (sizeof($user_id = array_filter(array_map(function ($v) {
                return (int)$v;
            }, $user_id), function ($v) {
                return ($v > 0);
            })) > 0) {
        ...
        ksort($log);
        return array_reduce($log, function ($c, $i) {
            foreach ($i as $v) {
                if (is_array($v)) {
                    $c[] = $v;
                }
     ...
     */
    private static function makeChangeLog($data, $options=null)
    {
        ...
        if (sizeof($data)) {
            ...
            foreach ($ids_by_table as $table_num => $row_ids) {
                ...
                if (!method_exists($t, 'getLabelTableName') || !$t->getLabelTableName()) {
                    continue;
                }
                    ...
                    foreach ($va_log_entries as $va_log_entry) {
                        ...
                        if (($vn_subject_table_num == $va_log_entry['logged_table_num'])) {
                            foreach ($va_log_entry['snapshot'] as $vs_field => $vs_value) {
                                ...
                                if ($t_user && !$t_user->getBundleAccessLevel($t_logged->tableName(), $vs_field)) {
                                    continue;
                                }	// does user have access to this bundle?
                                ...
                                if ((isset($va_field_info['DISPLAY_FIELD'])) && (is_array($va_field_info['DISPLAY_FIELD'])) && ($va_disp_fields = $va_field_info['DISPLAY_FIELD'])) {
                                    ...
                                    if (sizeof($va_keys)) {
                                        ...
                                        if ($t_user && !$t_user->getBundleAccessLevel($t_logged->tableName(), $va_keys['one_table'])) {
                                            continue;
                                        }	// does user have access to this bundle?
                        ...
                        if ($va_log_entry['logged_table_num'] == 3) {	// attribute_values
                            if ($t_element = ca_attributes::getElementInstance($va_log_entry['snapshot']['element_id'])) {
                                ...
                                if ($t_user && !$t_user->getBundleAccessLevel($t_logged->tableName(), $vs_element_code)) {
                                    continue;
                                }	// does user have access to this bundle?
                        ...
                        if ($t_subject) {
                            if ($t_logged->isRelationship()) {
                                if (method_exists($t_logged, 'getLeftTableNum')) {
                                    ...
                                    if ($t_user && !$t_user->getBundleAccessLevel($t_logged->tableName(), $t_related_table->tableName())) {
                                        continue;
                                    }	// does user have access to this bundle?
