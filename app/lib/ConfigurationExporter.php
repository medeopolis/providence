...
final class ConfigurationExporter
{
    ...
    public function getListsAsDOM()
    {
        ...
        while ($qr_lists->nextRow()) {
            ...
            if (($vn_exclude_lists_larger_than > 0) && ($t_list->numItemsInList($qr_lists->get('list_id')) > $vn_exclude_lists_larger_than)) {
                continue;
            }
        ...
        while ($qr_uis->nextRow()) {
            ...
            if (is_array($va_users) && (sizeof($va_users)>0)) {
                ...
                foreach ($va_users as $va_user_info) {
                    if (($acc = $this->_convertUserGroupAccessToString(intval($va_user_info['access']))) === false) {
                        continue;
                    }
            ...
            if (is_array($va_groups) && (sizeof($va_groups)>0)) {
                ...
                foreach ($va_groups as $va_group_info) {
                    if (($acc = $this->_convertUserGroupAccessToString(intval($va_group_info['access']))) === false) {
                        continue;
                    }
                    $vo_permission = $this->opo_dom->createElement("permission");
            ...
            if (is_array($va_roles) && (sizeof($va_roles)>0)) {
                ...
                foreach ($va_roles as $va_role_info) {
                    if (($acc = $this->_convertUserGroupAccessToString(intval($va_role_info['access']))) === false) {
                        continue;
                    }
                ...
                if (is_array($va_placements)) {
                    ...
                    foreach ($va_placements as $va_placement) {
                        ...
                        if (isset($va_placement['settings']['bundleTypeRestrictions']) && (is_array($va_type_restrictions = $va_placement['settings']['bundleTypeRestrictions']) || strlen($va_type_restrictions))) {
                            if ($va_type_restrictions && !is_array($va_type_restrictions)) {
                                $va_type_restrictions = [$va_type_restrictions];
                            }
    ...
    private function getRelationshipTypesForParentAsDOM($pn_parent_id, $pn_table_num)
    {
        ...
        if (!$qr_types->numRows() && !$this->opn_modified_after) {
            return false;
        }
    ...
    {
        if (!$this->opo_config->get('configuration_export_user_logins')) {
            return null;
        }
