...
final class ConfigurationCheck
{
    ...
    public static function performQuick($options=null)
    {
        ...
        foreach ($va_methods as $vo_method) {
            if (strpos($vo_method->name, "QuickCheck")!==false) {
                if (caGetOption('skipPathChecks', $options, false) && in_array($vo_method->name, ['caUrlRootQuickCheck', 'caBaseDirQuickCheck'])) {
                    continue;
                }
    ...
    public static function DBInnoDBQuickCheck()
    {
        ...
        while ($qr_engines->nextRow()) {
            if (!in_array($qr_engines->get('Support'), array('YES', 'DEFAULT'))) {
                continue;
            }
    ...
    public static function DBOutOfDateQuickCheck()
    {
        ...
        } elseif (($vn_schema_revision = self::getSchemaVersion()) < __CollectiveAccess_Schema_Rev__) {
    ...
    public static function caUrlRootQuickCheck()
    {
        ...
        if (caGetOSFamily() === OS_WIN32) {	// Windows paths are case insensitive
            if (!in_array(strtolower(__CA_URL_ROOT__), array_map(function ($v) {
                return strtolower($v);
            }, $possible_url_roots))) {
    ...
    public static function caBaseDirQuickCheck()
    {
        ...
        if (caGetOSFamily() === OS_WIN32) {	// Windows paths are case insensitive
            if (!in_array(strtolower(__CA_BASE_DIR__), array_map(function ($v) {
                return strtolower($v);
            }, $possible_bases))) {
    ...
    {
        if ((bool)self::$opo_config->get('dont_do_media_dir_expensive_check')) {
            return true;
        }
