...
class BaseFindController extends ActionController
{
            ...
            if ($this->request->config->get($this->ops_tablename.'_breakout_find_by_type_in_submenu') || $this->request->config->get($this->ops_tablename.'_breakout_find_by_type_in_menu')) {
                ...
                if ($res_type_id = $this->opo_result_context->getTypeRestriction($c)) {
                    ...
                    $type_ids = array_map(function ($v) {
                        return (int)$v;
                    }, $t_instance->getTypeList(['idsOnly' => true]));
    ...
    public function Index($pa_options=null)
    {
        ...
        if (!sizeof($va_displays)) {
            $va_displays = ['0' => _t('Default')];
        } // force default display if none are configured
        if (!isset($va_displays[$display_id])) {
            $display_id = array_shift(array_keys($va_displays));
        }
        ...
        if (method_exists($t_instance, 'getApplicableElementCodes')) {
            ...
            foreach ($display_list as $i => $va_display_item) {
                ...
                if ($t_instance->hasField($b = $tmp[1]) || $t_instance->hasField($b = $va_display_item['bundle_name'])) {
                    if ($tmp[0] === $va_display_item['bundle_name']) {
                        $va_display_item['bundle_name'] = $this->ops_tablename.".{$b}";
                    }
        ...
        if ($t_subject = Datamodel::getInstanceByTableName($this->ops_tablename, true)) {
            ...
            if (is_array($id_list) && sizeof($id_list)) {
                ...
                if (preg_match('!^attribute([\d]+)$!', $preferred_version, $m)) {
                    ...
                    $version = 'original';
                    if (!($t_element = ca_metadata_elements::getInstance($element_id = $m[1]))) {
                        throw new ApplicationException(_t('Invalid element_id'));
                    }
                    if (!($t_root = ca_metadata_elements::getInstance($root_id = $t_element->get('ca_metadata_elements.hier_element_id')))) {
                        throw new ApplicationException(_t('Invalid parent element_id'));
                    }
                    ...
                    while ($qr_res->nextHit()) {
                        if (!$element_code) {
                            ...
                            foreach ($paths as $i => $path) {
                                ...
                                if (!caCanRead($this->request->user->getPrimaryKey(), 'ca_object_representations', $representation_id)) {
                                    continue;
                                }
                                ...
                                if ($o_md_conf->get('do_metadata_embedding_for_search_result_media_download')) {
                                    if ($path_with_embedding = caEmbedMediaMetadataIntoFile(
                                        $path,
                                        'ca_objects',
                                        $qr_res->get('ca_objects.object_id'),
                                        caGetListItemIdno($qr_res->get('ca_objects.type_id')),
                                        $representation_id,
                                        $representation_type
                                    )) {
    ...
    {
        if (!$items_per_page = $this->opo_result_context->getItemsPerPage()) {
            $items_per_page = $this->opa_items_per_page[0];
        }
    ...
    public function resultsEditor()
    {
        ...
        // Get attribute sorts
        $this->opa_sorts = array_replace(
            $this->opa_sorts,
            caGetAvailableSortFields(
                $this->ops_tablename,
                $this->opn_type_restriction_id,
    ...
    public function getResultsEditorData()
    {
        ...
        if (($s = (int)$this->request->getParameter('s', pInteger)) < 0) {
            $s = 0;
        }
        if (($c = (int)$this->request->getParameter('c', pInteger)) < 1) {
            $c = 10;
        }
        ...
        $this->opa_sorts = caGetAvailableSortFields(
            $this->ops_tablename,
            $this->opn_type_restriction_id,
    ...
    public function saveResultsEditorData()
    {
        ...
        $response = $t_display->saveResultsEditorData(
            $this->ops_tablename,
            [
    ...
    private function _getDisplayList($display_id)
    {
        ...
        $ret = $t_display->getDisplayListForResultsEditor(
            $this->ops_tablename,
