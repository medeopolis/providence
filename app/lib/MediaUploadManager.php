...
class MediaUploadManager
{
     ...
     *
     */
    public static function newSession($user, $num_files, $size, $source='UPLOADER')
    {
     ...
     *
     */
    public static function findSession($key, $user_id=null)
    {
        if (!$key) {
            throw new MediaUploadSessionDoesNotExistException(_t('Empty session'));
        }
     ...
     *
     */
    public static function completeSession($key, $user_id=null)
    {
     ...
     *
     */
    public static function cancelSession($key, $user_id=null)
    {
        $s = MediaUploadManager::findSession($key, $user_id);
        $s->set('completed_on', _t('now'));
        $s->set('cancelled', 1);
     ...
     *
     */
    public static function getRecent(array $options)
    {
     ...
     * Get upload log
     */
    public static function getLog(array $options)
    {
        $user = caGetOption('user', $options, null);
        ...
        if ($sessions = ca_media_upload_sessions::find($params, ['returnAs' => 'arrays', 'sort' => 'created_on', 'sortDirection' => 'desc', 'allowWildcards' => true])) {
            ...
            $sessions = array_map(function ($s) use ($user_dir_path, $importer_forms, $t_session) {
                ...
                $received_bytes = array_reduce($files_proc, function ($c, $i) {
                    return $c + $i['bytes_received'];
                }, 0);
                $total_bytes = array_reduce($files_proc, function ($c, $i) {
                    return $c + $i['total_bytes'];
                }, 0);
     ...
     *
     */
    public static function getCurrentUploadSessionCount(array $options=null)
    {
        $params = [
            'completed_on' => null,
     ...
     *
     */
    public static function connectionsAvailable(array $options=null)
    {
     ...
     *
     */
    private static function _getUserID($user)
    {
     ...
     */
    public static function getTUSServer(int $user_id)
    {
            ...
            if (isset($rel_path) && strlen($rel_path) && file_exists($fileMeta['file_path'])) {
                ...
                foreach (preg_split('!/!', $rel_path) as $rel_path_dir) {
                    if (strlen($rel_path_dir = preg_replace('![^A-Za-z0-9\-_]+!', '_', $rel_path_dir))) {
                        ...
                        if (file_exists("{$path}/".join("/", $rel_path_proc))) {
                            continue;
                        }
     ...
     *
     */
    private static function updateStorageStats($user_id, $session, $fileMeta)
    {
     ...
     *
     */
    private static function _partialToFinalPath(string $filepath, array $options=null): string
    {
