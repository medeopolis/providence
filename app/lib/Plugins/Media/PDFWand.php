...
class WLPlugMediaPDFWand extends BaseMediaPlugin implements IWLPlugMedia
{
    ...
    public $opo_config;
    public $opo_search_config;
    public $ops_ghostscript_path;
    public $ops_pdftotext_path;
    ...
    #
    public $alternative_extensions = [];

    /**
     * Per-request caching of information extracted from read PDFs
     */
    public static $s_pdf_info_cache = [];
    ...
    */
    ...
    public function divineFileFormat($ps_filepath)
    {
        ...
        if ((isset(WLPlugMediaPDFWand::$s_pdf_info_cache[$ps_filepath]) && ($va_info = WLPlugMediaPDFWand::$s_pdf_info_cache[$ps_filepath])) || (WLPlugMediaPDFWand::$s_pdf_info_cache[$ps_filepath] = $va_info = caGetPDFInfo($ps_filepath))) {
            if (!WLPlugMediaPDFWand::$s_pdf_info_cache[$ps_filepath]) {
                return '';
            }
        ...
        if (sizeof(WLPlugMediaPDFWand::$s_pdf_info_cache) > 100) {
            WLPlugMediaPDFWand::$s_pdf_info_cache = array_slice(WLPlugMediaPDFWand::$s_pdf_info_cache, 50);
        }
    ...
    public function write($ps_filepath, $ps_mimetype, $pa_options=null)
    {
        ...
        if ($ps_mimetype == "application/pdf") {
            if ($this->ops_ghostscript_path && ($compress = strtolower($this->get("compress")))) {
                if (!in_array($compress, ['screen', 'ebook', 'printer', 'prepress', 'default'], true)) {
                    $compress = 'default';
                }
            ...
            if ($vn_scaling_correction == 1) {
                $vn_scaling_correction = 0;
            }
            $vs_res = "72x72";
            ...
            if ($vn_end_page > $this->get('pages')) {
                $vn_end_page = (int)$this->get('pages');
            }
            if ($vn_end_page < 1) {
                $vn_end_page = $vn_start_page;
            }
            if ($vn_start_page > $vn_end_page) {
                $vn_end_page = $vn_start_page;
            }
    ...
    {
        if (!(bool)$this->opo_config->get("document_preview_generate_pages") && (!isset($pa_options['force']) || !$pa_options['force'])) {
            return false;
        }
        ...
        if (($vn_quality = (int)$this->opo_config->get("document_preview_quality")) > 100) {
            $vn_quality = 75;
        }
