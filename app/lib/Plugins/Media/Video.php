...
class WLPlugMediaVideo extends BaseMediaPlugin implements IWLPlugMedia
{
    ...
    public $oproperties = array();
    public $media_metadata = array();

    public $path_to_ffmeg = null;
    ...
    public function read($filepath, $mimetype="", $options=null)
    {
        ...
        if (!$this->media_metadata['mime_type']) {
            return false;
    ...
    public function write($filepath, $mimetype, $options=null)
    {
        ...
        if ($start_secs > (float)$this->get('duration')) {
            $start_secs = 0;
        }
        # is mimetype valid?
        switch($mimetype) {
    ...
    {
        if (!(bool)$this->getAppConfig()->get("video_preview_generate_frames") && (!isset($options['force']) || !$options['force'])) {
            return false;
        }
        if (!caMediaPluginFFmpegInstalled()) {
            return false;
        ...
        if (($previewed_duration = ($start_at - $end_at)) < 0) {
            ...
            if ($previewed_duration == 0) {
                $previewed_duration = 0.1;
            }
        }
