...
class WLPlugMediaGmagick extends BaseMediaPlugin implements IWLPlugMedia
{
     ...
     */
    public static $s_metadata_read_cache = [];
    # ------------------------------------------------
    /**
     *
     */
    ...
    public function write($ps_filepath, $mimetype)
    {
        ...
        if ($mimetype == "image/tilepic") {
            ...
            if (($this->properties["colorspace"]) && ($this->properties["colorspace"] != "default")) {
                ...
                if (!is_null($vn_colorspace)) {
                    $this->handle->setimagecolorspace($vn_colorspace);
                }
            }
    ...
    public function &writePreviews($ps_filepath, $pa_options)
    {
        ...
        if ($num_previews > 1) {
            ...
            do {
                ...
                if ($dont_import_pages_for_tiffs && ($this->get('mimetype') === 'image/tiff')) {
                    break;
                }
    ...
    public function __destruct()
    {
        ...
        if (is_object($this->ohandle)) {
            $this->ohandle->destroy();
        }
        ...
        try {
            ...
            if (WLPlugMediaGmagick::$s_metadata_read_cache[$ps_filepath]) {
                ...
                if (function_exists('exif_read_data') && !($this->opo_config->get('dont_use_exif_read_data'))) {
                    ...
                    if (($vn_exif_size <= $this->opo_config->get('dont_use_exif_read_data_if_larger_than')) && (is_array($va_exif = caSanitizeArray($va_exif_data)))) {
                ...
                if (sizeof(WLPlugMediaGmagick::$s_metadata_read_cache) > 100) {
                    WLPlugMediaGmagick::$s_metadata_read_cache = array_slice(WLPlugMediaGmagick::$s_metadata_read_cache, 50);
                }
