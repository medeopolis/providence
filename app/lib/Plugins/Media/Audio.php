...
class WLPlugMediaAudio extends BaseMediaPlugin implements IWLPlugMedia
{
    ...
    public $input_bitrate;
    public $input_channels;
    public $input_sample_frequency;

    public $opo_config;
    public $ops_path_to_ffmpeg;

    public $ops_mediainfo_path;
    ...
    public function writeClip($ps_filepath, $ps_start, $ps_end, $pa_options=null)
    {
        ...
        if ($o_tc->parse($ps_start)) {
            $vn_start = (float)$o_tc->getSeconds();
        }
        if ($o_tc->parse($ps_end)) {
            $vn_end = (float)$o_tc->getSeconds();
        }
