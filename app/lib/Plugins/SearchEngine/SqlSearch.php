...
class WLPlugSearchEngineSqlSearch extends BaseSearchPlugin implements IWLPlugSearchEngine
{
    ...
    public static $s_word_cache = array();						// cached word-to-word_id values used when indexing
    public static $s_metadata_elements; 						// cached metadata element info
    public static $s_fieldnum_cache = array();				// cached field name-to-number values used when indexing
    ...
    private static $s_doc_content_buffer = [];			// content buffer used when indexing

    private static $s_asis_regexes;
    private static $s_search_tokenizer_regex;
    # -------------------------------------------------------
    public function __construct($po_db=null)
    {
        ...
        $this->opb_do_stemming = (int)trim($this->search_config->get('search_sql_search_do_stemming')) ? true : false;
    ...
    {
        if (($vn_max_indexing_buffer_size = (int)$this->search_config->get('max_indexing_buffer_size')) < 1) {
    ...
    private function initDbStatements()
    {
        ...
        $this->opqr_lookup_word = $this->db->prepare($this->ops_lookup_word_sql);
        ...
        $this->opqr_delete = $this->db->prepare($this->ops_delete_sql);
        ...
        $this->opqr_delete_with_field_num = $this->db->prepare($this->ops_delete_with_field_num_sql);
        ...
        $this->opqr_delete_with_field_num_without_rel_type_id = $this->db->prepare($this->ops_delete_with_field_num_sql_without_rel_type_id);
        ...
        $this->opqr_delete_with_field_row_id = $this->db->prepare($this->ops_delete_with_field_row_id_sql);
        ...
        $this->opqr_delete_with_field_row_id_and_num = $this->db->prepare($this->ops_delete_with_field_row_id_and_num);
        ...
        $this->opqr_delete_dependent_sql = $this->db->prepare($this->ops_delete_dependent_sql);
    }
    # -------------------------------------------------------
    /**
    ...
    {
        $this->db->query("DROP TABLE IF EXISTS {$ps_name}");
        $this->db->query("
    ...
    private function _getElementIDForAccessPoint($pn_subject_tablenum, $ps_access_point)
    {
        ...
        if (strtolower($vs_field) == 'count') {
            if (!is_array($va_rel_type_ids) || !sizeof($va_rel_type_ids)) {
                $va_rel_type_ids = [0];
            }	// for counts must pass "0" as relationship type to pull count for all reltypes in aggregate
            return array(
                    ...
                    switch($vs_op) {
                        case 'AND':
                            if ($vn_i > 0) {
                                ...
                                $qr_res = $this->db->query("TRUNCATE TABLE {$ps_dest_table}");

                                $qr_res = $this->db->query("INSERT INTO {$ps_dest_table} SELECT row_id, boost, field_container_id FROM {$ps_dest_table}_acc");
                                $this->_dropTempTable("{$ps_dest_table}_acc");
                            }
                            break;
                        case 'NOT':
                            $qr_res = $this->db->query("SELECT row_id FROM ca_sql_search_temp_{$pn_level}");

                            if (is_array($va_ids = $qr_res->getAllFieldValues('row_id')) && sizeof($va_ids)) {
                                $vs_sql = "
									DELETE FROM {$ps_dest_table} WHERE row_id IN (?)
								";
                                $qr_res = $this->db->query($vs_sql, array($va_ids));
                            }
                            break;
                        default:
                        case 'OR':
                ...
                case 'Zend_Search_Lucene_Search_Query_Range':
                    ...
                    switch(get_class($o_lucene_query_element)) {
                        case 'Zend_Search_Lucene_Search_Query_Range':
                            ...
                            if (is_array($va_tmp) && (sizeof($va_tmp) > 1) && is_array($va_indexed_fields) && isset($va_indexed_fields[$va_tmp[1]])) {
                                ...
                                if ($t_instance = Datamodel::getInstanceByTableNum($pn_subject_tablenum, true)) {
                                    $vs_direct_sql_query = "
                                        ...
                                        WHERE
                                            (".$va_lower_term->field." BETWEEN {$vn_lower_val} AND {$vn_upper_val})
                                    ";
                                }
                                break;
                            }
                            ...
                            if (($va_element['datatype'] !== 'COUNT') && !isset($va_indexed_fields['_ca_attribute_'.$va_element['element_id']]) && (!$vn_root_element_id || ($vn_root_element_id && !isset($va_indexed_fields['_ca_attribute_'.$vn_root_element_id])))) {
                                break(2);
                            } // skip if not indexed
                                ...
                                case __CA_ATTRIBUTE_VALUE_CURRENCY__:
                                    ...
                                    $vs_direct_sql_query = "
										...
										WHERE
											...
											AND
											(cav.value_longtext1 = '".$this->db->escape($vs_currency)."')
                            ...
                            if ($this->getOption('strictPhraseSearching') && !in_array($vs_first_term_table, array(_t('modified'), _t('created')))) {
                                ...
                                foreach ($o_lucene_query_element->getQueryTerms() as $o_term) {
                                    ...
                                    foreach ($va_terms as $vs_term) {
                                        if (strlen($vs_escaped_text = $this->db->escape($vs_term))) {
								...
								";
                                    $qr_res = $this->db->query($vs_sql, $vs_word, (int)$pn_subject_tablenum);

                                    $qr_count = $this->db->query("SELECT count(*) c FROM {$vs_temp_table}");
                                ...
                                $this->db->query("UPDATE {$vs_results_temp_table} SET row_id = row_id - 1");
                        ...
                        default:
                            ...
                            foreach ($va_term_objs as $o_term) {
                                    ...
                                    if ($vb_has_wildcard) {
                                        ...
                                        if ($vb_do_stemming) {
                                            ...
                                            if (!preg_match('!y$!u', $vs_to_stem) && (preg_match('!^[\d]+$!', $vs_to_stem) || preg_match('!^[\w\-]+$!', $vs_to_stem))) {	// don't stem things ending in 'y' as that can cause problems (eg "Bowery" becomes "Boweri")
                                                ...
                                                $va_ft_stem_terms[] = "'".$this->db->escape($vs_stem)."'";
                                            } else {
                                                $va_ft_terms[] = '"'.$this->db->escape($vs_term).'"';
                                            }
                                        } else {
                                            $va_ft_terms[] = '"'.$this->db->escape($vs_term).'"';
                                        }
                                ...
                                if ($vb_output_term) {
                                    $va_raw_terms[] = $vs_raw_term;
                                    $va_raw_terms_escaped[] = '"'.$this->db->escape($vs_raw_term).'"';
                                }
                            }
                                                    ...
                                                    case __CA_ATTRIBUTE_VALUE_DATERANGE__:
                                                        ...
                                                        switch(substr($vs_raw_term, 0, 4)) {
                                                            ...
                                                            default:
                                                                if ($this->opo_tep->parse($vs_raw_term)) {
                                                                    $va_dates = $this->opo_tep->getHistoricTimestamps();
                                                                    if (((int)$va_dates[0] === -2000000000) && $this->search_config->get('treat_before_dates_as_circa')) {
                                                                        $va_dates[0] = $va_dates['start'] = (int)$va_dates['end'] + 0.1231235959;
                                                                    }
                                                                    if (((int)$va_dates[1] === 2000000000) && $this->search_config->get('treat_after_dates_as_circa')) {
                    ...
                    if ($vb_is_blank_search) {
                        ...
                        if (sizeof($va_ft_terms)) {
                            if (($t_table) && (strlen($vs_fld_num) > 1)) {
                                ...
                                if (!is_array($va_field_info = $o_search->getFieldOptions($pn_subject_tablenum, $vs_table_num, $vs_field))) {
                                    $va_field_info = array();
                                }
                        ...
                        if (sizeof($va_ft_like_terms)) {
                            ...
                            foreach ($va_ft_like_terms as $vs_term) {
                                if ($vb_ft_bit_optimization) {
                                    $va_tmp[] = '(sw.word = \' '.$this->db->escape(trim($vs_term)).' \')';
                                } else {
                                    $va_tmp[] = '(sw.word LIKE \''.$this->db->escape(trim($vs_term)).'%\')';
                                }
                            }
                            if (($t_table) && (strlen($vs_fld_num) > 1)) {
                    ...
                    if (!$vb_is_blank_search && !$vb_is_not_blank_search) {
                        $vb_is_only_blank_searches = false;
                    }
                    ...
                    if ($vn_i == 0) {
                        ...
                        $this->db->query($vs_sql, $pa_direct_sql_query_params);
                        ...
                        switch($vs_op) {
                            case 'AND':
                                ...
                                $qr_res = $this->db->query($vs_sql, $pa_direct_sql_query_params);
                                ...
                                if (is_array($va_ids = $qr_res->getAllFieldValues(($vs_direct_sql_query && ($vn_direct_sql_target_table_num != $pn_subject_tablenum)) ? Datamodel::primaryKey($pn_subject_tablenum) : 'row_id')) && sizeof($va_ids)) {
                                    ...
                                    $this->db->query("DELETE FROM {$ps_dest_table}");
                                }
    ...
    public function startRowIndexing(int $pn_subject_tablenum, int $pn_subject_row_id): void
    {
        ...
        if ($this->debug) {
            Debug::msg("[SqlSearchDebug] startRowIndexing: $pn_subject_tablenum/$pn_subject_row_id");
        }
    ...
    public function indexField(int $pn_content_tablenum, string $ps_content_fieldname, int $pn_content_row_id, $pm_content, ?array $pa_options=null)
    {
        ...
        if ($this->debug) {
            Debug::msg("[SqlSearchDebug] indexField: $pn_content_tablenum/$ps_content_fieldname [$pn_content_row_id] =&gt; $pm_content");
        }
        ...
        if (!isset($pa_options['datatype'])) {
            $pa_options['datatype'] = null;
        }
        ...
        if ((!is_array($pm_content) && !strlen($pm_content)) || !sizeof($pm_content) || (((sizeof($pm_content) == 1) && strlen((string)$pm_content[0]) == 0))) {
            ...
            if (!$vb_tokenize) {
                $va_words = array_merge($va_words, $pm_content);
            }
        }
        ...
        if (!$va_words) {
            ...
            foreach ($va_words as $vs_word) {
                ...
                if (!($vn_word_id = (int)$this->getWordID($vs_word))) {
                    continue;
                }
    ...
    public function getWordID($ps_word)
    {
        ...
        if (mb_strlen($ps_word) > 255) {
            $ps_word = mb_substr($ps_word, 0, 255);
        }
        if (isset(WLPlugSearchEngineSqlSearch::$s_word_cache[$ps_word])) {
            return (int)WLPlugSearchEngineSqlSearch::$s_word_cache[$ps_word];
        }
        ...
        try {
            // insert word
            if (!($vs_stem = trim($this->opo_stemmer->stem($ps_word)))) {
                $vs_stem = $ps_word;
            }
            if (mb_strlen($vs_stem) > 255) {
                $vs_stem = mb_substr($vs_stem, 0, 255);
            }
    ...
    public function updateIndexingInPlace($pn_subject_tablenum, $pa_subject_row_ids, $pn_content_tablenum, $ps_content_fieldnum, $pn_content_container_id, $pn_content_row_id, $ps_content, $pa_options=null)
    {
        ...
        if (caGetOption("INDEX_AS_IDNO", $pa_options, false) || in_array('INDEX_AS_IDNO', $pa_options, true)) {
            ...
            if ($delimiters = caGetOption("IDNO_DELIMITERS", $pa_options, false)) {
                if ($delimiters && !is_array($delimiters)) {
                    $delimiters = [$delimiters];
                }
                if ($delimiters) {
                    $va_values = array_map(function ($v) {
                        return trim($v);
                    }, preg_split('!('.join('|', $delimiters).')!', $ps_content));
                }
            }
    ...
    {
        if (!is_array($pa_word_ids) || !sizeof($pa_word_ids)) {
            return array();
        }
    ...
    {
        if (isset(WLPlugSearchEngineSqlSearch::$s_fieldnum_cache[$pn_table_name_or_num.'/'.$ps_fieldname])) {
            return WLPlugSearchEngineSqlSearch::$s_fieldnum_cache[$pn_table_name_or_num.'/'.$ps_fieldname];
        }
