...
class WLPlugSearchEngineSqlSearch2 extends BaseSearchPlugin implements IWLPlugSearchEngine
{
    ...
    public static $whitespace_tokenizer_regex;
    public static $punctuation_tokenizer_regex;

    private static $word_cache = [];					// cached word-to-word_id values used when indexing
    private static $metadata_elements; 					// cached metadata element info
    private static $fieldnum_cache = [];				// cached field name-to-number values used when indexing
    private static $stop_words = null;
    private static $doc_content_buffer = [];			// content buffer used when indexing

    protected static $filter_stop_words = null;
    ...
    public function __construct($db=null)
    {
        ...
        if (is_null(self::$filter_stop_words)) {
            self::$filter_stop_words = $this->search_config->get('use_stop_words');
        }
        ...
        if (self::$filter_stop_words) {
            if (!is_array(self::$stop_words)) {
                if (CompositeCache::contains('stop_words', 'SqlSearch2')) {
                    ...
                    $langs = array_map(function ($v) {
                        return array_shift(explode('_', $v));
                    }, ca_locales::getCataloguingLocaleCodes());
    ...
    private function _processQueryChangeLog(int $subject_tablenum, Object $term)
    {
        switch(get_class($term)) {
            ...
            case 'Zend_Search_Lucene_Index_Term':
                ...
                break;
            case 'Zend_Search_Lucene_Search_Query_Phrase':
                $text = join(' ', array_map(function ($t) {
                    return $t->text;
                }, $terms = $term->getTerms()));
                $field = $terms[0]->field;
                break;
    ...
    private function _processMetadataDataType(int $subject_tablenum, array $ap, $query)
    {
        ...
        if ($ap['type'] === 'INTRINSIC') {
            ...
            if (!$t_instance->hasField($field_name)) {
                return [];
            }
        ...
        if (is_array($qinfo) && sizeof($qinfo)) {
            $params = $qinfo['params'];
            if ($ap['type'] !== 'INTRINSIC') {
                array_unshift($params, $ap['table_num']);
            }
            ...
            if (!($qr = caMakeSearchResult($ap['table_num'], array_keys($row_ids)))) {
                return [];
            }
    ...
    private function _filterQueryResult(int $subject_tablenum, ?array $hits, array $filters)
    {
        if (is_array($filters) && sizeof($filters) && is_array($hits) && sizeof($hits)) {
            ...
            foreach ($filters as $filter) {
                ...
                if (!($fi = Datamodel::getInstance($tmp[0], true))) {
                    continue;
                }
                if (!$fi->hasField($tmp[1])) {
                    continue;
                }
    ...
    {
        if ($this->debug) {
            Debug::msg("[SqlSearchDebug] startRowIndexing: {$subject_tablenum}/{$subject_row_id}");
        }
    ...
    public function indexField(int $content_tablenum, string $content_fieldname, int $content_row_id, $content, ?array $options=null)
    {
        ...
        if ($this->debug) {
            Debug::msg("[SqlSearchDebug] indexField: $content_tablenum/$content_fieldname [$content_row_id] =&gt; $content");
        }
        ...
        if (in_array('PRIVATE', $options, true)) {
            $options['PRIVATE'] = 1;
        }
        ...
        if ((!is_array($content) && !strlen($content)) || !sizeof($content) || (((sizeof($content) == 1) && strlen((string)$content[0]) == 0)) || ((sizeof($content) === 1) && ((string)mb_strtolower($content[0]) === mb_strtolower(caGetBlankLabelText(Datamodel::getTableName($content_tablenum)))))) {
            ...
            if (!$tokenize) {
                $words = array_merge($words, $content);
            }
        }
        ...
        if (!$words) {
            ...
            foreach ($words as $vs_word) {
                ...
                if (!($word_id = (int)$this->getWordID($vs_word))) {
                    continue;
                }
    ...
    public function getWordID(string $word): ?int
    {
        ...
        if (!strlen($word = trim(mb_strtolower($word, "UTF-8")))) {
            return null;
        }
        if (mb_strlen($word) > 255) {
            $word = mb_substr($word, 0, 255);
        }
        if (isset(WLPlugSearchEngineSqlSearch2::$word_cache[$word])) {
            return (int)WLPlugSearchEngineSqlSearch2::$word_cache[$word];
        }
        ...
        try {
            // insert word
            if (!($stem = trim($this->stemmer->stem($word)))) {
                $stem = $word;
            }
            if (mb_strlen($stem) > 255) {
                $stem = mb_substr($stem, 0, 255);
            }
            ...
            if (!($word_id = (int)$this->opqr_insert_word->getLastInsertID())) {
                return null;
            }
        } catch (Exception $e) {
            if ($qr_res = $this->q_lookup_word->execute($word)) {
    ...
    {
        if (isset(WLPlugSearchEngineSqlSearch2::$fieldnum_cache[$pn_table_name_or_num.'/'.$ps_fieldname])) {
            return WLPlugSearchEngineSqlSearch2::$fieldnum_cache[$pn_table_name_or_num.'/'.$ps_fieldname];
        }
    ...
    private function _getElementIDForAccessPoint($subject_tablenum, $access_point)
    {
        ...
        if ($table === '_fulltext') {
            return null;
        }	// ignore "_fulltext" specifier – just treat as text search
        ...
        if (strtolower($field) == 'count') {
            if (!is_array($rel_type_ids) || !sizeof($rel_type_ids)) {
                $rel_type_ids = [0];
    ...
    private function useSearchIndexForAP(array $ap): bool
    {
        if (is_array($ap['indexing_options'])) {
            foreach (['INDEX_AS_IDNO', 'INDEX_ANCESTORS', 'CHILDREN_INHERIT', 'ANCESTORS_INHERIT', 'INDEX_AS_MIMETYPE'] as $k) {
                if (in_array($k, $ap['indexing_options'], true) || isset($ap['indexing_options'][$k])) {
                    return true;
                }
            }
        }
