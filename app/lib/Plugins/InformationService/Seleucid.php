<?php
/** ---------------------------------------------------------------------
 * app/lib/Plugins/InformationService/Seleucid.php :
 * ----------------------------------------------------------------------
 * CollectiveAccess
...
global $g_information_service_settings_seleucid;
...
class WLPlugInformationServiceSeleucid extends BaseInformationServicePlugin implements IWLPlugInformationService
{
    ...
    public function __construct()
    {
        ...
        WLPlugInformationServiceSeleucid::$s_settings = $g_information_service_settings_seleucid;
        parent::__construct();
    ...
    # ------------------------------------------------
    /**
     * Perform lookup on Seleucid-based data service
     *
    ...
    public function lookup($pa_settings, $ps_search, $pa_options=null)
    {
        ...
        if (isURL($s) && preg_match("!^http://numismatics.org/sco/id/([A-Za-z0-9\.\-]+)$!", $s, $m)) {
            $ps_search = $m[1];
        }

        while ($count <= $maxcount) {
            $vs_data = caQueryExternalWebservice('http://numismatics.org/sco/apis/search?format=rss&q='.urlencode($ps_search));
                ...
                if ($o_xml) {
                    ...
                    if ($o_entries && sizeof($o_entries)) {
                        foreach ($o_entries as $o_entry) {
                            ...
                            $va_items[(string)$o_entry->{'title'}] = array('label' => (string)$o_entry->{'title'}, 'idno' => str_replace("http://numismatics.org/sco/id/", "", (string)$o_entry->{'link'}), 'url' => (string)$o_entry->{'link'});
                            $count++;
                        }
                    }
                }
    ...
    {
        if (!preg_match("!http://numismatics.org/sco/id/([A-Za-z0-9\.\-]+)!", $ps_url, $matches)) {
            return [];
        }
        $id = $matches[1];
        $vs_result = caQueryExternalWebservice("http://numismatics.org/sco/id/{$id}.jsonld");

        if (!$vs_result) {
            return [];
        }
        if (!is_array($va_data = json_decode($vs_result, true))) {
            return [];
        }
