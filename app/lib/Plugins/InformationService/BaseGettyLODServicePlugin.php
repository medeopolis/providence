...
abstract class BaseGettyLODServicePlugin extends BaseInformationServicePlugin
{
    ...
    public static function queryGetty($ps_query, $pa_options = null)
    {
        ...
        $pa_options[ CURLOPT_URL ]            = caGetOption(
            CURLOPT_URL,
            $pa_options,
        ...
        $pa_options[ CURLOPT_USERAGENT ]      = caGetOption(
            CURLOPT_USERAGENT,
            $pa_options,
    ...
    public function getExtendedInformation($pa_settings, $ps_url)
    {
        ...
        foreach ($va_service_conf['detail_view_info'] as $va_node) {
            ...
            if (strlen($vs_content = self::getLiteralFromRDFNode(
                $ps_url,
                $va_node['literal'],
                $vs_uri_for_pull,
                $va_node
     ...
     * @return string|bool
     */
    public static function getLiteralFromRDFNode(
        $ps_base_node,
        $ps_literal_propery,
        $ps_node_uri = null,
        $pa_options = array()
        ...
        if (strlen($ps_node_uri) > 0) {
            $va_pull_graphs = self::getListOfRelatedGraphs(
                $o_graph,
                $ps_base_node,
                $ps_node_uri,
                $pn_limit,
     ...
     */
    public static function getURIAsRDFGraph($ps_uri)
    {
        if (! $ps_uri) {
            return false;
        }
     ...
     * @return array
     */
    public static function getListOfRelatedGraphs($po_graph, $ps_base_node, $ps_node_uri, $pn_limit, $pb_recursive = false)
    {
        $vs_cache_key = md5(serialize(func_get_args()));
        ...
        if ($pb_recursive) {
            ...
            foreach ($va_pull_graphs as $vs_pull_uri => $o_pull_graph) {
                ...
                $va_sub_pull_graphs = array_merge(
                    $va_sub_pull_graphs,
