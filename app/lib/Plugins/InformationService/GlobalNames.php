...
class WLPlugInformationServiceGlobalNames extends BaseInformationServicePlugin implements IWLPlugInformationService
{
    ...
    public function lookup($settings, $search, $options=null)
    {
        ...
        while ($count <= $maxcount) {
            $data_json = caQueryExternalWebservice("http://resolver.globalnames.org/name_resolvers.json?names={$search}&data_source_ids={$source_id}&resolve_once=true&with_canonical_ranks=true");
                    ...
                    foreach ($data['data'] as $block) {
                        ...
                        foreach ($block['results'] as $entry) {
                            $items[(string)$entry['canonical_form']] = [
                                ...
                                'idno' => preg_replace("![^A-Za-z0-9\-\_]+!", "_", (string)$entry['data_source_title']).':'.(string)$entry['gni_uuid'],
                                'url' => "http://resolver.globalnames.org/name_resolvers.json?names=".urlencode($entry['canonical_form'])."&data_source_ids=".(string)$entry['data_source_id']."&resolve_once=true&best_match_only=true&with_canonical_ranks=true"
                            ];
                            $count++;
                        }
                    }
