...
class WLPlugGeographicMapLeaflet extends BaseGeographicMapPlugIn implements IWLPlugGeographicMap
{
    ...
    public function render($ps_format, $pa_options=null)
    {
        ...
        if (!($vs_path_color = caGetOption('pathColor', $pa_options, $this->opo_config->get('leaflet_maps_path_color')))) {
            $vs_path_color = '#ff0000';
        }
        if (($vn_path_weight = caGetOption('pathWeight', $pa_options, $this->opo_config->get('leaflet_maps_path_weight'))) < 1) {
            $vn_path_weight = 1;
        }
        if (($vn_path_opacity = caGetOption('pathOpacity', $pa_options, $this->opo_config->get('leaflet_maps_path_opacity'))) < 0) {
            $vn_path_opacity = 1;
        }

        if (!($vs_fill_color = caGetOption('fillColor', $pa_options, $this->opo_config->get('leaflet_maps_fill_color')))) {
            $vs_fill_color = '#ff0000';
        }
        if (($vn_fill_opacity = caGetOption('fillOpacity', $pa_options, $this->opo_config->get('leaflet_maps_fill_opacity'))) < 0) {
            $vn_fill_opacity = 1;
        }
...
<script type='text/javascript'>
    ...
    public function getAttributeBundleHTML($pa_element_info, $pa_options=null)
    {
        ...
        if ($default_location = caGetOption('defaultLocation', $pa_element_info['settings'], null)) {
            if (!caParseGISSearch($default_location)) {
