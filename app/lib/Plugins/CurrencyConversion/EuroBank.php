...
class WLPlugCurrencyConversionEuroBank extends BaseCurrencyConversionPlugIn implements IWLPlugCurrencyConversion
{
     ...
     */
    public const CONVERSION_SERVICE_URL = "http://www.ecb.europa.eu/stats/eurofxref/eurofxref-daily.xml";
    # ------------------------------------------------
    /**
     *
     */
    ...
    public static function convert($ps_value, $ps_to, $pa_options=null)
    {
        ...
        // or 1
        } elseif (preg_match("!^([\d\.\,]+)([^\d]+)$!", trim($ps_value), $va_matches)) {
            $vs_decimal_value = (float)$va_matches[1];
            $vs_currency_specifier = trim($va_matches[2]);
        // or 2
        } elseif (preg_match("!(^[\d\,\.]+$)!", trim($ps_value), $va_matches)) {
            $vs_decimal_value = (float)$va_matches[1];
