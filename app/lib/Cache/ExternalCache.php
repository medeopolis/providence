...
class ExternalCache
{
    ...
    {
        if (!in_array($pn_method, [Stash\Invalidation::NONE, Stash\Invalidation::PRECOMPUTE, Stash\Invalidation::OLD, Stash\Invalidation::VALUE, Stash\Invalidation::SLEEP])) {
            return false;
        }
    ...
    public static function fetch($ps_key, $ps_namespace='default')
    {
        ...
        if (!is_null(self::$invalidation_method)) {
            $item->setInvalidationMethod(self::$invalidation_method);
        }
