...
class Attribute
{
    ...
    public static $s_attribute_types = array();
     ...
     *
     */
    public static function renderDataType($pa_element_info, $pa_options=null)
    {
     ...
     *
     */
    public static function getValueDefault($pa_element_info)
    {
        $vs_element = Attribute::getValueInstance($pa_element_info['datatype']);
     ...
     *
     */
    public static function getValueInstance($pn_datatype, $pa_value_array=null, $pb_use_cache=false)
    {
        ...
        if (isset($va_types[$pn_datatype])) {
            ...
            if (!class_exists($vs_classname)) {
                if (!file_exists(__CA_LIB_DIR__.'/Attributes/Values/'.$vs_classname.'.php')) {
                    return null;
                }
     ...
     */
    public static function getSortFieldForDatatype($pn_datatype)
    {
     ...
     */
    public static function getValueDefaultSettingForDatatype($pn_datatype)
    {
        if ($t_instance = Attribute::getValueInstance($pn_datatype, null, true)) {
     ...
     *
     */
    public static function attributeTypes(?int $value=null, array $options=null)
    {
        $short_names = caGetOption('short', $options, false);
        $constants = get_defined_constants(true);
        $attr_types = [];
        foreach ($constants['user'] as $name => $v) {
            if (substr($name, 0, 21) !== '__CA_ATTRIBUTE_VALUE_') {
                continue;
            }

            if (!is_null($value) && ($value === $v)) {
                return $short_names ? substr($name, 21, strlen($name) - 23) : $name;
            }
            $attr_types[$short_names ? substr($name, 21, strlen($name) - 23) : $name] = $v;
        }
