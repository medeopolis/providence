...
class DateRangeAttributeValue extends AttributeValue implements IAttributeValue
{
     ...
     */
    private static $o_tep;

    /**
     *
     */
    private static $o_search_config;

    /**
     *
     */
    private static $o_lang;
     ...
     * @var array
     */
    private static $s_date_cache = array();
    # ------------------------------------------------------------------
    ...
    {
        if (!self::$o_search_config) {
            self::$o_search_config = caGetSearchConfig();
        };
        if (!self::$o_lang) {
            self::$o_lang = TimeExpressionParser::getSettingsForLanguage(self::$locale);
        }
        ...
        return ($d) ? array_map(function ($v) use ($d) {
            return "{$v} {$d}";
        }, $circa_indicators) : [];
    }
    # ------------------------------------------------------------------
