...
abstract class AuthorityAttributeValue extends AttributeValue
{
    ...
    public function htmlFormElement($pa_element_info, $pa_options=null)
    {
        ...
        if ($pa_options['request']) {
            if ($va_restrict_to_types = array_filter(caGetOption('restrictToTypes', $va_settings, [], ['castTo' => 'array']), function ($v) {
                return strlen($v);
            })) {
        ...
        $vs_quickadd_url = caNavUrl(
            $pa_options['request'],
            $va_pieces['module'],
            $va_pieces['controller'],
            $va_pieces['action'],
            array($t_instance->primaryKey() => 0)
        );
