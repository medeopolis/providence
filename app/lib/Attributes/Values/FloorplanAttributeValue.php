...
class FloorPlanAttributeValue extends AttributeValue implements IAttributeValue
{
    ...
    public function htmlFormElement($pa_element_info, $pa_options=null)
    {
        ...
        if (!($po_request = caGetOption('request', $pa_options, null))) {
            return _t('Floor plan is not supported outside of a request context');
        }
        ...
        if (!is_array($va_floor_plan_annotations = json_decode($t_subject->get($pa_element_info['element_code'])))) {
            $va_floor_plan_annotations = [];
        }
