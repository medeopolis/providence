...
class DisplayTemplateParser
{
     ...
     *
     */
    public static $template_cache = null;

    /**
     *
     */
    public static $value_cache = null;

    /**
     *
     */
    public static $value_count_cache = null;

    /**
     *
     */
    public static $join_tag_vals = [];

    # -------------------------------------------------------------------
    /**
     *  Statically evaluate an expression, returning the value
     */
    public static function evaluate($ps_template, $pm_tablename_or_num, $pa_row_ids, $pa_options=null)
    {
    ...
    public static function process($ps_template, $pm_tablename_or_num, array $pa_row_ids, array $pa_options=null)
    {
        ...
        while ($qr_res->nextHit()) {
            // skip deleted
            if ($vb_check_deleted && ($qr_res->get("{$ps_tablename}.deleted") !== '0')) {
                continue;
            }
            ...
            if ($pa_options['relativeToContainer']) {
                ...
                if (is_array($va_vals) && sizeof($va_vals)) {
                    foreach ($va_vals as $vn_index => $va_val_list) {
                        try {
                            if ($ps_skip_when && ExpressionParser::evaluate($ps_skip_when, $va_val_list)) {
                                continue;
                            }
                        } catch (Exception $e) {
                            // noop
        ...
        if (caGetOption('makeLink', $pa_options, true)) {
            $va_proc_templates = caCreateLinksFromText(
                ...
                $pa_row_ids,
                null,
                caGetOption('linkTarget', $pa_options, null),
            ...
            switch($vs_tag = $o_node->tag) {
                ...
                case 'if':
                    ...
                    foreach ($va_codes as $vs_code) {
                        ...
                        if ($ps_relative_to && !Datamodel::tableExists($va_code[0])) {
                            $vs_code = "{$ps_relative_to}.{$vs_code}";
                        }
            ...
            switch($vs_tag = strtolower($o_node->tag)) {
                ...
                case 'ifnotdef':
                    ...
                    if (!is_null($vb_omit_blanks)) {
                        $opts['includeBlankValuesInArray'] = !$vb_omit_blanks;
                    }
                ...
                case 'ifcount':
                    ...
                    if (($vn_limit = ($vn_max > 0) ? $vn_max : $vn_min) == 0) {
                        $vn_limit = 1;
                    }
                    $vn_limit++;
                    foreach ($va_codes as $vs_code) {
                        if ($vs_filter_regex) {
                            ...
                            $vals = array_filter($vals, function ($v) use ($vs_filter_regex) {
                                return preg_match($vs_filter_regex, $v);
                            });
                ...
                case 'unit':
                    ...
                    if ($va_relative_to_tmp[0] && !($t_rel_instance = Datamodel::getInstanceByTableName($va_relative_to_tmp[0], true))) {
                        continue(2);
                    }
                    ...
                    if (
                        ...
                        if ($vs_unit_filter_regex) {
                            $va_tmpl_val = array_filter($va_tmpl_val, function ($v) use ($vs_unit_filter_regex) {
                                return preg_match($vs_unit_filter_regex, $v);
                            });
                        }
                        ...
                        switch(strtolower($va_relative_to_tmp[1])) {
                            case 'hierarchy':
                                if (!is_array($va_relative_ids = $pr_res->get($t_rel_instance->tableName().".hierarchy.".$t_rel_instance->primaryKey(), $va_get_options))) {
                                    $va_relative_ids = [];
                                }
                                $va_relative_ids = array_values($va_relative_ids);
                                break;
                            case 'parent':
                                if (!is_array($va_relative_ids = $pr_res->get($t_rel_instance->tableName().".parent.".$t_rel_instance->primaryKey(), $va_get_options))) {
                                    $va_relative_ids = [];
                                }
                                $va_relative_ids = array_values($va_relative_ids);
                                break;
                            case 'children':
                                if (!is_array($va_relative_ids = $pr_res->get($t_rel_instance->tableName().".children.".$t_rel_instance->primaryKey(), $va_get_options))) {
                                    $va_relative_ids = [];
                                }
                                $va_relative_ids = array_values($va_relative_ids);
                                break;
                            case 'siblings':
                                if (!is_array($va_relative_ids = $pr_res->get($t_rel_instance->tableName().".siblings.".$t_rel_instance->primaryKey(), $va_get_options))) {
                                    $va_relative_ids = [];
                                }
                                $va_relative_ids = array_values($va_relative_ids);
                                break;
                            ...
                            default:
                                ...
                                if (method_exists($t_instance, 'isSelfRelationship') && $t_instance->isSelfRelationship() && is_array($pa_primary_ids) && isset($pa_primary_ids[$t_rel_instance->tableName()])) {
                                    if (!is_array($va_relative_ids = array_values($t_instance->getRelatedIDsForSelfRelationship($pa_primary_ids[$t_rel_instance->tableName()], array($pr_res->getPrimaryKey()))))) {
                                        $va_relative_ids = [];
                                    }
                                    ...
                                    if (method_exists($t_rel_instance, 'isRelationship') && $t_rel_instance->isRelationship() && ($t_opposite = $t_rel_instance->getInstanceOpposite($t_instance->tableName()))) {
                                        ...
                                        $va_relative_ids = $va_relation_ids = array_map(function ($v) {
                                            return $v['relation_id'];
                                        }, $va_related_items);
                                    } else {
                                        if (!is_array($va_relative_ids = $pr_res->get($t_rel_instance->tableName().".related.".$t_rel_instance->primaryKey(), $va_get_options))) {
                                            $va_relative_ids = [];
                                        }
                                        $va_relative_ids = array_values($va_relative_ids);
                                        ...
                                        $va_relationship_type_ids = array_values(array_map(function ($r) {
                                            return $r['relationship_type_id'];
                                        }, $rels));
                                        $va_relationship_type_orientations = array_values(array_map(function ($r) {
                                            return $r['direction'];
                                        }, $rels));
                                    }
                                    if (is_array($va_relation_ids) && sizeof($va_relation_ids)) {
     ...
     *
     */
    private static function _getValues(SearchResult $pr_res, array $pa_tags, array $pa_options=null)
    {
        unset($pa_options['returnAsArray']);
        ...
        if ($vs_relative_to_container = caGetOption('relativeToContainer', $pa_options, null)) {
            if (DisplayTemplateParser::$value_cache[$vs_cache_key]) {
                ...
                foreach (array_keys($pa_tags) as $vs_tag) {
                    ...
                    if (!preg_match("!^{$vs_relative_to_container}!", $vs_get_spec)) {
                        $vs_get_spec = $vs_relative_to_container.".".$vs_get_spec;
                    }
                ...
                if (isset($pa_options['sort']) && is_array($pa_options['sort']) && sizeof($pa_options['sort'])) {
                    $va_sortables = array();
                    if (!is_array($va_parsed_tag_opts['options'])) {
                        $va_parsed_tag_opts['options'] = [];
                    }
                    ...
                    switch(strtolower($vs_get_spec)) {
                        ...
                        default:
                            ...
                            if (is_array($va_parsed_tag_opts['modifiers']) && (sizeof($va_parsed_tag_opts['modifiers']) > 0)) {
                                $va_val_list = array_map(function ($v) use ($va_parsed_tag_opts) {
                                    return caProcessTemplateTagDirectives($v, $va_parsed_tag_opts['modifiers']);
                                }, $va_val_list);
                            }
                            ...
                            if (sizeof($va_val_list)) {
                                $vb_val_is_set = true;
                            }									// Flag that the data value was set to something
                            break;
                    }
                }
        ...
        if ($vb_rel_type_is_set && $vb_val_is_referenced && !$vb_val_is_set) {
            return [];
        }					// Return nothing when relationship type is set and a value is referenced but not set
     ...
     *
     */
    private static function _getValuesForCodeAttribute(SearchResult $pr_res, $po_node, array $pa_options=null)
    {
        if (!($va_codes = DisplayTemplateParser::_getCodesFromAttribute($po_node))) {
            return [];
        }
        ...
        foreach ($va_codes as $vs_code) {
            ...
            if (!is_array($va_val_list = $pr_res->get($va_parsed_tag_opts['tag'], array_merge($va_parsed_tag_opts['options'], ['filters' => $va_parsed_tag_opts['filters'], 'returnAsArray' => true])))) {
                continue;
            }
    ...
    private static function _evaluateCodeAttribute(SearchResult $pr_res, $po_node, array $pa_options=null)
    {
        ...
        foreach ($va_codes as $vs_code => $vs_bool) {
            if (isset(self::$join_tag_vals[$vs_code]) && strlen(self::$join_tag_vals[$vs_code])) {
                return true;
            }
            $va_parsed_tag_opts = DisplayTemplateParser::_parseTagOpts($vs_code);
            ...
            if (!is_array($va_val_list)) {  // no value
                ...
                if ($filter_regex) {
                    $va_val_list = array_filter($va_val_list, function ($v) use ($filter_regex) {
                        return preg_match($filter_regex, $v);
                    });
                }
                if (!is_null($pn_index)) {
     ...
     *
     */
    private static function _getCodesFromAttribute($po_node, array $pa_options=null)
    {
        ...
        if (!$po_node || !$po_node->{$vs_attribute}) {
            return null;
        }
        $va_codes = preg_split("![ ,;\|]+!", $po_node->{$vs_attribute});
        if ($pb_include_booleans) {
            preg_match_all("![ ,;\|]+!", $po_node->{$vs_attribute}, $va_matches);
            $va_matches = array_shift($va_matches);
        }

        $va_codes = array_filter(array_map(function ($v) {
            if (is_string($v) && (mb_substr($v, 0, 1) === '^')) {
                $v = mb_substr($v, 1);
            }
            return $v;
        }, $va_codes), function ($v) {
     ...
     *
     */
    private static function _getCodesBooleanModeAttribute($po_node, array $pa_options=null)
    {
        $vs_attribute = caGetOption('attribute', $pa_options, 'code');
        $vs_code_list = $po_node->{$vs_attribute};
        if (!$po_node || !$po_node->{$vs_attribute}) {
     ...
     *
     */
    private static function _parseTagOpts($ps_get_spec)
    {
        ...
        $va_tmp = explode('~', $ps_get_spec);
        if (!is_array($va_modifiers = array_slice($va_tmp, 1))) {
            $va_modifiers = null;
        }
        ...
        if (sizeof($va_tag_opt_tmp) > 0) {
            foreach ($va_tag_opt_tmp as $vs_tag_opt_raw) {
                ...
                if (sizeof($va_tag_tmp) == 1) {
                    $va_tag_tmp[1] = true;
                }	// value-less options are considered "true"

                if (in_array($va_tag_tmp[0], $va_skip_opts)) {
                    continue;
                }
    ...
    public static function _processLabelTemplate($t_instance, $ps_template, array $pa_row_ids, array $pa_options)
    {
        ...
        if (!is_array($va_tags) || (sizeof($va_tags) < 1)) {
            return [];
        }
     ...
     *
     */
    private static function createDimensionsUnitMap($ps_template, $pa_options=null)
    {
        $tags = array_map(function ($v) {
            return array_shift(explode("~", $v));
        }, $full_tags = caGetTemplateTags($ps_template));
        $units = array_map(function ($v) {
            return preg_replace("!^units:!i", "", array_shift(array_filter(array_slice(explode("~", $v), 1), function ($x) {
                $t=explode(":", $x);
                return ($t[0] == 'units');
            })));
        }, array_filter($full_tags, function ($v) {
     ...
     * @return string
     */
    private static function _processTemplateSubTemplates($po_nodes, array $pa_values, array &$pa_options=null)
    {
        $pb_is_case = caGetOption('isCase', $pa_options, false, ['castTo' => 'boolean']);
        $pb_mode = caGetOption('mode', $pa_options, 'present');	// value 'present' or 'not_present'
     ...
     * @return bool
     */
    private static function _processTemplateEvaluateCodeAttribute($po_node, $pa_values, array $pa_options=null)
    {
        if (!($va_codes = DisplayTemplateParser::_getCodesFromAttribute($po_node, ['includeBooleans' => true]))) {
            return false;
        }
        ...
        foreach ($va_codes as $vs_code => $vs_bool) {
            ...
            foreach ($vm_val as $v) {
                ...
                if (($vs_bool == 'OR') && $vb_has_value) {
                    return $vb_has_value;
                }
                if (($vs_bool == 'AND') && !$vb_has_value) {
                    return $vb_has_value;
                }
            }
        }
     ...
     * @return string Output of processed template
     */
    public static function processSimpleTemplate($ps_template, $pa_values, &$pa_options=null)
    {
        if (!isset($pa_options['tagIndex'])) {
            $pa_options['tagIndex'] = 0;
        }
        ...
        foreach ($va_tags as $vs_tag) {
            if ($pb_skip_tags_without_values && !isset($pa_values[$vs_tag])) {
                continue;
            }
            ...
            if ($t_instance && !isset($pa_values[$va_tmp[0]])) {
                ...
                if ($pb_quote || (!is_numeric($vs_val) & $pb_quote_strings)) {
     ...
     *
     */
    public static function processJoinTag($pa_vals, $pa_parsed_tag_opts)
    {
        ...
        $vn_unit_count = sizeof(array_reduce($va_acc, function ($A, $B) {
            $A[$B['units']] = true;
            return $A;
        }, []));
        $va_acc = array_map(function ($v) use ($vn_unit_count) {
            return ($vn_unit_count > 1) ? $v['val'] : $v['proc'];
        }, $va_acc);
        ...
        if (is_array($va_labels) && (sizeof($va_labels) > 0)) {
            if (is_null($vn_max_values_to_show_labels) || ($vn_max_values_to_show_labels >= sizeof(array_filter($va_acc, "strlen")))) {
                $va_acc = array_map(function ($v, $l) {
                    return (strlen($v) && strlen($l)) ? trim("{$v} {$l}") : $v;
                }, $va_acc, $va_labels);
            }
        }
     ...
     *
     */
    private static function _makeFilterRegex($filter)
    {
        $filter_regex = null;
        if ($filter = trim($filter)) {
            if (preg_match("!^/.*?/[a-z]*$!", $filter)) { // is regex
                ...
                $filter_terms = array_map(function ($v) {
                    return preg_quote($v, "!");
                }, explode("|", $filter));
     ...
     *
     */
    private static function _setPrimaryRepresentationFiltering($res, $value)
    {
        if (!is_a($res, "SearchResult") || !method_exists($res, "filterNonPrimaryRepresentations")) {
            return null;
        }

        $filter_opt = $value;
        $filter = true;
        if (!is_null($filter_opt) && (!(bool)$filter_opt || ($filter_opt === '0') || (strtolower($filter_opt) === 'no'))) {
            $filter = false;
        }
