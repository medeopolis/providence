...
class DelimitedDataParser implements IDataParser
{
    ...
    public function __construct($ps_delimiter="\t", $ps_text_marker='"', $ps_filepath=null, $pa_options=null)
    {
        ...
        if ($ps_filepath) {
            $this->parse($ps_filepath, $pa_options);
        }
     ...
     */
    public static function load($ps_filepath, $pa_options=null)
    {
    ...
    {
        if ($this->opr_file && ($this->ops_type == 'txt')) {
            fclose($this->opr_file);
        }
    }
    # ----------------------------------------
    /**
    ...
    public function parse($ps_filepath, $pa_options=null)
    {
        ...
        try {
            ...
            if (!($this->opr_file = fopen($ps_filepath, "r"))) {
                return false;
            }
