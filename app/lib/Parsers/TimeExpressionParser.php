...
class TimeExpressionParser
{
     ...
     */
    public static $s_language_settings_list_cache = array();
     ...
     */
    public static $early_mid_late_range_intervals = ['century' => 100, 'decade' => 10];
    public static $early_mid_late_range_lengths = ['century' => 20, 'decade' => 4];
    ...
    private function preprocess($ps_expression)
    {
        ...
        if (!is_array($early = $this->opo_language_settings->getList("earlyQualifier"))) {
            $early = [];
        }
        if (!is_array($mid = $this->opo_language_settings->getList("midQualifier"))) {
            $mid = [];
        }
        if (!is_array($late = $this->opo_language_settings->getList("lateQualifier"))) {
            $late = [];
        }
        ...
        if (!is_array($wordsWithSpaces = $this->opo_language_settings->getList("wordsWithSpaces"))) {
            $wordsWithSpaces = [];
        }
        if (!is_array($wordsWithSpacesReplacements = $this->opo_language_settings->getList("wordsWithSpacesReplacements"))) {
            $wordsWithSpacesReplacements = [];
        }
        ...
        foreach (['dateCircaIndicator', 'beforeQualifier', 'afterQualifier'] as $l) {
            if ($keywords = $this->getLanguageSettingsWordList($l)) {
                ...
                foreach ($keywords as $c) {
                    ...
                    if (!preg_match('!(^|[^A-Za-z]+)'.preg_quote($c, '!').'([\-\d]+)!i', $ps_expression, $m)) {
                        continue;
                    }
        ...
        if (sizeof($va_decade_indicators)) {
            if (
                ...
                if (sizeof($va_matches) === 4) {	// is range of decades with truncated end date
                    if ($va_matches[2] <= 99) {
                        $va_matches[2] += ((int)substr($va_matches[1], 0, 2) * 100);
                    }
    ...
    private function tokenize($ps_expression)
    {
        ...
        if (!is_array($this->opa_tokens)) {
            $this->opa_tokens = [];
        }	// Tokenization can fail if string is invalid UTF-8
    ...
    private function &getToken()
    {
        ...
        if (!is_array($va_ordinal_words = $this->opo_language_settings->getList("ordinalWords"))) {
            $va_ordinal_words = [];
        }
        $va_ordinal_word_alts = array_map(function ($v) use ($ordinal_suffix_default) {
            return preg_replace("!".preg_quote($ordinal_suffix_default, "!")."$!i", "", $v);
        }, $va_ordinal_words);
        ...
        switch(sizeof($va_tmp)) {
            case 2:
                if (!(is_numeric($va_tmp[0]) && ($va_tmp[0] == intval($va_tmp[0])))) {
                    break;
                }
                ...
                if (($va_tmp[0] >= 1) && ($va_tmp[0] <= 12)) {
                    ...
                    $va_next_tok = $this->peekToken();
                    if ($va_next_tok['type'] == TEP_TOKEN_MERIDIAN) {
                        break;
                    }		// is time
            ...
            case 3:
                ...
                if (!(is_numeric($va_tmp[1]) && ($va_tmp[1] == intval($va_tmp[1])))) {
                    break;
                }

                if (preg_match('!^[\?]+$!', $va_tmp[2])) {
                    $va_tmp[2] = 0;
                }
                if (!(is_numeric($va_tmp[2]) && ($va_tmp[2] == intval($va_tmp[2])))) {
                    break;
                }
    ...
    # -------------------------------------------------------------------
    public function &peekToken($vn_n=1)
    {
        $vn_c = 0;
    ...
    private function _processParseResults($pa_dates, $pa_options)
    {
        ...
        if (!is_array($pa_dates['end'])) {
            if (($pa_dates['start']['hours'] == 0) && ($pa_dates['start']['minutes'] == 0) && ($pa_dates['start']['seconds'] == 0)) {
                ...
                $pa_dates['end']['hours'] = 23;
                $pa_dates['end']['minutes'] = 59;
                $pa_dates['end']['seconds'] = 59;
            } else {
                $pa_dates['end'] = $pa_dates['start'];
            }
        }
        ...
        if (!$pa_dates['start']['day'] && !$pa_dates['start']['month'] && !$pa_dates['start']['year']) {
            ...
            if (($pa_dates['start']['year'] === TEP_START_OF_UNIVERSE) && ($pa_dates['end']['year'] !== TEP_END_OF_UNIVERSE)) {
                ...
                if ($pa_dates['end']['year'] === null) {
                    $pa_dates['end']['year'] = date("Y");
                }
                if ($pa_dates['end']['day'] === null) {
                    $pa_dates['end']['day'] = $this->daysInMonth($pa_dates['end']['month'], $pa_dates['end']['year']);
                }
            }

            if (($pa_dates['start']['year'] !== TEP_START_OF_UNIVERSE) && ($pa_dates['end']['year'] === TEP_END_OF_UNIVERSE)) {
                if ($pa_dates['start']['month'] === null) {
                    $pa_dates['start']['month'] = 1;
                }
                if ($pa_dates['start']['day'] === null) {
                    $pa_dates['start']['day'] = 1;
                }
                if ($pa_dates['start']['year'] === null) {
                    $pa_dates['start']['year'] = date("Y");
                }
            }
            ...
            if (($pa_dates['start']['day'] === null) && ($pa_dates['end']['day'] === null) && ($pa_dates['start']['year'] != TEP_START_OF_UNIVERSE) && $pa_dates['end']['year'] != TEP_END_OF_UNIVERSE) {
                $pa_dates['start']['day'] = 1;
                if (!$pa_dates['start']['month']) {
                    $pa_dates['start']['month'] = 1;
                }
                if (!$pa_dates['end']['month']) {
                    $pa_dates['end']['month'] = 12;
                }
            ...
            if ($pa_dates['start']['hours'] === null) {
                $pa_dates['start']['hours'] = 0;
            }
            if ($pa_dates['start']['minutes'] === null) {
                $pa_dates['start']['minutes'] = 0;
            }
            if ($pa_dates['start']['seconds'] === null) {
                $pa_dates['start']['seconds'] = 0;
            }
            if ($pa_dates['end']['hours'] === null) {
                $pa_dates['end']['hours'] = 23;
            }
            if ($pa_dates['end']['minutes'] === null) {
                $pa_dates['end']['minutes'] = 59;
            }
            if ($pa_dates['end']['seconds'] === null) {
                $pa_dates['end']['seconds'] = 59;
            }
            ...
            if (
                ...
                $vn_start_unixtime = mktime(
                    $pa_dates['start']['hours'],
                    $pa_dates['start']['minutes'],
                    $pa_dates['start']['seconds'],
                    $pa_dates['start']['month'],
                    $pa_dates['start']['day'],
                    $pa_dates['start']['year']
                );
                $vn_end_unixtime = mktime(
                    $pa_dates['end']['hours'],
                    $pa_dates['end']['minutes'],
                    $pa_dates['end']['seconds'],
                    $pa_dates['end']['month'],
                    $pa_dates['end']['day'],
                    $pa_dates['end']['year']
                );
            ...
            if ($pa_dates['start']['uncertainty'] > 0) {
                ...
                if ($vn_start_attributes > 1) {
                    $vn_start_uncertainty = intval($pa_dates['start']['uncertainty']);
                }
            }
            ...
            if ($pa_dates['end']['uncertainty'] > 0) {
                ...
                if ($vn_end_attributes > 1) {
                    $vn_end_uncertainty = intval($pa_dates['end']['uncertainty']);
                }
            }
    ...
    public function getText($pa_options=null)
    {
        ...
        if (($va_dates['start'] === null) && ($va_dates['end'] === null)) {
            if ($pa_options['isLifespan'] || !$pa_options['showUndated']) {
                return '';
            }	// no "undated" for lifedates
        ...
        if (isset($va_dates['start']) && ($va_dates['start'] != null)) {
                ...
                for ($vn_i=0; $vn_i < strlen($pa_options['format']); $vn_i++) {
                    ...
                    switch($vs_c = $pa_options['format'][$vn_i]) {
                        case 'Y':
                            ...
                            if (($vn_year == TEP_START_OF_UNIVERSE) || ($vn_year == TEP_END_OF_UNIVERSE)) {
                                $vn_year = null;
                            }
                            $vs_output .= $vn_year;
                            break;
            ...
            if ($va_dates['start'] <= TEP_START_OF_UNIVERSE) {
                ...
                if ($va_end_pieces['hours'] == 23 && $va_end_pieces['minutes'] == 59 && $va_end_pieces['seconds'] == 59) {
                    if ($va_end_pieces['day'] == 31 && $va_end_pieces['month'] == 12) {
                        ...
                        if ($va_end_pieces['day'] == $this->daysInMonth($va_end_pieces['month'], $va_end_pieces['year'])) {
                            unset($va_end_pieces['day']);
                        }
            ...
            if ($va_dates['end'] >= TEP_END_OF_UNIVERSE) {
                if ($va_start_pieces['hours'] == 0 && $va_start_pieces['minutes'] == 0 && $va_start_pieces['seconds'] == 0) {
                    if ($va_start_pieces['day'] == 1 && $va_start_pieces['month'] == 1) {
                        ...
                        if ($va_start_pieces['day'] == 1) {
                            unset($va_start_pieces['day']);
                        }
            ...
            if ($va_start_pieces['is_circa']) {
                $vs_start_circa = $vs_circa_indicator.' ';
            }
            if ($va_end_pieces['is_circa'] && !$va_start_pieces['is_circa']) {
                $vs_end_circa = $vs_circa_indicator.' ';
            }
            if ($va_start_pieces['is_probably']) {
                $vs_start_circa = $vs_probably_indicator.' ';
            }
            if ($va_end_pieces['is_probably'] && !$va_start_pieces['is_probably']) {
                $vs_end_circa = $vs_probably_indicator.' ';
            }
    ...
    private function _dateToText($pa_date_pieces, $pa_options=null)
    {
        ...
        if ($pa_options['dateFormat'] === 'ymd') {
            $va_date[] = $vs_year;
            if ($vs_month) {
                $va_date[] = sprintf("%02d", $vs_month);
            }
            if ($vs_day) {
                $va_date[] =  sprintf("%02d", $vs_day);
            }
            return join($vs_date_delimiter, $va_date);
        } elseif ($vb_month_comes_first) {
            if ($vs_month) {
                $va_date[] = (($pa_options['dateFormat'] == 'delimited') ? sprintf("%02d", $vs_month) : $vs_month);
            }
            if ($vs_day) {
                if (
     ...
     */
    public static function getSettingsForLanguage($ps_iso_code)
    {
        $vs_config_path = __CA_LIB_DIR__.'/Parsers/TimeExpressionParser/'.$ps_iso_code.'.lang';
        if (!file_exists($vs_config_path)) {
            return null;
        }
    ...
    {
        if (TimeExpressionParser::$s_language_settings_list_cache[$this->ops_language][$ps_key]) {
            return TimeExpressionParser::$s_language_settings_list_cache[$this->ops_language][$ps_key];
        }
    ...
    public function &getHistoricDateParts($pn_historic_date)
    {
        ...
        if (!$vs_uncertainty_units) {
            $vs_uncertainty_units = ((intval($vn_attributes) >> 2) == 1) ? 'y' : null;
        }
    ...
    public function getISODateTime($pa_date, $ps_mode='START', $pa_options=null)
    {
        ...
        if (!$pa_date['month']) {
            $pa_date['month'] = ($ps_mode == 'END') ? 12 : 1;
        }
        if (!$pa_date['day']) {
            $pa_date['day'] = ($ps_mode == 'END') ? 31 : 1;
        }

        if (($pa_date['year'] == TEP_END_OF_UNIVERSE) && !caGetOption('returnUnbounded', $pa_options, false)) {
            return '';
        }
        if ($pa_date['year'] == TEP_START_OF_UNIVERSE && !caGetOption('returnUnbounded', $pa_options, false)) {
            return '';
        }
    ...
    public function normalizeDateRange($pn_historic_start, $pn_historic_end, $ps_normalization)
    {
        ...
        switch($ps_normalization) {
            case 'decades':
                ...
                if ($vn_s <= TEP_START_OF_UNIVERSE) {
                    $vn_s = $vn_e;
                }
                if ($vn_e >= TEP_END_OF_UNIVERSE) {
                    $vn_e = $vn_s;
                }

                if (($vn_s <= TEP_START_OF_UNIVERSE) || ($vn_e >= TEP_END_OF_UNIVERSE) || ($vn_s == 0) || ($vn_e == 0)) {
                    break;
                }
            ...
            case 'months':
                ...
                if ($vn_s_year <= TEP_START_OF_UNIVERSE) {
                    $vn_s_year = $vn_e_year;
                    $vn_s_month = 1;
                }
                if ($vn_e_year >= TEP_END_OF_UNIVERSE) {
                    $vn_e_year = $vn_s_year;
                    $vn_e_month = 12;
                }

                if (($vn_s_year <= TEP_START_OF_UNIVERSE) || ($vn_e_year >= TEP_END_OF_UNIVERSE)) {
                    break;
                }
                ...
                if ($vn_s_year <= $vn_e_year) {
                    if (($vn_s_year == $vn_e_year) && ($vn_s_month > $vn_e_month)) {
                        break;
                    }
                    ...
                    for ($vn_y=$vn_s_year; $vn_y <= $vn_e_year; $vn_y++) {
                        if ($vn_y == $vn_s_year) {
                            $vn_start_month = $vn_s_month;
                        } else {
                            $vn_start_month = 1;
                        }
                        if ($vn_y == $vn_e_year) {
                            $vn_end_month = $vn_e_month;
                        } else {
                            $vn_end_month = 12;
                        }
            ...
            case 'days':
                ...
                if ($vn_s_day > ($vn_num_days = $this->daysInMonth($vn_s_month, $vn_s_year))) {
                    $vn_s_day = $vn_num_days;
                }
                ...
                if ($vn_e_day > ($vn_num_days = $this->daysInMonth($vn_e_month, $vn_e_year))) {
                    $vn_e_day = $vn_num_days;
                }
            ...
            case 'years':
                ...
                if (($vn_s <= TEP_START_OF_UNIVERSE) || ($vn_e >= TEP_END_OF_UNIVERSE)) {
                    break;
                }
    ...
    {
        if (!$this->opn_start_historic || !$this->opn_end_historic) {
            return null;
        }
        if ($this->opn_start_historic > $this->opn_end_historic) {
            return null;
    ...
    public static function applyPartOfRangeQualifier($qualifier, $start_or_end, $range_type, $year)
    {
        ...
        if (!in_array($qualifier, [TEP_TOKEN_EARLY, TEP_TOKEN_MID, TEP_TOKEN_LATE])) {
            return $year;
        }
        $start_or_end = strtolower($start_or_end);
        $range_type = strtolower($range_type);
        if (!isset(self::$early_mid_late_range_intervals[$range_type])) {
            return $year;
        }
    ...
    {
        if (!isset($dates['start']) || !is_array($start_pieces = $dates['start'])) {
            return null;
        }
        if (!isset($dates['end']) || !is_array($end_pieces = $dates['end'])) {
            return null;
        }

        if (!is_numeric($start_pieces['year']) || !is_numeric($end_pieces['year'])) {
            return null;
        }
