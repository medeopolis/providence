...
class TilepicParser
{
    ...
    #
    public $mimetype2magick = array(
            "image/gif" 	=> "GIF",
            "image/jpeg"	=> "JPEG",
            "image/png"		=> "PNG",
            "image/tiff"	=> "TIFF"
    );
    public $mimetype2ext = array(
            "image/gif" 	=> "gif",
            "image/jpeg"	=> "jpg",
            "image/png"		=> "png",
            "image/tiff"	=> "tiff"
    ...
    public $opo_config;
    public $ops_imagemagick_path;
    public $ops_graphicsmagick_path;
    ...
    #
    # ------------------------------------------------------------------------------------
    public function isTilepic($ps_filepath)
    {
    ...
    #
    # ------------------------------------------------------------------------------------
    public function encode($ps_filepath, $ps_output_path, $pa_options)
    {
        #
        # Default values for options
        #
        if (($pa_options["layer_ratio"] = (isset($pa_options["layer_ratio"])) ? $pa_options["layer_ratio"] : 2) <= 0) {
            $pa_options["layer_ratio"] = 2;
        }

        if (($pa_options["scale_factor"] = (isset($pa_options["scale_factor"])) ? $pa_options["scale_factor"] : 1) <= 0) {
            $pa_options["scale_factor"] = 1;
        }

        if (($pa_options["quality"] = (isset($pa_options["quality"])) ? $pa_options["quality"] : 75) < 1) {
            $pa_options["quality"] = 75;
        }
        ...
        if (($pa_options["antialiasing"] = (isset($pa_options["antialiasing"])) ? $pa_options["antialiasing"] : 1) < 0) {
            $pa_options["antialiasing"] = 1;
        }
    ...
    # ------------------------------------------------------------------------------------
    public function encode_imagemagick($ps_filepath, $ps_output_path, $pa_options)
    {
    ...
    # ------------------------------------------------------------------------------------
    public function encode_graphicsmagick($ps_filepath, $ps_output_path, $pa_options)
    {
        if (!($vs_tilepic_tmpdir = $this->opo_config->get('tilepic_tmpdir'))) {
            $vs_tilepic_tmpdir = caGetTempDirPath();
        }
        ...
        if ($pa_options["scale_factor"] != 1) {
            ...
            $vs_tmp_basename = tempnam($vs_tilepic_tmpdir, 'tpc_scale_');
            $vs_tmp_fname = $vs_tmp_basename.'.jpg';
            if (!($this->_graphicsMagickProcess(
                $vs_filepath,
                $vs_tmp_fname,
                array(
                    array(
    ...
    # ------------------------------------------------------------------------------------
    public function encode_imagick($ps_filepath, $ps_output_path, $pa_options)
    {
        if (!($magick = $this->mimetype2magick[$pa_options["output_mimetype"]])) {
            $this->error = "Invalid output format";
            return false;
    ...
    # ------------------------------------------------------------------------------------
    public function encode_gmagick($ps_filepath, $ps_output_path, $pa_options)
    {
        if (!($magick = $this->mimetype2magick[$pa_options["output_mimetype"]])) {
            $this->error = "Invalid output format";
            return false;
    ...
    # ------------------------------------------------------------------------------------
    public function encode_gd($ps_filepath, $ps_output_path, $pa_options)
    {
        if (!($vs_tilepic_tmpdir = $this->opo_config->get('tilepic_tmpdir'))) {
            $vs_tilepic_tmpdir = caGetTempDirPath();
        }
    ...
    #
    # ------------------------------------------------------------------------------------
    public function getTile($tile_number)
    {
    ...
    # ------------------------------------------------------------------------------------
    public function writeTiles($ps_dirpath, $ps_filestem="")
    {
        # --- get tile offsets (start of each tile)
        if (($this->fh) || ($this->fh = fopen($this->properties["filepath"], "r"))) {
    ...
    # ------------------------------------------------------------------------------------
    public function getLayer($layer_number, $output_mimetype = "image/jpeg")
    {
    ...
    # ------------------------------------------------------------------------------------
    public function getLayer_imagemagick($layer_number, $output_mimetype)
    {
        ...
        if ($vs_ext = $this->mimetype2ext[$output_mimetype]) {
            $vs_ext = '.'.$vs_ext;
        }
    ...
    # ------------------------------------------------------------------------------------
    public function getLayer_imagick($layer_number, $magick)
    {
        $layer_tiles = $this->getFileGeometry();
        if (!($tile_count = $layer_tiles[$layer_number]['tiles'])) {
            $this->error = "Invalid file";
    ...
    # ------------------------------------------------------------------------------------
    public function getLayer_gd($layer_number, $output_mimetype)
    {
        $layer_tiles = $this->getFileGeometry();
        if (!($tile_count = $layer_tiles[$layer_number]['tiles'])) {
            $this->error = "Invalid file";
    ...
    # ------------------------------------------------------------------------------------
    public function getFileGeometry()
    {
    ...
    #
    public static function unpackLargeInt($pn_the_int)
    {
