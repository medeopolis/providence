...
class XMPParser extends BaseMediaMetadataParser
{
     ...
     */
    public static $s_jpeg_segment_names = array(
    ...
    public function parse($ps_filepath)
    {
        ...
        if (!is_array($va_jpeg_header_data)) {
            $va_jpeg_header_data = array();
        }
    ...
    public function write($ps_filepath=null)
    {
        ...
        foreach ($this->opa_fields as $vs_field => $va_field_info) {
            if (!isset($this->opa_isset[$vs_field])) {
                $this->set($vs_field, '');
            }
        }
...
<?xpacket end="w"?>';
    ...
    {
        if (!isset($this->opa_fields[$ps_field])) {
            return false;
        }	// bad field
    ...
    private function removeJPEGHeaderSegment($ps_segment_name, $pa_jpeg_header_data)
    {
        ...
        foreach ($pa_jpeg_header_data as $vn_segno => $va_segment) {
            if ($va_segment['SegName'] == $ps_segment_name) {
                continue;
            }
     ...
     */
    public function getXMPData($pa_jpeg_header_data)
    {
