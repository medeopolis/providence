...
{
// Public variables
    public $LastError=false; 	// false if no error occured, else error text
    public $Streams=array();	// All the file data stored here

    // Private variables
    public $Data;
    public $cache;
    public $cachedir; 	// shall we use & write cache and where ?
    public $tagvendor; 		// change it to false if you don't want the oggclass to add its name to your vendor field when writting new comments
    public $StreamsUTF8; 		// if true, the vendor and comment information will be in utf8 inside the Streams array. Else, will be ansi.
    public $InputFile;
    public $OutputFile;
    public $MaxTime=0;
    public $refresh;
    ...
    public function Ogg($OggFile, $options=0, $cachedir="/tmp")
    {
        // Check php version
        if (intval(PHP_VERSION)<4) {
            return($this->creturn("PHP version must be >= 4 but we show PHP v".PHP_VERSION));
        }
        ...
        if ((strpos($OggFile, "://")===false)&&!file_exists($this->realpath($OggFile))) {
            return ($this->creturn("OGG file does not exist: ".$OggFile));
        }

        // handle caching
        if ($this->cache) {
            $this->cachedir=rtrim($cachedir, "/");
            if (!is_dir($this->realpath($this->cachedir))) {
                @mkdir($this->realpath($this->cachedir));
            }
            ...
            if ((($options&UPDATECACHE)==0)&&is_resource($cachefile = @fopen($this->realpath($this->Streams['cachefile']), "r"))) {
                if ($s=fread($cachefile, filesize($this->realpath($this->Streams['cachefile'])))) {
                    ...
                    if (isset($this->Streams['summary'])) {
                        $this->Streams['summary']=$this->Decode($this->Streams['summary']);
                    }
                    if (isset($this->Streams['theora']['vendor'])) {
                        $this->Streams['theora']['vendor']=$this->Decode($this->Streams['theora']['vendor']);
                    }
                    if (isset($this->Streams['vorbis']['vendor'])) {
                        $this->Streams['vorbis']['vendor']=$this->Decode($this->Streams['vorbis']['vendor']);
                    }
                    if (isset($this->Streams['theora']['comments'])) {
                        foreach ($this->Streams['theora']['comments'] as $key => $comment) {
                            $this->Streams['theora']['comments'][$key]=$this->Decode($comment);
                        }
                    }
                    if (isset($this->Streams['vorbis']['comments'])) {
                        foreach ($this->Streams['vorbis']['comments'] as $key => $comment) {
                            $this->Streams['vorbis']['comments'][$key]=$this->Decode($comment);
                        }
                    }
                    ...
                    if ((strpos($OggFile, "://")===false)&&($this->Streams['size']!=filesize($this->realpath($this->Streams['oggfile'])))) {
                        unset($this->Streams['source']);
                    }
                    $this->creturn(true);
                }
                fclose($cachefile);
...
// Public methods
    public function GetPicture($framepos=1, $where=false) //return $path of picture or false; $frame=frame number; $where=path to file to create, if false will be in cache;
    {
    ...
    public function WriteNewComments($refresh=5)
    {
        if ((strpos($this->Streams['oggfile'], "://")!==false)) {
            return ($this->creturn("It is not possible to change the comment tags on a remote server ! "));
        }

        $this->InputFile= fopen($this->realpath($this->Streams['oggfile']), "rb");
        if (!is_resource($this->InputFile)) {
            return ($this->creturn("Could not open OGG file ".$this->Streams['oggfile']));
        }
        ...
        if (!$vorbispacket && !$theorapacket) {
            return ($this->creturn("Could not find a stream to comment !"));
        }

        // Make sure the refresh time will not exceed the script time limit
        if ((get_cfg_var("max_execution_time")>0)&&($refresh>=get_cfg_var("max_execution_time"))) {
            $this->refresh=get_cfg_var("max_execution_time")-2;
        } else {
        ...
        if (!is_resource($this->OutputFile)) {
            return ($this->creturn("Could not create temp file ".$this->Streams['tmpfile']));
        }

        if ($vorbispacket&&$theorapacket) {
            if ($this->Streams['vorbis']['commentpos']<$this->Streams['theora']['commentpos']) {
                if ($this->Streams['vorbis']['commentpos']>0) {
                    fwrite($this->OutputFile, substr($this->Data, 0, $this->Streams['vorbis']['commentpos']));
                }
                ...
                if ($this->Streams['theora']['commentnext']<strlen($this->Data)) {
                    fwrite($this->OutputFile, substr($this->Data, $this->Streams['theora']['commentnext']));
                }
            } else {
                if ($this->Streams['theora']['commentpos']>0) {
                    fwrite($this->OutputFile, substr($this->Data, 0, $this->Streams['theora']['commentpos']));
                }
                fwrite($this->OutputFile, $theorapacket);
                ...
                fwrite($this->OutputFile, $vorbispacket);
                if ($this->Streams['vorbis']['commentnext']<strlen($this->Data)) {
                    fwrite($this->OutputFile, substr($this->Data, $this->Streams['vorbis']['commentnext']));
                }
            }
        } elseif ($vorbispacket) {
            if ($this->Streams['vorbis']['commentpos']>0) {
    ...
    public function ContinueWrite()
    {
        if ($this->MaxTime==0) { // not called from WriteNewComments()
            ...
            if (!is_resource($this->OutputFile)) {
                ...
                return ($this->creturn("Could not open tmp file ".$this->Streams['tmpfile']));
            }
            $this->MaxTime=time()+$this->refresh;
        ...
        while (!feof($this->InputFile)) {
            ...
            if (time()>=$this->MaxTime) {
                $interupted=ftell($this->InputFile);
                break;
            }
        }
        fclose($this->InputFile);
    ...
    public function CacheUpdate() // update cache file
    {
        if (!$this->cache) {
            return($this->creturn("CACHING disabled, therefore updating cache is not possible..."));
        }
    ...
    // Private methods
    public function creturn($error=false)
    {
        if (is_string($error)) {
    ...
    public function EncodeUTF8($string)
    {
        return((utf8_encode(utf8_decode($string)) == $string) ? $string : utf8_encode($string));
    }
    public function Read32LE(&$buffer, $pos)
    {
    ...
    public function readBits($nb, &$buffer, &$bitptr) //read and decodes an integer up to 32 bits from the buffer
    {
        if ($nb>32) {
            $nb=32;
        }
    ...
    public function realpath($path) // if a path to a file is absolute, add document_root
    {
    ...
    public function Picturable() //boolean, says if this server/library is able to extract a picture from movie
    {
        if (!isset($this->Streams['theora'])) {
            return($this->creturn("Only for video streams..."));
        }
        if ((strpos($this->Streams['oggfile'], "://")!==false)) {
            return ($this->creturn("Picture extraction not possible on a remote server ! "));
        }
        $extensions=get_loaded_extensions();
        if (!in_array("ffmpeg", $extensions)) {
            return($this->creturn("Module ffmpeg not found on this PHP server !"));
        }
        if ($gd=in_array("gd", $extensions)) {
            $GDArray=gd_info();
            if (intval(ereg_replace('[[:alpha:][:space:]()]+', '', $GDArray['GD Version'])) < 2) {
                $gd=false;
            }
        }
        if (!gd) {
            return($this->creturn("Module gd2 not found on this PHP server !"));
        }
        return($this->creturn(true));
    }


    public function crcOgg(&$str) 			// add a CRC to an ogg page $str (including headers, crc set to 0)
    {
        $crc=0;
        $polynom=0x04C11DB7;
    ...
    public function PackComments($stream)
    {
        if ($this->tagvendor && (strpos($stream['vendor'], OGGCLASSVERSION)===false)) {
            $stream['vendor'].=OGGCLASSVERSION;
        }
        // check that no empty comments or array
        if (isset($stream['comments'])) {
            foreach ($stream['comments'] as $key=>$comment) {
                if (!strlen($comment)) {
                    unset($stream['comments'][$key]);
                }
            }
        } else {
        ...
        foreach ($stream['comments'] as $comment) {
            if (!preg_match("`\A[A-Za-z]*=.+`", $comment)) {
                return($this->creturn("Invalid comment format : ".$comment));
            }
            $data.=pack("V", strlen($this->EncodeUTF8(($comment)))).$this->EncodeUTF8($comment);
        }
        if (isset($stream['channels'])) {
            $data.=chr(0x01);
        }		// vorbis stream adds this
        ...
        if (strlen($stream['commentleftSegments'])>0) {
            $packet.=$stream['commentleftSegments'];
        }
        $packet.=$data;
        if (($stream['commentpos']+$stream['commentlen'])<$stream['commentnext']) {
            $packet.=substr($this->Data, $stream['commentpos']+ord($this->Data[$stream['commentpos']+26])+27+$stream['commentlen'], $stream['commentnext']-$stream['commentpos']-$stream['commentlen']-27-ord($this->Data[$stream['commentpos']+26]));
        }
        $this->crcOgg($packet);
        return($packet);
    }

    public function analyze() // Parse headers to retrieve identification and comments infos
    {
        clearstatcache();
        ...
        if (strpos($this->Data, "OggS")===false) { // not a remote file or could cold read inside it
            ...
            if ($s=@filesize($this->realpath($this->Streams['oggfile']))) {
                $this->Streams['size']=$s;
            }
            ...
            if (! ($this->Data = fread($inputfile, 0xFFFF))) {
                return ($this->creturn("Could not read in OGG file ".$this->Streams['oggfile']));
            }
        }
        ...
        for ($pos=0;($pos=strpos($this->Data, "OggS", $pos))!==false;$pos++) { // parse OGG pages to retrieve interresting data
            ...
            if (isset($this->Streams['cmml']) && $this->Read32LE($this->Data, $pos+14)==$this->Streams['cmml']['serial']) { // read CMML data
                ...
                if (isset($this->Streams['cmml']['text'])) {
                    $this->Streams['cmml']['text'].="\n";
                }
            ...
            if ($this->Read32LE($this->Data, $pos+18)==0) { // Page Count = 0 : we can find the stream type and read identification headers
                if ((substr($this->Data, $packet+1, 6)=="vorbis") && !isset($this->Streams['vorbis'])) { // decode vorbis identification header
                    ...
                    if (ord($this->Data[$packet]) != 0x01) {
                        return($this->creturn(sprintf("Incorrect Vorbis Identification Header(0x%x)", ord($this->Data[$packet]))));
                    }
                    if ($this->Read32LE($this->Data, $packet+7)!=0) {
                        return($this->creturn("Incorrect Vorbis stream version = ".$this->Read32LE($this->Data, $packet+7)));
                    }
                    if (($vorbis['channels'] = ord($this->Data[$packet+11])) == 0) {
                        return($this->creturn("Incorrect Vorbis channels number = ".ord($this->Data[$packet+11])));
                    }
                    if (($vorbis['samplerate']= $this->Read32LE($this->Data, $packet+12)) == 0) {
                        return($this->creturn("Incorrect Vorbis sample rate = ".$this->Read32LE($this->Data, $packet+12)));
                    }
                    ...
                    if (ord($this->Data[$packet]) != 0x80) {
                        return($this->creturn(sprintf("Incorrect Theora Identification Header(0x%x)", ord($this->Data[$packet]))));
                    }
                    if ((($theora['vmaj']=ord($this->Data[$packet+7]))!=3) || (($theora['vmin']=ord($this->Data[$packet+8]))!=2)) {
                        return($this->creturn("Incorrect Theora stream version"));
                    }
                    $bitptr=($packet+9)*8;
                    ...
                    if (ord($this->Data[$packet+44])>0) {
                        $this->Streams['skeleton']['utc']=substr($this->Data, $packet+44, 20);
                    }
                ...
                if ($type=="vorbis") {
                    if (ord($this->Data[$packet])!=0x03) {
                        return($this->creturn(sprintf("Incorrect Vorbis Comment Header(0x%x)", ord($this->Data[$packet]))));
                    }
                    $table=&$this->Streams['vorbis'];
                } elseif ($type=="theora") {
                    if (ord($this->Data[$packet])!=0x81) {
                        return($this->creturn(sprintf("Incorrect Theora Comment Header(0x%x)", ord($this->Data[$packet]))));
                    }
            ...
            if (isset($this->Streams['vorbis']['vendor'])&&isset($this->Streams['theora']['vendor'])&&!isset($this->Streams['cmml'])) {
                break;
        ...
        if (isset($this->Streams['skeleton'])) { // read ogg skeleton secondary headers
            for ($pos=0;($pos=strpos($this->Data, "fisbone\0", $pos))!==false;$pos++) { // parse fisbone pages to retrieve interresting data
                ...
                if (strlen($message)>0) {
                    if (isset($this->Streams['theora']) && $this->Streams['theora']['serial']==$serial) {
                        $this->Streams['theora']['skeleton']=$message;
                    }
                    if (isset($this->Streams['vorbis']) && $this->Streams['vorbis']['serial']==$serial) {
                        $this->Streams['vorbis']['skeleton']=$message;
                    }
                }
            }
        }
        ...
        if (isset($this->Streams['size'])&&($this->Streams['size']>0xFFFF)) {
            if (is_resource($inputfile)) { // still open: so this is local file
                ...
                if (! ($endbuffer = fread($inputfile, 0xFFFF))) {
                    return ($this->creturn("Could not read in OGG file ".$this->Streams['oggfile']));
                }
                fclose($inputfile);
            } else {
        ...
        if (isset($this->Streams['theora'])) {
            ...
            if (isset($this->Streams['theora']['duration'])) {
                $this->Streams['summary'].=$this->Streams['theora']['duration']."s ";
            }
            ...
            if ($q=$this->Streams['theora']['qual']) {
                $this->Streams['summary'].=" Q=$q";
            }
        ...
        if (isset($this->Streams['vorbis'])) {
            ...
            $this->Streams['summary'].="): ";
            if (isset($this->Streams['vorbis']['duration'])) {
                $this->Streams['summary'].=$this->Streams['vorbis']['duration']."s ";
            }
