...
trait HistoryTrackingCurrentValueTrait
{
     ...
     *
     */
    public static $s_history_tracking_current_value_type_configuration_cache = [];

    /**
     *
     */
    public static $s_history_tracking_newly_added_current_values = [];

    /**
     *
     */
    public static $s_history_tracking_deleted_current_values = [];

    # ------------------------------------------------------
    /**
     *
     */
    public static function clearHistoryTrackingCurrentValues($options=null)
    {
     ...
     *
     */
    public static function getHistoryTrackingCurrentValuePolicyConfig($options=null)
    {
     ...
     */
    public static function getPolicyConfig($policy)
    {
     ...
     * @return array Configuration array or null if policy is not available.
     */
    public static function policy2bundleconfig($options=null)
    {
        ...
        if (!($policy_table = caGetOption('table', $policy_info, false))) {
            return [];
        }
            ...
            if (is_array($types)) {
                ...
                foreach ($types as $type_list => $config) {
                    ...
                    if (in_array($type_list, ['*', '__default__'])) {
                        ...
                        $tt = array_filter(array_map(function ($v) {
                            return $v['idno'];
                        }, $t_instance->getTypeList()), function ($v) use ($types) {
                            return !isset($types[$v]);
                        });
     ...
     */
    public static function getHistoryTrackingCurrentValuePolicyElement($policy, $table, $type=null, $options=null)
    {
     ...
     * @return array Element array or null if not available.
     */
    public static function historyTrackingPolicyUses($policy, $table, $type=null, $options=null)
    {
        if (is_array($policy = self::getHistoryTrackingCurrentValuePolicy($policy)) && is_array($map = $policy['elements']) && is_array($map[$table])) {
     ...
     * @return bool
     */
    public static function isValidHistoryTrackingCurrentValuePolicy($policy)
    {
     ...
     */
    public static function getHistoryTrackingCurrentValuePolicy($policy, $key=null)
    {
        if ($policy && is_array($history_tracking_policies = self::getHistoryTrackingCurrentValuePolicyConfig()) && is_array($history_tracking_policies['policies']) && is_array($history_tracking_policies['policies'][$policy])) {
            if ($key) {
                return isset($history_tracking_policies['policies'][$policy][$key]) ? $history_tracking_policies['policies'][$policy][$key] : null;
            }
            return $history_tracking_policies['policies'][$policy];
        }
        return null;
     ...
     * @return string Policy name
     */
    public static function getDefaultHistoryTrackingCurrentValuePolicyForTable(?string $table=null, ?array $options=null): ?string
    {
        if (is_null($table)) {
    ...
    public function getInspectorHistoryTrackingDisplayPolicy($key=null)
    {
        ...
        if (!isset($display_config[$table])) {
            return null;
        }
        if (!isset($display_config[$table][$type_code])) {
            $type_code = '__default__';
        }
        ...
        if ($key) {
            return isset($display_config[$table][$type_code][$key]) ? $display_config[$table][$type_code][$key] : null;
        }
    ...
    public static function getConfigurationForHistoryTrackingCurrentValue($policy, $table, $type_id=null, $options=null)
    {
        ...
        if (isset(self::$s_history_tracking_current_value_type_configuration_cache[$cache_key])) {
            return self::$s_history_tracking_current_value_type_configuration_cache[$cache_key];
        }
        $o_config = Configuration::load();
        ...
        if (!($t_instance = Datamodel::getInstance($table, true))) {
            return self::$s_history_tracking_current_value_type_configuration_cache[$cache_key] = null;
        }

        if (isset($map[$table])) {
            if ((!$type_id) && (isset($map[$table]['*']) || isset($map[$table]['__default__']))) {
                return self::$s_history_tracking_current_value_type_configuration_cache[caMakeCacheKeyFromOptions($options, "{$table}/{$type_id}")] = self::$s_history_tracking_current_value_type_configuration_cache[$cache_key] = $map[$table]['*'];
            ...
            if ($type_id) {
                ...
                if ($type && isset($map[$table][$type])) {
                    ...
                    if (!isset($map[$table]['__default__'])) {
                        $map[$table]['__default__'] = [];
                    }
                    if (!isset($map[$table]['*'])) {
                        $map[$table]['*'] = [];
                    }
                    if (!is_array($map[$table]['__default__'])) {
                        $map[$table]['__default__'] = [];
                    }
                    ...
                    if (is_array($facet_display_config) && (isset($facet_display_config[$table]['*']) || isset($facet_display_config[$table]['__default__']))) {
                        if (!isset($facet_display_config[$table]['__default__'])) {
                            $facet_display_config[$table]['__default__'] = [];
                        }
                        if (!isset($facet_display_config[$table]['*'])) {
                            $facet_display_config[$table]['*'] = [];
                        }
    ...
    {
        if (!($row_id = caGetOption('row_id', $options, null)) && !($row_id = $this->getPrimaryKey())) {
            return null;
        }
        if (!self::isValidHistoryTrackingCurrentValuePolicy($policy)) {
            return null;
        }
        ...
        if (!caGetOption('dontCheckRowIDs', $options, false)) {
            foreach ([$values['current_table_num'] => $values['current_row_id'], $values['tracked_table_num'] => $values['tracked_row_id']] as $t => $id) {
                if (!($table = Datamodel::getTableName($t))) {
                    continue;
                }
     ...
     * @throws ApplicationException
     */
    public static function getHistoryTrackingCurrentValuePolicies($table, $options=null)
    {
        ...
        foreach ($policy_config['policies'] as $policy => $policy_info) {
            if ($table !== $policy_info['table']) {
                continue;
            }
            if (is_array($type_restrictions) && is_array($policy_info['restrictToTypes']) && !sizeof(array_intersect($type_restrictions, $policy_info['restrictToTypes']))) {
                continue;
            }
     ...
     * @throws ApplicationException
     */
    public static function getDependentHistoryTrackingCurrentValuePolicies($table, $options=null)
    {
        ...
        foreach ($policy_config['policies'] as $policy => $policy_info) {
            ...
            foreach ($policy_info['elements'] as $dtable => $dinfo) {
                ...
                if (!in_array($table, array_keys($path))) {
                    continue;
                }
                if ($types && !sizeof(array_intersect(array_keys($dinfo), $types))) {
                    continue;
                }
                $policies[$policy] = $policy_info;
                break;
     ...
     */
    public static function getTablesWithHistoryTrackingPolicies($options=null)
    {
        $policy_config = self::getHistoryTrackingCurrentValuePolicyConfig();
        if (!is_array($policy_config) || !isset($policy_config['policies']) || !is_array($policy_config['policies'])) {
            return [];	// No policies are configured
                ...
                if (sizeof($h)) {
                    foreach ($h as $d => $by_date) {
                        foreach ($by_date as $entry) {
                            if ($omit_table && $omit_row_id && ($entry['tracked_table_num'] == $omit_table) && ($entry['tracked_row_id'] == $omit_row_id)) {
                                continue;
                            }
    ...
    public function updateDependentHistoryTrackingCurrentValues($options=null)
    {
        ...
        if (is_array($policies = self::getDependentHistoryTrackingCurrentValuePolicies($this->tableName(), ['type_id' => $this->getTypeID()]))) {
            ...
            foreach ($policies as $policy => $policy_info) {
                ...
                if ($spec_has_date && !$date_has_changed && !$this->get('deleted')) {
                    continue;
                }
                if (!($t = Datamodel::getInstance($policy_info['table'], true))) {
                    return null;
                }
            ...
            if ($num_updated > 0) {
                ExternalCache::flush("historyTrackingContent");
            }
            return true;
        }
        return false;
     ...
     *
     */
    public static function tablesTakeHistoryTracking()
    {
     ...
     *
     */
    public static function historyTrackingBaseTables()
    {
     ...
     *
     */
    public static function isHistoryTrackingCriterion($table)
    {
    ...
    {
        if (!$policy) {
            $policy = $this->getInspectorHistoryTrackingDisplayPolicy('policy');
        }
        if (!self::checkPolicyTypeRestrictions($policy, caGetOption('restrictToTypes', $options, []))) {
            return null;
        }
    ...
    {
        if (!is_array($policy_info = self::getHistoryTrackingCurrentValuePolicy($policy))) {
            return null;
        }
        if (!sizeof($type_restrictions)) {
            return true;
        }
        if (!is_array($policy_info['restrictToTypes']) || !sizeof($policy_info['restrictToTypes'])) {
            return true;
        }
        if (sizeof(array_intersect($type_restrictions, $policy_info['restrictToTypes'])) > 0) {
            return true;
        }
        return false;
    }
    ...
    {
        if (!$this->hasField('home_location_id')) {
            return null;
        }	// home location not supported
        if (!($home_location_id = $this->get('home_location_id'))) {
            return null;
    ...
    public function getHistory($options=null)
    {
        ...
        if ($type_id && !self::checkPolicyTypeRestrictions($policy, [$type_id])) {
            return [];
        }
    ...
    //
        ...
        if (is_array($path = Datamodel::getPath($table, 'ca_object_lots')) && (((sizeof($path) == 2) && ($vn_lot_id = $qr->get("{$table}.lot_id"))) || ((sizeof($path) == 3) && ($path = array_keys($path)) && ($linking_table = $path[1])))) {
            ...
            if (is_array($va_lot_types = caGetOption('ca_object_lots_showTypes', $pa_bundle_settings, null))) {
                ...
                if (caGetOption('ca_object_lots_includeFromChildren', $pa_bundle_settings, false)) {
                    ...
                    if ($pb_show_child_history) {
                        $va_lots = array_merge($va_lots, $va_child_lots);
                    }
                }
                ...
                while ($qr_lots->nextHit()) {
                    if ((string)$qr_lots->get('ca_object_lots.deleted') !== '0') {
                        continue;
                    }	// filter out deleted
                    ...
                    foreach ($va_dates as $va_date) {
                        ...
                        if ($pb_get_current_only && (($va_date['bounds'][0] > $vn_current_date))) {
                            continue;
                        }
        ...
        if (is_array($path = Datamodel::getPath($table, 'ca_loans')) && (sizeof($path) == 3) && ($path = array_keys($path)) && ($linking_table = $path[1])) {
            ...
            if (caGetOption('ca_loans_includeFromChildren', $pa_bundle_settings, false)) {
                $va_child_loans = array_reduce($qr->getWithTemplate("<unit relativeTo='{$table}.children' delimiter=';'>^{$linking_table}.relation_id</unit>", ['returnAsArray' => true]), function ($c, $i) {
                    return array_merge($c, explode(';', $i));
                }, []);
                if ($pb_show_child_history) {
                    $va_loans = array_merge($va_loans, $va_child_loans);
                }
            }
        ...
        if (is_array($path = Datamodel::getPath($table, 'ca_movements')) && (sizeof($path) == 3) && ($path = array_keys($path)) && ($linking_table = $path[1])) {
            ...
            if (caGetOption('ca_movements_includeFromChildren', $pa_bundle_settings, false)) {
                $va_child_movements = array_reduce($qr->getWithTemplate("<unit relativeTo='{$table}.children' delimiter=';'>^{$linking_table}.relation_id</unit>", ['returnAsArray' => true]), function ($c, $i) {
                    return array_merge($c, explode(';', $i));
                }, []);
                if ($pb_show_child_history) {
                    $va_movements = array_merge($va_movements, $va_child_movements);
                }
            }
            ...
            if (is_array($va_occurrence_types = caGetOption('ca_occurrences_showTypes', $pa_bundle_settings, null)) && is_array($va_occurrences)) {
                ...
                foreach ($va_occurrence_types as $vn_type_id) {
                    if (caGetOption("ca_occurrences_{$va_occurrence_type_info[$vn_type_id]['idno']}_includeFromChildren", $pa_bundle_settings, false)) {
                        $va_child_occurrences = array_reduce($qr->getWithTemplate("<unit relativeTo='{$table}.children' delimiter=';'>^{$linking_table}.relation_id</unit>", ['returnAsArray' => true]), function ($c, $i) {
                            return array_merge($c, explode(';', $i));
                        }, []);
                        if ($pb_show_child_history) {
                            $va_occurrences = array_merge($va_occurrences, $va_child_occurrences);
                        }
                    }
                }
            ...
            if (is_array($va_entity_types = caGetOption('ca_entities_showTypes', $pa_bundle_settings, null)) && is_array($va_entities)) {
                ...
                foreach ($va_entity_types as $vn_type_id) {
                    if (caGetOption("ca_entities_{$va_entity_type_info[$vn_type_id]['idno']}_includeFromChildren", $pa_bundle_settings, false)) {
                        $va_child_entities = array_reduce($qr->getWithTemplate("<unit relativeTo='{$table}.children' delimiter=';'>^{$linking_table}.relation_id</unit>", ['returnAsArray' => true]), function ($c, $i) {
                            return array_merge($c, explode(';', $i));
                        }, []);
                        if ($pb_show_child_history) {
                            $va_entities = array_merge($va_entities, $va_child_entities);
                        }
                    }
                }
                ...
                while ($qr_entities->nextHit()) {
                    if ((string)$qr_entities->get('ca_entities.deleted') !== '0') {
                        continue;
                    }	// filter out deleted
                    ...
                    foreach ($va_dates as $va_date) {
                        ...
                        if (!in_array($vn_type_id, $va_entity_types)) {
                            continue;
                        }
                        if ($pb_get_current_only && (($va_date['bounds'][0] > $vn_current_date) || ($va_date['bounds'][1] < $vn_current_date))) {
                            continue;
                        }
        ...
        if (is_array($path = Datamodel::getPath($table, 'ca_collections')) && (sizeof($path) == 3) && ($path = array_keys($path)) && ($linking_table = $path[1])) {
            ...
            if (caGetOption('ca_collections_includeFromChildren', $pa_bundle_settings, false)) {
                $va_child_collections = array_reduce($qr->getWithTemplate("<unit relativeTo='{$table}.children' delimiter=';'>^{$linking_table}.relation_id</unit>", ['returnAsArray' => true]), function ($c, $i) {
                    return array_merge($c, explode(';', $i));
                }, []);
                if ($pb_show_child_history) {
                    $va_collections = array_merge($va_collections, $va_child_collections);
                }
            }
            if (is_array($va_collection_types = caGetOption('ca_collections_showTypes', $pa_bundle_settings, null)) && is_array($va_collections)) {
                ...
                while ($qr_collections->nextHit()) {
                    if ((string)$qr_collections->get('ca_collections.deleted') !== '0') {
                        continue;
                    }	// filter out deleted
                    ...
                    foreach ($va_dates as $va_date) {
                        ...
                        if (!in_array($vn_type_id, $va_collection_types)) {
                            continue;
                        }
                        if ($pb_get_current_only && (($va_date['bounds'][0] > $vn_current_date) || ($va_date['bounds'][1] < $vn_current_date))) {
                            continue;
        ...
        if (is_array($path = Datamodel::getPath($table, 'ca_objects')) && (sizeof($path) == 3) && ($path = array_keys($path)) && ($linking_table = $path[1])) {
            ...
            if (caGetOption('ca_objects_includeFromChildren', $pa_bundle_settings, false)) {
                $va_child_objects = array_reduce($qr->getWithTemplate("<unit relativeTo='{$table}.children' delimiter=';'>^{$linking_table}.relation_id</unit>", ['returnAsArray' => true]), function ($c, $i) {
                    return array_merge($c, explode(';', $i));
                }, []);
                if ($pb_show_child_history) {
                    $va_objects = array_merge($va_objects, $va_child_objects);
                }
            }
            if (is_array($va_object_types = caGetOption('ca_objects_showTypes', $pa_bundle_settings, null)) && is_array($va_objects)) {
                ...
                while ($qr_objects->nextHit()) {
                    if ((string)$qr_objects->get('ca_objects.deleted') !== '0') {
                        continue;
                    }	// filter out deleted
                    ...
                    foreach ($va_dates as $va_date) {
                        ...
                        if (!in_array($vn_type_id, $va_object_types)) {
                            continue;
                        }
                        if ($pb_get_current_only && (($va_date['bounds'][0] > $vn_current_date) || ($va_date['bounds'][1] < $vn_current_date))) {
                            continue;
        ...
        if (is_array($path = Datamodel::getPath($table, 'ca_storage_locations')) && (sizeof($path) == 3) && ($path = array_keys($path)) && ($linking_table = $path[1])) {
            ...
            if (caGetOption('ca_storage_locations_includeFromChildren', $pa_bundle_settings, false)) {
                $va_child_locations = array_reduce($qr->getWithTemplate("<unit relativeTo='{$table}.children' delimiter=';'>^{$linking_table}.relation_id</unit>", ['returnAsArray' => true]), function ($c, $i) {
                    return array_merge($c, explode(';', $i));
                }, []);
                if ($pb_show_child_history) {
                    $va_locations = array_merge($va_locations, $va_child_locations);
                }
            }
            ...
            if (is_array($va_location_types = caGetOption('ca_storage_locations_showRelationshipTypes', $pa_bundle_settings, null)) && is_array($va_locations)) {
                ...
                if ($this->inTransaction()) {
                    $t_location->setTransaction($this->getTransaction());
                }
                ...
                while ($qr_locations->nextHit()) {
                    ...
                    if (sizeof($va_location_types) && sizeof($va_location_types) && !in_array($vn_rel_type_id = $qr_locations->get("{$linking_table}.type_id"), $va_location_types)) {
                        continue;
                    }
        ...
        if (caGetOption('sortDirection', $pa_bundle_settings, 'DESC', ['forceUppercase' => true]) !== 'ASC') {
    ...
    {
        if (!($row_id = caGetOption('row_id', $options, $this->getPrimaryKey()))) {
            return null;
        }
    ...
    public function getContentsForIDs(?string $policy, array $ids, $options=null)
    {
        ...
        if (!$policy) {
            if (!($policy = $this->getDefaultHistoryTrackingCurrentValuePolicy())) {
                return null;
            }
        }
        ...
        if (caGetOption('returnHistoryTrackingData', $options, false)) {
            return $values;
        }
        ...
        if (!($table_name = Datamodel::getTableName($row['table_num']))) {
            return null;
        }
    ...
    public function getHistoryTrackingChronologyHTMLFormBundle($po_request, $ps_form_name, $ps_placement_code, $pa_bundle_settings=null, $pa_options=null)
    {
        ...
        if (is_array($path = Datamodel::getPath($this->tableName(), 'ca_occurrences')) && ($path = array_keys($path)) && (sizeof($path) === 3)) {
            ...
            if (($t_occ_rel = Datamodel::getInstance($linking_table, true)) && ($t_occ = Datamodel::getInstance('ca_occurrences', true))) {
                $va_occ_types_to_show =  array_filter(caGetOption('add_to_occurrence_types', $pa_bundle_settings, array(), ['castTo' => 'array']), function ($v) {
                    return (bool)$v;
                });
                if (sizeof($va_occ_types_to_show) > 0) {
                    ...
                    foreach ($va_occ_types as $vn_type_id => $va_type_info) {
                        if (!in_array($vn_type_id, $va_occ_types_to_show) && !in_array($va_type_info['idno'], $va_occ_types_to_show)) {
                            unset($va_occ_types[$vn_type_id]);
                        }
                    }
                    ...
                    if (!is_array($bundle_config['ca_occurrences_showRelationshipTypes'])) {
                        $bundle_config['ca_occurrences_showRelationshipTypes'] = [];
                    }
        ...
        if (is_array($path = Datamodel::getPath($this->tableName(), 'ca_collections')) && ($path = array_keys($path)) && (sizeof($path) === 3)) {
            ...
            if (($t_coll_rel = Datamodel::getInstance($linking_table, true)) && ($t_coll = Datamodel::getInstance('ca_collections', true))) {
                ...
                foreach ($va_coll_types as $vn_type_id => $va_type_info) {
                    if (!in_array($vn_type_id, $va_coll_types_to_show) && !in_array($va_type_info['idno'], $va_coll_types_to_show)) {
                        unset($va_coll_types[$vn_type_id]);
                    }
                }
                ...
                if (!is_array($bundle_config['ca_collections_showRelationshipTypes'])) {
                    $bundle_config['ca_collections_showRelationshipTypes'] = [];
                }
        ...
        if (is_array($path = Datamodel::getPath($this->tableName(), 'ca_entities')) && ($path = array_keys($path)) && (sizeof($path) === 3)) {
            ...
            if (($t_entity_rel = Datamodel::getInstance($linking_table, true)) && ($t_entity = Datamodel::getInstance('ca_entities', true))) {
                ...
                foreach ($va_entity_types as $vn_type_id => $va_type_info) {
                    if (!in_array($vn_type_id, $va_entity_types_to_show) && !in_array($va_type_info['idno'], $va_entity_types_to_show)) {
                        unset($va_entity_types[$vn_type_id]);
                    }
                }
                ...
                if (!is_array($bundle_config['ca_entities_showRelationshipTypes'])) {
                    $bundle_config['ca_entities_showRelationshipTypes'] = [];
                }
        ...
        if (is_array($path = Datamodel::getPath($this->tableName(), 'ca_loans')) && ($path = array_keys($path)) && (sizeof($path) === 3)) {
            ...
            if ($t_loan_rel = Datamodel::getInstance($linking_table, true)) {
                ...
                if (!is_array($bundle_config['ca_loans_showRelationshipTypes'])) {
                    $bundle_config['ca_loans_showRelationshipTypes'] = [];
                }
        ...
        if (is_array($path = Datamodel::getPath($this->tableName(), 'ca_movements')) && ($path = array_keys($path)) && (sizeof($path) === 3)) {
            ...
            if ($t_movement_rel = Datamodel::getInstance($linking_table, true)) {
                ...
                if (!is_array($bundle_config['ca_movements_showRelationshipTypes'])) {
                    $bundle_config['ca_movements_showRelationshipTypes'] = [];
                }
        ...
        if (is_array($path = Datamodel::getPath($this->tableName(), 'ca_objects')) && ($path = array_keys($path)) && (sizeof($path) === 3)) {
            ...
            if ($t_object_rel = Datamodel::getInstance($linking_table, true)) {
                ...
                if (!is_array($bundle_config['ca_objects_showRelationshipTypes'])) {
                    $bundle_config['ca_objects_showRelationshipTypes'] = [];
                }
        ...
        if (is_array($path = Datamodel::getPath($this->tableName(), 'ca_storage_locations')) && ($path = array_keys($path)) && (sizeof($path) === 3)) {
            ...
            if ($t_location_rel = Datamodel::getInstance($linking_table, true)) {
                ...
                if (!is_array($bundle_config['ca_storage_locations_showRelationshipTypes'])) {
                    $bundle_config['ca_storage_locations_showRelationshipTypes'] = [];
                }
        ...
        $o_view->setVar('child_count', $child_count = sizeof(array_filter($h, function ($v) {
            return sizeof(array_filter($v, function ($x) {
    ...
    public function getHistoryTrackingCurrentValueHTMLFormBundle($po_request, $ps_form_name, $ps_placement_code, $pa_bundle_settings=null, $pa_options=null)
    {
        ...
        if (!($t_last_location = Datamodel::getInstance($last_location['tracked_table_num']))) {
    ...
    public static function getHistoryTrackingChronologyInterstitialElementAddHTMLForm($request, $id_prefix, $subject_table, $settings, $options=null)
    {
        ...
        if ((is_array($interstitial_elements = $settings["{$rel_table}_".($type_idno ? "{$type_idno}_" : "")."setInterstitialElementsOnAdd"])|| is_array($interstitial_elements = $settings["setInterstitialElementsOnAdd"])) && sizeof($interstitial_elements) && ($linking_table = Datamodel::getLinkingTableName($subject_table, $rel_table))) {
            ...
            if (!($t_rel = Datamodel::getInstance($linking_table, true))) {
                return null;
            }
                ...
                if (method_exists($this, "getHistory")) {
                    ...
                    if (is_array($h = $this->getHistory(array('limit' => 1, 'currentOnly' => true, 'policy' => $policy, 'row_id' => $pn_row_id))) && (sizeof($h) > 0)) {
                        ...
                        if (is_array($va_path_components) && $va_path_components['subfield_name']) {
                            ...
                            if (!is_array($path) || (sizeof($path) !== 3)) {
                                return null;
                            }
     ...
     * @return array
     */
    public static function getDependentCurrentValues($table_num, $row_id, $options=null)
    {
        ...
        $rows = array_reduce(array_merge($current, $tracked), function ($c, $i) {
            if (!$i['is_future']) {
                $c[$i['policy']][$i['table_num']][$i['row_id']] = true;
            } return $c;
        }, []);
     ...
     *
     */
    public static function getNewlyAddedCurrentValues()
    {
     ...
     *
     */
    public static function getDeletedCurrentValues()
    {
