...
class PDFRenderer
{
     ...
     *
     */
    public static $s_plugin_codes;

    /**
     *
     */
    public static $WLPDFRenderer_plugin_cache;
    ...
    */
    ...
    public function render($ps_content, $pa_options=null)
    {
        ...
        if (is_array($append = caGetOption('append', $pa_options, null)) && (sizeof($append) > 0) && caMediaPluginGhostscriptInstalled()) {
            if ($content = $this->renderer->render($ps_content, array_merge($pa_options, ['stream' => false]))) {
                ...
                caExec("{$ghostscript_path} -dBATCH -dNOPAUSE -q -sDEVICE=pdfwrite -dPDFSETTINGS=/prepress -sOutputFile={$outfile} {$basefile} ".join(" ", array_map(function ($v) {
                    return caEscapeShellArg($v);
                }, $append)).(caIsPOSIX() ? " 2> /dev/null" : ""), $va_output, $vn_return);
    ...
    {
        if (is_array(PDFRenderer::$s_plugin_codes)) {
        ...
        while (($vs_plugin = readdir($r_dir)) !== false) {
            if ($vs_plugin == "BasePDFRendererPlugin.php") {
                continue;
            }
    ...
    {
        if (preg_match('![^A-Za-z0-9_\-]+!', $ps_plugin_code)) {
            return null;
        }
        if (!file_exists(__CA_LIB_DIR__."/Plugins/PDFRenderer/{$ps_plugin_code}.php")) {
            return null;
        }
     ...
     */
    public static function isValidPageSize($ps_size)
    {
     ...
     */
    public static function isValidOrientation($ps_orientation)
    {
