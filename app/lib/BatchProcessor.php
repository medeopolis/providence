...
{
    # ----------------------------------------
    public const REGEXP_FILENAME_NO_EXT = '/\\.[^.\\s]+$/';
    /**
     *
     */
        ...
        if (isset($pa_options['sendMail']) && $pa_options['sendMail']) {
            if ($vs_email = trim($po_request->user->get('email'))) {
                caSendMessageUsingView(
                    $po_request,
                    array($vs_email => $po_request->user->get('fname').' '.$po_request->user->get('lname')),
                    __CA_ADMIN_EMAIL__,
                    _t('[%1] Batch edit completed', $po_request->config->get('app_display_name')),
                    'batch_processing_completed.tpl',
                    array(
    ...
    {
        if (preg_match("!(SynoResource|SynoEA)!", $ps_file)) {
            return true;
    ...
    public static function importMediaFromDirectory($po_request, $pa_options=null)
    {
        ...
        if (sizeof($batch_media_import_root_directories) === 0) {
            $o_eventlog->log([
                ...
                "MESSAGE" => $vs_msg
                    = _t(
                        "Specified import directory '%1' is not valid",
        ...
        foreach ($va_files_to_process as $vs_file) {
            ...
            if ($vs_import_mode === 'DIRECTORY_AS_HIERARCHY') {
                ...
                $t_instance = Datamodel::getInstance($vs_import_target);
            } elseif (!$t_instance->getPrimaryKey() && ($vs_import_mode !== 'DONT_MATCH')) {
            ...
            if (($id = $t_instance->getPrimaryKey()) && ($t_instance instanceof RepresentableBaseModel)) {
                ...
                if ($use_existing_representation_id) {
                    if (!($t_new_rep = $t_instance->addRelationship('ca_object_representations', $use_existing_representation_id, $vn_rel_type_id))) {
                        continue;
                    }
                } else {
                    ...
                    $t_new_rep = $t_instance->addRepresentation(
                        ...
                        $vn_rep_type_id, // path
                        $vn_locale_id,
                        $vn_object_representation_status,
                ...
                if (in_array($vs_import_mode, array('TRY_TO_MATCH', 'DONT_MATCH', 'DIRECTORY_AS_HIERARCHY'))) {
                    ...
                    $t_new_rep = $t_instance->addRepresentation(
                        $vs_directory.'/'.$f,
                        $vn_rep_type_id, // path, type_id
                        $vn_locale_id,
                        $vn_object_representation_status,
                        $vn_object_representation_access,
                        true, // locale, status, access, primary
                        array('idno' => $vs_rep_idno), // values
            ...
            if ($t_instance->getPrimaryKey()) {
                    ...
                    foreach ($media_metadata_extraction_defaults as $m => $importer_code) {
                        if (caCompareMimetypes($media_mimetype, $m)) {
                            if (!($vn_mapping_id = ca_data_importers::find(['importer_code' => $importer_code], ['returnAs' => 'firstId']))) {
                                if ($o_log) {
                                    $o_log->logInfo(_t('Could not find embedded metadata importer with code %1', $importer_code));
                                }
                            }
                            break;
                        }
                ...
                if ($vn_mapping_id && ($t_mapping = ca_data_importers::find(['importer_id' => $vn_mapping_id], ['returnAs' => 'firstModelInstance']))) {
                    ...
                    if ($o_log) {
                        $o_log->logDebug(_t('Using embedded media mapping %1 (format %2)', $t_mapping->get('importer_code'), $format));
                    }
        ...
        if ($po_request && isset($pa_options['sendMail']) && $pa_options['sendMail']) {
            if ($vs_email = trim($po_request->user->get('email'))) {
                ...
                caSendMessageUsingView(
                    ...
                    array($vs_email => $po_request->user->get('fname').' '.$po_request->user->get('lname')),
                    __CA_ADMIN_EMAIL__,
                    _t('[%1] Batch media import completed', $po_request->config->get('app_display_name')),
                    'batch_media_import_completed.tpl',
                    array(
                    ...
                    null,
                    null,
                    ['source' => 'Batch media import complete', 'attachments' => $attachments]
                );
            }
        }
    ...
    private static function _addNewRecord(BaseModel $t_instance, KLogger $o_log, array $options): array
    {
        ...
        if ($t_instance->tableName() == 'ca_entities') { // entity labels deserve special treatment
            ...
            $t_instance->addLabel(
                [$t_instance->getLabelDisplayField() => $vs_label_value],
                $locale_id,
                null,
                true
            );
    ...
    public static function importMetadata($po_request, $ps_source, $ps_importer, $ps_input_format, $pa_options=null)
    {
        ...
        if ($limit_log_to = caGetOption('limitLogTo', $pa_options, null) && !is_array($limit_log_to)) {
            $limit_log_to = array_map(function ($v) {
                return strtoupper($v);
            }, preg_split("![;,]+!", $limit_log_to));
        }
        ...
        if (isset($pa_options['sendMail']) && $pa_options['sendMail']) {
            if ($vs_email = trim($po_request->user->get('email'))) {
                ...
                caSendMessageUsingView(
                    ...
                    array($vs_email => $po_request->user->get('fname').' '.$po_request->user->get('lname')),
                    __CA_ADMIN_EMAIL__,
                    _t('[%1] Batch metadata import completed', $po_request->config->get('app_display_name')),
                    'batch_metadata_import_completed.tpl',
                    [
