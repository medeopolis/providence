...
// 			}
	...
	public static function importMediaFromDirectory($po_request, $pa_options=null) {
		...
		$dirs = explode(';', $pa_options['importFromDirectory']);
		
		foreach($dirs as $dir) {
			$dir = trim($dir, '/');
			$batch_media_import_root_directory = null;
			$batch_media_import_root_directories = array_filter(caGetAvailableMediaUploadPaths($vn_user_id), function($v) use ($dir) {
				$p = "{$v}/{$dir}";
			...
			foreach($batch_media_import_root_directories as $batch_media_import_root_directory) {
				$batch_media_import_directory = $batch_media_import_root_directory.'/'.$dir;
							
				if (!caIsValidMediaImportDirectory($batch_media_import_directory, ['user_id' => $vn_user_id, 'allowFiles' => true])) {
					BatchProcessor::$s_import_error_list[] = _t('%1 is not a valid directory', $batch_media_import_directory);
					continue;
				}
				...
				$va_files_to_process = is_file($batch_media_import_directory) ? [$batch_media_import_directory] : caGetDirectoryContentsAsList($batch_media_import_directory, $vb_include_subdirectories, false, true);
				
				$o_log->logInfo(_t('Found %1 files in directory \'%2\'', sizeof($va_files_to_process), $dir));
