...
class TaskQueue extends BaseObject
{
     ...
     *
     */
    public function getHandlerName($ps_handler)
    {
     ...
     *
     */
    public function getParametersForDisplay($pm_task_rec)
    {
     ...
     *
     */
    public function addTask($ps_handler, $pa_parameters, $pa_options)
    {
        #
        # Check user_id
        #
     ...
     */
    public function copyFileToQueueTmp($ps_handler, $ps_source)
    {
     ...
     *
     */
    public function processQueue($ps_handler="")
    {
     ...
     */
    public function cancelPendingTasksForEntity($ps_entity_key, $ps_handler="")
    {
     ...
     */
    public function retryFailedTask($pn_task_id)
    {
        $t_task = new ca_task_queue($pn_task_id);
        if (!$t_task->getPrimaryKey()) {
            return false;
        }
        if ((int)$t_task->get('error_code') === 0) {
            return false;
        }
     ...
     */
    public function cancelPendingTasksForRow($ps_row_key, $ps_handler="")
    {
    ...
    public function registerProcess()
    {
        ...
        if ($vn_max_opo_processes < 1) {
            $vn_max_opo_processes = 1;
        }
        $o_appvars = new ApplicationVars();
        $va_opo_processes = $o_appvars->getVar("taskqueue_opo_processes");
     ...
     *
     */
    public function updateRegisteredProcess($pn_proc_id, $ps_row_key='', $ps_entity_key='')
    {
        $o_appvars = new ApplicationVars();
        $va_opo_processes = $o_appvars->getVar("taskqueue_opo_processes");
     ...
     *
     */
    public function rowKeyIsBeingProcessed($ps_row_key)
    {
        $o_appvars = new ApplicationVars();
        $va_opo_processes = $o_appvars->getVar("taskqueue_opo_processes");
     ...
     *
     */
    public function entityKeyIsBeingProcessed($ps_row_key)
    {
        $o_appvars = new ApplicationVars();
        $va_opo_processes = $o_appvars->getVar("taskqueue_opo_processes");
     ...
     *
     */
    public function unregisterProcess($pn_proc_id)
    {
        $o_appvars = new ApplicationVars();
        $va_opo_processes = $o_appvars->getVar("taskqueue_opo_processes");
     ...
     *
     */
    public function &verifyProcesses($pa_opo_processes)
    {
        if (!is_array($pa_opo_processes)) {
            return array();
        }
     ...
     *
     */
    public function _microtime2float()
    {
