...
class Date extends Base
{
    ...
    public function check(&$t_instance)
    {
        ...
        if (is_array($va_filters = $va_values['element_filters']) && !sizeof($va_filters)) {
            $va_filters = null;
        }
                ...
                foreach ($va_elements = $t_root->getElementsInSet() as $va_element) {
                    if ($va_element['datatype'] == __CA_ATTRIBUTE_VALUE_LIST__) {
                        if ($vs_list = \ca_lists::getListAsHTMLFormElement(
                            $va_element['list_id'],
                            "{$ps_prefix_id}_element_filter_".$va_element['element_code']."[]",
    ...
    public function getTriggerQueryCriteria($pa_trigger_values)
    {
        ...
        // Get element code
        if (!($vs_element_code = \ca_metadata_elements::getElementCodeForId($pa_trigger_values['element_id']))) {
            return false;
        }
