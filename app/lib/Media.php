...
class Media extends BaseObject
{
     ...
     *
     */
    public static $WLMedia_plugin_cache = [];

    /**
     *
     */
    public static $WLMedia_unregistered_plugin_cache = [];

    /**
     *
     */
    public static $WLMedia_plugin_names = null;
     ...
     *
     */
    public static $s_file_extension_to_plugin_map = null;

    /**
     *
     */
    public static $s_divine_cache = [];
    ...
    {
        if (!Media::$plugin_path) {
            Media::$plugin_path = __CA_LIB_DIR__.'/Plugins/Media';
        }
    ...
    {
        if (is_array(Media::$WLMedia_plugin_names)) {
        ...
        if (!$dir) {
            throw new ApplicationException(_t('Cannot open media plugin directory %1', Media::$plugin_path));
        }

        $vb_binary_file_plugin_installed = false;
        while (($plugin = readdir($dir)) !== false) {
            if ($plugin == "BaseMediaPlugin.php") {
                continue;
            }
            if ($plugin == "BinaryFile.php") {
                $vb_binary_file_plugin_installed = true;
                continue;
            }
        ...
        if ($vb_binary_file_plugin_installed) {
            Media::$WLMedia_plugin_names[] = "BinaryFile";
        }
    ...
    public function getPlugin($plugin_name)
    {
        ...
        if ($vo_instance = $p->register()) {
            if ($this->DEBUG) {
                print "[DEBUG] LOADED $plugin_name<br>\n";
            }
            return Media::$WLMedia_plugin_cache[$plugin_name] = $vo_instance;
        } else {
            if ($this->DEBUG) {
                print "[DEBUG] DID NOT LOAD $plugin_name<br>\n";
            }
    ...
    public function checkPluginStatus($plugin_name)
    {
        ...
        if (isset(Media::$WLMedia_plugin_cache[$plugin_name])) {
            return Media::$WLMedia_plugin_cache[$plugin_name];
        }
     ...
     */
    public function divineFileFormat($ps_filepath, $options=null)
    {
        $pb_return_plugin_instance = caGetOption('returnPluginInstance', $options, false);
        $pb_no_cache = caGetOption('noCache', $options, false);

        if (!$pb_no_cache && $pb_return_plugin_instance && isset(Media::$s_divine_cache[$ps_filepath.'_plugin'])) {
            return Media::$s_divine_cache[$ps_filepath.'_plugin'];
        }
        if (!$pb_no_cache && isset(Media::$s_divine_cache[$ps_filepath])) {
            return Media::$s_divine_cache[$ps_filepath];
        }

        if (sizeof(Media::$s_divine_cache) > 200) {
            Media::$s_divine_cache = array_slice(Media::$s_divine_cache, 100);
        }
        ...
        foreach ($va_plugin_names as $vs_plugin_name) {
            if (!$va_plugin_info = $this->getPlugin($vs_plugin_name)) {
                continue;
            }
            $o_plugin = $va_plugin_info["INSTANCE"];
            if ($this->DEBUG) {
                print "[DEBUG] TRYING ".$vs_plugin_name."<br>\n";
            }
        ...
        if ($vs_mimetype) {
            if ($this->DEBUG) {
                print "[DEBUG] Plugin ".$vs_plugin_name." returned {$vs_mimetype}<br/>\n";
            }
            return $vs_mimetype;
        } else {
    ...
    public function writePreviews($options=null)
    {
        ...
        if (!method_exists($this->instance, 'writePreviews')) {
            return false;
        }
    ...
    public function joinArchiveContents($pa_files, $options = [])
    {
        ...
        if (!method_exists($this->instance, 'joinArchiveContents')) {
            return false;
        }
        $this->instance->set('version', '');
    ...
    public static function getImportFileExtensions()
    {
        ...
        $va_extensions = [];
        foreach ($va_plugin_names as $vs_plugin_name) {
            if (!$va_plugin_info = $o_media->getPlugin($vs_plugin_name)) {
                continue;
            }
            $o_plugin = $va_plugin_info["INSTANCE"];
