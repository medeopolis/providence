...
{
    # ------------------------------------------------------------------
    public static $s_label_cache = array();
     ...
     */
    public static $s_label_cache_size = 1024;


    public static $s_labels_by_id_cache = array();
     ...
     */
    public static $s_labels_by_id_cache_size = 1024;
    ...
    public function preferredLabelExistsForLocale($pn_locale_id)
    {
        ...
        if (!($t_label = Datamodel::getInstanceByTableName($this->getLabelTableName()))) {
            return null;
        }
    ...
    public function addLabel($pa_label_values, $pn_locale_id, $pn_type_id=null, $pb_is_preferred=false, $pa_options=null)
    {
        ...
        if ($pb_is_preferred && $this->preferredLabelExistsForLocale($pn_locale_id)) {
            return false;
        }
         ...
         */
        if (method_exists($this, "processLabelsAfterChange")) {
            $this->processLabelsAfterChange();
        }
        return $vn_label_id;
    }
    # ------------------------------------------------------------------
    ...
    public function editLabel($pn_label_id, $pa_label_values, $pn_locale_id, $pn_type_id=null, $pb_is_preferred=false, $pa_options=null)
    {
        ...
        if ($t_label->hasField('type_id')) {
            $t_label->set('type_id', $pn_type_id);
            if ($t_label->changed('type_id')) {
                $vb_has_changed = true;
            }
        }

        $t_label->set('locale_id', $pn_locale_id);
        if ($t_label->changed('locale_id')) {
            $vb_has_changed = true;
        }

        if ($t_label->hasField('is_preferred')) {
            $t_label->set('is_preferred', $pb_is_preferred ? 1 : 0);
            if ($t_label->changed('is_preferred')) {
                $vb_has_changed = true;
            }
        }
        ...
        if ($t_label->hasField('effective_date')) {
            $t_label->set('effective_date', $effective_date);
            if ($t_label->changed('effective_date')) {
                $vb_has_changed = true;
            }
        }
        if ($t_label->hasField('access')) {
            $t_label->set('access', $label_access);
            if ($t_label->changed('access')) {
                $vb_has_changed = true;
            }
        }
        if ($t_label->hasField('source_info')) {
            $t_label->set('source_info', $source_info);
            if ($t_label->changed('source_info')) {
                $vb_has_changed = true;
            }
        }
    ...
    public function removeLabel($pn_label_id, $pa_options = null)
    {
        ...
        if (!($t_label->get($this->primaryKey()) == $this->getPrimaryKey())) {
            return null;
        }
    ...
    public function removeAllLabels($pn_mode=__CA_LABEL_TYPE_ANY__, $pa_options = null)
    {
        ...
        $va_locale_ids = array_map(function ($v) {
            return is_numeric($v) ? $v : ca_locales::codeToID($v);
        }, caGetOption(['locales', 'locale'], $pa_options, null, ['castTo' => 'array']));
        ...
        foreach ($va_labels as $vn_id => $va_labels_by_locale) {
            foreach ($va_labels_by_locale as $vn_locale_id => $va_labels) {
                if (is_array($va_locale_ids) && sizeof($va_locale_ids) && !in_array($vn_locale_id, $va_locale_ids)) {
                    continue;
                }
    ...
    public function replaceLabel($pa_label_values, $pn_locale_id, $pn_type_id=null, $pb_is_preferred=true, $pa_options=null)
    {
         ...
         * @see LabelableBaseModelWithAttributes::addLabel()
         */
        if ($vn_rc && method_exists($this, "processLabelsAfterChange")) {
            $this->processLabelsAfterChange();
        }
    ...
    public static function find($pa_values, $pa_options=null)
    {
        ...
        foreach ($pa_values as $vs_field => $va_field_values) {
            foreach ($va_field_values as $va_field_value) {
                $vs_op = $va_field_value[0];
                $vm_value = $va_field_value[1];
                if (in_array($vs_field, array('preferred_labels', 'nonpreferred_labels')) && is_array($va_field_value) && sizeof($va_field_value)) {
        ...
        foreach ($pa_values as $vs_field => $va_field_values) {
            foreach ($va_field_values as $va_field_value) {
                ...
                if (!$t_instance->hasField($vs_field) && in_array($vs_field, $va_element_codes)) {
            ...
            foreach ($pa_values as $vs_field => $va_field_values) {
                foreach ($va_field_values as $va_field_value) {
                    ...
                    if ($t_instance->_getFieldTypeType($vs_field) == 0) {
                        if (!caIsValidSqlOperator($vs_op, ['type' => 'numeric', 'nullable' => true, 'isList' => (is_array($vm_value) && sizeof($vm_value))])) {
                            throw new ApplicationException(_t('Invalid numeric operator: %1', $vs_op));
                        }
                    } else {
                        if (!caIsValidSqlOperator($vs_op, ['type' => 'string', 'nullable' => true, 'isList' => (is_array($vm_value) && sizeof($vm_value))])) {
                            throw new ApplicationException(_t('Invalid string operator: %1', $vs_op));
                        }
                    }
        ...
        if (!sizeof($va_label_sql) && ($vn_attr_count == 0)) {
            return null;
        }
        ...
        $va_sql = [];
        if ($vs_wheres = join(" {$ps_boolean} ", $va_label_sql)) {
            $va_sql[] = "({$vs_wheres})";
        }
        if ($vs_deleted_sql) {
        ...
        switch($ps_return_as) {
            case 'firstmodelinstance':
                while ($qr_res->nextRow()) {
                    ...
                    if ($dont_filter_by_acl && method_exists($t_instance, "disableACL")) {
                        $o_instance->disableACL(true);
                    }
     ...
     */
    public static function getIDsForLabels($labels, $options=null)
    {
        if (!is_array($labels) && strlen($labels)) {
            $labels = [$labels];
        }
    ...
    public function hasBundle($ps_bundle, $pn_type_id=null)
    {
        ...
        if (($vn_num_bits == 1) && (in_array($ps_bundle, array('preferred_labels', 'nonpreferred_labels')))) {
            ...
            if (($va_bundle_bits[0] == $this->tableName()) && (in_array($va_bundle_bits[1], array('preferred_labels', 'nonpreferred_labels')))) {
                if (!($t_label = $this->getLabelTableInstance())) {
                    return false;
                }
    ...
    {
        if (!($t_label = Datamodel::getInstanceByTableName($this->getLabelTableName(), true))) {
            return null;
        }
        if ($this->inTransaction()) {
            $o_trans = $this->getTransaction();
    ...
    public function getLabels($pa_locale_ids=null, $pn_mode=__CA_LABEL_TYPE_ANY__, $pb_dont_cache=true, $pa_options=null)
    {
        ...
        if (!($vn_id = caGetOption('row_id', $pa_options, $this->getPrimaryKey()))) {
            return null;
        }
        ...
        if (!is_array($pa_locale_ids)) {
            $pa_locale_ids = [$pa_locale_ids];
        }
        $pa_locale_ids = array_filter($pa_locale_ids, function ($v) {
        ...
        if ($this->getPrimaryKey()) {
            ...
            if ($this->numErrors()) {
                foreach ($_REQUEST as $vs_key => $vs_value) {
                    if (!preg_match('/'.$ps_placement_code.$ps_form_name.'_Pref'.'locale_id_new_([\d]+)/', $vs_key, $va_matches)) {
                        continue;
                    }
    ...
    public function getNonPreferredLabelHTMLFormBundle($po_request, $ps_form_name, $ps_placement_code, $pa_bundle_settings=null, $pa_options=null)
    {
        ...
        if (!$bundle_preview) {
            ...
            $bundle_preview = join('; ', array_map(function ($v) use ($l) {
                return $v[$l];
            }, $va_inital_values));
        }
        $o_view->setVar('bundle_preview', $bundle_preview);
    ...
    public function getPreferredDisplayLabelsForIDs($pa_ids, $pa_options=null)
    {
        ...
        foreach ($va_ids as $vn_id) {
            if (!isset($va_labels[$vn_id]) || !$va_labels[$vn_id]) {
                continue;
            }
    ...
    public function getHierarchyWithLabels($pn_id=null, $pa_options=null)
    {
        ...
        if (!isset($pa_options['preferred_only'])) {
            $pa_options['preferred_only'] = true;
        }

        if (!($qr_res = $this->getHierarchy($pn_id, array('additionalTableToJoin' => $this->getLabelTableName())))) {
            return null;
        }
        ...
        $va_hier = array();
        while ($qr_res->nextRow()) {
            if (isset($pa_options['preferred_only']) && (bool)$pa_options['preferred_only'] && (!$qr_res->get('is_preferred'))) {
                continue;
            }
    ...
    {
        if (!($vn_id = caGetOption('row_id', $pa_options, null)) && !($vn_id = (int)$this->getPrimaryKey())) {
            return null;
        }
        if (!($vs_group_rel_table = $this->getProperty('USER_GROUPS_RELATIONSHIP_TABLE'))) {
            return null;
        }
        $vs_pk = $this->primaryKey();
    ...
    {
        if (!is_array($pm_group_id)) {
            $pm_group_id = array($pm_group_id);
        }
    ...
    *
    ...
    public function setUserGroups($pa_group_ids, $pa_effective_dates=null, $pa_options=null)
    {
        if (is_array($va_groups = $this->getUserGroups([]))) {
            ...
            if (!$this->removeUserGroups($va_group_ids_to_remove)) {
                return false;
            }
            if (!$this->addUserGroups($pa_group_ids, $pa_effective_dates)) {
                return false;
            }
        }
        return true;
    ...
    public function setUsers($pa_user_ids, $pa_effective_dates=null)
    {
        if (is_array($va_users = $this->getUsers([]))) {
            ...
            if (!$this->removeUsers($va_user_ids_to_remove)) {
                return false;
            }
            if (!$this->addUsers($pa_user_ids, $pa_effective_dates)) {
                return false;
            }
        }
        return true;
    ...
    public function getUserRoles($pa_options=null)
    {
        ...
        if (!($vs_role_rel_table = $this->getProperty('USER_ROLES_RELATIONSHIP_TABLE'))) {
            return null;
        }
        $vs_pk = $this->primaryKey();
    ...
    {
        if (!is_array($pm_role_id)) {
            $pm_role_id = array($pm_role_id);
        }
    ...
    *
    ...
    public function setUserRoles($pa_role_ids, $pa_options=null)
    {
        if (is_array($va_roles = $this->getUserRoles())) {
            ...
            if (!$this->removeUserRoles($va_role_ids_to_remove)) {
                return false;
            }
            if (!$this->addUserRoles($pa_role_ids, $pa_effective_dates)) {
                return false;
            }
        }
        return true;
    ...
    public function removeUserRoles($pa_role_ids)
    {
        ...
        foreach ($pa_role_ids as $vn_role_id) {
            if (!isset($va_current_roles[$vn_role_id]) && $va_current_roles[$vn_role_id]) {
                continue;
            }
