...
class PastPerfectXMLReader extends BaseXMLDataReader
{
    # -------------------------------------------------------
    /**
    ...
    public function __construct($ps_source=null, $pa_options=null)
    {
        ...
        $this->ops_title = _t('PastPerfect XML Reader');
        $this->ops_display_name = _t('PastPerfect XML');
        $this->ops_description = _t('Reads PastPerfect XML files');

        $this->opa_formats = array('pastperfectxml');	// must be all lowercase to allow for case-insensitive matching
    }
    # -------------------------------------------------------
}
