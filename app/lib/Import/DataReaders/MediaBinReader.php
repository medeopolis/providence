...
class MediaBinReader extends BaseXMLDataReader
{
    # -------------------------------------------------------
    /**
     ...
     * XPath to select
     */
    protected $ops_xpath = '//METADATA';

    /**
     *
     */
    protected $ops_root_tag = 'METADATA';
    ...
    public function __construct($ps_source=null, $pa_options=null)
    {
        ...
        $this->ops_title = _t('MediaBin XML Reader');
        $this->ops_display_name = _t('MediaBIN XML');
        $this->ops_description = _t('Reads MediaBin XML export files');

        $this->opa_formats = array('mediabin');	// must be all lowercase to allow for case-insensitive matching
    }
    # -------------------------------------------------------
}
