...
class CollectiveAccessDataReader extends BaseDataReader
{
    ...
    public function get($ps_col, $pa_options=null)
    {
        ...
        if (!is_array($pa_filter_to_types) && $pa_filter_to_types) {
            $pa_filter_to_types = array($pa_filter_to_types);
        }

        $pa_filter_to_relationship_types = isset($pa_options['filterToRelationshipTypes']) ? $pa_options['filterToRelationshipTypes'] : false;
        if (!is_array($pa_filter_to_relationship_types) && $pa_filter_to_relationship_types) {
            $pa_filter_to_relationship_types = array($pa_filter_to_relationship_types);
        }
        ...
        if ($va_col[0] == $this->ops_table) {	// field in this table
                            ...
                            if ($pb_return_as_array) {
                                if ($pb_return_all_locales) {
                                    foreach ($va_data['nonpreferred_labels'] as $loc => $labels) {
                                        $ret[$loc] = array_map(function ($v) use ($vs_display_field) {
                                            return $v[$vs_display_field];
                                        }, $labels);
                                    }
                                } else {
                                    foreach ($va_data['nonpreferred_labels'] as $loc => $labels) {
                                        $ret = array_merge($ret, array_map(function ($v) use ($vs_display_field) {
                                            return is_array($v) ? $v[$vs_display_field] : null;
                                        }, $labels));
                                    }
                                }
                                return $ret;
                ...
                case 3:
                    ...
                    if ($va_col[1] == 'nonpreferred_labels') {
                        if (is_array($va_data['nonpreferred_labels'])) {
                            $vals = array_map(function ($v) use ($va_col) {
                                return $v[$va_col[2]];
                            }, $va_data['nonpreferred_labels']);
                            if ($pb_return_as_array) {
                                if ($pb_return_all_locales) {
            ...
            // Related
            //
            if (!(in_array($va_col[0], ['tags', 'ca_item_tags'])) && !isset($va_data['related'][$va_col[0]])) {
                return null;
            }
                    ...
                    if ($va_col[1] == 'preferred_labels') {
                        ...
                        if ($t_instance = Datamodel::getInstanceByTableName($va_col[0], true)) {
                            ...
                            foreach ($va_rel_data as $vn_i => $va_rel) {
                                ...
                                if (is_array($pa_filter_to_types) && !in_array($va_rel['type_id_code'], $pa_filter_to_types)) {
                                    continue;
                                }
                                if (is_array($pa_filter_to_relationship_types) && !in_array($va_rel['relationship_typename'], $pa_filter_to_relationship_types) && !in_array($va_rel['relationship_type_code'], $pa_filter_to_relationship_types)) {
                                    continue;
                                }
