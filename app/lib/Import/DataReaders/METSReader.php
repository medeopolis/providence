...
class METSReader extends BaseXMLDataReader
{
    # -------------------------------------------------------
    /**
     ...
     */
    protected $ops_xml_namespace_prefix = 'mets';

    /**
     * XPath to select data for reading
     */
    protected $ops_xpath = '//mets:mets';

    /**
     *
     */
    protected $ops_root_tag = 'mets';
    ...
    public function __construct($ps_source=null, $pa_options=null)
    {
        ...
        $this->ops_title = _t('METS XML Reader');
        $this->ops_display_name = _t('METS XML');
        $this->ops_description = _t('Reads METS XML files');

        $this->opa_formats = array('mets');	// must be all lowercase to allow for case-insensitive matching
    }
    # -------------------------------------------------------
}
