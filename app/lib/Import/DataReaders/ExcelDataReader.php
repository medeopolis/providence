...
class ExcelDataReader extends BaseDataReader
{
        ...
        while (true) {
            ...
            if ($o_row = $this->opo_rows->current()) {
                ...
                foreach ($o_cells as $o_cell) {
                    ...
                    if ($vn_col > $this->opn_max_columns) {
                        break;
                    }
                }
    ...
    {
        if (($pn_dataset < 0) || ($pn_dataset >= $this->getDatasetCount())) {
            return false;
        }
        try {
        ...
        if ($o_value instanceof \PhpOffice\PhpSpreadsheet\RichText\RichText) {
            ...
            foreach ($va_elements as $o_element) {
                ...
                if ($o_element instanceof \PhpOffice\PhpSpreadsheet\RichText\Run) {
                    ...
                    if ($o_font->getItalic()) {
                        $vs_prefix .= "<i>";
                        $vs_suffix = "</i>{$vs_suffix}";
                    }
                    if ($o_font->getSuperScript()) {
                        $vs_prefix .= "<sup>";
                        $vs_suffix = "</sup>{$vs_suffix}";
                    }
                    if ($o_font->getSubScript()) {
                        $vs_prefix .= "<sub>";
                        $vs_suffix = "</sub>{$vs_suffix}";
                    }
                    ...
                    if ($o_font->getStrikethrough()) {
                        $vs_prefix .= "<strike>";
                        $vs_suffix = "</strike>{$vs_suffix}";
                    }
