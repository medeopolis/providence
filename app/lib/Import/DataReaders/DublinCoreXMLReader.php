...
class DublinCoreXMLReader extends BaseXMLDataReader
{
    # -------------------------------------------------------
    /**
     ...
     * XPath to select
     */
    protected $ops_xpath = '/dublin_core';
    ...
    public function __construct($ps_source=null, $pa_options=null)
    {
        ...
        $this->ops_title = _t('DublinCore XML Reader');
        $this->ops_display_name = _t('DublinCore XML');
        $this->ops_description = _t('Reads DublinCore XML files');

        $this->opa_formats = array('dcxml', 'dublincorexml');	// must be all lowercase to allow for case-insensitive matching
    }
    # -------------------------------------------------------
}
