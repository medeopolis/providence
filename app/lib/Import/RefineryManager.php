<?php
/** ---------------------------------------------------------------------
 * app/lib/Import/RefineryManager.php :
 * ----------------------------------------------------------------------
 * CollectiveAccess
...
class RefineryManager
{
     ...
     */
    public static $s_refinery_manager_did_do_refinery_init = false;

    /**
     *
     */
    public static $s_refinery_instances = array();
    # -------------------------------------------------------
    ...
    {
        if (RefineryManager::$s_refinery_manager_did_do_refinery_init) {
            return true;
        }
        ...
        $va_refinery_dirs = RefineryManager::getRefineryNames();
        foreach ($va_refinery_dirs as $vs_refinery_dir) {
            if (!file_exists($vs_base_refinery_dir.'/'.$vs_refinery_dir.'/'.$vs_refinery_dir.'Refinery.php')) {
                continue;
            }
            ...
            RefineryManager::$s_refinery_instances[$vs_refinery_dir] = $o_instance;
        }

        RefineryManager::$s_refinery_manager_did_do_refinery_init = true;
    ...
    {
        RefineryManager::initRefineries();
        if (isset(RefineryManager::$s_refinery_instances[$ps_refinery_name]) && RefineryManager::$s_refinery_instances[$ps_refinery_name]) {
            return RefineryManager::$s_refinery_instances[$ps_refinery_name];
        }
        return null;
    }
    # -------------------------------------------------------
    ...
    {
        $vs_base_refinery_dir = __CA_APP_DIR__.'/refineries';
        if (!file_exists($vs_base_refinery_dir)) {
            return array();
        }
    ...
    public static function checkRefineryStatus($ps_refinery_name)
    {
        ...
        if (isset(RefineryManager::$s_refinery_instances[$ps_refinery_name]) && is_object(RefineryManager::$s_refinery_instances[$ps_refinery_name])) {
            return RefineryManager::$s_refinery_instances[$ps_refinery_name]->checkStatus();
        }
    ...
    public static function getRefineryTitle($ps_refinery_name)
    {
        ...
        if (isset(RefineryManager::$s_refinery_instances[$ps_refinery_name]) && is_object(RefineryManager::$s_refinery_instances[$ps_refinery_name])) {
            return RefineryManager::$s_refinery_instances[$ps_refinery_name]->getTitle();
        }
        ...
        if (RefineryManager::$s_refinery_instances[$ps_refinery_name] && is_object(RefineryManager::$s_refinery_instances[$ps_refinery_name])) {
            ...
            foreach ($va_available_settings as $vs_setting_name => $va_setting_info) {
                ...
                if (isset($va_setting_info['scope']) && ($va_setting_info['scope'] == 'application')) {
                    ...
                    $pa_settings[$vs_setting_name] = $o_appvar->getVar('refinery_settings_'.$ps_refinery_name.'_'.$vs_setting_name);
                }
                $vs_buf .= RefineryManager::$s_refinery_instances[$ps_refinery_name]->settingHTMLFormElement($ps_refinery_id, $vs_setting_name, array('value' => $pa_settings[$vs_setting_name]));
            }
        }
        return $vs_buf;
    }
    ...
    public function getRefineryAvailableSettings($ps_refinery_name)
    {
        RefineryManager::initRefineries();
        if (RefineryManager::$s_refinery_instances[$ps_refinery_name] && is_object(RefineryManager::$s_refinery_instances[$ps_refinery_name])) {
            return RefineryManager::$s_refinery_instances[$ps_refinery_name]->getAvailableSettings();
        }
