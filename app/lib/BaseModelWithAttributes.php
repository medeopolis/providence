...
{
    # ------------------------------------------------------------------
    public static $s_applicable_element_code_cache = array();
    public static $s_element_label_cache = array();
    # ------------------------------------------------------------------
    ...
    public function addAttribute($pa_values, $pm_element_code_or_id, $ps_error_source=null, $pa_options=null)
    {
        ...
        if (!$ps_error_source) {
            $ps_error_source = $this->tableName().'.'.$t_element->get('element_code');
        }
        ...
        if ($takes_locale && $single_value_per_locale) {
            $attrs_to_remove = array_map(function ($v) {
                return (int)$v['attribute_id'];
            }, $this->opa_attributes_to_remove);
            ...
            if (is_array($attrs_for_element)) {
                ...
                $extant_locales = array_unique(
                    array_merge(
                        (is_array($va_attrs) ? array_map(function ($a) {
                            return $a->getLocaleID();
                        }, $va_attrs) : []),
                ...
                $extant_locales = array_filter($extant_locales, function ($v) {
                    return (bool)$v;
                });
    ...
    public function editAttribute($pn_attribute_id, $pm_element_code_or_id, $pa_values, $ps_error_source=null, $pa_options=null)
    {
        ...
        if (array_key_exists('locale_id', $pa_values)) {
            $num_values--;
        }
        ...
        if (caGetOption('skipExistingValues', $pa_options, false) || !caGetOption('allowDuplicateValues', $element_info, false)) {
            ...
            if (is_array($va_attrs = $this->getAttributesByElement($pm_element_code_or_id))) {
                ...
                $vals = [];
                foreach ($va_attrs as $o_attr) {
                    if ($o_attr->getAttributeID() == $pn_attribute_id) {
                        continue;
                    }
                    foreach ($o_attr->getValues() as $o_value) {
        ...
        if (
            ...
            is_array($elements = array_filter(ca_metadata_elements::getElementsForSet($vn_attr_element_id, ['omitContainers' => true]), function ($v) {
                return (int)$v['datatype'] !== 0;
            }))
            &&
            ...
            if (!$elements) {
                $elements = array_filter(ca_metadata_elements::getElementsForSet($vn_attr_element_id, ['omitContainers' => true]), function ($v) {
                    return (int)$v['datatype'] !== 0;
                });
            }
        ...
        if (!$ps_error_source) {
            $ps_error_source = $this->tableName().'.'.ca_metadata_elements::getElementCodeForId($pm_element_code_or_id);
        }
    ...
    public function removeAttribute($pn_attribute_id, $ps_error_source=null, $pa_extra_info=null, $pa_options=null)
    {
        ...
        if (!$ps_error_source) {
            $ps_error_source = $this->tableName().'.'.ca_metadata_elements::getElementCodeForId($vn_element_id);
        }
        ...
        if (!($t_element = ca_metadata_elements::getInstance($t_attr->get('element_id')))) {
            return false;
        }
                            ...
                            foreach ($va_v_list as $vn_index => $va_values) {
                                if (is_array($va_values)) {
                                    foreach ($va_values as $vs_sub_code => $vs_value) {
                                        ...
                                        if (!$t_element = ca_metadata_elements::getInstance($vs_sub_code)) {
                                            continue;
                                        }
    ...
    {
        if (!isset($this->SOURCE_ID_FLD) || !$this->SOURCE_ID_FLD) {
            return null;
        }
    ...
    {
        global $g_request;
        if (!($po_request = caGetOption('request', $pa_options, null))) {
    ...
    public function getSourceInstance($pn_source_id=null)
    {
        ...
        if ($pn_source_id) {
            ...
            if (!($vn_source_id = $this->get($this->SOURCE_ID_FLD))) {
                return null;
            }
        }
    ...
    {
        if (!property_exists($this->tableName(), 'ATTRIBUTE_TYPE_ID_FLD')) {
            return null;
        }
    ...
    {
        if ($pn_type_id && !is_numeric($pn_type_id)) {
            $pn_type_id = $this->getTypeIDForCode($pn_type_id);
        }
    ...
    {
        if (!$pn_type_id) {
            $pn_type_id = $this->get($this->ATTRIBUTE_TYPE_ID_FLD);
        }
        if (!$pn_type_id) {
            return null;
        }
    ...
    {
        $va_tmp = explode('.', $ps_field);
        if (($va_tmp[0] != $this->tableName()) && !in_array($va_tmp[0], array('created', 'modified', 'lastModified'))) {
            return null;
        }
        if (!$this->hasField($va_tmp[1]) && !in_array($va_tmp[1], array('created', 'modified', 'lastModified')) && !in_array($va_tmp[0], array('created', 'modified', 'lastModified'))) {
            $va_tmp[1] = preg_replace('!^ca_attribute_!', '', $va_tmp[1]);	// if field space is a bundle placement-style bundlename (eg. ca_attribute_<element_code>) then strip it before trying to pull label
            ...
            if ($va_tmp[0] == $this->tableName() && $vs_autocomplete) {
                if (!caGetOption('nojs', $pa_options, false)) {
                    ...
                    if ($vs_rel_types = join(";", caGetOption('restrictToRelationshipTypes', $pa_options, []))) {
                        $vs_rel_types_proc = "_{$vs_rel_types}";
                        $vs_rel_types = "/{$vs_rel_types}";
                    }
    ...
    public function getAttributeHTMLFormBundle($po_request, $ps_form_name, $pm_element_code_or_id, $ps_placement_code, $pa_bundle_settings, $pa_options)
    {
        ...
        if ($t_restriction = $this->getTypeRestrictionInstance($t_element->get('element_id'))) {
            ...
            if (($vn_min_repeats < 1) && (isset($va_element['settings']['requireValue'])) && ((bool)$va_element['settings']['requireValue'])) {
                $vn_min_repeats = 1;
            }
    ...
    {
        if (!($vn_element_id = ca_metadata_elements::getElementID($pm_element_code_or_id))) {
            return null;
        }
        return ca_attributes::getReferencedAttributes($this->getDb(), $this->tableNum(), $pa_reference_limit_ids, array('element_id' => $vn_element_id));
        s;
    }
    # ------------------------------------------------------------------
    /**
    ...
    {
        if (!($vn_row_id = caGetOption('row_id', $pa_options, $this->getPrimaryKey()))) {
            return null;
        }
    ...
    public function getSimpleAttributeValue($pm_element_code_or_id, $pn_index=0, $pa_options=null)
    {
        ...
        if (sizeof($va_attrs)) {
            if (($pn_index >= 0) && ($pn_index < sizeof($va_attrs))) {
                $pn_index = 0;
            }
    ...
    public function getRawValue($pn_row_id, $pm_element_code_or_id, $pm_sub_element_code_or_id=null, $ps_delimiter=',', $pa_options=null)
    {
        ...
        if (!is_array($va_attributes = $va_attributes[$vn_element_id])) {
            return null;
        }
    ...
    public function getAttributesForDisplay($pm_element_code_or_id, $ps_template=null, $pa_options=null)
    {
        ...
        if (!$pm_element_code_or_id || !($t_element = ca_metadata_elements::getInstance($pm_element_code_or_id))) {
            return null;
        }
        if (!is_array($pa_options)) {
            $pa_options = array();
    ...
    public function copyAttributesTo($pn_row_id, $pa_options=null)
    {
        ...
        if (!is_array($va_exclude_attributes_by_codes)) {
            $va_exclude_attributes_by_codes = [];
        }

        if (!($t_dupe = Datamodel::getInstanceByTableNum($this->tableNum()))) {
            return null;
        }
    ...
    public function getAuthorityElementReferences($pa_options=null)
    {
        ...
        while ($qr_res->nextRow()) {
            $va_row = $qr_res->getRow();
            if (!is_array($va_references[$va_row['table_num']][$va_row['row_id']])) {
                $va_references[$va_row['table_num']][$va_row['row_id']] = [];
            }
            if (!in_array($va_row['element_id'], $va_references[$va_row['table_num']][$va_row['row_id']])) {
    ...
    public function getAuthorityElementList($pa_options=null)
    {
        ...
        if (caGetOption('omitLists', $pa_options, true)) {
            unset($va_element_types['ca_list_items']);
        }
        ...
        if ($pb_root_ids_only || $pb_ids_only) {
            return array_keys($va_elements);
        }
        if (caGetOption('countOnly', $pa_options, false)) {
            return sizeof($va_elements);
        }
        return $va_elements;
    }
    # ------------------------------------------------------------------
    ...
    public function moveAttributes($pn_to_id, $pa_element_codes_or_ids, $pa_options=null)
    {
        ...
        if (is_array($pa_element_codes_or_ids) && sizeof($pa_element_codes_or_ids)) {
            ...
            if (!$t_instance->load($pn_to_id) || (bool)$t_instance->get('deleted')) {
                throw new ApplicationException(_t('Invalid target ID'));
            }

            $va_element_ids = [];
            $va_changed_values = [];
            foreach ($pa_element_codes_or_ids as $vm_element) {
                if (!($vn_element_id = ca_metadata_elements::getElementID($vm_element))) {
                    continue;
                }

                // is element valid for target? (we use top-level container if the element is a container sub-element)
                if (!$t_instance->hasElement(ca_metadata_elements::getElementCodeForId($vn_hier_element_id = ca_metadata_elements::getElementHierarchyID($vn_element_id)))) {
                    continue;
                }
     ...
     */
    public static function valueExistsForElement($pm_element_code_or_id, $ps_value, $pa_options=null)
    {
        if (!($vn_element_id = ca_metadata_elements::getElementID($pm_element_code_or_id))) {
            return false;
        }
    ...
    public function attributeHasChanged($pm_element_code_or_id)
    {
        ...
        // not an element?
        if (!$vs_code || (!$this->hasElement($vs_code))) {
            return false;
        }
    ...
    {
        if (!is_array($cf = $this->_FIELD_VALUE_CHANGED)) {
            return false;
        }
        if (sizeof($cf) === 0) {
            return false;
        }
        if (sizeof(array_filter(array_keys($cf), function ($v) {
            return substr($v, 0, 14) === '_ca_attribute_';
        })) > 0) {
            return true;
        }
        return false;
    ...
    public function attributeDidChange($pm_element_code_or_id)
    {
        ...
        // not an element?
        if (!$vs_code || (!$this->hasElement($vs_code, null, true))) {
            return false;
        }
    ...
    {
        if (!is_array($cf = $this->_FIELD_VALUE_DID_CHANGE)) {
            return false;
        }
        if (sizeof($cf) === 0) {
            return false;
     ...
     */
    public static function getIDsForAttributeValues(string $element_code, array $values, ?array $options=null): array
    {
