...
class RequestHTTP extends Request
{
    ...
    public function __construct($po_response, $pa_options=null)
    {
        ...
        if (!caUseCleanUrls() && !preg_match("!/index.php!", $this->ops_full_path) && !preg_match("!/service.php!", $this->ops_full_path)) {
    ...
    # -------------------------------------------------------
    public function isDownload($pb_set_download=null)
    {
    ...
    public function parameterExists($pa_name, $ps_http_method=null, $pa_options=array())
    {
        ...
        foreach ($pa_name as $ps_name) {
            ...
            if (is_array($vm_val) || (strlen($vm_val) > 0)) {
                break;
            }
        }
    ...
    public function getParameter($pa_name, $pn_type, $ps_http_method=null, $pa_options=array())
    {
        ...
        if ((caGetOption('purify', $pa_options, true) && $this->config->get('purify_all_text_input')) || caGetOption('forcePurify', $pa_options, false)) {
            if (is_array($vm_val)) {
                $vm_val = array_map(function ($v) {
                    return is_array($v) ? $v : str_replace("&amp;", "&", RequestHTTP::getPurifier()->purify(rawurldecode($v)));
                }, $vm_val);
            } else {
    ...
    {
        if (!$pa_http_methods) {
            $pa_http_methods = array('GET', 'POST', 'COOKIE', 'PATH', 'REQUEST');
        }
        if ($pa_http_methods && !is_array($pa_http_methods)) {
            $pa_http_methods = array($pa_http_methods);
        }
        $va_params = array();
        if (!is_array($pa_http_methods)) {
            $pa_http_methods = array('GET', 'POST', 'COOKIE', 'PATH', 'REQUEST');
        }
    ...
    # -------------------------------------------------------
    public function setParameter($ps_name, $pm_value, $ps_http_method='GET')
    {
        if (in_array($ps_http_method, array('GET', 'POST', 'COOKIE', 'PATH', 'REQUEST'))) {
