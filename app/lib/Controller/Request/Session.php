...
class Session
{
    ...
    public static function init($ps_app_name=null, $pb_dont_create_new_session=false)
    {
        ...
        if (!$pb_dont_create_new_session) {
            ...
            if (!$session_id) {
                ...
                if (!caIsRunFromCLI()) {
                    setcookie(Session::$name, $_COOKIE[Session::$name] = $session_id = caGenerateGUID(), Session::$lifetime ? time() + Session::$lifetime : null, $vs_cookiepath, null, $secure, true);
                }
            }
     ...
     * @throws Exception
     */
    public static function getServiceAuthToken($pb_dont_create_new_token=false)
    {
    ...
    public static function save()
    {
        ...
        if ($g_errored) {
            return;
        }	// don't save session on routing errors
