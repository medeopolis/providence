...
class LanguageDetection
{
    ...
    public $opn_ngram_max_length = 4;					// maximum length of n-gram
    public $opn_num_ngram_in_lm = 400;					// default number of ngrams in LM-fingerprints

    //analysis defaults
    public $opn_num_ngram_in_analysis = 50; 			// default nb of ngrams created from analyzed text
    public $opn_max_num_input_words_in_analysis = 100;	// maximum number of input words use in analysis
    public $opn_max_delta = 140000;					// difference limit after which evaluation is halted
    ...
    # ------------------------------------------------------------------
    public function analyze($ps_text)
    {
        if (!empty($ps_text)) {
    ...
    public function loadFingerprints()
    {
        ...
        if (is_array($LANGUAGE_DETECTION_LM_DATA) && sizeof($LANGUAGE_DETECTION_LM_DATA) > 0) {
            return true;
        }
    ...
    public function createNGrams($ps_text)
    {
    ...
    # ------------------------------------------------------------------
    public function compareNGrams(&$va_ngrams)
    {
        global $LANGUAGE_DETECTION_LM_DATA;
        $va_results = array();
