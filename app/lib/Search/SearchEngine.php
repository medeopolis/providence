...
class SearchEngine extends SearchBase
{
    ...
    public function doSearch($ps_search, $po_result=null, $pa_options=null)
    {
        ...
        if (!$vb_from_cache) {
            ...
            if (!$vb_no_types) {
                ...
                if ($pa_options['expandToIncludeParents'] && sizeof($va_hits)) {
                    ...
                    while ($qr_exp->nextHit()) {
                        ...
                        if (($va_type_ids) && (!in_array($qr_exp->get('type_id'), $va_type_ids))) {
                            continue;
                        }
            ...
            if ((!isset($pa_options['dontFilterByACL']) || !$pa_options['dontFilterByACL']) && $vb_do_acl) {
                ...
                if ($vn_limit > 0) {
                    $va_hits = array_slice($va_hits, 0, $vn_limit);
                }
            }
    ...
    public function filterHitsBySets($pa_hits, $pa_set_ids, $pa_options=null)
    {
        ...
        if (!sizeof($pa_set_ids)) {
            return $pa_hits;
        }
        if (!($t_table = Datamodel::getInstanceByTableNum($this->opn_tablenum, true))) {
            return $pa_hits;
        }
    ...
    private function _rewriteTerm($po_term, $pb_sign)
    {
        ...
        if (is_array($va_access_points = $this->getAccessPoints($this->opn_tablenum)) && sizeof($va_access_points)) {
            ...
            if (
                ...
                if (sizeof($va_terms['signs']) > 0) {
                    array_pop($va_terms['signs']);
                }
                return $va_terms;
            }
        }
        ...
        foreach ($pa_type_codes_or_ids as $vs_code_or_id) {
            ...
            if (isset($va_type_list[$vn_type_id]) && $va_type_list[$vn_type_id]) {	// is valid type for this subject
                if (caGetOption('includeSubtypes', $pa_options, true)) {
                    ...
                    if ($exclude) {
                        $va_ids = array_filter($va_ids, function ($v) use ($pa_type_codes_or_ids) {
                            return in_array($v, $pa_type_codes_or_ids);
                        });
                    }
    ...
    public function getSourceRestrictionList()
    {
        if (function_exists("caGetSourceRestrictionsForUser")) {
            ...
            if (!is_array($va_pervasive_sources)) {
     ...
     *
     */
    public static function quickSearch($ps_search, $ps_tablename, $pn_tablenum, $pa_options=null)
    {
        $o_config = Configuration::load();
        ...
        if (!@require_once(__CA_LIB_DIR__.'/Plugins/SearchEngine/'.$ps_plugin_name.'.php')) {
            return null;
        }
     ...
     * @return array
     */
    private static function _getFieldList($po_query)
    {
        $va_fields = array();
