...
class SearchResult extends BaseObject
{
    ...
    public static $s_prefetch_cache = array();
    public static $s_instance_cache = array();
    public static $s_timestamp_cache = array();
    public static $s_rel_prefetch_cache = array();
    public static $s_parsed_field_component_cache = array();
    public static $opa_hierarchy_parent_prefetch_cache = array();
    public static $opa_hierarchy_children_prefetch_cache = array();
    public static $opa_hierarchy_parent_prefetch_cache_index = array();
    public static $opa_hierarchy_children_prefetch_cache_index = array();
    public static $opa_hierarchy_siblings_prefetch_cache = array();
    public static $opa_hierarchy_siblings_prefetch_cache_index = array();
     ...
     */
    public static $s_cache_size_limit = 2048;
    ...
    public function __construct($po_engine_result=null, $pa_tables=null)
    {
        ...
        if (!SearchResult::$opt_list) {
            SearchResult::$opt_list = Datamodel::getInstanceByTableName('ca_lists', true);
        }
        ...
        if (!$GLOBALS["_DbResult_mediainfocoder"]) {
            $GLOBALS["_DbResult_mediainfocoder"] = new MediaInfoCoder();
        }
    ...
    {
        if ($this->opa_row_ids_to_prefetch_cache[$pn_start.'/'.$pn_num_rows]) {
            return $this->opa_row_ids_to_prefetch_cache[$pn_start.'/'.$pn_num_rows];
        }
        $va_row_ids = array();
    ...
    public function prefetchHierarchyParents($ps_tablename, $pn_start, $pn_num_rows, $pa_options=null)
    {
        ...
        while (true) {
            if (!sizeof($va_row_ids_in_current_level) || !is_array($va_params) || !is_array($va_params[0]) || !sizeof($va_params[0])) {
                break;
            }
            $qr_rel = $this->opo_subject_instance->getDb()->query($vs_sql, $va_params);
            if (!$qr_rel || ($qr_rel->numRows() == 0)) {
                break;
            }

            while ($qr_rel->nextRow()) {
                $va_row = $qr_rel->getRow();
                if (!$va_row[$vs_parent_id_fld]) {
                    continue;
                }
    ...
    public function prefetchHierarchyChildren($ps_tablename, $pn_start, $pn_num_rows, $pa_options=null)
    {
        ...
        while (true) {
            if (!is_array($va_params) || !sizeof($va_params) || !is_array($va_params[0]) || !sizeof($va_params[0])) {
                break;
            }
            $qr_rel = $this->opo_subject_instance->getDb()->query($vs_sql, $va_params);
    ...
    public function prefetch($ps_tablename, $pn_start, $pn_num_rows, $pa_options=null)
    {
        ...
        if ($ps_tablename != $this->ops_table_name) {
            ...
            if (!is_array($va_linking_tables)) {
                $va_linking_tables = array();
            }
    ...
    public function prefetchRelated($ps_tablename, $pn_start, $pn_num_rows, $pa_options)
    {
        ...
        if (!method_exists($this->opo_subject_instance, "getRelatedItems")) {
            return false;
        }
        ...
        if (!isset($va_opts['limit'])) {
            if (!($va_opts['limit'] = (int)Configuration::load()->get('maximum_related_prefetch_values'))) {
                $va_opts['limit'] = 100000;
            }
        }
        ...
        if (!is_array($va_rel_items) || !sizeof($va_rel_items)) {
            return;
        }
    ...
    public function get($ps_field, $pa_options=null)
    {
        ...
        if ($pa_options['filterTypes'] && !is_array($pa_options['filterTypes'])) {
            $pa_options['filterTypes'] = preg_split('![,;]+!', $pa_options['filterTypes']);
        }
        ...
        if (!($vs_output = (isset($pa_options['output']) ? (string)$pa_options['output'] : null))) {
            ...
            if (!$vs_output && $vb_convert_codes_to_value) {
                $vs_output = "value";
            }
        }
        ...
        if (!(($vs_value = $this->opo_engine_result->get($ps_field, $pa_options)) === false)) {
            if (in_array($vs_value, $va_exclude_values)) {
                return $vb_return_as_array ? [] : null;
            }
            ...
            switch($va_path_components['hierarchical_modifier']) {
                ...
                case 'hierarchy':
                            ...
                            if ($va_path_components['related']) {
                                foreach ($va_ids as $vn_id) {
                                    if (is_array(SearchResult::$opa_hierarchy_parent_prefetch_cache[$va_path_components['table_name']][$vn_id][$vs_opt_md5])) {
                                        ...
                                        if ($vs_hierarchy_direction === 'desc') {
                                            $va_hier_id_list = array_reverse($va_hier_id_list);
                                        }
                                ...
                                if ($qr_hier = caMakeSearchResult($va_path_components['table_name'], $va_ancestor_ids, $pa_options)) {
                                    while ($qr_hier->nextHit()) {
                                        if (is_array($filter_by_types) && sizeof($filter_by_types)) {
                                            if (!is_array($type_struct = $qr_hier->get($type_spec, array('returnWithStructure' => true, 'returnAllLocales' => true, 'useLocaleCodes' => $pa_options['useLocaleCodes'])))) {
                                                continue;
                                            }

                                            $type_id = array_shift(array_shift($type_struct));
                                            if (!in_array($type_id, $filter_by_types)) {
                                                continue;
                                            }
                                        }
                    ...
                    foreach ($va_hier_list as $vn_h => $va_hier_item) {
                        if (!$vb_return_all_locales) {
                            $va_hier_item = caExtractValuesByUserLocale($va_hier_item, null, $locale ? [$locale] : null);
                        }
...
//
            ...
            if ($vb_return_as_count) {
                return is_array($vm_val) ? array_map('intval', $vm_val) : (int)$vm_val;
            }
            goto filter;
        } else {
            if (!$va_path_components['hierarchical_modifier']) {
...
//
        ...
        if ($va_sort_fields && !is_array($va_sort_fields)) {
            $va_sort_fields = [$va_sort_fields];
        }
        if ($vb_return_as_array && is_array($vm_val) && is_array($va_sort_fields) && sizeof($va_sort_fields)) {
            $va_keys = array_map(function ($v) {
        ...
        if (sizeof($va_exclude_values) > 0) {
                        ...
                        foreach ($vm_val as $vn_locale_id => $va_by_locale) {
                            foreach ($va_by_locale as $vn_i => $va_by_value) {
                                foreach ($va_by_locale as $vs_subfield => $vs_val) {
                                    if (in_array($vs_val, $va_exclude_values)) {
                                        unset($vm_val[$vn_locale_id][$vn_i]);
                                    }
                                }
                            }
                        }
            ...
            } elseif (in_array($vm_val, $va_exclude_values)) {
                $vm_val = null;
            }
        }
        ...
        if ($vb_strip_tags) {
            if (is_array($vm_val)) {
                return array_map(function ($v) {
                    return !is_array($v) ? strip_tags($v) : $v;
                }, $vm_val);
            } else {
    ...
    private function _getRelatedValue($pa_value_list, $pa_options=null)
    {
        ...
        if (!is_array($pa_exclude_idnos	= $pa_options['excludeIdnos'])) {
            $pa_exclude_idnos = [];
        }
        ...
        foreach ($pa_value_list as $vn_i => $va_rel_item) {
            if ($pb_primary_only && isset($va_rel_item['is_primary']) && !$va_rel_item['is_primary']) {
                continue;
            }
        ...
        if (!($qr_rel = caMakeSearchResult($va_path_components['table_name'], $va_ids, array('instance' => $t_rel_instance)))) {
            return null;
        }
        ...
        if (is_array($pa_restrict_to_lists)) {
            $pa_restrict_to_lists = caMakeListIDList($pa_restrict_to_lists);
        }

        // Make sure spec has a table name, otherwise we can get caught in an infinite loop when we pull using the spec
        if ((substr($va_spec[0], 0, 3) !== 'ca_') || !Datamodel::tableExists($va_spec[0])) {
            array_unshift($va_spec, $va_path_components['table_name']);
        }
    ...
    private function _getLabelValue($pa_value_list, $pt_instance, $pa_options)
    {
        ...
        if (!isset($pa_options['output']) && $vb_convert_codes_to_display_text) {
            $pa_options['output'] = 'text';
        }
        if (!isset($pa_options['output']) && $vb_convert_codes_to_idno) {
            $pa_options['output'] = 'idno';
        }
        if (!isset($pa_options['output']) && $vb_convert_codes_to_value) {
            $pa_options['output'] = 'value';
        }
        ...
        if (is_array($pa_value_list)) {
            foreach ($pa_value_list as $vn_locale_id => $va_labels_by_locale) {
                if ($pa_options['useLocaleCodes']) {
                    if (!$vn_locale_id || !($vm_locale_id = SearchResult::$opo_locales->localeIDToCode($vn_locale_id))) {
                ...
                foreach ($va_labels_by_locale as $vn_id => $va_label) {
                    ...
                    if (is_array($va_restrict_to_type_ids) && sizeof($va_restrict_to_type_ids) && !in_array($va_label['type_id'], $va_restrict_to_type_ids)) {
                        continue;
                    }
                    if (is_array($va_exclude_type_ids) && sizeof($va_exclude_type_ids) && in_array($va_label['type_id'], $va_exclude_type_ids)) {
                        continue;
                    }
        ...
        if (!$pa_options['returnAllLocales']) {
            $va_return_values = caExtractValuesByUserLocale($va_return_values, null, $locale ? [$locale] : null);
        }
        if ($pa_options['returnWithStructure']) {
            return is_array($va_return_values) ? $va_return_values : array();
        }
    ...
    private function _getAttributeValue($pa_value_list, $pt_instance, $pa_options)
    {
        ...
        if (!is_array($pa_exclude_idnos) && $pa_exclude_idnos) {
            $pa_exclude_idnos = [$pa_exclude_idnos];
        }
            ...
            foreach ($pa_value_list as $o_attribute) {
                ...
                foreach ($va_values as $o_value) {
                    ...
                    if (is_a($o_value, "AuthorityAttributeValue")) {
                        ...
                        if (!$vb_has_field_spec) {
                            $va_auth_spec = [Datamodel::primaryKey($vs_auth_table_name)];
                        }
        ...
        if (!$pa_options['returnAllLocales'] && !$vb_return_value_id) {
            $va_return_values = caExtractValuesByUserLocale($va_return_values, null, $locale ? [$locale] : null);
        }
        ...
        // Flatten array for return as string or simple array value
        //
        $va_flattened_values = array_map(function ($v) use ($pa_options) {
            return is_array($v) ? join($pa_options['delimiter'], $v) : $v;
        }, $this->_flattenArray($va_return_values, $pa_options));
                    ...
                    foreach ($va_values as $vn_i => $va_value) {
                        ...
                        if ((isset($d['EXIF']) || isset($d['XMP']) || isset($d['IFD0'])) && (in_array(strtolower($va_path_components['subfield_name']), ['title', 'description', 'creator', 'subjects', 'rights', 'copyright', 'date', 'location', 'orientation']))) {
                            ...
                            switch(strtolower($va_path_components['subfield_name'])) {
                                case 'title':
                                    if (isset($d['XMP']['Title'])) {
                                        $exif_value = $d['XMP']['Title'];
                                    }
                                    break;
                                case 'description':
                                    if (isset($d['XMP']['Description'])) {
                                        $exif_value = $d['XMP']['Description'];
                                    }
                                    if (!$exif_value && isset($d['EXIF']['IFD0']['ImageDescription'])) {
                                        $exif_value = $d['EXIF']['IFD0']['ImageDescription'];
                                    }
                                    break;
                                case 'creator':
                                    if (isset($d['XMP']['Creator'])) {
                                        $exif_value = $d['XMP']['Creator'];
                                    }
                                    if (!$exif_value && isset($d['EXIF']['IFD0']['Artist'])) {
                                        $exif_value = $d['EXIF']['IFD0']['Artist'];
                                    }
                                    break;
                                case 'subjects':
                                    if (isset($d['XMP']['Subjects'])) {
                                        $exif_value = $d['XMP']['Subjects'];
                                    }
                                    break;
                                case 'rights':
                                case 'copyright':
                                    if (isset($d['XMP']['Rights'])) {
                                        $exif_value = $d['XMP']['Rights'];
                                    }
                                    if (!$exif_value && isset($d['EXIF']['IFD0']['Copyright'])) {
                                        $exif_value = $d['EXIF']['IFD0']['Copyright'];
                                    }
                                    if (!$exif_value && isset($d['EXIF']['COMPUTED']['Copyright'])) {
                                        $exif_value = $d['EXIF']['COMPUTED']['Copyright'];
                                    }
                                    break;
                                case 'date':
                                    $exif_date = null;
                                    if (isset($d['EXIF']['IFD0']['DateTime'])) {
                                        $exif_date = $d['EXIF']['IFD0']['DateTime'];
                                    }
                                    if (isset($d['EXIF']['EXIF']['DateTimeOriginal'])) {
                                        $exif_date = $d['EXIF']['EXIF']['DateTimeOriginal'];
                                    }
                                    if (isset($d['EXIF']['EXIF']['DateTimeDigitized'])) {
                                        $exif_date = $d['EXIF']['EXIF']['DateTimeDigitized'];
                                    }
                            ...
                            $c = $va_path_components['components'];
                            array_shift($c);
                            array_shift($c);
                            while (sizeof($c) > 0) {
                                $slot = array_shift($c);
    ...
    private function _flattenArray($pa_array, $pa_options=null)
    {
        ...
        if (caGetOption('sort', $pa_options, null)) {
            ...
            if (caGetOption('sortDirection', $pa_options, null, ['forceLowercase' => true]) == 'desc') {
                $va_flattened_values = array_reverse($va_flattened_values);
            }
        }
        return $va_flattened_values;
    }
    ...
    private function getCacheKeyForGetWithTemplate($ps_template, $pa_options)
    {
        ...
        foreach ($pa_options as $vs_k => $vs_v) {
            if (in_array($vs_k, array('useSingular', 'maximumLength', 'delimiter', 'purify', 'restrict_to_types', 'restrict_to_relationship_types',  'restrictToTypes', 'restrictToRelationshipTypes', 'returnAsArray',  'excludeTypes', 'excludeRelationshipTypes'))) {
                continue;
            }
    ...
    public function hasData($pa_fields, $pa_options=null)
    {
        ...
        if (!is_array($pa_fields) && ($pa_fields)) {
            $pa_fields = array($pa_fields);
        }
        ...
        foreach ($pa_fields as $vn_i => $vs_field) {
            ...
            if (!($t_instance = Datamodel::getInstanceByTableName($va_tmp[0], true))) {
                unset($pa_fields[$vn_i]);
                continue;
            }
            if (!$t_instance->hasField($va_tmp[1]) && (!$t_instance->hasElement($va_tmp[1], null, false, array('dontCache' => false)))) {
                unset($pa_fields[$vn_i]);
            }
        }

        $vn_c = 0;
        if (($vn_limit = caGetOption('limit', $pa_options, 10000)) < 1) {
            $vn_limit = 10000;
        }
     ...
     */
    public function getFieldInfo($ps_field)
    {
        $va_tmp = explode(".", $ps_field);
     ...
     *
     */
    public function getMediaPath($ps_field, $ps_version, $pa_options=null)
    {
        if (!is_array($pa_options)) {
            $pa_options = [];
        }
     ...
     *
     */
    public function getMediaPaths($ps_field, $ps_version, $pa_options=null)
    {
        if (!is_array($pa_options)) {
            $pa_options = [];
        }
     ...
     *
     */
    public function getMediaUrls($ps_field, $ps_version, $pa_options=null)
    {
        if (!is_array($pa_options)) {
            $pa_options = [];
        }
     ...
     *
     */
    public function getMediaTags($ps_field, $ps_version, $pa_options=null)
    {
        if (!is_array($pa_options)) {
            $pa_options = [];
        }
     ...
     *
     */
    public function getMediaInfos($ps_field)
    {
        if (!$this->opa_field_media_info[$ps_field]) {
            $this->opa_field_media_info[$ps_field] = $this->get($ps_field, array("unserialize" => true, 'returnWithStructure' => true));
        }
     ...
     *
     */
    public function getMediaVersions($ps_field)
    {
        if (!$this->opa_field_media_info[$ps_field]) {
            $this->opa_field_media_info[$ps_field] = $this->get($ps_field, array("unserialize" => true, 'returnWithStructure' => true));
        }
     ...
     *
     */
    public function hasMedia($ps_field)
    {
        if (!$this->opa_field_media_info[$ps_field]) {
            $this->opa_field_media_info[$ps_field] = $this->get($ps_field, array("unserialize" => true, 'returnWithStructure' => true));
        }
     ...
     *
     */
    public function getMediaScale($ps_field)
    {
        if (!$this->opa_field_media_info[$ps_field]) {
            $this->opa_field_media_info[$ps_field] = $this->get($ps_field, array("unserialize" => true, 'returnWithStructure' => true));
        }
     ...
     *
     */
    public function getMediaMirrorStatus($ps_field, $ps_version, $ps_mirror=null)
    {
        if (!$this->opa_field_media_info[$ps_field]) {
            $this->opa_field_media_info[$ps_field] = $this->get($ps_field, array("unserialize" => true, 'returnWithStructure' => true));
        }
     ...
     *
     */
    public function hasFile($ps_field)
    {
        $va_field = $this->getFieldInfo($ps_field);
     ...
     *
     */
    public function getFileConversionPath($ps_field, $ps_mimetype)
    {
        $va_field = $this->getFieldInfo($ps_field);
     ...
     *
     */
    public function getFileConversionUrl($ps_field, $ps_mimetype)
    {
        $va_field = $this->getFieldInfo($ps_field);
     ...
     *
     */
    public function getDate($ps_field, $pa_options=null)
    {
        $va_field = $this->getFieldInfo($ps_field);
     ...
     *
     */
    public function getTimecode($ps_field, $ps_format=null)
    {
        $va_field = $this->getFieldInfo($ps_field);
        if (is_object($va_field["instance"])) {
     ...
     *
     */
    public function getChoiceListValue($ps_field)
    {
        $va_field = $this->getFieldInfo($ps_field);
        if (is_object($va_field["instance"])) {
     ...
     *
     */
    public function getVars($ps_field)
    {
        $va_field = $this->getFieldInfo($ps_field);
        if (is_object($va_field["instance"])) {
    ...
    {
        if (isset(SearchResult::$s_parsed_field_component_cache[$this->ops_table_name.'/'.$ps_path])) {
            return SearchResult::$s_parsed_field_component_cache[$this->ops_table_name.'/'.$ps_path];
        }
    ...
    public function getResultCountForFieldValues($pa_field_list, $vb_sort=false)
    {
        ...
        if (isset($this->opa_cached_result_counts[$vs_key])) {
            return  $this->opa_cached_result_counts[$vs_key];
        }
     ...
     *
     */
    public static function _isHierarchyModifier($pm_modifier)
    {
