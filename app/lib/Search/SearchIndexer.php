...
class SearchIndexer extends SearchBase
{
    ...
    public static $s_search_indexing_queue_inserts = [];
    public static $s_search_unindexing_queue_inserts = [];

    /**
     *
     */
    public static $s_search_config_field_entry_to_index_field_nums_cache = [];
    ...
    {
        if (ExternalCache::contains('getIndexedTables', 'SearchIndexer')) {
            return ExternalCache::fetch('getIndexedTables', 'SearchIndexer');
        };
    ...
    {
        if (!($t_instance = Datamodel::getInstance($pm_table_name_or_num))) {
            return null;
        }
    ...
    public function indexRow($pn_subject_table_num, $pn_subject_row_id, $pa_field_data, $pb_reindex_mode=false, $pa_exclusion_list=null, $pa_changed_fields=null, $pa_options=null)
    {
        ...
        if (!$pb_reindex_mode && !$for_current_value_reindex && is_array($pa_changed_fields) && !sizeof($pa_changed_fields)) {
            return;
        ...
        if (is_array($pa_exclusion_list[$pn_subject_table_num]) && (isset($pa_exclusion_list[$pn_subject_table_num][$pn_subject_row_id]))) {
            return;
        }
            ...
            if (is_array($va_fields_to_index)) {
                ...
                foreach ($va_fields_to_index as $vs_field => $va_data) {
                        ...
                        if ($va_data['datatype'] === 0) {
                            ...
                            if (is_array($child_ids = ca_metadata_elements::getElementsForSet($va_matches[1], ['idsOnly' => true]))) {
                                foreach ($child_ids as $child_id) {
                                    if (!is_array($va_fields_to_index['_ca_attribute_'.$child_id])) {
                                        continue;
                                    }
                    ...
                    //
                        ...
                        if (is_null($vn_fld_num = $t_subject->fieldNum($vs_field))) {
                            continue;
                        }
                    ...
                    //
                        ...
                        if (((isset($va_data['INDEX_AS_IDNO']) && $va_data['INDEX_AS_IDNO']) || in_array('INDEX_AS_IDNO', $va_data, true)) && method_exists($t_subject, "getIDNoPlugInInstance") && ($o_idno = $t_subject->getIDNoPlugInInstance())) {
                            if ((is_array($va_data['IDNO_DELIMITERS']) && count($va_data['IDNO_DELIMITERS'])) || strlen($va_data['IDNO_DELIMITERS'])) {
                                if (!is_array($va_data['IDNO_DELIMITERS'])) {
                                    $va_data['IDNO_DELIMITERS'] = [$va_data['IDNO_DELIMITERS']];
                                }
                                $va_values = array_map(function ($v) {
                                    return trim($v);
                                }, preg_split('!('.join('|', $va_data['IDNO_DELIMITERS']).')!', $pa_field_data[$vs_field]));
                            } else {
                        ...
                        if (in_array($va_field_list[$vs_field]['FIELD_TYPE'], array(FT_DATERANGE,FT_HISTORIC_DATERANGE))) {
                            ...
                            if (
                                ...
                                if (!$t_item) {
                                    continue;
                                }		// list item doesn't exist (can happen for access and status values)
                ...
                foreach ($va_related_tables as $vs_related_table) {
                    ...
                    if (is_array($policies = $va_info['current_values'])) {
                        foreach ($policies as $p => $pinfo) {
                            ...
                            if (is_array($history) && (sizeof($history) > 0)) {
                                ...
                                if ($current_history_type !== $current_value['type']) {
                                    continue;
                                }
                    ...
                    foreach ($va_queries as $vn_i => $va_query) {
                        ...
                        while ($qr_res->nextRow()) {
                            ...
                            if ($for_current_value_reindex) {
                                $fields_to_index_list = ['current_values' => $va_cv_fields_to_index];
                            }
                            ...
                            foreach ($fields_to_index_list as $m => $field_lists) {
                                ...
                                foreach ($field_lists as $p => $field_list) {
                                    if ($m == 'current_values') {
                                        if (!isset($current_value_ids[$vn_rel_subject_row_id]) || !in_array($p, $current_value_ids[$vn_rel_subject_row_id])) {
                                            continue;
                                        }
                                    }
        ...
        //
                                        ...
                                        if (substr($vs_rel_field, 0, 14) === '_ca_attribute_') {
                                            ...
                                            if ($va_rel_field_info['datatype'] === 0) {
                                                ...
                                                if (is_array($child_ids = ca_metadata_elements::getElementsForSet($va_matches[1], ['idsOnly' => true]))) {
                                                    foreach ($child_ids as $child_id) {
                                                        if (!is_array($field_list['_ca_attribute_'.$child_id])) {
                                                            continue;
                                                        }
                                                    ...
                                                    foreach ($field_nums as $field_num => $is_generic) {
                                                        if (((isset($va_rel_field_info['INDEX_AS_IDNO']) && $va_rel_field_info['INDEX_AS_IDNO']) || in_array('INDEX_AS_IDNO', $va_rel_field_info, true)) && method_exists($t_rel, "getIDNoPlugInInstance") && ($o_idno = $t_rel->getIDNoPlugInInstance())) {
                                                            ...
                                                            if (strlen($va_rel_field_info['IDNO_DELIMITERS']) || (is_array($va_rel_field_info['IDNO_DELIMITERS']) && count($va_rel_field_info['IDNO_DELIMITERS']))) {
                                                                if (!is_array($va_rel_field_info['IDNO_DELIMITERS'])) {
                                                                    $va_rel_field_info['IDNO_DELIMITERS'] = [$va_rel_field_info['IDNO_DELIMITERS']];
                                                                }
                                                                $va_values = array_map(function ($v) {
                                                                    return trim($v);
                                                                }, preg_split('!('.join('|', $va_rel_field_info['IDNO_DELIMITERS']).')!', $vs_fld_data));
                                                            } else {
                    ...
                    if ($index_ancestors) {
                        ...
                        if ($t_label->load($va_row_to_reindex['field_row_id'], false)) {
                            ...
                            foreach ($va_row_to_reindex['row_ids'] as $vn_row_id) {
                                ...
                                if (!is_array($va_children_ids = $t_instance->getHierarchyAsList($vn_row_id, ['idsOnly' => true]))) {
                                    $va_children_ids = [];
                                }
                                ...
                                if (is_array($va_row_to_reindex['cv_indexing_info'])) {
                                    foreach ($va_row_to_reindex['cv_indexing_info'] as $p => $pinfo) {
                                        if (!isset($current_values[$p][$va_row_to_reindex['table_num']][$vn_row_id])) {
                                            continue;
                                        }
                                        foreach ($va_children_ids as $id) {		// Apply indexing to each child
                                            foreach ($va_content['values'] as $v_id => $v) {		// Apply each hierarchical value with correct field row_id
                                        ...
                                        foreach ($va_attributes as $vo_attribute) {
                                            foreach ($vo_attribute->getValues() as $vo_value) {
                                                ...
                                                if ($t_to_reindex = Datamodel::getInstanceByTableNum($va_row_to_reindex['table_num'], true)) {
                                                    ...
                                                    if (is_array($va_row_to_reindex['cv_indexing_info'])) {
                                                        foreach ($va_row_to_reindex['cv_indexing_info'] as $p => $pinfo) {
                                                            if (!isset($current_values[$p][$va_row_to_reindex['table_num']][$vn_rel_row_id])) {
                                                                continue;
                                                            }
    ...
    {
        if (!method_exists($pt_subject, "getAttributesByElement")) {
            return true;
        }
                    ...
                    foreach ($va_attributes as $i => $vo_attribute) {
                        ...
                        if (is_array($va_sub_element_ids) && sizeof($va_sub_element_ids)) {
                            ...
                            foreach ($vo_attribute->getValues() as $vo_value) {
                                ...
                                $va_sub_data = array_filter($va_sub_data, function ($v) {
                                    return !is_array($v) && (strlen($v) > 0);
                                });
                    ...
                    foreach ($va_attributes as $vo_attribute) {
                        foreach ($vo_attribute->getValues() as $vo_value) {
                            ...
                            if ($vn_datatype == __CA_ATTRIBUTE_VALUE_LIST__) {
                                ...
                                $this->_genIndexInheritance(
                                    $t_inheritance_subject ? $t_inheritance_subject : $pt_subject,
                                    $t_inheritance_subject ? $pt_subject : null,
                                    $field_num_prefix.$vn_element_id,
                                    $pn_inheritance_subject_id ? $pn_inheritance_subject_id :
                                        ($pn_inheritance_subject_id ? $pn_inheritance_subject_id : $pn_row_id),
                                    $pn_row_id,
                                    [$vs_v],
                                    array_merge($pa_data, ['DONT_TOKENIZE' => 1])
                                );
                            }
                        ...
                        foreach ($vo_attribute->getValues() as $vo_value) {
                            ...
                            if (((isset($pa_data['INDEX_AS_IDNO']) && $pa_data['INDEX_AS_IDNO']) || in_array('INDEX_AS_IDNO', $pa_data, true)) && method_exists($pt_subject, "getIDNoPlugInInstance") && ($o_idno = $pt_subject->getIDNoPlugInInstance())) {
                                if (strlen($pa_data['IDNO_DELIMITERS']) || (is_array($pa_data['IDNO_DELIMITERS']) && count($pa_data['IDNO_DELIMITERS']))) {
                                    if (!is_array($pa_data['IDNO_DELIMITERS'])) {
                                        $pa_data['IDNO_DELIMITERS'] = [$pa_data['IDNO_DELIMITERS']];
                                    }
                                    $va_values = array_map(function ($v) {
                                        return trim($v);
                                    }, preg_split('!('.join('|', $pa_data['IDNO_DELIMITERS']).')!', $vs_value_to_index));
                                } else {
        ...
        if (is_array($this->opa_dependencies_to_update)) {
            ...
            if (!$vb_can_do_incremental_indexing) {
                ...
                foreach ($this->opa_dependencies_to_update as $va_item) {
                    ...
                    if (isset($va_seen_items[$va_item['table_num']][$va_item['row_id']])) {
                        continue;
                    }
            ...
            foreach ($va_dep_rel_indexing_tables as $vs_dep_rel_table) {
                ...
                foreach ($va_rel_tables_to_index_list as $vs_rel_table) {
                    ...
                    foreach ($va_table_path as $vs_n => $va_linking_tables_config) {
                        ...
                        if (!in_array($vs_dep_table, $va_table_list)) {
                            array_unshift($va_table_list, $vs_dep_table);
                        }
                        if (!in_array($vs_rel_table, $va_table_list)) {
                            $va_table_list[] = $vs_rel_table;
                        }
                        if (!in_array($vs_subject_tablename, $va_table_list)) {
                            continue;
                        }
                        ...
                        foreach ($va_cv_fields_to_index as $p => $pinfo) {
                            foreach ($pinfo as $pf => $pi) {
                                if (!isset($all_fields_to_index[$pf])) {
                                    $all_fields_to_index[$pf] = $pi;
                                }
                            }
                        }

                        if ($vs_rel_table == $vs_subject_tablename) {
                            if (is_array($pa_changed_field_nums) && !$this->_indexedFieldsHaveChanged($all_fields_to_index, $pa_changed_field_nums)) {
                                continue;
                            } // check if the current field actually needs indexing; only do this check if we've been passed a list of changed fields, otherwise we have to assume that everything has changed
                        }
    ...
    private function _getRelatedRows($pa_tables, $pa_linking_tables_config, $pa_table_keys, $ps_subject_tablename, $pn_row_id, $ps_table_to_index, $pa_fields_to_index)
    {
        ...
        if (true) { //!MemoryCache::contains($vs_key, 'SearchIndexerRelatedRowsJoins')) {
            ...
            foreach ($pa_tables as $vs_right_table) {
                ...
                if (is_array($pa_table_keys) && (isset($pa_table_keys[$vs_right_table][$vs_left_table]) || isset($pa_table_keys[$vs_left_table][$vs_right_table]))) {		// are the keys for this join specified in the indexing config?
                    ...
                    if (isset($va_field_names[$vs_left])) {
                        unset($va_flds[$va_field_names[$vs_left]]);
                    }
                    $va_flds[$va_field_names[$vs_left] = "{$vs_prev_alias}.{$vs_left}"] = true;

                    if (isset($va_field_names[$vs_right])) {
                        unset($va_flds[$va_field_names[$vs_right]]);
                    }
                    ...
                    if (isset($va_field_names[$vs_many])) {
                        unset($va_flds[$va_field_names[$vs_many]]);
                    }
                    ...
                    if (isset($va_field_names[$vs_one])) {
                        unset($va_flds[$va_field_names[$vs_one]]);
                    }
            ...
            if ($t_indexed_table = Datamodel::getInstanceByTableName($ps_table_to_index, true)) {
                foreach ($pa_fields_to_index as $vs_f => $va_field_info) {
                    if (!$t_indexed_table->hasField($vs_f)) {
                        continue;
                    }
                    if (in_array($t_indexed_table->getFieldInfo($vs_f, 'FIELD_TYPE'), array(FT_MEDIA, FT_FILE, FT_VARS, FT_DATERANGE, FT_HISTORIC_DATERANGE, FT_TIMERANGE))) {
                        continue;
                    }
                    if (isset($va_fld_names[$vs_f]) && $va_fld_names[$vs_f]) {
                        continue;
                    }

                    if (isset($va_fld_names[$vs_f])) {
                        continue;
                    }
            ...
            foreach ($va_tmp as $vn_i => $vs_v) {
                if (in_array($t_select->getFieldInfo($vs_v, 'FIELD_TYPE'), array(FT_MEDIA, FT_FILE, FT_VARS, FT_DATERANGE, FT_HISTORIC_DATERANGE, FT_TIMERANGE))) {
                    continue;
                }
                if (isset($va_fld_names[$vs_v]) && $va_fld_names[$vs_v]) {
                    continue;
                }
        ...
        foreach ($va_joins as $vn_i => $va_join_list) {
            if (!is_array($va_joins[$vn_i])) {
                $va_joins[$vn_i] = array($va_joins[$vn_i]);
            }
    ...
    private function _getQueriesForRelatedRows($pt_subject, $pn_subject_row_id, $pt_rel, $pb_reindex_mode, $pa_options=null)
    {
        ...
        if (!$pb_force_related && ($vs_subject_tablename == $vs_related_table)) {
            // self-relation
            if (!($vs_self_rel_table_name = $pt_rel->getSelfRelationTableName())) {
                return null;
            }
            $t_self_rel = Datamodel::getInstanceByTableName($vs_self_rel_table_name, true);
            ...
            $va_self_info = $this->getTableIndexingInfo($vs_subject_tablename, $vs_subject_tablename);
            if (!is_array($va_fields_to_index = $va_self_info['related']['fields'])) {
                $va_fields_to_index = [];
            }
            if (!is_array($va_cv_fields_to_index = $va_self_info['related']['current_values'])) {
                $va_cv_fields_to_index = [];
            }
            ...
            if ($pt_rel->hasField('type_id')) {
                $va_proc_field_list[] = $vs_related_table.'.type_id';
            }
            if ($pt_rel->hasField('access')) {
                $va_proc_field_list[] = $vs_related_table.'.access';
            }
            ...
            if (!is_array($va_fields_to_index = $this->getFieldsToIndex($vs_subject_tablename, $pb_force_related ? "{$vs_related_table}.related" : $vs_related_table))) {
                $va_fields_to_index = [];
            }
            if (!is_array($va_cv_fields_to_index = $this->getFieldsToIndex($vs_subject_tablename, $pb_force_related ? "{$vs_related_table}.related" : $vs_related_table, ['currentValueFields' => true]))) {
                $va_cv_fields_to_index = [];
            }
            $va_field_list = array_keys($va_fields_to_index);
            ...
            foreach ($va_table_list_list as $vs_list_name => $va_linking_tables_config) {
                ...
                if ($vs_type_id_fld) {
                    $va_proc_field_list[] = $vs_type_id_fld;
                }
    ...
    private static function _genQueryFilters(string $table, array $table_info, ?array $linking_tables_config, string $alias)
    {
        ...
        if ($filters = caGetOption('filter', $table_info, caGetOption('filter', $linking_tables_config, null))) {
            ...
            foreach ($filters as $k => $v) {
                if (!($field_info = Datamodel::getFieldInfo($table, $k))) {
                    continue;
                }
                switch($k) {
                    case 'status':
    ...
    private function _doCountIndexing($pt_subject, $pn_subject_row_id, $pt_rel, $pb_reindex_mode, $pa_options=null)
    {
        ...
        if (isset($va_fields_to_index['_count']) && is_array($va_fields_to_index['_count'])) {
            ...
            if (!is_array($va_rel_field_info = $va_fields_to_index['_count'])) {
                $va_rel_field_info = [];
            }
