...
class SearchBase extends BaseFindEngine
{
    ...
    public static $s_fields_to_index_cache = array();
    # ------------------------------------------------
    /**
     ...
     */
    public static function newSearchEngine($plugin_name=null, $db=null)
    {
        if (!$plugin_name) {
            $plugin_name = self::searchEngineName();
        }
        if (!file_exists(__CA_LIB_DIR__.'/Plugins/SearchEngine/'.$plugin_name.'.php')) {
            return null;
        }
     ...
     */
    public static function searchEngineName(): string
    {
     ...
     * @return string Class name of search engine (WLPlugSearchEngine<engine name>)
     */
    public static function searchEngineClassName(): string
    {
        $o_config = Configuration::load();
        if (!($plugin_name = $o_config->get('search_engine_plugin'))) {
            throw ApplicationException(_t('No search engine plugin configured'));
    ...
    public function getFieldsToIndex($pm_subject_table, $pm_content_table=null, $pa_options=null)
    {
        ...
        if (!$current_value_fields_only) {
            $va_fields_to_index = $va_fields_to_index[0];
        }
    ...
    public function getRelatedIndexingTables($pm_subject_table)
    {
        ...
        if (!isset($va_info[$pm_subject_table]['related']) && (!$vs_label_table || !isset($va_info[$vs_label_table]['related']))) {
            unset($va_info[$pm_subject_table]);
        }	// remove subject table _unless_ 'related' indexing is enabled in subject or subject's label
    ...
    public function getTableIndexingInfo($pm_subject_table, $pm_content_table)
    {
        ...
        if (!isset($va_info[$pm_content_table]['tables']) || !$va_info[$pm_content_table]['tables']) {
            $va_info[$pm_content_table]['tables'] = array();
        }
