...
class BrowseEngine extends BaseFindEngine
{
     ...
     * @var Type_id cache
     */
    public static $s_type_id_cache = array();
    public static $s_source_id_cache = array();
    # ------------------------------------------------------
    /**
     *
     */
    ...
    public function addCriteria($ps_facet_name, $pa_row_ids, $pa_display_strings=null)
    {
        ...
        if ($va_facet_info['type'] == 'hierarchy') {
            $va_criteria[$ps_facet_name] = [];
        }
        ...
        foreach ($pa_row_ids as $vn_i => $vn_row_id) {
            ...
            if (isset($pa_display_strings[$vn_i])) {
                $va_criteria_display_strings[$ps_facet_name][urldecode($vn_row_id)] = $pa_display_strings[$vn_i];
            }
        }
    ...
    {
        if ($ps_facet_name && !$this->isValidFacetName($ps_facet_name)) {
            return false;
        }
    ...
    public function getCriterionLabel($ps_facet_name, $pn_row_id)
    {
        ...
        switch($va_facet_info['type']) {
            # -----------------------------------------------------
            case 'hierarchy':
                if (!($t_table = Datamodel::getInstanceByTableName($this->ops_browse_table_name, true))) {
                    break;
                }
            ...
            case 'has':
                ...
                # -----------------------------------------------------
            case 'label':
                if (!($t_table = Datamodel::getInstanceByTableName((isset($va_facet_info['relative_to']) && $va_facet_info['relative_to']) ? $va_facet_info['relative_to'] : $this->ops_browse_table_name, true))) {
                    break;
                }
                if (isset($va_facet_info['browse_by_first_letter']) && $va_facet_info['browse_by_first_letter']) {
                    return _t('Beginning with %1', $pn_row_id);
                }
                if (!$t_table->load($pn_row_id)) {
                    return '???';
                }
                ...
                # -----------------------------------------------------
            case 'authority':
                if (!($t_table = Datamodel::getInstanceByTableName($va_facet_info['table'], true))) {
                    break;
                }
                if (!$t_table->load($pn_row_id)) {
                    return '???';
            ...
            case 'relationship_types':
                if (!($t_rel = Datamodel::getInstanceByTableName($va_facet_info['relationship_table'], true))) {
                    break;
                }
            ...
            case 'attribute':
                ...
                # -----------------------------------------------------
            case 'field':
                if (!($t_item = Datamodel::getInstanceByTableName($va_facet_info['relative_to'] ? $va_facet_info['relative_to'] : $this->ops_browse_table_name, true))) {
                    break;
                }
                ...
                # -----------------------------------------------------
            case 'violations':
                if (!($t_rule = Datamodel::getInstanceByTableName('ca_metadata_dictionary_rules', true))) {
                    break;
                }
            ...
            case 'normalizedDates':
                ...
                # -----------------------------------------------------
            case 'fieldList':
                if (!($t_item = Datamodel::getInstanceByTableName($this->ops_browse_table_name, true))) {
                    break;
                }
                    ...
                    switch($va_facet_info['type']) {
                        ...
                        case 'has':
                            ...
                            if ($va_facet_info['element_code']) {
                                ...
                                if (!is_array($va_restrict_to_types = $this->_convertTypeCodesToIDs($va_restrict_to_types, array('instance' => $t_rel_item)))) {
                                    $va_restrict_to_types = [];
                                }
                                ...
                                if (!is_array($va_exclude_types = $this->_convertTypeCodesToIDs($va_exclude_types, array('instance' => $t_rel_item)))) {
                                    $va_exclude_types = [];
                                }
                            ...
                            if ($t_item_rel && isset($va_facet_info['filter_on_interstitial']) && is_array($va_facet_info['filter_on_interstitial']) && sizeof($va_facet_info['filter_on_interstitial'])) {
                                foreach ($va_facet_info['filter_on_interstitial'] as $vs_field_name => $va_values) {
                                    ...
                                    if (!($vn_element_id = (int)ca_metadata_elements::getElementID($vs_field_name))) {
                                        continue;
                                    }
                                    if (!($o_value = Attribute::getValueInstance(ca_metadata_elements::getElementDatatype($vs_field_name)))) {
                                        continue;
                                    }
                        ...
                        case 'field':
                            ...
                            if ($t_user && $t_user->isLoaded() && ($t_user->getBundleAccessLevel($vs_table_name, $bundle_name) < __CA_BUNDLE_ACCESS_READONLY__)) {
                                break;
                            }
                        ...
                        case 'attribute':
                            ...
                            if ($t_user && $t_user->isLoaded() && ($t_user->getBundleAccessLevel($this->ops_browse_table_name, array_shift(explode(".", $va_facet_info['element_code']))) < __CA_BUNDLE_ACCESS_READONLY__)) {
                                break;
                            }

                            if ($t_user->getBundleAccessLevel($this->ops_browse_table_name, $va_facet_info['element_code']) < __CA_BUNDLE_ACCESS_READONLY__) {
                                break;
                            }
                                        ...
                                        switch($vn_datatype) {
                                            case __CA_ATTRIBUTE_VALUE_LIST__:
                                                ...
                                                if (!caGetOption(['dontExpandHierarchically', 'dont_expand_hierarchically'], $va_facet_info, false)) {
                                                    ...
                                                    $va_attr_values[] = array_map(function ($v) {
                                                        return (int)$v;
                                                    }, array_unique($va_item_ids));
                                                } else {
                        ...
                        case 'relationship_types':
                            ...
                            if ($t_user && $t_user->isLoaded() && ($t_user->getBundleAccessLevel($this->ops_browse_table_name, $vs_rel_table_name) < __CA_BUNDLE_ACCESS_READONLY__)) {
                                break;
                            }
                            ...
                            foreach ($va_row_ids as $vn_row_id) {
                                ...
                                if ($this->ops_browse_table_name == $vs_rel_table_name) {
                                    ...
                                    if (!($self_rel_table = $t_rel_item->getProperty('SELF_RELATION_TABLE_NAME'))) {
                                        break;
                                    }
                        ...
                        case 'location':
                        case 'current_value':
                            if ($t_user && $t_user->isLoaded() && ($t_user->getBundleAccessLevel($this->ops_browse_table_name, 'ca_objects_location') < __CA_BUNDLE_ACCESS_READONLY__)) {
                                return [];
                            }
                            ...
                            foreach ($va_row_ids as $vn_row_id) {
                                ...
                                if ($va_row_tmp[0] == 89) { // ca_storage_locations
                                    ...
                                    if (!is_array($va_loc_ids = $t_loc->getHierarchy($va_row_tmp[2], ['returnAsArray' => true, 'includeSelf' => true, 'idsOnly' => true])) || !sizeof($va_loc_ids)) {
                                        continue;
                                    }
                        ...
                        case 'inHomeLocation':
                            ...
                            if ($this->ops_browse_table_name !== 'ca_objects') {
                                return [];
                            }
                        ...
                        case 'fieldList':
                            ...
                            foreach ($va_row_ids as $vn_row_id) {
                                ...
                                if (($vs_field_name === 'access') || ($vs_field_name === 'status')) {
                                    ...
                                    if (!($t_list_item = ca_list_items::find($vn_row_id, ['returnAs' => 'firstModelInstance', 'checkAccess' => $pa_options['checkAccess']]))) {
                                        break;
                                    }
                            ...
                            switch($vs_facet_name) {
                                case '_search':
                                    ...
                                    if (is_array($va_row_ids) && sizeof($va_row_ids) > 1) {
                                        ...
                                        $va_row_ids = array_filter($va_row_ids, function ($a) {
                                            return !($a === '*');
                                        });
                                    }
                                ...
                                case '_reltypes':
                                    ...
                                    foreach ($va_row_ids as $vs_tmp) {
                                        ...
                                        if (!($t_target = Datamodel::getInstanceByTableName($va_tmp[0], true))) {
                                            break;
                                        }
                                        ...
                                        if (is_array($va_path_to_target) && (sizeof($va_path_to_target) == 3)) {
                                            if (!($t_target_rel = Datamodel::getInstanceByTableName($va_path_to_target[1], true)) || !$t_target_rel->isRelationship() || !$t_target_rel->hasField('type_id')) {
                                                break;
                                            }
    ...
    public function loadFacetContent($pa_options=null)
    {
        ...
        if (($o_results->numHits() > 1) || !sizeof($va_criteria)) {
            ...
            foreach ($va_facets as $vs_facet_name) {
                $va_facet_info = $this->getInfoForFacet($vs_facet_name);
                if (($va_facet_info['type'] === 'field') && ($va_facet_info['field'] === 'is_deaccessioned') && $pa_options['filterDeaccessionedRecords']) {
                    continue;
                }
                if (
    ...
    public function getFacet($ps_facet_name, $pa_options=null)
    {
        ...
        if ($vb_needs_caching) {
            if (!is_array($va_facet_content)) {
                $va_facet_content = array();
            }
    ...
    public function getHierarchyIDsForFacet($ps_facet_name, $pa_options=null)
    {
        ...
        foreach ($va_facet_cache[$ps_facet_name] as $vn_id => $va_item) {
            if (!isset($va_item['hierarchy_id']) || !$va_item['hierarchy_id']) {
                continue;
            }
    ...
    public function getFacetContent($ps_facet_name, $pa_options=null)
    {
        ...
        if (!is_array($va_criteria = $this->getCriteria($ps_facet_name))) {
            $va_criteria = [];
        }
        ...
        if (caGetOption('returnFullFacet', $pa_options, false)) {
            $va_results = null;
            $va_container_ids = null;
        } else {
            $va_results = $this->opo_ca_browse_cache->getResults();
            if (!is_array($va_container_ids = $this->opo_ca_browse_cache->getParameter('container_ids'))) {
                $va_container_ids = [];
            }
        }
        ...
        switch($va_facet_info['type']) {
            ...
            case 'has':
                ...
                if ($va_facet_info['element_code']) {
                    ...
                    foreach ($va_facet_values as $vs_state_name => $va_state_info) {
                        ...
                        if ($filter_deaccessioned) {
                            $va_wheres[] = $filter_deaccessioned;
                        }
                        $va_joins = array();
            ...
            case 'label':
                ...
                if ($t_user && $t_user->isLoaded() && ($t_user->getBundleAccessLevel($vs_browse_table_name, 'preferred_labels') < __CA_BUNDLE_ACCESS_READONLY__)) {
                    return [];
                }
                ...
                if (sizeof($va_label_order_by_fields = isset($va_facet_info['order_by_label_fields']) ? $va_facet_info['order_by_label_fields'] : [])) {
                    $va_label_order_by_fields = array_map(function ($v) {
                        return "l.{$v}";
                ...
                if (is_array($va_label_ui_fields) && sizeof($va_label_ui_fields)) {
                    $va_label_ui_fields = array_map(function ($v) {
                        return "l.{$v}";
                    }, $va_label_ui_fields);
                }

                foreach ($va_label_ui_fields as $x) {
                    if (!in_array($x, $va_label_order_by_fields)) {
                        $va_label_order_by_fields[] = $x;
                    }
                }
                ...
                if ($vb_check_availability_only) {
                    ...
                    if ($vs_parent_fld_select) {
                        $group_by_fields[] = $vs_parent_fld_select;
                    }
                    $vs_sql = "
                    ...
                    while ($qr_res->nextRow()) {
                        ...
                        if ($browse_by_first_letter) {
                            ...
                            if (!preg_match("!^[a-z]+$!", $letter)) {
                                $letter = "#";
                            }	// map all non-alphabetic values to "#"
            ...
            case 'attribute':
                ...
                if ($t_user && $t_user->isLoaded() && ($t_user->getBundleAccessLevel($vs_browse_table_name, array_shift(explode(".", $va_facet_info['element_code']))) < __CA_BUNDLE_ACCESS_READONLY__)) {
                    return [];
                }
                ...
                if ($vb_check_availability_only) {
                    ...
                    if (is_array($va_suppress_values = caGetOption(['suppress', 'exclude_values'], $va_facet_info, null))) {
                        $suppress_null = (sizeof(array_filter($va_suppress_values, function ($v) {
                            return trim(strtolower($v)) === 'null';
                        })) > 0);
                        $va_suppress_values = array_filter($va_suppress_values, function ($v) {
                            return trim(strtolower($v)) !== 'null';
                        }); // remove
                    ...
                    switch($vn_element_type) {
                        case __CA_ATTRIBUTE_VALUE_LIST__:
                            if (!is_array($va_restrict_to_types = $this->_convertTypeCodesToIDs($va_facet_info['restrict_to_types'], array('instance' => new ca_list_items(), 'dontExpandHierarchically' => true)))) {
                                $va_restrict_to_types = array();
                            }
                            ...
                            foreach ($va_values as $i => $vn_val) {
                                ...
                                if ($va_list_item_cache[$vn_val]['deleted'] > 0) {
                                    continue;
                                }
                                ...
                                if (is_array($va_restrict_to_types) && sizeof($va_restrict_to_types) && !in_array($va_list_item_cache[$vn_val]['type_id'], $va_restrict_to_types)) {
                                    continue;
                                }
                                ...
                                if (!($vs_label = html_entity_decode($va_list_label_cache[$vn_val]))) {
                                    $vs_label = '['.caGetBlankLabelText('ca_list_items').']';
                                }
                                if (is_array($va_facet_info['relabel']) && isset($va_facet_info['relabel'][$vs_label])) {
                                    $vs_label = $va_facet_info['relabel'][$vs_label];
                                }
                            ...
                            if (!caGetOption(['dontExpandHierarchically', 'dont_expand_hierarchically'], $va_facet_info, false)) {
                                ...
                                if ($qr_ancestors) {
                                    ...
                                    while ($qr_ancestors->nextHit()) {
                                        if ($qr_ancestors->get('deleted')) {
                                            continue;
                                        }
                                        $vn_ancestor_id = (int)$qr_ancestors->get("{$vs_rel_pk}");

                                        $vn_parent_type_id = $qr_ancestors->get('type_id');
                                        if (is_array($va_suppress_values) && (in_array($vn_ancestor_id, $va_suppress_values))) {
                                            continue;
                                        }
                                        if (is_array($va_exclude_types) && (sizeof($va_exclude_types) > 0) && in_array($vn_parent_type_id, $va_exclude_types)) {
                                            continue;
                                        }
                                        if (is_array($va_restrict_to_types) && (sizeof($va_restrict_to_types) > 0) && !in_array($vn_parent_type_id, $va_restrict_to_types)) {
                                            continue;
                                        }
                    ...
                    while ($qr_res->nextRow()) {
                        ...
                        if (is_array($access) && isset($access[$qr_res->get('value_integer1')]) && ($access[$qr_res->get('value_integer1')] != 1)) {
                            continue;
                        }
                        switch($vn_element_type) {
                            case __CA_ATTRIBUTE_VALUE_LIST__:
            ...
            case 'current_value':
                $t_item = Datamodel::getInstanceByTableName($vs_browse_table_name, true);
                if ($t_user && $t_user->isLoaded() && ($t_user->getBundleAccessLevel($vs_browse_table_name, 'ca_objects_location') < __CA_BUNDLE_ACCESS_READONLY__)) {
                    return [];
                }
                if (!is_array($va_restrict_to_types = $va_facet_info['restrict_to_types'])) {
                    $va_restrict_to_types = array();
                ...
                if ($vb_check_availability_only) {
                    ...
                    while ($qr_res->nextRow()) {
                        if (!($current_table_num = trim($qr_res->get('current_table_num')))) {
                            continue;
                        }
                        if (!($current_row_id = trim($qr_res->get('current_row_id')))) {
                            continue;
                        }
                    ...
                    foreach ($values_by_table as $current_table_num => $current_row_ids_by_type) {
                        foreach ($current_row_ids_by_type as $current_type_id => $current_row_info) {
                            $current_row_ids = array_map(function ($v) {
                                return $v['current_row_id'];
                            }, $current_row_info);
                            if (!sizeof($current_row_ids)) {
                                continue;
                            }
                            ...
                            if ($current_table_name == 'ca_storage_locations') {
                                ...
                                if (!$vn_max_browse_depth) {
                                    $vn_max_browse_depth = null;
                                }
                            ...
                            while ($qr_res->nextHit()) {
                                ...
                                if (isset($va_collapse_map[$current_table_name]) && isset($va_collapse_map[$current_table_name][$type_id]) && $va_collapse_map[$current_table_name][$type_id]) {
                                    if (!($vs_label = $va_collapse_map[$current_table_name][$type_id])) {
                                        continue;
                                    }
                                ...
                                if (!$row_id || !($vs_label = $qr_res->getWithTemplate($vs_template, $va_config))) {
                                    continue;
                                }
            ...
            case 'fieldList':
                ...
                // Map deaccession fields to deaccession bundle, which is used for access control on all
                $bundle_name = (in_array($vs_field_name, ['deaccession_type_id'])) ? 'ca_objects_deaccession' : $vs_field_name;
                if ($t_user && $t_user->isLoaded() && ($t_user->getBundleAccessLevel($vs_browse_table_name, $bundle_name) < __CA_BUNDLE_ACCESS_READONLY__)) {
                    return [];
                }
                ...
                if ($vb_check_availability_only) {
                    ...
                    while ($qr_res->nextRow()) {
                        if ($t_rule->load($qr_res->get('rule_id'))) {
                            if (!($vs_val = trim(caExtractSettingsValueByUserLocale('label', $t_rule->getSettings())))) {
                                continue;
                            }
            ...
            case 'normalizedDates':
                ...
                $vs_container_code = (is_array($va_element_code) && (sizeof($va_element_code) > 0)) ? array_pop($va_element_code) : null;
                if ($t_user && $t_user->isLoaded() && ($t_user->getBundleAccessLevel($vs_browse_table_name, $vs_container_code) < __CA_BUNDLE_ACCESS_READONLY__)) {
                    return [];
                }
                        ...
                        while ($qr_res->nextRow()) {
                            ...
                            foreach ($va_normalized_values as $vn_sort_value => $vs_normalized_value) {
                                if ($va_criteria[$vs_normalized_value]) {
                                    continue;
                                }		// skip items that are used as browse criteria - don't want to browse on something you're already browsing on

                                if ($seen[$vs_normalized_value][$row_id]) {
                                    continue;
                                }
            ...
            case 'relationship_types':
                ...
                if ($t_user && $t_user->isLoaded() && ($t_user->getBundleAccessLevel($vs_browse_table_name, $vs_rel_table_name) < __CA_BUNDLE_ACCESS_READONLY__)) {
                    return [];
                }
                ...
                if ($vb_check_availability_only) {
                    ...
                    $vs_sql .= " GROUP BY ".join(', ', array_map(function ($v) {
                        return str_replace(' rel_type_id', '', $v);
                    }, $va_select_flds));
                    ...
                    if (isset($pa_options['checkAccess']) && is_array($pa_options['checkAccess']) && sizeof($pa_options['checkAccess'])) {
                        $checkAccess = array_map(function ($v) {
                            return intval($v);
                        }, $pa_options['checkAccess']);
                    }
                    ...
                    while ($qr_res->nextRow()) {
                        ...
                        if ($va_facet_info['type'] === 'relationship_types') {
                            if (sizeof($va_restrict_to_relationship_types) && !in_array($va_fetched_row['rel_type_id'], $va_restrict_to_relationship_types)) {
                                continue;
                            }
                            if (sizeof($va_exclude_relationship_types) && in_array($va_fetched_row['rel_type_id'], $va_exclude_relationship_types)) {
                                continue;
                            }
                    ...
                    if (!caGetOption(['dontExpandHierarchically', 'dont_expand_hierarchically'], $va_facet_info, false)) {
                        ...
                        if ($qr_ancestors) {
                            ...
                            while ($qr_ancestors->nextHit()) {
                                if ($qr_ancestors->get("{$vs_rel_table}.deleted")) {
                                    continue;
                                }
                                if (!($vn_parent_type_id = $qr_ancestors->get('type_id'))) {
                                    continue;
                                }

                                $vn_ancestor_id = (int)$qr_ancestors->get("{$vs_rel_pk}");
                                if (isset($va_criteria[$vn_ancestor_id])) {
                                    continue;
                                }		// skip items that are used as browse critera - don't want to browse on something you're already browsing on
                                ...
                                $ancestor_count = array_reduce($children, function ($c, $i) use ($va_facet_items) {
                                    return isset($va_facet_items[$i]) ? $c + $va_facet_items[$i]['content_count'] : $c;
                                }, 0);
                    ...
                    if (sizeof($va_row_ids = array_keys($va_facet_items))) {
                        if ($vs_label_table_name = $t_rel_item->getLabelTableName()) {
                            ...
                            while ($qr_labels->nextRow()) {
                                ...
                                if ($idno_fld && $va_facet_info['include_idno'] && $va_fetched_row[$idno_fld]) {
                                    $l .= " (".$va_fetched_row[$idno_fld].")";
                                }
    ...
    {
        if (!is_array($va_cache = $this->opo_ca_browse_cache->getParameter('facet_html'))) {
            $va_cache = array();
        }
    ...
    {
        if (!$ps_subject_table_name) {
            $ps_subject_table_name = $this->ops_browse_table_name;
        }
    ...
    public function setTypeExclusions($pa_type_codes_or_ids, $pa_options=null)
    {
        ...
        $type_list = array_filter($type_list, function ($v) use ($types_to_exclude) {
            return !in_array($v, $types_to_exclude);
        });
    ...
    public function getSourceRestrictionList()
    {
        if (function_exists("caGetSourceRestrictionsForUser")) {
            ...
            if (!is_array($va_pervasive_sources) || !sizeof($va_pervasive_sources)) {
    ...
    private function _getRelativeExecuteSQLData($ps_relative_to_table, $pa_options=null)
    {
        if ($vs_relative_to_table = $this->_isParentRelative($ps_relative_to_table)) {
            ...
            if (!($t_target = Datamodel::getInstanceByTableName($vs_relative_to_table, true))) {
                return null;
            }
        ...
        if (!($t_target = Datamodel::getInstanceByTableName($ps_relative_to_table, true))) {
            return null;
        }
        $vs_target_browse_table_num = $t_target->tableNum();
        $vs_target_browse_table_pk = $t_target->primaryKey();
    ...
    public function getAvailableRelationshipTypesForCurrentResult($ps_rel_table, $pa_rel_ids, $pa_options=null)
    {
        ...
        if (!is_array($pa_rel_ids)) {
            $pa_rel_ids = [$pa_rel_ids];
        }

        $pa_rel_ids = array_map(function ($v) {
            return (int)$v;
        }, $pa_rel_ids);
