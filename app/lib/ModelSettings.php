...
trait ModelSettings
{
    ...
    public function setSettingsFromHTMLForm($po_request, $pa_options=null)
    {
        ...
        if (!is_array($va_available_settings = $this->getAvailableSettings())) {
            $va_available_settings = [];
        }
        ...
        switch($va_properties['displayType']) {
            ...
            case DT_FIELD:
                ...
                foreach ($va_locales as $vs_locale => $va_locale_info) {
                    ...
                    if ($va_properties['usewysiwygeditor']) {
                        ...
                        if (!is_array($va_toolbar_config = $config->getAssoc('wysiwyg_editor_toolbar'))) {
                            $va_toolbar_config = [];
                        }
            ...
            case DT_CHECKBOXES:
                ...
                if (isset($va_properties['showOnSelect'])) {
                    if (!is_array($va_properties['showOnSelect'])) {
                        $va_properties['showOnSelect'] = array($va_properties['showOnSelect']);
                    }
	...
	jQuery(document).ready(function() {
		jQuery('".join(",", $va_ids)."').hide();
            ...
            case DT_SELECT:
                ...
                if ($va_properties['showTypesForTable']) {
                    ...
                    if ($t_show_types_for_table instanceof BaseRelationshipModel) { // interstitial type restriction incoming
                        ...
                        if (!is_array($va_rel_type_list)) {
                            break;
                        }

                        foreach ($va_rel_type_list as $vn_type_id => $va_type_info) {
                            if (!$va_type_info['parent_id']) {
                                continue;
                            }
                    ...
                    if (isset($va_properties['showSortableElementsFor']) && ($va_properties['showSortableElementsFor'] > 0)) {
                        ...
                        $va_select_opts = array_filter($va_select_opts, function ($v) {
                            return ($v !== '_natural');
                        });
                        ...
                        if (!is_array($va_properties['table'])) {
                            $va_properties['table'] = [$va_properties['table']];
                        }
                        ...
                        foreach ($va_properties['table'] as $vs_table) {
                            ...
                            if ($va_properties['includeIntrinsics']) {
                                if (!($t_rep = Datamodel::getInstance($vs_table, true))) {
                                    continue;
                                }

                                foreach ($t_rep->getFormFields() as $vs_f => $va_field_info) {
                                    if (is_array($va_properties['includeIntrinsics']) && !in_array($vs_f, $va_properties['includeIntrinsics'])) {
                                        continue;
                                    }
                                    if (in_array($va_field_info['DT_DISPLAY'], array('DT_OMIT', 'DT_HIDDEN'))) {
                                        continue;
                                    }
            ...
            case DT_INTERVAL:
                ...
                if (isset($va_properties['prefix'])) {
                    $vs_return .= $va_properties['prefix'];
                }
                ...
                if (isset($va_properties['suffix'])) {
                    $vs_return .= $va_properties['suffix'];
                }
                $vs_return .= "
