...
class Configuration
{
    ...
    public static $s_get_cache;
    public static $s_config_cache = null;
    public static $s_have_to_write_config_cache = false;
     ...
     */
    public static function load($ps_file_path=__CA_APP_CONFIG__, $pb_dont_cache=false, $pb_dont_cache_instance=false, $pb_dont_load_from_default_path=false)
    {
        if (!$ps_file_path) {
    ...
    public function loadFile($ps_filepath, $pb_die_on_error=false, $pn_num_lines_to_read=null)
    {
        ...
        while (!feof($r_file)) {
            ...
            if (($pn_num_lines_to_read > 0) && ($vn_line_num > $pn_num_lines_to_read)) {
                break;
            }
            $vs_buffer = trim(fgets($r_file, 32000));

            # skip comments (start with '#') or blank lines
            if (strtolower(substr($vs_buffer, 0, 7)) == '#!merge') {
                $vb_merge_mode = true;
            }
            if (strtolower(substr($vs_buffer, 0, 9)) == '#!replace') {
                $vb_merge_mode = false;
            }
            if (!$vs_buffer || (substr($vs_buffer, 0, 1) === "#")) {
                continue;
            }
            ...
            while (sizeof($va_tokens)) {
                ...
                if (sizeof($va_token_history) > 50) {
                    array_shift($va_token_history);
                }
                switch($vn_state) {
                    # ------------------------------------
                    # init
                    ...
                    case 20:
                        ...
                        # handle list values
                    case 30:
                        if ($vb_quoted_item_is_closed && (!in_array(trim($vs_token), [',', ']', ')']))) {
                            break;
                        }
                        switch($vs_token) {
                            # -------------------
    ...
    public function get($pm_key)
    {
        ...
        foreach ($pm_key as $ps_key) {
            if (isset(Configuration::$s_get_cache[$this->ops_md5_path][$ps_key]) && Configuration::$s_get_cache[$this->ops_md5_path][$ps_key]) {
            ...
            if (!is_array($vs_tmp) && !strlen($vs_tmp)) {
                if (is_array($vs_tmp = $this->getAssoc($ps_key))) {
                    $assoc_exists = true;
                }
            }
    ...
    public function exists($ps_key)
    {
        ...
        if (array_key_exists($ps_key, $this->ops_config_settings["scalars"])) {
            return true;
        }
        if (array_key_exists($ps_key, $this->ops_config_settings["lists"])) {
            return true;
        }
        if (array_key_exists($ps_key, $this->ops_config_settings["assoc"])) {
            return true;
        }
    ...
    public function validate()
    {
        ...
        if (!($schema = $loader->loadSchema("https://collectiveaccess.org/{$f}.schema.json"))) {
            return null;
        } 	// no schema loaded
