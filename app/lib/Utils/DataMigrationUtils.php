...
class DataMigrationUtils
{
     ...
     */
    public static function getEntityID($pa_entity_name, $pn_type_id, $locale_id, $pa_values=null, $options=null)
    {
     ...
     * @see DataMigrationUtils::_getID()
     */
    public static function getPlaceID($ps_place_name, $pn_parent_id, $pn_type_id, $locale_id, $pn_hierarchy_id=null, $pa_values=null, $options=null)
    {
     ...
     * @see DataMigrationUtils::_getID()
     */
    public static function getOccurrenceID($ps_occ_name, $pn_parent_id, $pn_type_id, $locale_id, $pa_values=null, $options=null)
    {
     ...
     * @see DataMigrationUtils::_getID()
     */
    public static function getListItemID($pm_list_code_or_id, $ps_item_idno, $pn_type_id, $locale_id, $pa_values=null, $options=null)
    {
        if (!is_array($options)) {
            $options = array();
        }
        ...
        if ($options['cache'] && isset(DataMigrationUtils::$s_cached_list_item_ids[$vs_cache_key])) {
            ...
            if ($o_log) {
                $o_log->logDebug(_t("%3Found existing list item %1 (member of list %2) in DataMigrationUtils::getListItemID() using idno", $ps_item_idno, $pm_list_code_or_id, $log_reference_str));
            }
        ...
        if (!($vn_list_id = ca_lists::getListID($pm_list_code_or_id))) {
            ...
            if ($o_log) {
                $o_log->logError(_t("%2Could not find list with list code %1", $pm_list_code_or_id, $log_reference_str));
            }
        ...
        if (isset($options['transaction']) && $options['transaction'] instanceof Transaction) {
            ...
            $t_item->setTransaction($options['transaction']);
            if ($o_event) {
                $o_event->setTransaction($options['transaction']);
            }
        }
            ...
            switch(strtolower($vs_match_on)) {
                ...
                case 'nonpreferred_labels':
                    ...
                    if (trim($vs_singular_label) || trim($vs_plural_label)) {
                        ...
                        if (($vn_parent_id !== false) && !$pb_ignore_parent) {
                        ...
                        if ($vn_item_id = (ca_list_items::find($va_criteria, array('returnAs' => 'firstId', 'purifyWithFallback' => true, 'transaction' => $options['transaction'], 'restrictToTypes' => $va_restrict_to_types)))) {
                            if ($o_log) {
                                $o_log->logDebug(_t("%4Found existing list item %1 (member of list %2) in DataMigrationUtils::getListItemID() using singular label %3", $ps_item_idno, $pm_list_code_or_id, $vs_singular_label, $log_reference_str));
                            }
                            break(2);
                        } else {
                            ...
                            if ($vn_item_id = (ca_list_items::find($va_criteria, array('returnAs' => 'firstId', 'purifyWithFallback' => true, 'transaction' => $options['transaction'])))) {
                                if ($o_log) {
                                    $o_log->logDebug(_t("%4Found existing list item %1 (member of list %2) in DataMigrationUtils::getListItemID() using plural label %3", $ps_item_idno, $pm_list_code_or_id, $vs_plural_label, $log_reference_str));
                                }
                                break(2);
                            }
                        }
                ...
                case 'idno':
                    ...
                    if ($vn_item_id = (ca_list_items::find($va_criteria, array('returnAs' => 'firstId', 'purifyWithFallback' => true, 'transaction' => $options['transaction'], 'restrictToTypes' => $va_restrict_to_types)))) {
                        if ($o_log) {
                            $o_log->logDebug(_t("%4Found existing list item %1 (member of list %2) in DataMigrationUtils::getListItemID() using idno with %3", $ps_item_idno, $pm_list_code_or_id, $ps_item_idno, $log_reference_str));
                        }
                        break(2);
                    }
                    break;
        ...
        if (!$t_list->load($vn_list_id)) {
            if ($o_log) {
                $o_log->logError(_t("%2Could not find list with list id %1", $vn_list_id, $log_reference_str));
            }
            return null;
        }
        ...
        if ($t_item = $t_list->addItem($ps_item_idno, $pa_values['is_enabled'], $pa_values['is_default'], $vn_parent_id, $pn_type_id, $ps_item_idno, '', (int)$pa_values['status'], (int)$pa_values['access'], $pa_values['rank'])) {
            ...
            if ($t_item->numErrors()) {
                ...
                if ($o_log) {
                    $o_log->logError(_t("%3Could not set preferred label for list item %1: %2", "{$vs_singular_label}/{$vs_plural_label}/{$ps_item_idno}", join('; ', $t_item->getErrors()), $log_reference_str));
                }
            ...
            if ($o_log) {
                $o_log->logInfo(_t("%3Created new list item %1 in list %2", "{$vs_singular_label}/{$vs_plural_label}/{$ps_item_idno}", $pm_list_code_or_id, $log_reference_str));
            }
            ...
            if ($o_log) {
                $o_log->logError(_t("%2Could not find add item to list: %1", join("; ", $t_list->getErrors()), $log_reference_str));
            }
        }
        return null;
    }
     ...
     * @see DataMigrationUtils::_getID()
     */
    public static function getCollectionID($ps_collection_name, $pn_type_id, $locale_id, $pa_values=null, $options=null)
    {
     ...
     * @see DataMigrationUtils::_getID()
     */
    public static function getStorageLocationID($ps_location_name, $pn_parent_id, $pn_type_id, $locale_id, $pa_values=null, $options=null)
    {
     ...
     * @see DataMigrationUtils::_getID()
     */
    public static function getObjectID($ps_object_name, $pn_parent_id, $pn_type_id, $locale_id, $pa_values=null, $options=null)
    {
     ...
     * @see DataMigrationUtils::_getID()
     */
    public static function getObjectLotID($ps_idno_stub, $ps_lot_name, $pn_type_id, $locale_id, $pa_values=null, $options=null)
    {
     ...
     * @see DataMigrationUtils::_getID()
     */
    public static function getObjectRepresentationID($ps_representation_name, $pn_type_id, $locale_id, $pa_values=null, $options=null)
    {
     ...
     * @see DataMigrationUtils::_getID()
     */
    public static function getLoanID($ps_loan_name, $pn_type_id, $locale_id, $pa_values=null, $options=null)
    {
     ...
     * @see DataMigrationUtils::_getID()
     */
    public static function getMovementID($ps_movement_name, $pn_type_id, $locale_id, $pa_values=null, $options=null)
    {
     ...
     * @return string
     */
    public static function transformTextEncoding($text, $options=null)
    {
     ...
     */
    public static function splitEntityName($text, $options=null)
    {
        global $g_ui_locale;
        ...
        if (!$locale && defined('__CA_DEFAULT_LOCALE__')) {
        ...
        if ($suffix_for_name && $is_corporation) {
            ...
            if (sizeof($tmp) > 1) {
                $tmp2 = array_filter(preg_split("![ ]+!", $tmp[1]), function ($v) {
                    return (bool)strlen(trim($v));
                });
        ...
        if ($class === 'ORG') {
            $options['displaynameFormat'] = 'forenamemiddlenamesurname';
        }
    ...
    private static function _setAttributes($pt_instance, $locale_id, $pa_values, $options=null)
    {
        ...
        if (is_array($pa_values)) {
            foreach ($pa_values as $vs_element => $va_values) {
                if (!$pt_instance->hasElement($vs_element)) {
                    continue;
                }
                ...
                foreach ($va_values as $va_value) {
                    if (is_array($va_value)) {
                        if (($vs_delimiter = caGetOption('delimiter', $va_value, null)) && !sizeof(array_filter($va_value, function ($v) {
                            return is_array($v);
                        }))) {
                            ...
                            foreach ($va_value as $vs_k => $vs_v) {
                                ...
                                if (in_array($vs_k, ['delimiter', 'matchOn'])) {
                                    continue;
                                }
                                ...
                                $pt_instance->addAttribute(
                                    ...
                                    [
                                        ...
                                        'skipExistingValues' => (
                                            caGetOption('skipExistingValues', $options, true)
                                            ||
                                            caGetOption('_skipExistingValues', $va_values, true)
                                        ), // default to skipping attribute values if they already exist (until v1.7.9 default was _not_ to skip)
                                        'matchOn' => caGetOption('_matchOn', $va_values, null)]
                                );
                            }
                        }
            ...
            if ($pt_instance->numErrors()) {
                ...
                if ($o_log) {
                    $o_log->logError(_t("Could not set values for %1 %2: %3", $pt_instance->getProperty('NAME_SINGULAR'), $pt_instance->getLabelForDisplay(), join('; ', $pt_instance->getErrors())));
                }
        ...
        if (is_array($va_nonpreferred_labels = caGetOption("nonPreferredLabels", $options, null))) {
            ...
            foreach ($va_labels as $va_label) {
                ...
                if ($pt_instance->numErrors()) {
                    ...
                    if ($o_log) {
                        $o_log->logError(_t("Could not set non-preferred label for %1 %2: %3", $pt_instance->getProperty('NAME_SINGULAR'), $pt_instance->getLabelForDisplay(), join('; ', $pt_instance->getErrors())));
                    }
                } else {
                    $vn_count++;
                }
        ...
        if ($o_idno = $pt_instance->getIDNoPlugInInstance()) {
            ...
            if (($vs_proc_idno = join($vs_sep, $va_values)) && ($vs_proc_idno != $ps_idno)) {
                ...
                if ($pt_instance->numErrors()) {
                    ...
                    if ($o_log) {
                        $o_log->logError(_t("Could not update idno for %1 %2: %3", $pt_instance->getProperty('NAME_SINGULAR'), $pt_instance->getLabelForDisplay(), join('; ', $pt_instance->getErrors())));
                    }
                    return null;
                }
            }
     ...
     */
    public static function getIDFor($ps_table, $pa_label, $pn_parent_id, $pn_type_id, $locale_id, $pa_values=null, $options=null)
    {
    ...
    private static function _getID($ps_table, $pa_label, $pn_parent_id, $pn_type_id, $locale_id, $pa_values=null, $options=null)
    {
        ...
        if (!is_array($pa_label)) {
            $pa_label[$vs_label_display_fld] = $pa_label;
        }
        ...
        if (is_array($vs_idno)) {
            $vs_idno = $vs_idno[$vs_idno_fld];
        }	// when passed via caProcessRefineryAttributes() might be in attribute-y array
                ...
                case 'idno_stub':
                    ...
                    switch ($vs_table_class) {
                        ...
                        default:
                            ...
                            if (!$pb_ignore_parent && $vn_parent_id) {
                                $va_find_vals['parent_id'] = $vn_parent_id;
                            }
        ...
        if (!$vn_id) {
            ...
            if (!$t_instance = Datamodel::getInstanceByTableName($ps_table, false)) {
                return null;
            }
            ...
            if ($t_instance->hasField('locale_id')) {
                $t_instance->set('locale_id', $locale_id);
            }
            if ($t_instance->hasField('type_id')) {
            ...
            if ($t_instance->hasField('media') && ($t_instance->getFieldInfo('media', 'FIELD_TYPE') == FT_MEDIA) && isset($pa_values['media']) && $pa_values['media']) {
                ...
                if (($pb_match_media_without_ext) && !isURL($pa_values['media']) && !file_exists($pa_values['media'])) {
                    ...
                    foreach ($va_files_in_dir as $vs_filepath) {
                        if ($o_log) {
                            $o_log->logDebug(_t("%4Trying media %1 in place of %2/%3", $vs_filepath, $vs_original_path, $vs_filename, $log_reference_str));
                        }
                        if (pathinfo($vs_filepath, PATHINFO_FILENAME) == $vs_filename) {
                            if ($o_log) {
                                $o_log->logNotice(_t("%4Found media %1 for %2/%3", $vs_filepath, $vs_original_path, $vs_filename, $log_reference_str));
                            }
            ...
            if ($t_instance->numErrors()) {
                ...
                if ($o_log) {
                    $o_log->logError(_t("%4Could not insert %1 %2: %3", $vs_table_display_name, $pa_label[$vs_label_display_fld], join('; ', $t_instance->getErrors()), $log_reference_str));
                }
                return null;
            }
            ...
            if ($t_instance->numErrors()) {
                ...
                if ($o_log) {
                    $o_log->logError(_t("%4Could not set preferred label for %1 %2: %3", $vs_table_display_name, $pa_label[$vs_label_display_fld], join('; ', $t_instance->getErrors()), $log_reference_str));
                }
            ...
            if ($o_log) {
                $o_log->logInfo(_t("%3Created new %1 %2", $vs_table_display_name, $pa_label[$vs_label_display_fld], $log_reference_str));
            }
            ...
            if ($o_log) {
                $o_log->logDebug(_t("%3Found existing %1 %2 in DataMigrationUtils::_getID()", $vs_table_display_name, $pa_label[$vs_label_display_fld], $log_reference_str));
            }

            $vb_attr_errors = false;
            if (($vb_force_update = caGetOption('forceUpdate', $options, false)) || ($vb_return_instance = caGetOption('returnInstance', $options, false))) {
                if (!$t_instance = Datamodel::getInstanceByTableName($vs_table_class, false)) {
                    return null;
                }
                if (isset($options['transaction']) && $options['transaction'] instanceof Transaction) {
                    $t_instance->setTransaction($options['transaction']);
                ...
                if (!$vn_rc) {
                    if ($o_log) {
                        $o_log->logError(_t("%4Could not load existing %1 with id %2 (%3) in DataMigrationUtils::_getID() [THIS SHOULD NOT HAPPEN]", $vs_table_display_name, $vn_id, $pa_label[$vs_label_display_fld], $log_reference_str));
                    }
                    return null;
                } else {
                    if ($vb_force_update && $vb_has_attr) {
                        if ($vb_attr_errors = !DataMigrationUtils::_setAttributes($t_instance, $locale_id, $pa_values, $options)) {
                            if ($o_log) {
                                $o_log->logError(_t("%5Could not set attributes for %1 with id %2 (%3) in DataMigrationUtils::_getID(): %4", $vs_table_display_name, $vn_id, $pa_label[$vs_label_display_fld], join("; ", $t_instance->getErrors()), $log_reference_str));
                            }
                        }
                    }
     ...
     * @return string
     */
    public static function postError($po_object, $ps_message, $pn_level=__CA_DATA_IMPORT_ERROR__, $options=null)
    {
        if (!$po_object->numErrors()) {
            return null;
        }
        $vs_error = '';
