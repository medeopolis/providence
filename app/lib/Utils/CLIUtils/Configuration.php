...
trait CLIUtilsConfiguration
{
    ...
    public static function install($po_opts=null, $pb_installing = true)
    {
        ...
        if (!$vb_quiet) {
            CLIUtils::addMessage(_t("Processing metadata alerts"));
        }
...
\tIf the database schema already exists, this operation will fail, unless the --overwrite flag is set, in which case all existing data will be deleted (use with caution!).");
    ...
    public static function push_config_changes($po_opts=null)
    {
        ...
        if ($o_log) {
            $o_log->logDebug(_t("[push-config-changes] Start preparing to push config changes"));
        }
        ...
        foreach ($va_targets as $vs_target) {
            ...
            if ($o_log) {
                $o_log->logDebug(_t("[push-config-changes] Processing target %1", $vs_target));
            }

            if (!isURL($vs_target)) {
                CLIUtils::addError(_t("The target '%1' doesn't seem to be in URL format", $vs_target));
                if ($o_log) {
                    $o_log->logError(_t("[push-config-changes] The target '%1' doesn't seem to be in URL format", $vs_target));
                }
                return false;
            }
            ...
            if ($o_log) {
                $o_log->logDebug(_t("[push-config-changes] Service endpoint is '%1'. Timestamp for diff config is %2", $vs_target, $vn_target_timestamp));
            }
            ...
            if ($o_log) {
                $o_log->logDebug(_t("[push-config-changes] Configuration fragment for target '%1' is \n %2", $vs_target, $vs_config));
            }
            ...
            if ($vn_code != 200) {
                ...
                if ($o_log) {
                    $o_log->logError(_t("[push-config-changes] Pushing to target '%1' seems to have failed. HTTP response code was %2. Enable debug logging mode to get more info below.", $vs_target, $vn_code));
                }
            }

            if ($o_log) {
                $o_log->logDebug(_t("[push-config-changes] Target '%1' responded with 200 OK", $vs_target));
            }

            $va_response = @json_decode($vs_exec, true);

            if ($o_log) {
                $o_log->logDebug(_t("[push-config-changes] Decoded response from target '%1' is '%2'", $vs_target, print_r($va_response, true)));
            }
            ...
            if ($o_log) {
                $o_log->logDebug(_t("[push-config-changes] Finished processing target '%1'", $vs_target));
            }
        }
        ...
        $o_vars->save();
        if ($o_log) {
            $o_log->logDebug(_t("[push-config-changes] Saved sync timestamps are: %1", print_r($va_timestamps, true)));
        }

        CLIUtils::addMessage(_t("All done"));
        if ($o_log) {
            $o_log->logDebug(_t("[push-config-changes] Finished ..."));
        }
    }
