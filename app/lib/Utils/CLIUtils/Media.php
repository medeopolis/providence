...
trait CLIUtilsMedia
{
    ...
    public static function reprocess_media($po_opts=null)
    {
        ...
        if ($o_log) {
            $o_log->logDebug(_t("[reprocess-media] Start preparing to reprocess media"));
        }
                        ...
                        foreach ($va_vals as $vn_value_id) {
                            ...
                            if ($t_attr_val->getPrimaryKey()) {
                                ...
                                if ($t_attr_val->numErrors()) {
                                    $vs_message = _t(
                                        "Error processing attribute media: %1",
        ...
        if ((in_array('all', $pa_kinds)  || in_array('icons', $pa_kinds)) && (!$vn_start && !$vn_end)) {
            ...
            foreach ($icon_tables as $icon_table) {
                if (!($t_instance = Datamodel::getInstance($icon_table, true))) {
                    continue;
                }
                if (!$quiet) {
                    print CLIProgressBar::start($icon_table::find('*', ['returnAs' => 'count']), _t('Re-processing icons'));
                }
