...
trait CLIUtilsMaintenance
{
    ...
    public static function rebuild_sort_values($po_opts=null)
    {
        ...
        foreach ($tables as $table) {
            ...
            if (!($t_table = Datamodel::getInstance($table))) {
                continue;
            }
...
\tNote that depending upon the size of your database reloading sort values can take from a few minutes to an hour or more. During the reloading process the system will remain usable but search and browse functions may return incorrectly sorted results.");
    ...
    public static function remove_unused_media($po_opts=null)
    {
        ...
        foreach ($va_contents as $vs_path) {
            print CLIProgressBar::next();
            if (!preg_match('!_ca_object_representation!', $vs_path)) {
                continue;
            } // skip non object representation files
            if (!$va_paths[$vs_path]) {
                $vn_delete_count++;
                if ($vb_delete_opt) {
                    @unlink($vs_path);
    ...
    public static function purge_deleted($po_opts=null)
    {
        ...
        foreach ($va_tables as $vs_table) {
            if (is_array($va_tables_to_process) && sizeof($va_tables_to_process) && !in_array($vs_table, $va_tables_to_process)) {
                continue;
            }
            if (!$t_instance = Datamodel::getInstanceByTableName($vs_table)) {
                continue;
            }
    ...
    public static function fix_permissions($po_opts=null)
    {
        ...
        if (!$po_opts->getOption("quiet")) {
            CLIUtils::addMessage(_t("Fixing permissions for the HTMLPurifier definition cache directory (vendor/ezyang/htmlpurifier/library/HTMLPurifier/DefinitionCache/Serializer) for ownership by \"%1\"...", $vs_user));
        }
        ...
        if (!$po_opts->getOption("quiet")) {
            CLIUtils::addMessage(_t("Fixing permissions for the log directory (app/log) for ownership by \"%1\"...", $vs_user));
        }
        ...
        if (!$po_opts->getOption("quiet")) {
            CLIUtils::addMessage(_t("Fixing permissions for the user media upload directory (app/log) for ownership by \"%1\"...", $vs_user));
        }
    ...
    public static function validate_using_metadata_dictionary_rules($po_opts=null)
    {
        ...
        $tables = array_unique(array_map(function ($v) {
            return $v['table_num'];
        }, $rules));
            ...
            if ($qr_reps->numRows() > 0) {
                ...
                while ($qr_reps->nextRow()) {
                    ...
                    foreach ($va_media_versions as $vs_version) {
                        if (!($vs_path = $qr_reps->getMediaPath('media', $vs_version))) {
                            continue;
                        }
                        if (!($vs_database_md5 = $qr_reps->getMediaInfo('media', $vs_version, 'MD5'))) {
                            continue;
                        }		// skip missing MD5s - indicates empty file
                    ...
                    foreach ($va_elements as $vs_element_code => $va_element_info) {
                        ...
                        foreach ($va_vals as $vn_value_id) {
                            ...
                            if ($t_attr_val->getPrimaryKey()) {
                                ...
                                foreach ($va_media_versions as $vs_version) {
                                    if (!($vs_path = $t_attr_val->getMediaPath('value_blob', $vs_version))) {
                                        continue;
                                    }

                                    if (!($vs_database_md5 = $t_attr_val->getMediaInfo('value_blob', $vs_version, 'MD5'))) {
                                        continue;
                                    }	// skip missing MD5s - indicates empty file
                                    $vs_file_md5 = md5_file($vs_path);
                    ...
                    foreach ($va_elements as $vs_element_code => $va_element_info) {
                        ...
                        foreach ($va_vals as $vn_value_id) {
                            ...
                            if ($t_attr_val->getPrimaryKey()) {
                                ...
                                if (!($vs_path = $t_attr_val->getFilePath('value_blob'))) {
                                    continue;
                                }

                                if (!($vs_database_md5 = $t_attr_val->getFileInfo('value_blob', 'MD5'))) {
                                    continue;
                                }	// skip missing MD5s - indicates empty file
                                $vs_file_md5 = md5_file($vs_path);
    ...
    public static function garbage_collection($po_opts=null)
    {
        ...
        if (!$quiet) {
            CLIUtils::addMessage(_t('Performing garbage collection on application caches and temporary directories...'));
        }
    ...
    public static function reload_service_values($po_opts=null)
    {
        $va_infoservice_elements = ca_metadata_elements::getElementsAsList(
            ...
            false,
            false,
            array(__CA_ATTRIBUTE_VALUE_INFORMATIONSERVICE__)
        );
    ...
    public static function precache_content($po_opts=null)
    {
        ...
        if (!is_array($va_exclude_from_precache = $o_cache_config->get('exclude_from_precache'))) {
            $va_exclude_from_precache = [];
        }
        ...
        foreach ($va_cached_actions as $vs_controller => $va_actions) {
            if (in_array($vs_controller, $va_exclude_from_precache)) {
                continue;
            }
                ...
                case 'Detail':
                    ...
                    foreach ($va_actions as $vs_action => $vn_ttl) {
                        if (is_array($va_detail_types[$vs_action])) {
                            ...
                            if (!file_exists(__CA_MODELS_DIR__."/{$vs_table}.php")) {
                                continue;
                            }
    ...
    public static function regenerate_dependent_field_values($po_opts=null)
    {
        ...
        foreach ($va_elements as $va_element) {
            ...
            foreach ($va_type_res_list as $va_type_res) {
                if (!($t_instance = Datamodel::getInstanceByTableNum($va_type_res['table_num']))) {
                    continue;
                }
    ...
    public static function set_default_field_values($po_opts=null)
    {
        ...
        foreach ($va_elements as $va_element) {
            print_R($va_element);
            if (!strlen($default_value = trim($va_element['settings']['default_text']))) {
                continue;
            }
            ...
            foreach ($va_type_res_list as $va_type_res) {
                ...
                while ($qr_res->nextHit()) {
                    ...
                    foreach ($va_value_list as $vn_row_id => $va_values_by_attribute_id) {
                        foreach ($va_values_by_attribute_id as $vn_attr_id => $va_values) {
                            if ($va_values[$va_element['element_code']]) {
                                continue;
                            }
                            CLIUtils::addMessage(_t('Processing row %1 for %2.%3', $vn_row_id, $vs_root_code, $va_element['element_code']));
