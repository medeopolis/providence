...
trait CLIUtilsDeveloper
{
    ...
    public static function dump_log($opts=null)
    {
        ...
        if ($t_logged) {
            ...
            $log = array_filter($log, function ($v) use ($logged_table_num) {
                return $v['logged_table_num'] == $logged_table_num;
            });
        }

        $log = array_map(function ($v) {
            $v['log_datetime_display'] = date("d-M-Y h:m:s", $v['log_datetime']);
            return $v;
        }, $log);
        ...
        switch($return) {
            case 'tables':
                $tables = array_unique(array_map(function ($v) {
                    return $v['logged_table_num'];
                }, $log));
                print_r(array_values(array_map(function ($v) {
                    return Datamodel::getTableName($v);
                }, $tables))); // TODO: improve formatting
                break;
            default:
