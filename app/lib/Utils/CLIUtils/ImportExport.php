...
trait CLIUtilsImportExport
{
    ...
    public static function import_mediaParamList()
    {
        ...
        $access_status_list_str = join(", ", array_map(function ($v, $k) {
            return "{$k} ({$v})";
        }, $access_status_values, array_keys($access_status_values)));
        $access_status_default_str = join(", ", array_map(function ($v, $k) {
            return "{$k} ({$v})";
        }, $access_status_default, array_keys($access_status_default)));
        ...
        $workflow_status_list_str = join(", ", array_map(function ($v, $k) {
            return "{$k} ({$v})";
        }, $workflow_status_values, array_keys($workflow_status_values)));
        $workflow_status_default_str = join(", ", array_map(function ($v, $k) {
            return "{$k} ({$v})";
        }, $workflow_status_default, array_keys($workflow_status_default)));
        ...
        $object_type_list_str = join(", ", array_map(function ($v, $k) {
            return "{$k} ({$v})";
        }, $object_type_values, array_keys($object_type_values)));
        $object_type_default_str = join(", ", array_map(function ($v, $k) {
            return "{$k} ({$v})";
        }, $object_type_default, array_keys($object_type_default)));
        ...
        $representation_type_list_str = join(", ", array_map(function ($v, $k) {
            return "{$k} ({$v})";
        }, $representation_type_values, array_keys($representation_type_values)));
        $representation_type_default_str = join(", ", array_map(function ($v, $k) {
            return "{$k} ({$v})";
        }, $representation_type_default, array_keys($representation_type_default)));
    ...
    public static function import_data($po_opts=null)
    {
        ...
        if (!$t_importer->importDataFromSource(
            $vs_data_source,
            $vs_mapping,
    ...
    public static function load_chenhall_nomenclature($po_opts=null)
    {
        ...
        while ($o_rows->valid() && ($o_row = $o_rows->current())) {
            ...
            $non_preferred_terms = array_filter($non_preferred_terms, function ($v) {
                return strlen($v);
            });
