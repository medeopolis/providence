...
class CLITools extends CLIBaseUtils
{
    ...
    private static function _makeList($pa_list, $pn_indent=0, $pa_stack=null, $locale=null)
    {
        ...
        foreach ($pa_list as $vn_i => $va_item) {
            ...
            $np_label_list = array_map(function ($v) {
...
{$vn_ident}\t\t</label>
{$vn_ident}\t</labels>\n".
    ((is_array($va_item['subitems']) && sizeof($va_item['subitems'])) ? "{$vn_ident}\t<types>\n{$vn_indent}".CLITools::_makeTypeList($va_item['subitems'], $pn_indent + 2, array_merge($pa_stack, array(substr($vs_label_proc, 0, 10))))."{$vn_ident}\t</types>" : '')."
{$vn_ident}</type>\n";
        }
