...
class Timer
{
     ...
     * @return Timer
     */
    public static function start($ps_name, $pa_options=null)
    {
        Timer::$s_timers_enabled[$ps_name] = true;
        if (Timer::$s_timers[$ps_name]) {
            Timer::$s_timers[$ps_name]->start = microtime(true);
            return Timer::$s_timers[$ps_name];
        }
     ...
     */
    public static function stop($ps_name, $pa_options=null)
    {
        if (Timer::$s_timers[$ps_name]) {
     ...
     * @return Timer
     */
    public static function reset($ps_name, $pa_options=null)
    {
        Timer::$s_timers_enabled[$ps_name] = true;
        if (Timer::$s_timers[$ps_name]) {
            Timer::$s_timers[$ps_name]->elapsed = 0;
            Timer::$s_timers[$ps_name]->start = microtime(true);
            return Timer::$s_timers[$ps_name];
        }
        return Timer::$s_timers[$ps_name] = new Timer();
     ...
     * @return bool True on success, null if named timer does not exist.
     */
    public static function destroy($ps_name, $pa_options=null)
    {
        if (Timer::$s_timers[$ps_name]) {
            Timer::$s_timers_enabled[$ps_name] = false;
     ...
     * @return void
     */
    public static function p($ps_name, $ps_message="[%name] %time seconds<br/>\n", $pa_options=null)
    {
     ...
     * @return void
     */
    public static function printAndStop($ps_name, $ps_message="[%name] %time seconds<br/>\n", $pa_options=null)
    {
        print Timer::get($ps_name, $ps_message, $pa_options);
     ...
     * @return string
     */
    public static function get($ps_name, $ps_message="[%name] %time seconds<br/>\n", $pa_options=null)
    {
        if (Timer::$s_timers[$ps_name] instanceof Timer) {
            if (!Timer::$s_timers_enabled[$ps_name]) {
                return false;
            }
     ...
     * @return bool True on success, null if named timer does not exist.
     */
    public static function enable($ps_name, $pb_enabled=true)
    {
        if (Timer::$s_timers[$ps_name]) {
     ...
     * @return bool True on success, null if named timer does not exist.
     */
    public static function disable($ps_name)
    {
        if (Timer::$s_timers[$ps_name]) {
            Timer::$s_timers_enabled[$ps_name] = false;
            return true;
     ...
     * @return bool True on success, null if named timer does not exist.
     */
    public static function enabled($ps_name)
    {
        if (Timer::$s_timers[$ps_name]) {
