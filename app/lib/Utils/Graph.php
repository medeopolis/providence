...
class Graph
{
    ...
    {
        if ($ps_node2) {
            if (!isset($this->opa_graph["EDGES"][$ps_node1][$ps_node2])) {
                return null;
            }
            return $this->opa_graph["EDGES"][$ps_node1][$ps_node2];
        } else {						# add attribute array to node
            if (!isset($this->opa_graph["NODES"][$ps_node1])) {
                return null;
            }
        ...
        while (isset($vs_closest_node)) {
            ...
            if (is_array($va_edges[$vs_closest_node])) {
                foreach ($va_edges[$vs_closest_node] as $vs_vertex => $va_attr) {
                    ...
                    if (isset($va_marked[$vs_vertex]) && $va_marked[$vs_vertex]) {
                        continue;
                    }
    ...
    private function _getTopoInDegree($ps_node)
    {
        ...
        foreach ($va_graph_nodes as $vs_key => $va_node) {
            ...
            if (!$this->getAttribute('topo_visited', $vs_key) && $this->hasRelationship($ps_node, $vs_key)) {
                $vn_result++;
            }
        }
        return $vn_result;
