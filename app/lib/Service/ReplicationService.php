...
class ReplicationService
{
    ...
    public static function getLastReplicatedLogID($po_request)
    {
        ...
        if (!strlen($vs_guid)) {
            throw new Exception('must provide a system guid');
        }
    ...
    public static function getLogIDForTimestamp($po_request)
    {
        ...
        if (!strlen($vn_timestamp)) {
            throw new Exception('must provide a timestamp');
        }

        $vn_log_id = ca_change_log::getLogIDForTimestamp($vn_timestamp);
        if (!$vn_log_id) {
            throw new Exception('could not figure out log_id for given timestamp');
        }
    ...
    public static function applyLog($po_request)
    {
        ...
        if (!is_array($va_log)) {
            throw new \Exception('Log must be array: '.$c);
        }
        $o_db = new Db();
        ...
        foreach ($va_log as $vn_log_id => $va_log_entry) {
            ...
            try {
                if ($va_log_entry['SKIP']) {
                    throw new CA\Sync\LogEntry\IrrelevantLogEntry(_t('Skip log entry'));
                }
