...
class InformationServiceManager
{
     ...
     */
    public static $s_information_service_manager_did_do_init = false;

    /**
     *
     */
    public static $s_information_service_instances = array();
    # -------------------------------------------------------
    ...
    {
        if (InformationServiceManager::$s_information_service_manager_did_do_init) {
            return true;
        }
        ...
        foreach ($va_services as $vs_service) {
            if ($vs_service == 'BaseInformationServicePlugin') {
                continue;
            }
            if (!file_exists("{$vs_base_service_dir}/{$vs_service}.php")) {
                continue;
            }
            require_once("{$vs_base_service_dir}/{$vs_service}.php");
            $vs_service_classname = "WLPlugInformationService{$vs_service}";
            if (!class_exists($vs_service_classname)) {
                continue;
            }
