...
{
    # -------------------------------------
    public static $s_table_names = [];

    public static $s_mimetype_cache = [];
    public static $s_class_cache = [];
    ...
    private function _getLinkingTableName()
    {
        ...
        if (isset(self::$s_table_names[$t])) {
            return self::$s_table_names[$t];
        }
    ...
    {
        if (!($table = $this->_getLinkingTableName())) {
            return false;
        }
    ...
    private function _prefetchMimeTypes($pa_options)
    {
        ...
        $vs_sql_access = '';
        if (is_array($va_check_access) && sizeof($va_check_access)) {
            $vs_sql_access = ' AND caor.access IN ('.join(",", $va_check_access).')';
        }
     ...
     */
     ...
     {
         if (!is_array($pa_access_values)) {
             $pa_access_values = [];
         }
         if (!sizeof($pa_access_values)) {
             return true;
         }
