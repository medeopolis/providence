...
function caBatchGetMediaFilenameToIdnoRegexList($options=null)
{
    ...
    foreach ($regex_list as $k => $v) {
        if (!is_array($v['regexes']) || !sizeof($v['regexes'])) {
            if ($o_log) {
                $o_log->logError(_t("No filename-to-idno matching patterns set for %1. Check your configuration.", $k));
            }
            unset($regex_list[$k]);
            continue;
        }
        foreach ($v['regexes'] as $match) {
            if (@preg_match('!'.$match.'!', "Just a test") === false) {
                if ($o_log) {
                    $o_log->logError(_t("Filename-to-idno matching replacement pattern %1 in %2 is not a valid PCRE. Check your configuration.", $match, $k));
                }
                unset($regex_list[$k]);
                continue(2);
            }
...
function caBatchGetIdnoRegexList($options=null)
{
    ...
    foreach ($regex_list as $k => $v) {
        ...
        foreach ($v['regexes'] as $match => $replace) {
            if (@preg_match('!'.$match.'!', "Just a test") === false) {
                if ($o_log) {
                    $o_log->logError(_t("Idno replacement pattern %1 in %2 is not a valid PCRE. Check your configuration.", $match, $k));
                }
                unset($regex_list[$k]);
                continue(2);
            }
...
function caBatchGetMediaFilenameReplacementRegexList($options=null)
{
    ...
    foreach ($regex_list as $k => $v) {
        ...
        foreach ($v['regexes'] as $match => $replace) {
            if (@preg_match('!'.$match.'!', "Just a test") === false) {
                if ($o_log) {
                    $o_log->logError(_t("Filename replacement pattern %1 in %2 is not a valid PCRE. Check your configuration.", $match, $k));
                }
                unset($regex_list[$k]);
                continue(2);
            }
            ...
            foreach ($va_regex_info['regexes'] as $vs_regex) {
                ...
                foreach ($va_names_to_match as $vs_match_name) {
                    if (preg_match('!'.$vs_regex.'!u', $vs_match_name, $va_matches)) {
                        ...
                        if ($o_log) {
                            $o_log->logDebug(_t("Extracted value from name %1 using regex %2", $vs_match_name, $vs_regex));
                        }
                        ...
                        switch(strtoupper($ps_match_type)) {
                            case 'STARTS':
                                ...
                                if ($o_log) {
                                    $o_log->logDebug(_t("STARTS match on %1 to value %2", $match, $ps_value));
                                }
                                break;
                            case 'ENDS':
                                $vb_match = preg_match('!'.$ps_value.'$!i', $match, $va_matches);
                                if ($o_log) {
                                    $o_log->logDebug(_t("ENDS match on %1 to value %2", $match, $ps_value));
                                }
                                break;
                            case 'CONTAINS':
                                $vb_match = preg_match('!'.$ps_value.'!i', $match, $va_matches);
                                if ($o_log) {
                                    $o_log->logDebug(_t("CONTAINS match on %1 to value %2", $match, $ps_value));
                                }
                                break;
                            case 'EXACT':
                                ...
                                if ($o_log) {
                                    $o_log->logDebug(_t("EXACT match on %1 to value %2", $match, $ps_value));
                                }
                                break;
                            ...
                            default:
                                ...
                                if ($o_log) {
                                    $o_log->logDebug(_t("Case-insensitive match on %1 to value %2", $match, $ps_value));
                                }
                                break;
                        }
                        if ($vb_match) {
...
{
    if (!($user_name = ca_users::userNameFor($user))) {
        return null;
    }
...
function caGetAvailableMediaUploadModes(array $options=null)
{
    ...
    $import_modes = [];
    if (!is_array($available_modes = $config->getList('media_importer_allowed_modes'))) {
        $available_modes = [];
    }
    foreach ([
