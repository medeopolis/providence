...
function caGetUserLocaleRules($ps_item_locale=null, $pa_preferred_locales=null)
{
    ...
    if (isset($g_user_locale_rules[$ps_item_locale])) {
        return $g_user_locale_rules[$ps_item_locale];
    }
    ...
    $va_fallback_locales = array_filter($va_fallback_locales, function ($v, $k) use ($ps_item_locale, $va_fallback_locales, $va_preferred_locales) {
        return !isset($va_preferred_locales[$k]) && ($k !== $ps_item_locale);
...
{
    global $g_ui_locale;
    if (!isset($setting_values[$setting])) {
        return caGetOption('default', $options, null);
    }
    if (!is_array($v = $setting_values[$setting])) {
        return strlen($v) ? $v : caGetOption('default', $options, null);
    }
    ...
    switch($vs_instance_table) {
        ...
        default:
            ...
            foreach ($va_tables as $vs_table) {
                if (!is_array($va_items = $t_instance->getRelatedItems($vs_table))) {
                    $va_items = [];
                }
            ...
            if (is_array($va_references_from = $t_instance->getAuthorityElementList()) && sizeof($va_references_from)) {
                foreach ($va_references_from as $va_ref) {
                    if (!($t_element = ca_metadata_elements::getInstance($va_ref['hier_element_id']))) {
                        continue;
                    }
...
function _caFormatMediaMetadataArray($pa_array, $pn_level=0, $ps_key=null)
{
    ...
    foreach ($pa_array as $vs_key => $vs_val) {
        if (preg_match("!^Undefined!i", $vs_key)) {
            continue;
        }
...
function caEditorInspector($po_view, $pa_options=null)
{
    ...
    if (!is_array($va_reps = $po_view->getVar('representations'))) {
        $va_reps = [];
    }
            ...
            if ($t_item->hasField('is_deaccessioned') && $t_item->get('is_deaccessioned') && ($t_item->get('deaccession_date', array('getDirectDate' => true)) <= caDateToHistoricTimestamp(_t('now')))) {
                ...
                if ($po_view->request->user->canDoAction('can_see_current_location_in_inspector_'.$vs_table_name)) {
                    ...
                    if (method_exists($t_item, "getHistory") && ($inspector_current_value_label = $t_item->getInspectorHistoryTrackingDisplayPolicy('label'))) {
                        if ($inspector_current_value = $t_item->getCurrentValueForDisplay()) {
                            $vs_buf .= "<div class='inspectorCurrentLocation'><strong>{$inspector_current_value_label}:</strong><br/>{$inspector_current_value}".(($is_home) ? ' '._t('[HOME]') : '')."</div>";
                        }
                    }
                    ...
                    if ((!$is_home || !$inspector_current_value) && caHomeLocationsEnabled($t_item->tableName(), $t_item->getTypeCode()) && ($home_location_id = $t_item->get('home_location_id')) && ($t_home_loc = ca_storage_locations::find($home_location_id, ['returnAs' => 'firstModelInstance']))) {
                        if (!($template = $po_view->request->config->get('inspector_home_location_display_template'))) {
		...
		function caToggleItemWatch() {
            ...
            if ($va_creation['timestamp'] || $va_last_change['timestamp']) {
                ...
                if (method_exists($t_item, 'getMetadataDictionaryRuleViolations') && is_array($va_violations = $t_item->getMetadataDictionaryRuleViolations()) && (($total_num_violations = (sizeof($va_violations))) > 0)) {
                    ...
                    if (!$screen_id || !is_array($violations_for_current_screen = $t_item->getMetadataDictionaryRuleViolations(null, ['screen_id' => $screen_id]))) {
                        $violations_for_current_screen = [];
                    }
...
function caTableIsActive($pm_table)
{
    ...
    if (is_subclass_of($t_instance, "BaseRelationshipModel")) {
        ...
        if ($vs_left_table_name == 'ca_tour_stops') {
            $vs_left_table_name = 'ca_tours';
        }
        $vs_right_table_name = $t_instance->getRightTableName();
        if ($vs_right_table_name == 'ca_tour_stops') {
            $vs_right_table_name = 'ca_tours';
        }

        if ((int)($o_config->get("{$vs_left_table_name}_disable"))) {
            return false;
        }
        if ((int)($o_config->get("{$vs_right_table_name}_disable"))) {
            return false;
        }
    } else {
        switch($vs_table_name) {
            case 'ca_tour_stops':
                if ((int)($o_config->get('ca_tours_disable'))) {
                    return false;
                }
                break;
            default:
                if ((int)($o_config->get($vs_table_name.'_disable'))) {
                    return false;
                }
                break;
        }
...
function caGetMediaDisplayInfo($ps_context, $ps_mimetype)
{
    ...
    if (!is_array($va_context = $o_media_display_config->getAssoc($ps_context))) {
        return null;
    }
    ...
    foreach ($va_context as $vs_media_class => $va_media_class_info) {
        if (!is_array($va_mimetypes = $va_media_class_info['mimetypes'])) {
            continue;
        }
...
function caGetDefaultMediaViewer($ps_mimetype)
{
    ...
    if (!is_array($va_defaults = $o_media_display_config->getAssoc('default_viewers'))) {
        return null;
    }

    foreach ($va_defaults as $vs_media_class => $va_info) {
        if (!is_array($va_mimetypes = $va_info['mimetypes'])) {
            continue;
        }
...
function caGetTemplateTags($ps_template, $pa_options=null)
{
    ...
    if (MemoryCache::contains($key, 'DisplayTemplateParserUtils')) {
        return MemoryCache::fetch($key, 'DisplayTemplateParserUtils');
    }
...
{
    global $g_ui_locale;
    if (!is_array($pa_directives) || !sizeof($pa_directives)) {
        return $ps_value;
    }
        ...
        switch(strtoupper($va_tmp[0])) {
            case 'UNITS':
                try {
                    ...
                    switch($vs_units) {
                        ...
                        case 'fractionalenglish':
                            ...
                            if (!$force_english_units && ($vn_value_in_inches > $o_dimensions_config->get('use_feet_for_display_up_to'))) {
                                if ($vn_in_miles = (int)($vn_value_in_inches / (12 * 5280))) {
                                    $va_measure_conv[] = "{$vn_in_miles} miles".((!$pb_omit_units && in_array('MILE', $va_add_periods_list)) ? '.' : '');
                                }
                            ...
                            if (($force_english_units == 'ft') || (!$force_english_units && ($vn_value_in_inches > $o_dimensions_config->get('use_inches_for_display_up_to')))) {
                                if ($vn_in_feet = (int)($vn_value_in_inches / 12)) {
                                    $va_measure_conv[] = "{$vn_in_feet} ft".((!$pb_omit_units && in_array('FEET', $va_add_periods_list)) ? '.' : '');
                                }
                    ...
                    if ($vs_measure_conv) {
                        if ($pb_omit_units) {
                            $vs_measure_conv = trim(preg_replace("![^\d\-\.\/ ]+!", "", $vs_measure_conv));
                        }
    ...
    if (is_object($qr_rel_items)) {
        if (!$qr_rel_items->numHits()) {
            ...
            while ($qr_rel_items->nextHit()) {
                ...
                $va_item['_display'] = caProcessTemplateForIDs(
                    $vs_template,
                    $vs_table,
                ...
                if ($vs_idno_fld) {
                    $va_item['idno'] = mb_strtolower($qr_rel_items->get("{$vs_table}.{$vs_idno_fld}"));
                }
...
{
    if (caGetOption('dont_allow_bundle_show_hide', $pa_settings, false)) {
        return '';
    }
...
function caEditorBundleMetadataDictionary($po_request, $ps_id_prefix, $pa_settings)
{
    ...
    if (!($vs_definition = trim($definition))) {
        return '';
    }
...
function caGetDefaultEditorBundleSortConfiguration(string $table, string $related_table, array $settings): array
{
    ...
    if (!is_array($default_sorts) || !is_array($default_sort_options = caGetOption('options', $default_sorts, null))) {
...
function caEditorBundleSortControls($po_request, $ps_id_prefix, $ps_table, $ps_related_table, $pa_options=null)
{
    ...
    if (!is_array($va_sort_fields = caGetAvailableSortFields($ps_table, $type_id, array_merge(['request' => $po_request], $pa_options, ['naturalSortLabel' => _t('default'), 'includeInterstitialSortsFor' => $ps_related_table]))) || !sizeof($va_sort_fields)) {
        return '';
    }
    ...
    if (!is_array($allowed_sorts) || !sizeof($allowed_sorts)) {
        ...
        $default_sort_options = array_merge(['_natural' => false], array_reduce($default_sort_options, function ($carry, $item) use ($sort_field_trans) {
            if (array_key_exists($item, $sort_field_trans)) {
                $item = $sort_field_trans[$item];
            }	// rewrite truncated fields
    ...
    $va_sort_fields = array_map(function ($v) {
        return mb_strtolower($v);
    }, $va_sort_fields);
    return "<div class='editorBundleSortControl'>"._t(
        'Sort using %1 %2',
        caHTMLSelect(
        "{$ps_id_prefix}_RelationBundleSortControl",
...
function caHomeLocationsEnabled(string $table, $type=null, array $options=null)
{
    ...
    if (caGetOption('enableIfAnyTypeSet', $options, false) && ($t_instance = Datamodel::getInstance($table, true))) {
        if (is_array($types = array_map(function ($v) {
            return $v['idno'];
...
{
    if (!is_array($pconfig = ca_objects::getPolicyConfig($ps_policy))) {
        return null;
    }
    if (!($pt_related = Datamodel::getInstance($pconfig['table'], true))) {
        return null;
    }

    $target = $pt_related->tableName();
    if (!$pt_related->getAppConfig()->get("{$target}_enable_home_location")) {
        return null;
    }
    $policies = array_filter(ca_objects::getHistoryTrackingCurrentValuePolicies($target), function ($v) {
...
function caProcessBottomLineTemplateForDisplay($po_request, $pt_display, $pr_res, $pa_options=null)
{
    ...
    $va_bundles_by_code = [];
    if (!is_array($va_bundles = $pt_display->getPlacementsInDisplay())) {
        return null;
    }
        ...
        switch(strtolower($va_tag_bits[0])) {
            case 'sum':
                ...
                foreach ($va_fields as $vs_field) {
                    if (!isset($va_bundles_by_code[$vs_field])) {
                        continue;
                    }
                    ...
                    case 6:		// currency
                        ...
                        if (is_array($va_values)) {
                            foreach ($va_values as $vs_value) {
                                ...
                                if (is_null($va_tag_values[$vs_value_name]['MIN']) || ($vn_value < $va_tag_values[$vs_value_name]['MIN'])) {
                                    $va_tag_values[$vs_value_name]['MIN'] = $vn_value;
                                }
                                if (is_null($va_tag_values[$vs_value_name]['MAX']) || ($vn_value > $va_tag_values[$vs_value_name]['MAX'])) {
                                    $va_tag_values[$vs_value_name]['MAX'] = $vn_value;
                                }

                                if (($vn_c >= $pn_page_start) && ($vn_c <= $pn_page_end)) {
                                    $va_tag_values[$vs_value_name]['PAGESUM'] += $vn_value;
                                    if (is_null($va_tag_values[$vs_value_name]['PAGEMIN']) || ($vn_value < $va_tag_values[$vs_value_name]['PAGEMIN'])) {
                                        $va_tag_values[$vs_value_name]['PAGEMIN'] = $vn_value;
                                    }
                                    if (is_null($va_tag_values[$vs_value_name]['PAGEMAX']) || ($vn_value > $va_tag_values[$vs_value_name]['PAGEMAX'])) {
                                        $va_tag_values[$vs_value_name]['PAGEMAX'] = $vn_value;
                                    }
                                    $vn_page_len++;
                                }
                            }
    ...
    foreach ($va_subelements_to_process as $vs_subelement => $vn_subelement_datatype) {
        ...
        switch($vn_subelement_datatype) {
            ...
            case 10:	// timecode
                ...
                foreach ($va_tag_values[$vs_value_name] as $vs_tag => $vn_val) {
                    if (!$vb_has_timecode) {
                        $va_tag_values[$vs_value_name][$vs_tag] = '';
                        continue;
                    }
        ...
        foreach ($va_tag_values as $vs_value_name => $va_tag_data) {
            foreach ($va_tag_data as $vs_tag => $vs_tag_value) {
                if (strpos($vs_value_name, '.') !== false) {
                    ...
                    if ($vs_name) {
                        $vs_template = str_replace("^{$vs_tag}:{$vs_name}", $vs_tag_value, $vs_template);
                    }
                }
...
function caRepresentationViewer($po_request, $po_data, $pt_subject, $pa_options=null)
{
    ...
    if (($show_only_media_types) && !is_array($show_only_media_types)) {
        $show_only_media_types = [$show_only_media_types];
    }

    $show_only_media_types_when_present = caGetOption('representationViewerShowOnlyMediaTypesWhenPresent', $pa_options, null);
    if (($show_only_media_types_when_present) && !is_array($show_only_media_types_when_present)) {
        $show_only_media_types_when_present = [$show_only_media_types_when_present];
    }
    ...
    $va_rep_ids = array();
    if (method_exists($vo_data, 'filterNonPrimaryRepresentations')) {
        $vo_data->filterNonPrimaryRepresentations(false);
    }
    while ($vo_data->nextHit()) {
        if (!($vn_representation_id = $vo_data->get('ca_object_representations.representation_id', ['checkAccess' => $va_access_values, 'limit' => 1]))) {
            continue;
        }
        ...
        if (sizeof($va_rep_ids) > 0) {
            ...
            while ($qr_reps->nextHit()) {
                ...
                if ($show_only_media_types && !caMimetypeIsValid($mimetype, $show_only_media_types)) {
                    continue;
                }

                if ($show_only_media_types_when_present_reduced && !caMimetypeIsValid($mimetype, $show_only_media_types_when_present_reduced)) {
                    continue;
                }
            ...
            $o_view->setVar('representation_ids', array_values(array_map(function ($v) {
                return $v['rep_id'];
            }, $va_rep_info)));
...
function caGetMediaAnnotationList($po_data, $pa_options=null)
{
    ...
    if (
        ...
        while ($qr_annotations->nextHit()) {
            if (!preg_match('!^TimeBased!', $qr_annotations->getAnnotationType())) {
                continue;
            }
...
function caDoTemplateTagSubstitution($po_view, $pm_subject, $ps_template_path, $pa_options=null)
{
    ...
    if (caGetOption('clearVars', $pa_options, false)) {
...
function caDragAndDropSortingForHierarchyEnabled($pt_request, $ps_table, $pn_id=null)
{
    ...
    if (!$pt_request->isLoggedIn() || (!$pt_request->user->canDoAction("can_edit_{$ps_table}") && (($vs_hier_table = $t_instance->getProperty('HIERARCHY_DEFINITION_TABLE')) ? !$pt_request->user->canDoAction("can_edit_{$vs_hier_table}") : false))) {
        return false;
    }
    if (!$t_instance->isHierarchical()) {
        return false;
    }
    if (!($vs_rank_fld = $t_instance->getProperty('RANK'))) {
        return false;
    }
    if (!is_null($pn_id) && !$t_instance->load($pn_id)) {
        return false;
    }
    ...
    if ($vs_def_table_name && ($t_def = Datamodel::getInstanceByTableName($vs_def_table_name, true)) && ($t_def->load($t_instance->get($vs_def_id_fld))) && ($t_def->hasField('default_sort')) && ((int)$t_def->get('default_sort') === __CA_LISTS_SORT_BY_RANK__)) {
        ...
        if ((sizeof($va_sort_values) >= 1) && ($va_sort_values[0] === "{$ps_table}.{$vs_rank_fld}")) {
            return true;
        }
    }
    return false;
...
function caExtractSettingValueByLocale($pa_settings, $ps_key, $ps_locale)
{
    if (isset($pa_settings[$ps_key])) {
        if (isset($pa_settings[$ps_key]) && !is_array($pa_settings[$ps_key])) {
            return $pa_settings[$ps_key];
        }
        if (isset($pa_settings[$ps_key][$ps_locale]) && ($pa_settings[$ps_key][$ps_locale])) {
            ...
            foreach ($va_locales_for_language as $vs_locale) {
                if ($pa_settings[$ps_key][$vs_locale]) {
                    return $pa_settings[$ps_key][$vs_locale];
                }
            }
        }
    }
...
function caProcessReferenceTags($po_request, $ps_text, $pa_options=null)
{
    ...
    if (!is_array($va_access_values = caGetUserAccessValues($po_request)) || !sizeof($va_access_values)) {
        ...
        if (preg_match_all("!\[{$vs_ref_tag} ([^\]]+)\]([^\[]+)\[/{$vs_ref_tag}\]!", $ps_text, $va_matches)) {
            foreach ($va_matches[1] as $i => $vs_attr_string) {
                if (sizeof($va_vals = caParseAttributes($vs_attr_string, ['id', 'idno', 'class', 'version'])) > 0) {
                    ...
                    $va_idnos[$vs_ref_type][$va_matches[0][$i]] = array_filter($va_vals, function ($v) {
                        return !is_null($v);
                    });
                }
            }
                ...
                case 'ca_site_page_media':
                    foreach ($va_idnos[$vs_ref_type] as $vs_tag => $va_l) {
                        ...
                        while ($qr_m->nextHit()) {
                            ...
                            if (!($caption = trim($qr_m->get('caption')))) {
                                $caption = null;
                            }
                            if (!($title = trim($qr_m->get('title')))) {
                                $title = null;
                            }
                            if (!($idno = trim($qr_m->get('idno')))) {
                                $idno = null;
                            }
                    ...
                    break;
                default:
                    $va_map = call_user_func($vs_ref_type.'::getIDsForIdnos', array_map(function ($v) {
                        return $v['idno'];
                    }, $va_idnos[$vs_ref_type]), ['forceToLowercase' => true, 'checkAccess' => $va_access_values]);

                    $va_idnos[$vs_ref_type] = array_map(function ($v) use ($va_map) {
                        $v['id'] = $va_map[$v['idno']];
                        return $v;
                    }, $va_tags);
...
{
    if (MemoryCache::contains('blank_label_text_'.$table)) {
        return MemoryCache::fetch('blank_label_text_'.$table);
    }
    $config = Configuration::load();
    ...
    if ($label_text = $config->get($d)) {
        ...
        if ($table !== $table_orig) {
            MemoryCache::save('blank_label_text_'.$table_orig, $l);
        }
        return $l;
    }
...
{
    if (!in_array($type, ['menuBar', 'login', 'report'], true)) {
        return null;
    }
