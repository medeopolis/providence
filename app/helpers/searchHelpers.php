...
function caSearchUrl($po_request, $ps_table, $ps_search=null, $pb_return_url_as_pieces=false, $pa_additional_parameters=null, $pa_options=null)
{
    if (is_numeric($ps_table)) {
        if (!($t_table = Datamodel::getInstanceByTableNum($ps_table, true))) {
            return null;
        }
    } else {
...
function caPuppySearch($po_request, $ps_search_expression, $pa_blocks, $pa_options=null)
{
    ...
    if ($purifier = RequestHTTP::getPurifier()) {
        $ps_search_expression = $purifier->purify($ps_search_expression);
    }
    ...
    foreach ($pa_blocks as $vs_block => $va_block_info) {
        ...
        if (is_array($va_types) && sizeof($va_types)) {
            $o_search->setTypeRestrictions($va_types, $va_block_info);
        }
        ...
        if (is_array($base_criteria)) {
            if (!($o_browse = caGetBrowseInstance($va_block_info['table']))) {
                continue;
            }
...
{
    if (!($t_instance = Datamodel::getInstanceByTableName($pr_res->tableName(), true))) {
        return null;
    }

    if (!($vs_type_fld = $t_instance->getTypeFieldName())) {
        return null;
    }
        ...
        switch($vs_element) {
            case '_fieldlist':
                foreach ($pa_form_values[$vs_dotless_element.'_field'] as $vn_j => $vs_fieldlist_field) {
                    if (!strlen(trim($vs_fieldlist_field))) {
                        continue;
                    }
                ...
                if (is_array($pa_form_values[$vs_dotless_element])) {
                    ...
                    if (isset($pa_form_values["{$vs_dotless_element}{$vs_element_rel_type}_autocomplete"])) {
                        ...
                        foreach ($pa_form_values["{$vs_dotless_element}{$vs_element_rel_type}"] as $vn_j => $vs_element_value) {
                            if ($t_table) {
                                $vs_search_element = $t_table->primaryKey(true);
                            }
                    ...
                    foreach ($pa_form_values["{$vs_dotless_element}{$vs_element_rel_type}"] as $vn_j => $vs_element_value) {
                        if (!strlen(trim($vs_element_value))) {
                            continue;
                        }
            ...
            foreach ($va_value_list as $vn_i => $vs_value) {
                ...
                switch($vs_element) {
                    ...
                    case '_fieldlist_field':
                        if (!strlen(trim($pa_form_values['_fieldlist_value'][$vn_i]))) {
                            continue(2);
                        }
                    ...
                    default:
                        ...
                        if ((sizeof($va_tmp) > 2) && ($va_tmp[1] == 'related')) {
                            unset($va_tmp[1]);
                            $vs_element = join('.', $va_tmp);
                        }
...
function caGetDisplayStringForHTMLFormInput($po_result_context, $pa_options=null)
{
    ...
    foreach ($va_form_contents as $vn_i => $vs_element) {
        ...
        if (!($vs_label = trim($pa_form_values["{$vs_dotless_element}_label"]))) {
            $vs_label = "???";
        }

        if (isset($pa_form_values["{$vs_dotless_element}_autocomplete"])) {
            if (!is_array($pa_form_values["{$vs_dotless_element}_autocomplete"])) {
                $pa_form_values["{$vs_dotless_element}_autocomplete"] = [$pa_form_values["{$vs_dotless_element}_autocomplete"]];
            }
        ...
        if ((sizeof($va_fld) > 2) && ($va_fld[1] == 'related')) {
            unset($va_fld[1]);
            $vs_element = join('.', $va_fld);
            $va_fld = array_values($va_fld);
        }
        $t_table = Datamodel::getInstanceByTableName($va_fld[0], true);
...
function caGetDisplayStringForSearch($ps_search, $pa_options=null)
{
    ...
    if ($purifier = RequestHTTP::getPurifier()) {
        $ps_search = $purifier->purify($ps_search);
    }
...
{
    if (file_exists(__CA_THEME_DIR__."/helpers/namedSearchHelpers.php")) {
        require_once(__CA_THEME_DIR__."/helpers/namedSearchHelpers.php");
    }
...
{
    if (!is_array($pa_values) && ($va_tmp = json_decode($pa_values))) {
        $pa_values = $va_tmp;
    }
...
function caRewriteElasticSearchTermFieldSpec($po_term)
{
    if (strlen($po_term->field) > 0) {
        ...
        if (strtolower($va_tmp[1]) == 'count') {
            $va_tmp[1] = 'COUNT';
        }	// uppercase count field
        if (preg_match("/^count\|(.*)$/", strtolower($va_tmp[1]), $va_matches)) {
            $va_tmp[1] = 'COUNT|'.$va_matches[1];
        }	// uppercase count field with optional type
...
function caRewriteDateForElasticSearch($ps_date, $pb_is_start=true)
{
    ...
    if (preg_match("/^([\-]{0,1}[\d]+)\-(\d+)\-(\d+)T(\d+)\:(\d+)\:(\d+)Z/", $ps_date, $va_date_parts)) {
        ...
        if (intval($va_date_parts[1]) > 9999) {
            $va_date_parts[1] = 9999;
        }
        if (intval($va_date_parts[1]) < -9999) {
            $va_date_parts[1] = -9999;
        }
        ...
        if (intval($va_date_parts[2]) < 1) {
            $va_date_parts[2]  = ($pb_is_start ? "01" : "12");
        }
        // fix messed up months
        if (intval($va_date_parts[2]) > 12) {
            $va_date_parts[2] = "12";
        }
        // fix day-less dates
        if (intval($va_date_parts[3]) < 1) {
            $va_date_parts[3]  = ($pb_is_start ? "01" : "31");
        }
        // fix messed up days
        $vn_days_in_month = cal_days_in_month(CAL_GREGORIAN, intval($va_date_parts[2]), intval(abs($va_date_parts[1])));
        if (intval($va_date_parts[3]) > $vn_days_in_month) {
            $va_date_parts[3] = (string) $vn_days_in_month;
        }

        // fix hours
        if (intval($va_date_parts[4]) > 23) {
            $va_date_parts[4] = "23";
        }
        if (intval($va_date_parts[4]) < 0) {
            $va_date_parts[4]  = ($pb_is_start ? "00" : "23");
        }
        // minutes and seconds
        if (intval($va_date_parts[5]) > 59) {
            $va_date_parts[5] = "59";
        }
        if (intval($va_date_parts[5]) < 0) {
            $va_date_parts[5]  = ($pb_is_start ? "00" : "59");
        }
        if (intval($va_date_parts[6]) > 59) {
            $va_date_parts[6] = "59";
        }
        if (intval($va_date_parts[6]) < 0) {
            $va_date_parts[6]  = ($pb_is_start ? "00" : "59");
        }
...
{
    if (caGetOption('disableSorts', $options, false)) {
        return [];
    }
    ...
    $o_config = Configuration::load();
    if ((bool)$o_config->get('do_available_sort_list_caching') && CompositeCache::contains('available_sorts', 'SearchBuilder') && is_array($cached_data = CompositeCache::fetch('available_sorts', 'SearchBuilder')) && isset($cached_data[$cache_key])) {
    ...
    if ($ps_table) {
        ...
        foreach ($va_base_fields as $k => $v) {
            foreach ([$k, preg_replace("!_sort$!", "", $k)] as $kp) {
                if (isset($ui_bundle_label_map[$kp])) {
                    ...
                    array_pop($tmp);
                    if (isset($ui_bundle_label_map[join('.', $tmp)])) {
                        $va_base_fields[$k] = $ui_bundle_label_map[join('.', $tmp)];
                    }
                }
            }
        }
        ...
        if (is_array($config_sorts = $o_config->get("{$ps_table}_include_result_sorts")) && sizeof($config_sorts)) {
            $config_sorts = array_filter($config_sorts, function ($v) {
                return is_string($v);
            });
    ...
    if (($pa_allowed_sorts = caGetOption('allowedSorts', $options, null)) && !is_array($pa_allowed_sorts)) {
        $pa_allowed_sorts = [$pa_allowed_sorts];
    }

    if (is_array($pa_allowed_sorts) && sizeof($pa_allowed_sorts) > 0) {
        foreach ($va_base_fields as $vs_k => $vs_v) {
            if (!in_array($vs_k, $pa_allowed_sorts) && !isset($config_sorts[$vs_k])) {
                unset($va_base_fields[$vs_k]);
            }
        }
    }
    ...
    $va_base_fields = array_map(function ($v) {
        return caUcFirstUTF8Safe($v);
    }, $va_base_fields);
        ...
        if (ca_metadata_elements::getElementDatatype($element_code) === __CA_ATTRIBUTE_VALUE_CONTAINER__) {
            if (is_array($sub_elements = ca_metadata_elements::getElementsForSet($element_code))) {
                foreach ($sub_elements as $sub_element) {
                    if (((int)$sub_element['datatype'] === __CA_ATTRIBUTE_VALUE_CONTAINER__) && ($sub_element['parent_id'] > 0)) {
                        continue;
                    }	// skip sub-containers
                    $sub_element_code = $sub_element['element_code'];

                    if ($sub_element['element_id'] == $sub_element['hier_element_id']) {	// is root
                        if (!is_array($b) && !is_array($bundles[$b])) {
                            $bundles[$b] = [];
                        }
...
function caGetSearchBuilderFilters(BaseModel $t_subject, Configuration $po_query_builder_config)
{
    ...
    if (CompositeCache::contains($key, 'SearchBuilder') && is_array($cached_data = CompositeCache::fetch($key, 'SearchBuilder'))) {
        return $cached_data;
    }
