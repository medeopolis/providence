...
function caGetPrintTemplateDirectoryPath($ps_type)
{
    ...
    switch($ps_type) {
        case 'results':
            if (is_dir(__CA_THEME_DIR__.'/printTemplates/results')) {
                $va_paths[] = __CA_THEME_DIR__.'/printTemplates/results';
            }
            $va_paths[] = __CA_APP_DIR__.'/printTemplates/results';
            break;
        case 'summary':
            if (is_dir(__CA_THEME_DIR__.'/printTemplates/summary')) {
                $va_paths[] = __CA_THEME_DIR__.'/printTemplates/summary';
            }
            $va_paths[] = __CA_APP_DIR__.'/printTemplates/summary';
            break;
        case 'labels':
            if (is_dir(__CA_THEME_DIR__.'/printTemplates/labels')) {
                $va_paths[] = __CA_THEME_DIR__.'/printTemplates/labels';
            }
            $va_paths[] = __CA_APP_DIR__.'/printTemplates/labels';
            break;
        case 'bundles':
            if (is_dir(__CA_THEME_DIR__.'/printTemplates/bundles')) {
                $va_paths[] = __CA_THEME_DIR__.'/printTemplates/bundles';
            }
            $va_paths[] = __CA_APP_DIR__.'/printTemplates/bundles';
            break;
        case 'sets':
            if (is_dir(__CA_THEME_DIR__.'/printTemplates/sets')) {
                $va_paths[] = __CA_THEME_DIR__.'/printTemplates/sets';
            }
...
{
    if (!is_array($va_template_paths = caGetPrintTemplateDirectoryPath($ps_type))) {
        $va_template_paths = [];
    }
    ...
    if (!is_array($va_show_only_in = caGetOption('showOnlyIn', $pa_options, null))) {
        $va_show_only_in = array_map(function ($v) {
            return trim($v);
        }, explode(',', $va_show_only_in));
    }
            ...
            if (is_resource($r_dir = opendir($vs_path))) {
                while (($vs_template = readdir($r_dir)) !== false) {
                    ...
                    if (is_array($va_template_info = caGetPrintTemplateDetails($ps_type, $vs_template_tag))) {
                        if (caGetOption('type', $va_template_info, null) !== $vs_type) {
                            continue;
                        }
                        if (caGetOption('disabled', $va_template_info, false, ['castTo' => 'bool'])) {
                            continue;
                        }

                        if (!is_array($template_restrict_to_types = caGetOption('restrictToTypes', $va_template_info, null))) {
                            $template_restrict_to_types = [];
                        }
                        ...
                        $va_template_show_only_in = array_filter(array_map(function ($v) {
                            return trim($v);
                        }, explode(",", caGetOption('showOnlyIn', $va_template_info, null))), function ($v) {
                            return (bool)strlen($v);
                        });
                        if (is_array($va_show_only_in) && (sizeof($va_show_only_in) > 0) && is_array($va_template_show_only_in) && (sizeof($va_template_show_only_in) > 0) && !sizeof(array_intersect($va_template_show_only_in, $va_show_only_in))) {
                            continue;
                        }

                        if ($vs_element_code && (caGetOption('elementCode', $va_template_info, null) !== $vs_element_code)) {
                            continue;
                        }
...
function caGetPrintTemplateDetails($ps_type, $ps_template, $pa_options=null)
{
    ...
    foreach ($va_template_paths as $vs_template_path) {
        ...
        if (!$va_info['fileFormat']) {
            $va_info['fileFormat'] = 'pdf';
        }    // pdf is assumed for templates without a specific file format
...
function caConvertMeasurementToPoints($ps_value, $pa_options=null)
{
    ...
    if (isset($g_print_measurement_cache[$ps_value])) {
        return $g_print_measurement_cache[$ps_value];
    }
...
function caGetPrintFormatsListAsHTMLForSetItemBundles($ps_id_prefix, $po_request, $pt_set, $pa_row_ids)
{
    ...
    if (!is_array($va_formats) || (sizeof($va_formats) == 0)) {
        return '';
    }
...
function caEditorPrintSummaryControls($view)
{
    ...
    if (is_array($print_templates)) {
        ...
        foreach ($print_templates as $k => $v) {
            if (($num_available_templates > 1) && (bool)$v['generic']) {
                continue;
            }    // omit generics from list when specific templates are available
            if ((bool)$v['standalone']) {
                continue;
            }
