...
function caProcessRefineryParents($ps_refinery_name, $ps_table, $pa_parents, $pa_source_data, $pa_item, $pn_c, $pa_options=null)
{
    ...
    if ($hier_delimiter && !is_array($hier_delimiter)) {
        $hier_delimiter = [$hier_delimiter];
    }
    ...
    foreach ($pa_parents as $vn_i => $va_parent) {
        ...
        if (is_array($hier_delimiter)) {
            $name_hier = array_map(function ($v) {
                return trim($v);
            }, preg_split("!(".join("|", $hier_delimiter).")!", $vs_name));
            $idno_hier = array_map(function ($v) {
                return trim($v);
            }, preg_split("!(".join("|", $hier_delimiter).")!", $vs_idno));
        } else {
            $name_hier = [$vs_name];
            $idno_hier = [$vs_idno];
        }
        foreach ($name_hier as $x => $vs_name) {
            ...
            if ((get_class($id_numberer) === 'MultipartIDNumber') && caGetOption('normalizeIdno', $va_parent, false)) {
                ...
                if (is_array($id_number_elements) && (sizeof($id_number_elements) > 1)) {		// don't bother normalizing single-element formats
                    ...
                    if ($last !== '%') {
                        array_push($n, $last);
                    }	// remove trailing SERIAL placeholder if present
            ...
            if (!$vs_name && isset($va_parent['name'])) {
                continue;
            }
            ...
            switch($ps_table) {
                ...
                case 'ca_list_items':
                    ...
                    if (!isset($va_attributes['is_enabled'])) {
                        $va_attributes['is_enabled'] = 1;
                    }
                    ...
                    foreach ($va_val['_related_related'] as $vs_table => $va_rels) {
                        foreach ($va_rels as $va_rel) {
                            if (!$t_subject->addRelationship($vs_table, $va_rel['id'], $va_rel['_relationship_type'], null, null, null, null, ['setErrorOnDuplicate' => true])) {
                                if ($o_log) {
                                    $o_log->logDebug(_t('[%6] Could not create relationship between parent %1 and %2 for ids %3 and %4 with type %5: %7', $ps_table, $vs_table, $vn_id, $va_rel['id'], $va_rel['_relationship_type'], $ps_refinery_name, join("; ", $t_subject->getErrors())));
                                }
                            }
                        }
                    }
        ...
        foreach ($pa_attributes as $vs_element_code => $va_attrs) {
            ...
            if (caIsIndexedArray($va_attrs)) {
                ...
                foreach ($va_attrs as $va_attrs_i) {
                    ...
                    if (!is_array($va_attrs_i)) {
                        $va_attrs_i = [$va_attrs_i];
                    }
                    foreach ($va_attrs_i as $vs_k => $vs_v) {
                        ...
                        if (sizeof($va_vals) > 1) {
                            $vb_is_repeating = true;
                        }

                        if ($vb_is_repeating) {
                            if (is_null($vn_num_repeats)) {
                                $vn_num_repeats = sizeof($va_vals);
                            }

                            $vn_c = 0;
                            foreach ($va_vals as $vn_x => $va_v) {
                                if (!$va_v || (!is_array($va_v) && !trim($va_v))) {
                                    continue;
                                }
                                if ($vs_prefix && is_array($va_v)) {
                                    $va_v = array_map(function ($v) use ($vs_prefix) {
                                        return $vs_prefix.$v;
                                    });
                            ...
                            if ($vm_val_to_import = trim((is_array($vm_v = BaseRefinery::parsePlaceholder($vs_v, $pa_source_data, $pa_item, $pn_c, array('delimiter' => caGetOption('delimiter', $pa_options, null), 'returnAsString' => true, 'reader' => $o_reader, 'applyImportItemSettings' => $apply_import_item_settings)))) ? join(" ", $vm_v) : $vm_v)) {
                                if (!file_exists($vs_path = $vs_prefix.$vm_val_to_import) && ($va_candidates = array_filter($va_prefix_file_list, function ($v) use ($vs_path) {
                                    return preg_match("!^{$vs_path}!", $v);
                                })) && is_array($va_candidates) && sizeof($va_candidates)) {
...
function caProcessInterstitialAttributes($ps_refinery_name, $pm_import_tablename_or_num, $pm_target_tablename_or_num, $pa_source_data, $pa_item, $pn_c, $pa_options=null)
{
    ...
    if (is_array($pa_item['settings']["{$ps_refinery_name}_interstitial"])) {
        if (!($ps_import_tablename = Datamodel::getTableName($pm_import_tablename_or_num))) {
            return null;
        }
        if (!($ps_target_tablename = Datamodel::getTableName($pm_target_tablename_or_num))) {
            return null;
        }
        if (!($t_target = Datamodel::getInstanceByTableName($ps_target_tablename, true))) {
            return null;
        }
        if ($o_trans) {
...
function caProcessRefineryRelated($po_refinery_instance, $ps_related_table, $pa_related_option_list, $pa_source_data, $pa_item, $pn_c, $pa_options=null)
{
    ...
    foreach ($pa_related_option_list as $vn_i => $pa_related_options) {
        ...
        if (!is_array($va_name = BaseRefinery::parsePlaceholder($vs_name_spec, $pa_source_data, $pa_item, $pn_c, array('reader' => $o_reader, 'returnAsString' => false, 'delimiter' => $vs_delimiter, 'applyImportItemSettings' => $apply_import_item_settings)))) {
            $va_name = [$va_name];
        }
        if (!is_array($va_idno = BaseRefinery::parsePlaceholder($vs_idno_spec, $pa_source_data, $pa_item, $pn_c, array('reader' => $o_reader, 'returnAsString' => false, 'delimiter' => $vs_delimiter, 'applyImportItemSettings' => $apply_import_item_settings)))) {
            $va_idno = [$va_idno];
        }
        if (!is_array($va_type = BaseRefinery::parsePlaceholder($vs_type_spec, $pa_source_data, $pa_item, $pn_c, array('reader' => $o_reader, 'returnAsString' => false, 'delimiter' => $vs_delimiter, 'applyImportItemSettings' => $apply_import_item_settings)))) {
            $va_type = [$va_type];
        }
        if (!is_array($va_parent_id = BaseRefinery::parsePlaceholder($vs_parent_id_spec, $pa_source_data, $pa_item, $pn_c, array('reader' => $o_reader, 'returnAsString' => false, 'delimiter' => $vs_delimiter, 'applyImportItemSettings' => $apply_import_item_settings)))) {
        ...
        foreach ($va_name as $i => $vs_name) {
            ...
            if (!$vs_name && (strpos($vs_idno, '%') === false)) {
                $vs_name = $vs_idno;
            }
            if (!$vs_name) {
                continue;
            ...
            if (!$vs_type) {
                $vs_type = $va_type[sizeof($va_type) - 1];
            }
            if (!$vs_parent_id) {
                $vs_parent_id = $va_parent_id[sizeof($va_parent_id) - 1];
            }
...
function caGenericImportSplitter($ps_refinery_name, $ps_item_prefix, $ps_table, $po_refinery_instance, &$pa_destination_data, $pa_group, $pa_item, $pa_source_data, $pa_options)
{
    ...
    if (!is_array($va_delimited_items)) {
        $va_delimited_items = [$va_delimited_items];
    }
    $va_delimiter = $pa_item['settings']["{$ps_refinery_name}_delimiter"];
    if (!is_array($va_delimiter)) {
        $va_delimiter = [$va_delimiter];
    }
    ...
    if (!isset($pa_options['matchOn'])) {
        $pa_options['matchOn'] = array('idno', 'label');
    }
        ...
        foreach ($va_delimited_items as $vn_x => $vs_delimited_item) {
            ...
            foreach ($va_items as $vn_i => $vs_item) {
                ...
                if (!($vs_item = trim($vs_item))) {
                    if (is_array($va_parents) && (sizeof($va_parents) > 0)) {
                        ...
                        while (sizeof($va_parents) > 0) {
                            ...
                            if (!isset($va_p[$vs_display_field])) {
                                $vs_display_field = 'name';
                            }
                ...
                if (is_array($va_skip_values = $pa_item['settings']["{$ps_refinery_name}_skipIfValue"]) && in_array($vs_item, $va_skip_values)) {
                    if ($o_log) {
                        $o_log->logDebug(_t('[%1] Skipped %2 because it was in the skipIfValue list', $ps_refinery_name, $vs_item));
                    }
                    continue;
                }
                ...
                if ($va_hierarchy_setting = $pa_item['settings']["{$ps_refinery_name}_hierarchy"]) {
                    ...
                    if (
                        ...
                        $va_item_hier = array_map(function ($v) {
                            return trim($v);
                        }, preg_split("!(".join("|", $va_hier_delimiter).")!", $vs_item));
                            ...
                            foreach ($va_item_hier as $vn_ix => $vs_parent) {
                                ...
                                switch($ps_refinery_name) {
                                    ...
                                    case 'listItemSplitter':
                                        $vals = ['idno' => $vs_parent];
                                        if (!is_null($vn_hier_item_id)) {
                                            $vals['parent_id'] = $vn_hier_item_id;
                                        }
                ...
                if (($o_reader instanceof CollectiveAccessDataReader) && ($vs_table_idno_fld = Datamodel::getTableProperty($ps_table, 'ID_NUMBERING_ID_FIELD')) && in_array($vs_table_idno_fld, $pa_options['matchOn']) && !isset($va_val[$vs_table_idno_fld])) {
                    ...
                    if (isset($va_idno_value_list[$vn_i]) && strlen($va_idno_value_list[$vn_i])) {
                        $va_val[$vs_table_idno_fld] = $va_idno_value_list[$vn_i];
                    }
                }
                ...
                if (
                    ...
                    switch($ps_table) {
                        ...
                        case 'ca_list_items':
                            ...
                            if (!isset($va_attr_vals_with_parent['is_enabled'])) {
                                $va_attr_vals_with_parent['is_enabled'] = 1;
                            }
                            if (is_array($vs_idno = caGetOption('idno', $va_attr_vals_with_parent, null))) {
                                $vs_idno = caGetOption('idno', $vs_idno, null);
                            }
                            if (!$vs_idno) {
                                $vs_idno = $vs_item;
                            }
                    ...
                    switch($ps_table) {
                        ...
                        case 'ca_object_representations':
                            ...
                            if ($va_val['media']['media'] || $vs_item) {
                                ...
                                foreach (caGetAvailableMediaUploadPaths() as $d) {
                                    $va_files = caBatchFindMatchingMedia($d
                                        ...
                                        'matchMode' => caGetOption(
                                            'objectRepresentationSplitter_matchMode',
                                            $pa_item['settings'],
                                            'FILE_NAME'
                                        ),
                                        'matchType' => caGetOption(
                                            'objectRepresentationSplitter_matchType',
                                            $pa_item['settings'],
                                            null
                                        ),
                                        'log'       => $o_log
                            ...
                            if (!isset($va_val['idno'])) {
                                $va_val['idno'] = pathinfo($vs_item, PATHINFO_FILENAME);
                            }
                            break;
                        default:
                            if ($o_log) {
                ...
                if (isset($pa_options['ignoreParent']) && $pa_options['ignoreParent']) {
                    $va_val['_ignoreParent'] = 1;
                }
                $va_vals[] = $va_val;
            }
        }
    ...
    $va_vals = array_filter($va_vals, function ($v) {
        ...
        foreach ($keys as $k) {
            if ($k[0] !== '_') {
                return true;
            }	// directive keys start with "_"; if there are any keys starting something else, we have data
        }
        return false;
    });
        ...
        foreach ($va_relationships as $va_relationship_settings) {
            if ($vs_table_name = caGetOption('relatedTable', $va_relationship_settings)) {
                if ($skip_if_all_empty = caGetOption('skipIfAllEmpty', $va_relationship_settings, null, [])) {
                    if (!is_array($skip_if_all_empty)) {
                        $skip_if_all_empty = [$skip_if_all_empty];
                    }
                    $skip_if_all_empty = array_map(function ($v) {
                ...
                if ($skip_if_any_empty = caGetOption('skipIfAnyEmpty', $va_relationship_settings, null)) {
                    $skip_if_any_empty = array_map(function ($v) {
                        return preg_replace('!^\^!', '', $v);
                    }, $skip_if_any_empty);
                    $is_empty = false;
                ...
                if (is_array($va_rels = caProcessRefineryRelated($po_refinery_instance, $vs_table_name, array($va_relationship_settings), $pa_source_data, $pa_item, $pn_value_index, array_merge($pa_options, ['dontCreate' => caGetOption('dontCreate', $va_relationship_settings, false), 'list_id' => caGetOption('list', $va_relationship_settings, null)])))) {
                    ...
                    if (is_array($va_rel_rels)) {
                        if (!is_array($va_val['_related_related'])) {
                            $va_val['_related_related'] = [];
                        }
                        foreach ($va_rel_rels as $vs_rel_table => $va_rel_info) {
                            if (!is_array($va_val['_related_related'][$vs_rel_table])) {
                                $va_val['_related_related'][$vs_rel_table] = [];
                            }
