...
function _p($ps_key)
{
    ...
    if (!sizeof(func_get_args()) & isset($g_translation_cache[$ps_key])) {
        print $g_translation_cache[$ps_key];
        return;
    }
...
function &caGetDirectoryContentsAsList($dir, $pb_recursive=true, $pb_include_hidden_files=false, $pb_sort=false, $pb_include_directories=false, $pa_options=null)
{
    ...
    if ($va_paths = @scandir($dir, 0)) {
        foreach ($va_paths as $item) {
            ...
            if (($limit > 0) && (sizeof($va_file_list) >= $limit)) {
                break;
            }
        }
    }
...
function caDirectorySize($dir, array $options=null)
{
    ...
    if (caGetOption('forDisplay', $options, false)) {
        return caHumanFilesize($size);
    }
...
function caIsValidFilePath($ps_path)
{
    ...
    $ps_path = preg_replace("!\"$!", "", $ps_path);
    if (!$ps_path || (preg_match("/[^\/A-Za-z0-9\.:\ _\(\)\\\-]+/", $ps_path))) {
        return false;
    }
...
{
    if (caGetOption('useAppTmpDir', $options, true)) {
        return __CA_APP_DIR__."/tmp";
    }
...
{
    if (preg_match("!^".caGetTempDirPath()."/!", $ps_file)) {
        return true;
    }
    return false;
}
...
function caModRewriteIsAvailable()
{
    ...
    if (is_bool($g_mod_write_is_available)) {
        return $g_mod_write_is_available;
    }
 ...
 */
function caLengthSortHelper($a, $b)
{
...
{
    if (!($ps_dimension = trim($ps_dimension))) {
        $ps_dimension = caGetOption('default', $pa_options, null);
    }
    if (!$ps_dimension) {
        return null;
    }
...
function caSortArrayByKeyInValue($pa_values, $pa_sort_keys, $ps_sort_direction="ASC", $pa_options=null)
{
    ...
    foreach ($pa_values as $vn_id => $va_data) {
        ...
        foreach ($va_sort_keys as $vs_sort_key) {
            ...
            if ($pb_natural_sort || $pb_case_insensitive) {
                $k = mb_strtolower($k);
            }
            if ($pb_natural_sort) {
...
{
    if (!$ps_cache_dir) {
        $ps_cache_dir = __CA_APP_DIR__.'/tmp';
    }
...
{
    if (!is_array($pa_options)) {
        return md5($pa_options.$ps_additional_text);
    }
    foreach ($pa_options as $vs_key => $vm_value) {
        if (is_object($vm_value)) {
            unset($pa_options[$vs_key]);
        }
    }
...
function caFormatDate($ps_date_expression, $ps_format = 'c')
{
    ...
    if (!is_numeric($va_unix_timestamps['start'])) {
        return null;
    }
...
function caGetOption($pm_option, $pa_options, $pm_default=null, $pa_parse_options=null)
{
    ...
    if (isset($pa_parse_options['validValues']) && is_array($pa_parse_options['validValues'])) {
        ...
        if (!isset($pa_parse_options['caseSensitive']) || !$pa_parse_options['caseSensitive']) {
            $va_valid_values = array_map(function ($v) {
    ...
    if (isset($pa_parse_options['defaultOnEmptyString']) && $pa_parse_options['defaultOnEmptyString'] && is_string($vm_val) && (strlen($vm_val) === 0)) {
        $vm_val = $pm_default;
    }
    ...
    if (is_string($vm_val) && (isset($pa_parse_options['delimiter']) && ($va_delimiter = $pa_parse_options['delimiter']))) {
        ...
        if (is_array($va_valid_values)) {
            ...
            foreach ($va_split_vals as $vm_val) {
                ...
                if (!sizeof($va_filtered_vals) && $pm_default) {
                    $va_filtered_vals[] = $pm_default;
                }
...
function caGetOptions($pa_options, $pa_defaults)
{
    ...
    foreach ($pa_defaults as $vs_opt => $vs_opt_default_val) {
        if (!isset($va_proc_options[$vs_opt])) {
            $va_proc_options[$vs_opt] = $vs_opt_default_val;
        }
    }
...
function caMakeSearchResult($ps_table, $pa_ids, $pa_options=null)
{
    if ($t_instance = Datamodel::getInstanceByTableName('ca_objects', true)) {	// get an instance of a model inherits from BundlableLabelableBaseModelWithAttributes; doesn't matter which one
        if ($trans = caGetOption('transaction', $pa_options, null)) {
...
function caExtractValuesFromArrayList($pa_array, $ps_key, $pa_options=null)
{
    ...
    foreach ($pa_array as $vs_k => $va_v) {
        if (!$vb_include_blanks && (!isset($va_v[$ps_key]) ||(strlen($va_v[$ps_key]) == 0))) {
            continue;
        }
        if ($vb_preserve_keys) {
...
function caGetCurrencySymbol($value, $element_code_or_id=null)
{
    ...
    if (!$dollars_are_this) {
        $dollars_are_this = strtolower($o_config->get('default_dollar_currency'));
    }
...
function caParseCurrencyValue(string $value, $locale=null): ?array
{
    ...
    try {
        ...
        // or 1
        } elseif (preg_match("!^([\d\.\,]+)([^\d]+)$!", trim($value), $matches)) {
        ...
        // or 2
        } elseif (preg_match("!(^[\d\,\.]+$)!", trim($value), $matches)) {
            $decimal_value = (strpos($matches[1], ',') !== false) ? Zend_Locale_Format::getNumber($matches[1], ['locale' => $g_locale, 'precision' => 2]) : (float)$matches[1];
...
function caParseLengthExpression($ps_expression, $pa_options=null)
{
    ...
    foreach ($va_measurements as $vn_i => $vs_measurement) {
        ...
        try {
            if (!($vo_parsed_measurement = caParseLengthDimension($vs_measurement))) {
                ...
                if (!strlen($vn_parse_precision = $pn_precision) && !strlen($vn_parse_precision = $o_dimensions_config->get(caGetLengthUnitType($vs_extracted_units, ['code' => true])."_decimal_precision"))) {
                    $vn_parse_precision = 4;
                }
                $vs_measurement = trim($vo_parsed_measurement->toString($vn_parse_precision));
                if (!$vs_specified_units) {
                    $vs_specified_units = $vs_extracted_units;
                }
            }
        } catch(Exception $e) {
    ...
    foreach ($va_extracted_measurements as $vn_i => $va_measurement) {
        if (!$va_unit_map[$vn_i]) {
            ...
            if (!($vs_inferred_units = $ps_units)) {
                $ps_units = "in";
            }
            for ($x = $vn_i + 1; $x < sizeof($va_unit_map); $x++) {
                if ($va_unit_map[$x]) {
                    $vs_inferred_units = $va_unit_map[$x];
                    break;
                }
            }
            ...
            try {
                if ($vo_parsed_measurement = caParseLengthDimension($va_measurement['string']." {$vs_inferred_units}")) {
                    if (!strlen($vn_parse_precision = $pn_precision) && !strlen($vn_parse_precision = $o_dimensions_config->get(caGetLengthUnitType($vs_inferred_units, ['code' => true])."_decimal_precision"))) {
                        $vn_parse_precision = 4;
                    }
    ...
    if ($pb_return_extracted_measurements) {
        return $va_extracted_measurements;
    }
...
function caGenerateCSRFToken($po_request=null)
{
    ...
    if ($po_request) {
        if (!is_array($va_tokens = PersistentCache::fetch("csrf_tokens_{$session_id}", "csrf_tokens"))) {
            $va_tokens = [];
        }
        if (sizeof($va_tokens) > 2000) {
            $va_tokens = array_filter($va_tokens, function ($v) {
                return ($v > (time() - 86400));
            });	// delete any token older than eight hours
        }
        if (sizeof($va_tokens) > 2000) {
            $va_tokens = array_slice($va_tokens, 0, 500, true); // delete last quarter of token buffer if it gets too long
        }

        if (!isset($va_tokens[$vs_token])) {
...
function caValidateCSRFToken($po_request, $ps_token=null, $pa_options=null)
{
    ...
    if (!$ps_token) {
        $ps_token = $po_request->getParameter('csrfToken', pString);
    }
    if (!is_array($va_tokens = PersistentCache::fetch("csrf_tokens_{$session_id}", "csrf_tokens"))) {
        $va_tokens = [];
    }
...
function caGetFileMTime($ps_file)
{
    ...
    if (!is_array($va_stat) || !isset($va_stat['mtime'])) {
        return false;
    }
...
{
    $va_return = array();
    array_walk_recursive($pa_array, function ($a) use (&$va_return) {
        $va_return[] = $a;
    });

    if (caGetOption('unique', $pa_options, false)) {
        $va_return = array_unique($va_return);
    }
    return $va_return;
}
# ----------------------------------------
...
function caLengthToFractions($pn_inches_as_float, $pn_denom, $pb_reduce = true, $pa_options=null)
{
    ...
    if (is_null($pa_allow_fractions_for)) {
        $pa_allow_fractions_for = $o_display_config->get('display_fractions_for');
    }
    if (!is_array($pa_allow_fractions_for)) {
        $pa_allow_fractions_for = [];
    }

    $pa_use_unicode_fraction_glyphs_for = caGetOption('useUnicodeFractionGlyphsFor', $pa_options, null);
    if (is_null($pa_use_unicode_fraction_glyphs_for)) {
        $pa_use_unicode_fraction_glyphs_for = $o_display_config->get('use_unicode_fraction_glyphs_for');
    }
    if (!is_array($pa_use_unicode_fraction_glyphs_for)) {
        $pa_use_unicode_fraction_glyphs_for = [];
    }

    $pb_use_unicode_fraction_glyphs = (is_array($pa_use_unicode_fraction_glyphs_for) && sizeof($pa_use_unicode_fraction_glyphs_for));
    if (is_null($pa_use_unicode_fraction_glyphs_for)) {
        $pb_use_unicode_fraction_glyphs = $o_config->get('use_unicode_fractions_for_measurements');
    }
...
function caSortableValue($ps_text, $pa_options=null)
{
    ...
    if (!$ps_locale) {
        return mb_substr($ps_text, 0, $max_length);
    }
...
function caGetPrimaryTables($pb_include_rel_tables=false, ?array $additional_tables=null)
{
    ...
    if (is_array($additional_tables) && sizeof($additional_tables)) {
        foreach ($additional_tables as $table) {
            if (!($t=Datamodel::getInstance($table, true))) {
                continue;
            }
    ...
    foreach ($pa_values as $vs_key => $vm_val) {
        if (is_array($vm_val)) {
            ...
            foreach ($vm_val as $vs_key2 => $vm_list_vals) {
                ...
                if (!is_array($vm_list_vals)) {
                    $vm_list_vals = [$vm_list_vals];
                }

                foreach ($vm_list_vals as $vm_list_val) {
                    if (!is_array($vm_list_val)) {
                        $vm_list_val = ['=', $vm_list_val];
                    }
...
function caExtractTagsFromTemplate($ps_template, $pa_options=null)
{
    ...
    foreach ($va_tags as $vn_i => $vs_tag) {
        if ((($p = strpos($vs_tag, "~")) !== false) && ($p < (mb_strlen($vs_tag) - 1))) {
            continue;
        }	// don't clip trailing characters when there's a tag directive specified (eg. a tilde that is not at the end of the tag)
...
function caSnakeToCamel($text)
{
    ...
    $vals = array_map(function ($v) {
        return preg_replace_callback("!_([A-Za-z0-9]{1})!", function ($m) {
...
function caTextToCamel($text)
{
    ...
    $vals = array_map(function ($v) {
        return trim(preg_replace_callback("! ([A-Za-z0-9]{1})!", function ($m) {
            return strtoupper($m[1]);
        }, strtolower($v)));
    }, $text);
...
function caCamelOrSnakeToText($text, ?array $options=null)
{
    ...
    $vals = array_map(function ($v) {
        ...
        if ($is_snake) {
            return trim(preg_replace_callback("!_([A-Za-z0-9]{1})!", function ($m) {
                return ' '.strtolower($m[1]);
            }, $v));
        } else {
            return trim(preg_replace_callback("!([A-Z]{1})!", function ($m) {
                return ' '.strtolower($m[1]);
            }, $v));
        }
    }, $text);
...
function caGenerateRandomPassword($length=6, $options=null)
{
    ...
    if (caGetOption('uppercase', $options, false)) {
        $pw = strtoupper($pw);
    }
    return $pw;
}
...
function caIdnoUseCount($idno, $table=null)
{
    ...
    if (!($idno_fld = $instance->getProperty('ID_NUMBERING_ID_FIELD'))) {
        return false;
    }
