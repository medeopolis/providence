...
function caGetExternalApplicationPath($ps_application_name, $options=null)
{
    ...
    foreach ($app_paths as $p) {
        if ($e = caGetOption('executableName', $options, false)) {
            $p .= "/{$e}";
        }
        if (file_exists($p)) {
            return $p;
...
{
    if (!caGetOption('noCache', $options, defined('__CA_DONT_CACHE_EXTERNAL_APPLICATION_PATHS__')) && CompositeCache::contains("mediahelper_imagemagick_installed", "mediaPluginInfo")) {
        return CompositeCache::fetch("mediahelper_imagemagick_installed", "mediaPluginInfo");
    }
    if (!$ps_imagemagick_path) {
        $ps_imagemagick_path = caGetExternalApplicationPath('imagemagick', ['executableName' => 'identify']);
    }
...
{
    if (!caGetOption('noCache', $options, defined('__CA_DONT_CACHE_EXTERNAL_APPLICATION_PATHS__')) && CompositeCache::contains("mediahelper_graphicsmagick_installed", "mediaPluginInfo")) {
        return CompositeCache::fetch("mediahelper_graphicsmagick_installed", "mediaPluginInfo");
    }
    if (!$ps_graphicsmagick_path) {
        $ps_graphicsmagick_path = caGetExternalApplicationPath('graphicsmagick');
    }
...
{
    if (!caGetOption('noCache', $options, defined('__CA_DONT_CACHE_EXTERNAL_APPLICATION_PATHS__')) && CompositeCache::contains("mediahelper_dcraw_installed", "mediaPluginInfo")) {
        return CompositeCache::fetch("mediahelper_dcraw_installed", "mediaPluginInfo");
    }
    if (!$ps_path_to_dcraw) {
        $ps_path_to_dcraw = caGetExternalApplicationPath('dcraw');
    }
...
{
    if (!caGetOption('noCache', $options, defined('__CA_DONT_CACHE_EXTERNAL_APPLICATION_PATHS__')) && CompositeCache::contains("mediahelper_ffmpeg_installed", "mediaPluginInfo")) {
        return CompositeCache::fetch("mediahelper_ffmpeg_installed", "mediaPluginInfo");
    }
    if (!$ps_path_to_ffmpeg) {
        $ps_path_to_ffmpeg = caGetExternalApplicationPath('ffmpeg');
    }
...
{
    if (!caGetOption('noCache', $options, defined('__CA_DONT_CACHE_EXTERNAL_APPLICATION_PATHS__')) && CompositeCache::contains("mediahelper_ghostscript_installed", "mediaPluginInfo")) {
        return CompositeCache::fetch("mediahelper_ghostscript_installed", "mediaPluginInfo");
    }
    if (!$ps_path_to_ghostscript) {
        $ps_path_to_ghostscript = caGetExternalApplicationPath('ghostscript');
    }
...
{
    if (!caGetOption('noCache', $options, defined('__CA_DONT_CACHE_EXTERNAL_APPLICATION_PATHS__')) && CompositeCache::contains("mediahelper_pdftotext_installed", "mediaPluginInfo")) {
        return CompositeCache::fetch("mediahelper_pdftotext_installed", "mediaPluginInfo");
    }
    if (!$ps_path_to_pdf_to_text) {
        $ps_path_to_pdf_to_text = caGetExternalApplicationPath('pdftotext');
    }
...
{
    if (!caGetOption('noCache', $options, defined('__CA_DONT_CACHE_EXTERNAL_APPLICATION_PATHS__')) && CompositeCache::contains("mediahelper_libreoffice_installed", "mediaPluginInfo")) {
        return CompositeCache::fetch("mediahelper_libreoffice_installed", "mediaPluginInfo");
    }
    if (!$ps_path_to_libreoffice) {
        $ps_path_to_libreoffice = caGetExternalApplicationPath('libreoffice');
    }
...
function caMediaPluginImagickInstalled($options=null)
{
    ...
    if ($o_config->get('dont_use_imagick')) {
        return false;
    }
...
{
    if (!caGetOption('noCache', $options, defined('__CA_DONT_CACHE_EXTERNAL_APPLICATION_PATHS__')) && CompositeCache::contains("mediahelper_mediainfo_installed", "mediaPluginInfo")) {
        return CompositeCache::fetch("mediahelper_mediainfo_installed", "mediaPluginInfo");
    }
...
{
    if (!caGetOption('noCache', $options, defined('__CA_DONT_CACHE_EXTERNAL_APPLICATION_PATHS__')) && CompositeCache::contains("mediahelper_meshlabserver_installed", "mediaPluginInfo")) {
        return CompositeCache::fetch("mediahelper_meshlabserver_installed", "mediaPluginInfo");
    }
    if (!$ps_meshlabserver_path) {
        $ps_meshlabserver_path = caGetExternalApplicationPath('meshlabserver');
    }
...
{
    if (!caGetOption('noCache', $options, defined('__CA_DONT_CACHE_EXTERNAL_APPLICATION_PATHS__')) && CompositeCache::contains("mediahelper_wkhtmltopdf_installed", "mediaPluginInfo")) {
        return CompositeCache::fetch("mediahelper_wkhtmltopdf_installed", "mediaPluginInfo");
    }
    if (!$ps_wkhtmltopdf_path) {
        $ps_wkhtmltopdf_path = caGetExternalApplicationPath('wkhtmltopdf');
    }
...
{
    if (!caGetOption('noCache', $options, defined('__CA_DONT_CACHE_EXTERNAL_APPLICATION_PATHS__')) && CompositeCache::contains("mediahelper_youtube_dl_installed", "mediaPluginInfo")) {
        return CompositeCache::fetch("mediahelper_youtube_dl_installed", "mediaPluginInfo");
    }
    if (!$youtube_dl_path) {
        $youtube_dl_path = caGetExternalApplicationPath('youtube-dl');
    }
...
{
    if (!caGetOption('noCache', $options, defined('__CA_DONT_CACHE_EXTERNAL_APPLICATION_PATHS__')) && CompositeCache::contains("mediahelper_exiftool_installed", "mediaPluginInfo")) {
        return CompositeCache::fetch("mediahelper_exiftool_installed", "mediaPluginInfo");
    }
    if (!$ps_exiftool_path) {
...
function caExtractMetadataWithExifTool($ps_filepath, $pb_skip_unknown=false)
{
    if ($vs_path_to_exif_tool = caExifToolInstalled()) {
        ...
        if ($vn_return == 0) {
            ...
            if (isset($va_data['GPS']['GPSLatitude'])) {
                $va_data['GPS']['GPSLatitude'] .= " ".substr($va_data['GPS']['GPSLatitudeRef'], 0, 1);
            }
            if (isset($va_data['GPS']['GPSLongitude'])) {
                $va_data['GPS']['GPSLongitude'] .= " ".substr($va_data['GPS']['GPSLongitudeRef'], 0, 1);
            }
...
function caExtractEmbeddedMetadata($po_instance, $pa_metadata, $pn_locale_id)
{
    ...
    if (!($vs_media_metadata_config = __CA_CONF_DIR__.'/media_metadata.conf')) {
        return false;
    }
...
function caExportMediaMetadataForRecord($ps_table, $ps_type_code, $pn_id)
{
    ...
    if (!($vs_media_metadata_config = $o_app_config->get('media_metadata'))) {
        return false;
    }
    $o_metadata_config = Configuration::load($vs_media_metadata_config);

    $va_mappings = $o_metadata_config->getAssoc('export_mappings');
    if (!isset($va_mappings[$ps_table])) {
        return false;
    }
...
function caParseMediaIdentifier($ps_identifier, $pa_options=null)
{
    ...
    switch($vs_type = strtolower($va_tmp[0])) {
        case 'representation':
            ...
            if (!($t_rep = ca_object_representations::find((int)$va_tmp[1], $pa_options))) {
                return null;
            } // ca_object_representations::find() performs checkAccess
        ...
        case 'attribute':
            ...
            if (!($t_subject = $vs_table_name::find($vn_subject_id, $pa_options))) {
                return null;
            } // table::find() performs checkAccess
