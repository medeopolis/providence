...
{
    $va_locale_paths = [];
    if (file_exists($vs_locale_path = realpath(__CA_THEME_DIR__.'/locale/'.$ps_locale.'/messages.mo'))) {
        $va_locale_paths[] = $vs_locale_path;
    }
    if (file_exists($vs_locale_path = realpath(__CA_THEMES_DIR__.'/default/locale/'.$ps_locale.'/messages.mo'))) {
        $va_locale_paths[] = $vs_locale_path;
    }
    if (file_exists($vs_locale_path = realpath(__CA_APP_DIR__.'/locale/user/'.$ps_locale.'/messages.mo'))) {
        $va_locale_paths[] = $vs_locale_path;
    }
    if (file_exists($vs_locale_path = realpath(__CA_APP_DIR__.'/locale/'.$ps_locale.'/messages.mo'))) {
        $va_locale_paths[] = $vs_locale_path;
    }
...
function initializeLocale($ps_locale)
{
    ...
    if (CookieOptionsManager::allow('performance')) {
        ...
        if (!headers_sent()) {
            setcookie('CA_'.__CA_APP_NAME__.'_ui_locale', $ps_locale, time() + Session::lifetime(), $cookiepath, null, $secure);
        }
...
function caGetArticlesForLocale($ps_locale, $pa_options=null)
{
    if (sizeof($va_tmp = explode('_', $ps_locale)) == 1) {
        $va_locales = array_map(function ($v) {
            return pathinfo($v, PATHINFO_BASENAME);
        }, caGetDirectoryContentsAsList(__CA_LIB_DIR__."/Parsers/TimeExpressionParser", false));
        $va_locales = array_filter($va_locales, function ($v) use ($ps_locale) {
            return preg_match("!^{$ps_locale}_!", $v);
        });
        if (sizeof($va_locales) > 0) {
            $ps_locale = str_replace(".lang", "", array_shift($va_locales));
        } else {
            return null;
        }
    }

    if (!file_exists($vs_path = __CA_LIB_DIR__."/Parsers/TimeExpressionParser/{$ps_locale}.lang")) {
        return null;
    }
