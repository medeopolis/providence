...
function caHTMLSelect($ps_name, $pa_content, $pa_attributes=null, $pa_options=null)
{
    ...
    if (isset($pa_options['contentArrayUsesKeysForValues']) && $pa_options['contentArrayUsesKeysForValues']) {
        foreach ($pa_content as $vs_val => $vs_opt) {
            if ($vb_use_options_for_values) {
                $vs_val = preg_replace("!^[\s]+!", "", preg_replace("![\s]+$!", "", str_replace("&nbsp;", "", $vs_opt)));
            }
            if ($COLOR = ($vs_color = $va_colors[$vs_val]) ? " data-color='#{$vs_color}'" : '') {
                $vb_uses_color = true;
            }
...
function caHTMLTextInput($ps_name, $pa_attributes=null, $pa_options=null)
{
    ...
    if ($vb_use_wysiwyg_editor) {
        ...
        if (!is_array($va_toolbar_config = $o_config->getAssoc(caGetOption('cktoolbar', $pa_options, 'wysiwyg_editor_toolbar')))) {
            $va_toolbar_config = [];
        }
...
{
    if (caGetOption('checked', $pa_options, false)) {
        $pa_attributes['checked'] = 1;
    }
    if (caGetOption('disabled', $pa_options, false)) {
...
{
    if (array_key_exists('checked', $pa_attributes) && !$pa_attributes['checked']) {
...
function caHTMLImage($ps_url, $pa_options=null)
{
    ...
    foreach ($pa_options as $vs_k => $vs_v) {
        if (substr($vs_k, 0, 5) == 'data-') {
            $va_attributes[$vs_k] = $vs_v;
        }
    }
    ...
    if (preg_match("/\.tpc\$/", $ps_url)) {
        ...
        if (!is_array($va_viewer_opts_from_app_config = $o_config->getAssoc('image_viewer_options'))) {
            $va_viewer_opts_from_app_config = array();
        }
...
function _caHTMLMakeAttributeString($pa_attributes, $pa_options=null)
{
    ...
    if (is_array($pa_attributes)) {
        foreach ($pa_attributes as $vs_attr => $vs_attr_val) {
            if (is_array($vs_attr_val)) {
                $vs_attr_val = join(" ", $vs_attr_val);
            }
            if (is_object($vs_attr_val)) {
                continue;
            }
