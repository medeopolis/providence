...
function caGetListID($ps_list, $pa_options=null)
{
    ...
    if (isset($g_list_id_cache[$vs_cache_key])) {
        return $g_list_id_cache[$vs_cache_key];
    }
    $t_list = new ca_lists();
...
function caGetListItemID($ps_list_code, $ps_idno, $pa_options=null)
{
    ...
    if (!caGetOption(['noCache', 'dontCache'], $pa_options, false)) {
        if (isset($g_list_item_id_cache[$vs_cache_key])) {
            return $g_list_item_id_cache[$vs_cache_key];
        }
    }
    $t_list = new ca_lists();
    if ($o_trans = caGetOption('transaction', $pa_options, null)) {
...
function caGetListCode($pn_list_id, $pa_options=null)
{
    ...
    if (!caGetOption(['noCache', 'dontCache'], $pa_options, false)) {
        if (isset($g_ca_get_list_code_cache[$vs_cache_key])) {
            return $g_ca_get_list_code_cache[$vs_cache_key];
        }
    }
...
function caGetListName($pm_list_code_or_id, $pa_options=null)
{
    ...
    if (!caGetOption(['noCache', 'dontCache'], $pa_options, false)) {
        if (isset($g_ca_get_list_name_cache[$vs_cache_key])) {
            return $g_ca_get_list_name_cache[$vs_cache_key];
        }
    }
...
function caGetListItemIdno($pn_item_id, $pa_options=null)
{
    ...
    if (!caGetOption(['noCache', 'dontCache'], $pa_options, false)) {
        if (isset($g_list_item_idno_cache[$vs_cache_key])) {
            return $g_list_item_idno_cache[$vs_cache_key];
        }
    }
...
function caGetListItemForDisplay($ps_list_code, $ps_idno, $pa_options=null)
{
    ...
    if (!caGetOption(['noCache', 'dontCache'], $pa_options, false)) {
        if (isset($g_list_item_label_cache[$vs_cache_key])) {
            return $g_list_item_label_cache[$vs_cache_key];
        }
    }
...
function caGetListItemForValue($ps_list_code, $ps_value, $pa_options=null)
{
    ...
    if (!caGetOption(['noCache', 'dontCache'], $pa_options, false)) {
        if (isset($g_list_items_for_values[$vs_cache_key])) {
            return $g_list_items_for_values[$vs_cache_key];
        }
    }
...
function caGetListItemValueForID($pn_id, $pa_options=null)
{
    ...
    if (!$pn_id || !is_numeric($pn_id)) {
        return null;
    }
    $vs_cache_key = caMakeCacheKeyFromOptions($pa_options, $pn_id);

    if (!caGetOption(['noCache', 'dontCache'], $pa_options, false)) {
        if (isset($g_list_item_values_for_ids[$vs_cache_key])) {
            return $g_list_item_values_for_ids[$vs_cache_key];
        }
    }
...
function caGetListItemIDForLabel($ps_list_code, $ps_label, $pa_options=null)
{
    ...
    if (!caGetOption(['noCache', 'dontCache'], $pa_options, false)) {
        if (isset($g_list_item_id_for_label_cache[$vs_cache_key])) {
            return $g_list_item_id_for_label_cache[$vs_cache_key];
        }
    }
...
function caGetListItems($ps_list_code, $pa_options=null)
{
    ...
    if (!caGetOption(['noCache', 'dontCache'], $pa_options, false)) {
        if (isset($g_list_items_cache[$vs_cache_key])) {
            return $g_list_items_cache[$vs_cache_key];
        }
    }
    ...
    foreach ($list_items as $id => $item) {
        if ($default_only && !$item['is_default']) {
            continue;
        }
...
function caGetDefaultItemID($ps_list_code, $pa_options=null)
{
    ...
    if (!caGetOption(['noCache', 'dontCache'], $pa_options, false)) {
        if (isset($g_default_list_item_id_cache[$vs_cache_key])) {
            return $g_default_list_item_id_cache[$vs_cache_key];
        }
    }
...
function caGetListItemSettingValue(string $list_code, string $idno, string $setting, ?array $options=null)
{
    ...
    if (!is_array($g_default_list_item_setting_cache)) {
        $g_default_list_item_setting_cache = [];
    }
    $cache_key = caMakeCacheKeyFromOptions($options, $list_code.'/'.$idno.'/'.$setting);

    if (!caGetOption(['noCache', 'dontCache'], $options, false)) {
        if (array_key_exists($cache_key, $g_default_list_item_setting_cache)) {
            return $g_default_list_item_setting_cache[$cache_key];
        }
    }
    ...
    $t_item = new ca_list_items();
    if ($o_trans = caGetOption('transaction', $options, null)) {
        $t_item->setTransaction($o_trans);
    }
...
function caGetAncestorsForItemID($pm_item_id, $pa_options=null)
{
    ...
    if (!caGetOption(['noCache', 'dontCache'], $pa_options, false)) {
        if (isset($g_list_item_id_ancestors_cache[$vs_cache_key])) {
            return $g_list_item_id_ancestors_cache[$vs_cache_key];
        }
    }
    ...
    if (!is_array($pm_item_id)) {
        $pm_item_id = [$pm_item_id];
    }
