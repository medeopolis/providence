...
function caGetTypeRestrictionsForUser($table_name_or_num, $options=null)
{
    ...
    if (isset($g_access_helpers_type_restriction_cache[$cache_key])) {
        return $g_access_helpers_type_restriction_cache[$cache_key];
    }
...
function caGetSourceRestrictionsForUser($pm_table_name_or_num, $pa_options=null)
{
    ...
    if (isset($g_access_helpers_source_restriction_cache[$vs_cache_key])) {
        return $g_access_helpers_source_restriction_cache[$vs_cache_key];
    }
    $o_config = Configuration::load();
...
function caMakeTypeIDList($pm_table_name_or_num, $pa_types, $pa_options=null)
{
    ...
    if (is_a($t_instance, 'BaseLabel')) {
        $t_instance = $t_instance->getSubjectTableInstance();
    }
    if (!($vs_type_list_code = $t_instance->getTypeListCode())) {
        return null;
    }	// table doesn't use types
...
function caValidateTypeList($table_name_or_num, $types, ?array $options=null)
{
    ...
    if (!($type_list_code = $t_instance->getTypeListCode())) {
        return null;
    }	// table doesn't use types

    $t_list = new ca_lists();
    if (!is_array($idnos_in_list = $t_list->getItemsForList($type_list_code, ['idnosOnly' => true]))) {
        return null;
    }
    if (!is_array($ids_in_list = $t_list->getItemsForList($type_list_code, ['idsOnly' => true]))) {
        return null;
    }
...
{
    if (!is_array($pa_item_idnos)) {
        $pa_item_idnos = strlen($pa_item_idnos) ? [$pa_item_idnos] : [];
    }
    if (!is_array($pa_options)) {
        $pa_options = [];
    }
    ...
    if (!is_array($va_item_ids_in_list = $t_list->getItemsForList($pm_list_code_or_id, ['idsOnly' => true]))) {
        $va_item_ids_in_list = [];
    }
    ...
    foreach ($pa_item_idnos as $vm_item) {
        ...
        if (!in_array($vn_type_id, $va_item_ids_in_list)) {
            continue;
        }	// skip type_id if it's not in list $pm_list_code_or_id
...
{
    if (is_array($pa_type_ids) && !sizeof($pa_type_ids)) {
        return array();
    }
    if (!is_array($pa_type_ids)) {
        $pa_type_ids = strlen($pa_type_ids) ? [$pa_type_ids] : [];
    }
...
function caMakeSourceIDList($pm_table_name_or_num, $pa_sources, $pa_options=null)
{
    ...
    if (!is_array($pa_sources)) {
        $pa_sources = strlen($pa_sources) ? [$pa_sources] : [];
    }
...
function caMakeRelationshipTypeCodeList($table_name_or_num, $type_ids, $options=null)
{
    ...
    if (isset($options['dontIncludeSubtypesInTypeRestriction']) && (!isset($options['dont_include_subtypes_in_type_restriction']) || !$options['dont_include_subtypes_in_type_restriction'])) {
        $options['dont_include_subtypes_in_type_restriction'] = $options['dontIncludeSubtypesInTypeRestriction'];
    }
...
function caGetBundleAccessLevel($ps_table_name, $ps_bundle_name)
{
    ...
    if (isset($g_bundle_access_level_cache[$ps_table_name][$ps_bundle_name])) {
        return $g_bundle_access_level_cache[$ps_table_name][$ps_bundle_name];
    }
...
function caGetTypeAccessLevel($ps_table_name, $pm_type_code_or_id)
{
    ...
    if (isset($g_type_access_level_cache[$ps_table_name][$pm_type_code_or_id])) {
        return $g_type_access_level_cache[$ps_table_name][$pm_type_code_or_id];
    }
    if ($g_request) {
...
function caGetSourceAccessLevel($ps_table_name, $pm_source_code_or_id)
{
    ...
    if (isset($g_source_access_level_cache[$ps_table_name][$pm_source_code_or_id])) {
        return $g_source_access_level_cache[$ps_table_name][$pm_source_code_or_id];
    }
    if ($g_request) {
...
function caCanRead($pn_user_id, $pm_table, $pm_id, $ps_bundle_name=null, $pa_options=null)
{
    ...
    if (!($t_instance = Datamodel::getInstanceByTableName($ps_table_name, true))) {
        return null;
    }
