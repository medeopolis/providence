...
function caGetThemeGraphic($po_request, $ps_file_path, $pa_attributes=null, $pa_options=null)
{
    ...
    if ($o_config->get('allowThemeInheritance')) {
        ...
        while ($vs_inherit_from_theme = trim(trim($o_config->get(['inheritFrom', 'inherit_from'])), "/")) {
            ...
            if (!file_exists(__CA_THEMES_DIR__."/{$vs_inherit_from_theme}/conf/app.conf")) {
                break;
            }
...
function caSetPageCSSClasses($pa_page_classes)
{
    ...
    if (!is_array($pa_page_classes) && $pa_page_classes) {
        $pa_page_classes = array($pa_page_classes);
    }
    if (!is_array($pa_page_classes)) {
        $pa_page_classes = array();
    }
...
function caObjectRepresentationThumbnails($po_request, $pn_representation_id, $pt_object, $pa_options)
{
    ...
    if ($show_only_media_types_when_present) {
        $mimetypes_present = array_reduce($va_reps, function ($c, $i) {
            $c[$i['mimetype']] = true;
            return $c;
        }, []);
    ...
    switch($ps_return_as) {
        ...
        case "list":
            ...
            foreach ($va_links as $vn_rep_id => $vs_link) {
                if ($vs_link) {
                    $vs_formatted_thumbs .= "<li id='detailRepresentationThumbnail{$vn_rep_id}'".(($vn_rep_id == $pn_representation_id) ? " class='{$ps_current_rep_class}'" : "").">{$vs_link}</li>\n";
                }
            }
        ...
        case "bsCols":
            ...
            foreach ($va_links as $vn_rep_id => $vs_link) {
                if ($vs_link) {
                    $vs_formatted_thumbs .= "<div id='detailRepresentationThumbnail{$vn_rep_id}' class='{$ps_bs_col_classes}".(($vn_rep_id == $pn_representation_id) ? " {$ps_current_rep_class}" : "")."'>{$vs_link}</div>\n";
                }
            }
    ...
    if (is_array($va_comments) && (sizeof($va_comments) > 0)) {
        foreach ($va_comments as $va_comment) {
            ...
            if ($va_comment["media1"]) {
                ...
                TooltipManager::add(
                    "#commentMedia".$va_comment["comment_id"],
                    $va_comment["media1"]["large_preview"]["TAG"]
                );
            }
...
function caGetDetailForType($pm_table, $pm_type=null, $pa_options=null)
{
    ...
    if (!($vs_table = Datamodel::getTableName($pm_table))) {
        return null;
    }
...
{
    if (!($t_instance = Datamodel::getInstanceByTableName($pm_table, true))) {
        return null;
    }
...
function caSetAdvancedSearchFormInView($po_view, $ps_function, $ps_view, $pa_options=null)
{
    ...
    if (!($pt_subject = Datamodel::getInstanceByTableName($va_search_info['table'], true))) {
        return null;
    }
    ...
    if (!is_array($pa_tags) || !sizeof($pa_tags)) {
        return null;
    }
        ...
        switch(strtolower($vs_tag_proc)) {
            ...
            default:
                if (preg_match("!^(.*):label$!", $vs_tag_proc, $va_matches)) {
                    ...
                    if ($vs_rel_types = join(";", caGetOption('restrictToRelationshipTypes', $va_opts, array()))) {
                    ...
                    if ($vs_tag_val = $pt_subject->htmlFormElementForSearch($po_request, $vs_tag_proc, $va_opts)) {
                        switch(strtolower($vs_tag_proc)) {
                            case '_fulltext':		// Set default label for _fulltext if needed
                                if (!isset($va_opts['label'])) {
                                    $va_opts['label'] = _t('Keywords');
                                }
                                break;
                        }
                ...
                if ($vs_tag_val) {
                    $va_form_elements[] = $vs_tag_proc.$vs_rel_types;
                }		// add relationship types to field name
                break;
        }
    }
...
function caGetAdvancedSearchFormAutocompleteJS($po_request, $ps_field, $pt_instance, $pa_options=null)
{
    ...
    if ($vs_rel_types = join("_", caGetOption(['restrictToRelationshipTypes', 'relationshipType'], $pa_options, []))) {
    ...
    if (!is_array($va_json_lookup_info = caJSONLookupServiceUrl($po_request, $pt_instance->tableName()))) {
        return null;
    }
...
{
    if (!($ps_view = caGetOption('view', $pa_options, null)) && ($o_request = caGetOption('request', $pa_options, null))) {
        $ps_view = $o_request->getParameter('view', pString);
    }
...
{
    if (!is_array($va_comparison_list = Session::getVar("{$ps_table}_comparison_list"))) {
        $va_comparison_list = [];
    }

    // Get title template from config
    $va_compare_config = $po_request->config->get('compare_images');
    if (!is_array($va_compare_config = $va_compare_config[$ps_table])) {
        $va_compare_config = [];
    }
...
function caGetBrowseLinks($t_instance, string $bundle, ?array $options=null): ?array
{
    ...
    if ($restrict_to_types && !is_array($restrict_to_types)) {
    ...
    if ($restrict_to_relationship_types && !is_array($restrict_to_relationship_types)) {
    ...
    if ($facet) {
        ...
        if (!sizeof(array_filter($text, 'strlen'))) {
            return null;
        }
        ...
        $links = caCreateBrowseLinksFromText($text, $bt, array_map(function ($v) use ($facet) {
            return ['facet' => $facet, 'id' => $v];
        }, $ids), '', []);
...
function caGetSearchLinks($t_instance, string $bundle, ?array $options=null): ?array
{
    ...
    switch($bundle_type) {
        case 'related_table':
            if ($template) {
                $template = "<unit relativeTo='{$bundle}' delimiter='|'>{$template}</unit>";
            }
            break;
    }
    ...
    $links =  caCreateSearchLinksFromText($text, $st, array_map(function ($s) use ($bundle) {
        return "{$bundle}:\"{$s}\"";
    }, $values), '', []);
...
{
    return caCreateNavigationLinksFromText($text, '', 'Search', $search_type, array_map(function ($s) {
        return ['search' => $s];
    }, $searches), $class, $options);
}
# ------------------------------------------------------------------
/**
