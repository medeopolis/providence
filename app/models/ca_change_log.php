...
class ca_change_log extends BaseModel
{
                    ...
                    switch($vs_fld) {
                        ...
                        case 'type_id':
                            if (preg_match("!^ca_relationship_type!", $t_instance->tableName())) {
                                ...
                                if ($t_instance instanceof BaseRelationshipModel) {
                                    if (!($va_snapshot['type_code'] = caGetRelationshipTypeCode($vm_val))) {
                                        $va_snapshot = ['SKIP' => true];
                                        continue(2);
                                    }
                                } elseif ($t_instance instanceof BaseModel) {
                                    if (!($va_snapshot['type_code'] = caGetListItemIdno($vm_val)) && (!$t_instance->getFieldInfo('type_id', 'IS_NULL'))) {
                                        continue(2);
                                    }
                        ...
                        default:
                            ...
                            if (!is_null($vm_val) && ($va_fld_info = $t_instance->getFieldInfo($vs_fld))) {
                                ...
                                if (!isset($va_snapshot[$vs_fld . '_guid'])) {
     ...
     *
     */
    public static function logEntryExists($pn_log_id)
    {
        $o_db = new Db();

        $qr_res = $o_db->query("SELECT log_id FROM ca_change_log WHERE log_id = ?", [(int)$pn_log_id]);
        if ($qr_res->nextRow() && $qr_res->get('log_id')) {
            return true;
        }
        return false;
    }
     ...
     *
     */
    public static function logEntryHasAccess($pa_log_entry, $pa_access)
    {
        $o_db = new Db();

        if (!($t_instance = Datamodel::getInstanceByTableNum($pa_log_entry['logged_table_num'], true))) {
            return false;
        }
        if ($t_instance->hasField('access')) {
            ...
            if ($qr_res->nextRow()) {
                if (in_array($vn_access = $qr_res->get('access'), $pa_access)) {
                    return true;
                }
            }
        }
        ...
        if (is_array($pa_log_entry['subjects'])) {
            foreach ($pa_log_entry['subjects'] as $va_subject) {
                if (!($t_instance = Datamodel::getInstanceByTableNum($va_subject['subject_table_num'], true))) {
                    continue;
                }
                if ($t_instance->hasField('access')) {
                    $qr_res = $o_db->query("SELECT access FROM ".$t_instance->tableName()." WHERE ".$t_instance->primaryKey()." = ?", [(int)$va_subject['subject_row_id']]);
                    if ($qr_res->nextRow()) {
                        if (in_array($qr_res->get('access'), $pa_access)) {
                            return true;
                        }
                    }
                }
     ...
     *
     */
    public static function rowHasAccess($pn_table_num, $pn_row_id, $pa_access)
    {
        $o_db = new Db();
