...
class ca_metadata_alert_triggers extends BaseModel
{
    ...
    public static function fireApplicableTriggers(&$t_subject, int $pn_type = __CA_MD_ALERT_CHECK_TYPE_SAVE__, ?array $additional_data=null)
    {
        ...
        if (!is_array($va_triggers) || !sizeof($va_triggers)) {
            return;
        }
        ...
        foreach ($va_triggers as $va_trigger) {
            ...
            if ($o_trigger->getTriggerType() != $pn_type) {
                continue;
            }
    ...
    public static function fireTrigger($po_trigger, &$t_subject, array $pa_trigger, ?array $additional_data=null)
    {
        ...
        if ($po_trigger->check($t_subject)) {
            if (!$t_rule->load($pa_trigger['rule_id'])) {
                return false;
            }
            ...
            if (is_array($va_users)) {
                foreach ($va_users as $va_user) {
                    if ($va_user['access'] >= __CA_ALERT_RULE_ACCESS_NOTIFICATION__) {
                        ...
                        if ($t_user->notificationExists(__CA_NOTIFICATION_TYPE_METADATA_ALERT__, $vs_notification_key)) {
                            continue;
                        }
    ...
    private static function getApplicableTriggersForInstance($t_subject=null)
    {
        ...
        if (!is_array($va_rules) || !sizeof($va_rules)) {
            return;
        }
        ...
        foreach ($va_rules as $t_rule) {
            ...
            if (is_array($va_restrictions) && sizeof($va_restrictions)) {
                ...
                if (is_object($t_subject) && !in_array($t_subject->getTypeID(), $va_type_ids)) {
                    continue;
                }
            }
    ...
    public static function firePeriodicTriggers()
    {
        foreach (self::tablesWithRules() as $vn_table_num => $vs_table) {
            ...
            if (!is_array($va_triggers)) {
                continue;
            }

            foreach ($va_triggers as $va_trigger) {
                $o_trigger = CA\MetadataAlerts\TriggerTypes\Base::getInstance($va_trigger['info']['trigger_type'], $va_trigger['info']);
                if ($o_trigger->getTriggerType() != __CA_MD_ALERT_CHECK_TYPE_PERIODIC__) {
                    continue;
                }	// only periodic triggers here
