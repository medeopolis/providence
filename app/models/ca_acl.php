...
if (!defined('__CA_ACL_NO_ACCESS__')) {
    define('__CA_ACL_NO_ACCESS__', 0);
}
if (!defined('__CA_ACL_READONLY_ACCESS__')) {
    define('__CA_ACL_READONLY_ACCESS__', 1);
}
if (!defined('__CA_ACL_EDIT_ACCESS__')) {
    define('__CA_ACL_EDIT_ACCESS__', 2);
}
if (!defined('__CA_ACL_EDIT_DELETE_ACCESS__')) {
...
class ca_acl extends BaseModel
{
    ...
    public static $s_acl_access_value_cache = array();
    ...
    public static function applyACLInheritanceToRelatedFromRow($t_subject, $ps_target)
    {
        ...
        if ($t_link = $t_subject->getRelationshipInstance($ps_target)) {
            if ($t_rel_item = Datamodel::getInstanceByTableName($ps_target, false)) {
                ...
                if (!$t_rel_item->hasField("acl_inherit_from_{$vs_table}")) {
                    return false;
                }
        ...
        if ($t_link = $t_subject->getRelationshipInstance($ps_target)) {
            if ($t_rel_item = Datamodel::getInstanceByTableName($ps_target, false)) {
                ...
                if (!isset($pa_options['deleteACLOnly']) || !$pa_options['deleteACLOnly']) {
                    if (!$t_rel_item->hasField("acl_inherit_from_{$vs_subject}")) {
                        return false;
                    }
                }
                ...
                if (!isset($pa_options['deleteACLOnly']) || !$pa_options['deleteACLOnly']) {
                    ...
                    if (!$qr_inherit->nextRow()) {
                        return false;
                    }
                    if (!$qr_inherit->get("acl_inherit_from_{$vs_subject}")) {
                        return false;
                    }
