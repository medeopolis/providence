...
class ca_guids extends BaseModel
{
    ...
    public static function getAccessForGUID($ps_guid, $pa_access, $pa_options=null)
    {
        ...
        if (!($va_info = self::getInfoForGUID($ps_guid))) {
            return null;
        }
        ...
        if (in_array(Datamodel::getTableName($va_info['table_num']), ['ca_lists', 'ca_list_items', 'ca_list_labels', 'ca_list_item_labels', 'ca_object_lots', 'ca_object_lot_labels'])) {   //TODO: make tables for which we should ignore access configurable
            ...
            if ($t_rel = Datamodel::getInstanceByTableNum($va_info['table_num'], true)) {
                ...
                if ($t_left->load($t_rel->get($t_rel->getLeftTableFieldName()))) {
                    if ($t_left->hasField('deleted')) {
                        if ((int)$t_left->get('deleted') !== 0) {
                            return false;
                        }
                    }
                    if ($t_left->hasField('access')) {
                        if (!in_array((int)$t_left->get('access'), $pa_access, true)) {
                            return false;
                        }
                    }
                    $vb_left = true;
                ...
                if ($t_right->load($t_rel->get($t_rel->getRightTableFieldName()))) {
                    if ($t_right->hasField('deleted')) {
                        if ((int)$t_right->get('deleted') !== 0) {
                            return false;
                        }
                    }
                    if ($t_right->hasField('access')) {
                        if (!in_array((int)$t_right->get('access'), $pa_access, true)) {
                            return false;
                        }
                    }
                    $vb_right = true;
                }
                if (is_null($vb_left) && is_null($vb_right)) {
                    return null;
                }
                return true;
            }
            ...
            if (in_array(Datamodel::getTableName($vn_table_num), ['ca_object_lots', 'ca_object_lot_labels', 'ca_lists', 'ca_list_items', 'ca_list_labels', 'ca_list_item_labels'])) {
                return true;
            }

            if (!Datamodel::getFieldInfo($vn_table_num, 'access')) {
                return false;
            }        // TODO: support attributes on non-acess control tables (eg. config tables; interstitial attributes on relationships)
            $qr_guid = $o_db->query('
            ...
            if (!Datamodel::getFieldInfo($va_info['table_num'], 'access')) {
                return null;
            }
            $qr_guid = $o_db->query('
