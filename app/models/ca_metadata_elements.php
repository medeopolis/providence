...
class ca_metadata_elements extends LabelableBaseModelWithAttributes implements ITakesSettings
{
    ...
    public function insert($pa_options=null)
    {
        ...
        if ($vn_rc =  parent::insert($pa_options)) {
            if (!caGetOption('noFlush', $pa_options, false)) {
     ...
     */
    public static function getElementsForSet($pn_element_id, $pa_options=null)
    {
        if (!is_numeric($pn_element_id)) {
            $pn_element_id = self::getElementID($pn_element_id);
        }
    ...
    {
        if (!isset($pa_element_set[$pn_parent_id]) || !$pa_element_set[$pn_parent_id]) {
            return array();
        }
    ...
    public static function getElementSetsWithSetting($ps_setting, $pm_value=null, $pa_options=null)
    {
        return array_map(function ($v) {
            $v['settings'] = caUnserializeForDatabase($v['settings']);
            return $v;
        }, array_filter(ca_metadata_elements::find('*', ['returnAs' => 'arrays']), function ($v) use ($ps_setting, $pm_value) {
        ...
        switch($va_properties['displayType']) {
            ...
            case DT_SELECT:
                ...
                if ($va_properties['showMediaElementBundles']) {
                    if (!is_array($va_restrictions = $this->getTypeRestrictions())) {
                        $va_restrictions = [];
                    }
    ...
    public static function getElementListAsHTMLSelect($ps_select_name, array $pa_attributes=[], array $pa_options=[])
    {
        ...
        if (is_array($pa_add_items)) {
            $va_list = array_merge($va_list, $pa_add_items);
        }
        ...
        foreach ($va_elements as $va_element) {
            ...
            if (is_array($pa_restrict_to_datatypes) && !in_array($va_element['datatype'], $pa_restrict_to_datatypes)) {
                continue;
            }
    ...
    public static function getElementsAsList($pb_root_elements_only=false, $pm_table_name_or_num=null, $pm_type_name_or_id=null, $pb_use_cache=true, $pb_return_stats=false, $pb_index_by_element_code=false, $pa_data_types=null, $options=null)
    {
        ...
        if (!is_array($va_element_list_cache_keys)) {
            $va_element_list_cache_keys = array();
        }
    ...
    public static function getSortableElements($pm_table_name_or_num, $pm_type_name_or_id=null, $options=null)
    {
        ...
        foreach ($elements as $vn_id => $element) {
            if (!isset($element['settings']['canBeUsedInSort'])) {
                $element['settings']['canBeUsedInSort'] = true;
            }
                ...
                if ((int)$element['datatype'] === 0) {
                    ...
                    if (is_array($sub_elements = $t_element->getElementsInSet($vn_id, !$no_cache, $options))) {
                        ...
                        foreach ($sub_elements as $sub_element) {
                            if (!$sub_element['parent_id']) {
                                continue;
                            }
                            if ((int)$sub_element['datatype'] === 0) {
                                continue;
                            }
    ...
    public static function getDataTypeForElementCode($ps_element_code)
    {
        if (is_numeric($ps_element_code)) {
            $ps_element_code = self::getElementCodeForId($ps_element_code);
        }
    ...
    public static function getUIUsageCounts($pm_element_code_or_id=null)
    {
        ...
        while ($qr_use_counts->nextRow()) {
            ...
            if (preg_match("!^($vs_table\.|ca_attribute_)([A-Za-z0-9_\\-]+)\$!", $qr_use_counts->get('bundle_name'), $va_matches)) {
                if (!($t_table = Datamodel::getInstanceByTableNum($vn_table_num, true))) {
                    continue;
                }
    ...
    public static function getTypeRestrictionsAsList($pm_element_code_or_id=null)
    {
        ...
        while ($qr_restrictions->nextRow()) {
            if (!($t_table = Datamodel::getInstanceByTableNum($qr_restrictions->get('table_num'), true))) {
                continue;
            }
    ...
    public static function getElementDatatype($pm_element_code_or_id)
    {
        ...
        if (is_numeric($pm_element_code_or_id)) {
            $pm_element_code_or_id = (int) $pm_element_code_or_id;
        }
     ...
     * @throws MemoryCacheInvalidParameterException
     */
    public static function isAuthorityDatatype($pm_element_code_or_id)
    {
     ...
     * @throws MemoryCacheInvalidParameterException
     */
    public static function getElementCodeForId($pm_element_id)
    {
        if (!$pm_element_id) {
            return null;
        }
        if (is_numeric($pm_element_id)) {
            $pm_element_id = (int) $pm_element_id;
        }
        ...
        $vm_return = null;
        if (!$t_element = self::getInstance($pm_element_id)) {
            return null;
        }
     ...
     * @throws MemoryCacheInvalidParameterException
     */
    public static function getElementLabel($pm_element_id)
    {
        if (!$pm_element_id) {
            return null;
        }
     ...
     * @throws MemoryCacheInvalidParameterException
     */
    public static function getParentCode($pm_element_id)
    {
        if (!$pm_element_id) {
            return null;
        }
     ...
     * @throws MemoryCacheInvalidParameterException
     */
    public static function getElementID($pm_element_code_or_id, $pa_options=null)
    {
        if (!$pm_element_code_or_id) {
            return null;
        }
     ...
     * @throws MemoryCacheInvalidParameterException
     */
    public static function getElementHierarchyID($pm_element_code_or_id)
    {
        if (!$pm_element_code_or_id) {
            return null;
        }
     ...
     * @throws MemoryCacheInvalidParameterException
     */
    public static function getElementListID($pm_element_code_or_id)
    {
        if (!$pm_element_code_or_id) {
            return null;
        }
        ...
        $vm_return = null;
        if (!($t_element = self::getInstance($pm_element_code_or_id))) {
            return null;
        }
     ...
     * @throws MemoryCacheInvalidParameterException
     */
    public static function getElementSettingsForId($pm_element_id)
    {
        if (!$pm_element_id) {
            return null;
        }
     ...
     * @throws MemoryCacheInvalidParameterException
     */
    public static function getElementDefaultValue($pm_element_code_or_id)
    {
        if (!$pm_element_code_or_id) {
            return null;
        }
