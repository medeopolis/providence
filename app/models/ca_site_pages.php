...
class ca_site_pages extends BundlableLabelableBaseModelWithAttributes
{
    ...
    public function getHTMLFormElements($options=null)
    {
        ...
        if (!($vn_template_id = $this->get('template_id'))) {
            return null;
        }

        if (!is_array($va_page_content = $this->get('content'))) {
            $va_page_content = [];
        }
    ...
    public static function renderPageForPath($po_controller, $ps_path, $options=null)
    {
        ...
        if (
            ...
            if (sizeof($va_media_to_render) > 0) {
                $va_media_list = array_values($t_page->getPageMedia(array_unique(array_map(function ($v) {
                    return $v['version'];
                }, $va_media_to_render))));

                if (!is_array($va_access_values = caGetUserAccessValues($po_controller->request)) || !sizeof($va_access_values)) {
                ...
                foreach ($va_media_to_render as $va_media) {
                    ...
                    if ($vn_index > sizeof($va_media_list) - 1) {
                        $vn_index = sizeof($va_media_list) - 1;
                    }

                    if (!isset($va_media_list[$vn_index])) {
                        continue;
                    }
                    if (is_array($va_access_values) && !in_array($va_media_list[$vn_index]['access'], $va_access_values)) {
                        continue;
                    }
    ...
    {
        if (!($vn_page_id = $this->getPrimaryKey())) {
            return null;
        }
    ...
    public function addMedia($ps_path, $ps_title, $ps_caption, $ps_idno, $pn_access, $options=null)
    {
        ...
        if ($o_trans = caGetOption('transaction', $options, null)) {
            $t_media->setTransaction($o_trans);
        }
        $t_media->setMode(ACCESS_WRITE);

        if (!$ps_idno) {
            $ps_idno = uniqid("Media-".$this->getPrimaryKey());
        }
        if (!$ps_title) {
            $ps_title = $ps_idno;
        }
    ...
    public function removeMedia($pn_media_id, $options=null)
    {
        ...
        $t_media = new ca_site_page_media($pn_media_id);
        if ($t_media->get('page_id') != $this->getPrimaryKey()) {
            return false;
        }
        if ($o_trans = caGetOption('transaction', $options, null)) {
            $t_media->setTransaction($o_trans);
    ...
    {
        if (!is_array($va_media = $this->getPageMedia(array('thumbnail', 'original'), $options))) {
            $va_media = [];
        }
