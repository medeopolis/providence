...
class ca_attribute_values extends BaseModel
{
                ...
                if (is_array($va_attrs = $t_instance->getAttributesByElement($vn_attr_element_id = $t_attr->get('element_id')))) {
                    ...
                    foreach ($va_attrs as $o_attr) {
                        foreach ($o_attr->getValues() as $o_val) {
                            if ((int)$o_val->getElementID() !== (int)$pa_element_info['element_id']) {
                                continue;
                            }
     ...
     *
     */
    public static function getAttributeValueArray($pn_element_id, $pa_options=null)
    {
        if (!$pn_element_id) {
            return null;
        }
     ...
     */
    public static function getValueIDFor($element_id, $value, $options=null)
    {
     ...
     */
    public static function getValuesFor($value_id, $options=null)
    {
        if ($value_id > 0) {
            $db = ($trans = caGetOption('transaction', $options, null)) ? $trans->getDb() : new Db();
