...
class ca_search_forms extends BundlableLabelableBaseModelWithAttributes
{
    ...
    public static $s_have_access_to_form_cache = [];
    ...
    public function getFormCount($pa_options=null)
    {
        ...
        if (is_array($va_forms)) {
            return sizeof($va_forms);
        } else {
            return 0;
        }
    }
     ...
     */
    public static function getFormTypeNames($pa_form_ids, $pa_options=null)
    {
        if (!is_array($pa_form_ids) && $pa_form_ids) {
            $pa_form_ids = [$pa_form_ids];
        }
        if (!$pa_form_ids) {
            return null;
        }
        if (!sizeof($pa_form_ids = array_filter($pa_form_ids, "intval"))) {
            return null;
        }
        ...
        while ($qr_res->nextRow()) {
            ...
            $va_restriction_names = array_map(function ($v) {
                return caUcFirstUTF8Safe(caGetListItemByIDForDisplay($v['type_id'], !$vb_use_singular));
            }, $t_form->getTypeRestrictions(null, ['form_id' => $qr_res->get('form_id')]));
    ...
    public function getAvailableBundles($pm_table_name_or_num=null, $pa_options=null)
    {
        ...
        if (is_array($va_search_settings)) {
            foreach ($va_search_settings as $vs_table => $va_fields) {
                if (preg_match("!\.related$!", $vs_table)) {
                    continue;
                }
                ...
                if ($vs_table == $vs_primary_table) {
                    ...
                    foreach ($va_field_list as $vs_field => $va_field_indexing_info) {
                        ...
                        if (Datamodel::getFieldInfo($vs_table, $vs_field, 'DONT_INCLUDE_IN_SEARCH_FORM')) {
                            continue;
                        }
                        ...
                        if (($va_field_info = $t_instance->getFieldInfo($vs_field))) {
                            ...
                            // self-related?
                            if (caGetBundleAccessLevel($vs_primary_table, $bundle_bits[2]) == __CA_BUNDLE_ACCESS_NONE__) {
                                continue;
                            }
    ...
    public function getHTMLFormElements($po_request, $pa_form_data=null)
    {
        ...
        if (!($vs_form_table_name = Datamodel::getTableName($this->get('table_num')))) {
            return null;
        }
            ...
            if ($t_element->load(array('element_code' => $element_code))) {
                ...
                if (sizeof($va_sub_elements = $t_element->getElementsInSet()) > 1) {
                    foreach ($va_sub_elements as $vn_element_id => $va_element_info) {
                        if ($element_code == $va_element_info['element_code']) {
                            continue;
                        }
    ...
    {
        if (!($vn_form_id = (int)$this->getPrimaryKey())) {
            return null;
        }		// ui must be loaded
    ...
    public function getTypeRestrictions($pn_type_id=null, $pa_options=null)
    {
        if (!($pn_form_id = caGetOption('form_id', $pa_options, null))) {
            if (!($pn_form_id = (int)$this->getPrimaryKey())) {
                return null;
            }
        }
