...
class ca_user_groups extends BaseModel
{
    ...
    {
        if (!$this->get('code')) {
            $this->set('code', caGenerateRandomPassword(6, ['uppercase' => true]));
        }
 ...
 */
    public function roleListAsHTMLFormElement($pa_options=null)
    {
        ...
        $vs_buf = '';
        if (sizeof($va_roles)) {
            if (!$va_group_roles = $this->getGroupRoles()) {
                $va_group_roles = array();
            }
     ...
     */
    public function addUsers($pm_user_ids)
    {
    ...
    * @return bool Returns true on success, false on error.
    */
    public function removeUsers($pm_user_ids)
    {
        if (!is_array($pm_user_ids)) {
            $pm_user_ids = array($pm_user_ids);
        }
