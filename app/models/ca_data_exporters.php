...
class ca_data_exporters extends BundlableLabelableBaseModelWithAttributes
{
    ...
    # ------------------------------------------------------
    public static function getFormatSettings($ps_format)
    {
     ...
     * @return bool
     */
    public static function exporterExists($exporter_code, $table=null, $options=null)
    {
        $d = ['exporter_code' => $exporter_code];
        if (!is_null($table)) {
            $d['table_num'] = Datamodel::getTableName($table);
        }
     ...
     */
    public static function getExporterListAsHTMLFormElement($ps_name, $pn_table_num=null, $pa_attributes=null, $pa_options=null)
    {
     ...
     */
    public static function getExporterCount($pn_table_num=null)
    {
     ...
     * @return boolean success state
     */
    public static function exportRDFMode($ps_config, $ps_filename, $pa_options=array())
    {
     ...
     * @return boolean success state
     */
    public static function exportRecordsFromSearchResult($ps_exporter_code, $po_result, $ps_filename=null, $pa_options=array())
    {
        ...
        if ($vs_export_format === 'CSV') {
            $individual_files = false;
        } // no individual file output with CSV
    ...
    public function processExporterItem($pn_item_id, $pn_table_num, $pn_record_id, $pa_options=array())
    {
        ...
        if (!$vb_ignore_context && ($contexts = $settings['context'])) {
            ...
            for ($i=0; $i < sizeof($contexts); $i = $i + 2) {
                $context = $contexts[$i];
                if (!is_array($context_settings = $contexts[$i+1])) {
                    $context_settings = [];
                }

                $va_filter_types = $context_settings['filterTypes'];
                if (!is_array($va_filter_types) && $va_filter_types) {
                    $va_filter_types = [$va_filter_types];
                }

                $va_restrict_to_types = $context_settings['restrictToTypes'];
                if (!is_array($va_restrict_to_rel_types = $context_settings['restrictToRelationshipTypes'])) {
                    $va_restrict_to_rel_types = [];
                }
                            ...
                            if ($vn_new_table_num) {
                                ...
                                if (is_array($va_attrs) && sizeof($va_attrs)>0) {
                                    ...
                                    foreach ($va_attrs as $vo_attr) {
                                        $va_attribute_export = $this->processExporterItem(
                                            $pn_item_id,
                                            $cur_table_num,
                                            $cur_record_id,
        ...
        if ($vn_attribute_id) {
            ...
            if (preg_match("/^_CONSTANT_:(.*)$/", $vs_source, $va_matches)) {
            ...
            } elseif (in_array($vs_source, array("relationship_type_id", "relationship_type_code", "relationship_typename"))) {
                ...
                if (!$vb_repeat) {
                    ...
                    if ($deduplicate) {
                        $va_values = array_unique($va_values);
                    }
        ...
        foreach ($va_item_info as $vn_key => &$va_item) {
            ...
            } elseif ((strlen($vs_prefix)>0) || (strlen($vs_suffix)>0)) {
            ...
            if ($vb_strip_newlines) {
                $va_item['text'] = preg_replace("![\n\r]+!", '', $va_item['text']);
            }
        }
        ...
        if ($settings['omitIfNoChildren']) {
            if (sizeof(array_filter($va_info['children'], function ($v) {
                return substr($v['element'], 0, 1) !== '@';
            })) === 0) {
                return [];
            }
        }
     ...
     */
    public static function loadInstanceByID($pn_record_id, $pn_table_num, $pa_options=null)
    {
        ...
        if (isset(ca_data_exporters::$s_instance_cache[$cache_key])) {
            ...
            if (!($table = Datamodel::getTableName($pn_table_num))) {
                return false;
            }
     ...
     * @return bool
     */
    public static function writeExporterToFile($exporter_code, $file)
    {
