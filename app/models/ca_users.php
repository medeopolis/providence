...
class ca_users extends BaseModel
{
     ...
     */
    public static $s_user_role_cache = [];
    public static $s_user_group_cache = [];
    public static $s_group_role_cache = [];
    public static $s_user_type_access_cache = [];
    public static $s_user_source_access_cache = [];
    public static $s_user_bundle_access_cache = [];
    public static $s_user_action_access_cache = [];
    public static $s_user_type_with_access_cache = [];
    public static $s_user_source_with_access_cache = [];
     ...
     * @var array
     */
    public static $s_user_name_cache = [];
    public static $s_user_info_cache = [];

    /**
     * List of tables that can have bundle- or type-level access control
     */
    public static $s_bundlable_tables = array(
     ...
     *
     */
    public static function applyPasswordPolicy($password)
    {
        $auth_config = Configuration::load(__CA_APP_DIR__."/conf/authentication.conf");
        if (strtolower($auth_config->get('auth_adapter')) !== 'causers') {
            return true;
        }	// password policies only apply to integral auth system
     ...
     *
     */
    public static function getPasswordPolicyAsText()
    {
        $auth_config = Configuration::load(__CA_APP_DIR__."/conf/authentication.conf");
        if (strtolower($auth_config->get('auth_adapter')) !== 'causers') {
            return '';
        ...
        if (is_array($policies = $auth_config->get('password_policies')) && sizeof($policies)) {
            ...
            return caMakeCommaListWithConjunction(array_filter($criteria, function ($v) {
                return strlen($v);
            }));
        }
        return '';
     ...
     *
     */
    private static function _getPasswordPolicyRuleAsText($rule, $value)
    {
        $v = (int)$value;
        switch($rule) {
            case 'min_length':
    ...
    public function getUserList($pa_options=null)
    {
        ...
        if (!is_array($pa_userclass)) {
            $pa_userclass = array($pa_userclass);
        }
        ...
        while ($qr_users->nextRow()) {
            if (!is_array($va_vars = $qr_users->getVars('vars'))) {
                $va_vars = array();
            }
    ...
    public function roleListAsHTMLFormElement($pa_options=null)
    {
        ...
        $vs_buf = '';
        if (sizeof($va_roles)) {
            if (!$va_user_roles = $this->getUserRoles(['skipVars' => true])) {
                $va_user_roles = array();
            }
    ...
    {
        if ($pn_user_id = $this->getPrimaryKey()) {
            if (isset(ca_users::$s_user_group_cache[$pn_user_id])) {
                return ca_users::$s_user_group_cache[$pn_user_id];
            }
            $o_db = $this->getDb();
            $qr_res = $o_db->query("
				SELECT 
    ...
    public function groupListAsHTMLFormElement($pa_options=null)
    {
        ...
        if (sizeof($va_groups)) {
            if (!$va_user_groups = $this->getUserGroups()) {
                    ...
                    switch($va_pref_info['formatType']) {
                        case 'FT_UI_LOCALE':
                            ...
                            foreach ($va_locales as $vs_code => $va_parts) {
                                if (is_array($va_restrict_to_ui_locales) && sizeof($va_restrict_to_ui_locales) && !in_array($vs_code, $va_restrict_to_ui_locales)) {
                                    continue;
                                }
                                try {
 ...
 */
    ...
    public function addSavedSearch($pm_table_name_or_num, $ps_type, $pa_search)
    {
        ...
        if (!is_array($va_searches = $this->getVar('saved_searches'))) {
            $va_searches = array();
        }
     ...
     */
     public static function exists($ps_user_name_or_id, $pa_options=null)
     {
    ...
    /**
      ...
      */
    public static function userIDFor($user)
    {
     ...
     */
    public static function userNameFor($user)
    {
        if ($info = self::userInfoFor($user)) {
     ...
     * @return string user_name value, or null if no match was found
     */
    public static function userInfoFor($user)
    {
     ...
     * @return string user_name value, or null if no match was found
     */
    private static function _getUserInfoFromInstance($user)
    {
        $info = [];
    ...
    public function addIp($pn_ip1, $pn_ip2, $pn_ip3, $pn_ip4s, $pn_ip4e, $ps_notes)
    {
        ...
        if (($pn_ip1 < 1) || ($pn_ip1 > 255)) {
            return false;
        }
        if (($pn_ip2 < 0) || ($pn_ip2 > 255)) {
            return false;
        }
        if (!$pn_ip3 || ($pn_ip3 < 0) || ($pn_ip3 > 255)) {
            $pn_ip3 = "NULL";
        }
    ...
    public function requestPasswordReset()
    {
        ...
        if (!($this->isActive())) {
            return false;
        } // no password resets for locked users
    ...
    public function getTypeAccessLevel($ps_table_name, $pm_type_code_or_id)
    {
        ...
        if (isset(ca_users::$s_user_type_access_cache[$vs_cache_key])) {
            return ca_users::$s_user_type_access_cache[$vs_cache_key];
        }
        ...
        if (in_array($ps_table_name, ca_users::$s_bundlable_tables)) { // type-level access control only applies to these tables
            ...
            if (is_numeric($pm_type_code_or_id)) {
                ...
                if (!($vs_type_list_code = $t_instance->getTypeListCode())) {
                    return __CA_BUNDLE_ACCESS_EDIT__;
                } // no type-level acces control for tables without type lists (like ca_lists)
    ...
    public function getTypesWithAccess($ps_table_name, $pn_access)
    {
        ...
        if (isset(ca_users::$s_user_type_with_access_cache[$vs_cache_key])) {
            return ca_users::$s_user_type_with_access_cache[$vs_cache_key];
        }
        $va_roles = array_merge($this->getUserRoles(), $this->getGroupRoles());
    ...
    public function getSourcesWithAccess($ps_table_name, $pn_access, $pa_options=null)
    {
        $vb_exact = caGetOption('exactAccess', $pa_options, false);
        $vs_cache_key = $ps_table_name."/".(int)$pn_access."/".$this->getPrimaryKey().(int)$vb_exact;
        if (isset(ca_users::$s_user_source_with_access_cache[$vs_cache_key])) {
            return ca_users::$s_user_source_with_access_cache[$vs_cache_key];
        }
        $va_roles = array_merge($this->getUserRoles(), $this->getGroupRoles());
        ...
        if (!method_exists($t_instance, "getSourceList")) {
            return null;
        }
