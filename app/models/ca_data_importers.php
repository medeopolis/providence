...
class ca_data_importers extends BundlableLabelableBaseModelWithAttributes
{
     ...
     *
     */
    public static function getAvailableInputFormats()
    {
     ...
     *
     */
    public static function getInputFormatListAsHTMLFormElement($ps_name, $pa_attributes=null, $pa_options=null)
    {
    ...
    public static function getImporters($pn_table_num=null, $pa_options=null)
    {
        ...
        while ($qr_res->nextRow()) {
            ...
            if ($vb_dont_include_worksheet) {
                unset($va_row['worksheet']);
            }
     ...
     * @return int
     */
    public static function getImporterListAsHTMLFormElement($ps_name, $pn_table_num=null, $pa_attributes=null, $pa_options=null)
    {
     ...
     * @return int
     */
    public static function getImporterCount($pn_table_num=null)
    {
    ...
    public function getRules()
    {
        ...
        if (!is_array($va_rules)) {
            return array();
        }
        if (!is_array($va_rules['rules'])) {
            $va_rules = array('rules' => $va_rules);
        }
    ...
    public function getEnvironment()
    {
        ...
        if (!is_array($va_rules['environment'])) {
            return array();
        }
    ...
    public static function loadImporterFromFile($ps_source, &$pa_errors, $pa_options=null, &$is_new=null)
    {
            ...
            switch($vs_mode) {
                ...
                case 'constant':
                    ...
                    if (!$vs_source) {
                        ...
                        if ($o_log) {
                            $o_log->logWarn(_t("[loadImporterFromFile:%1] Skipped mapping at row %2 because source was not defined", $ps_source, $vn_row_num));
                        }
                        continue(2);
                    }
                    if (!$vs_destination) {
                        ...
                        if ($o_log) {
                            $o_log->logWarn(_t("[loadImporterFromFile:%1] Skipped mapping at row %2 because destination was not defined", $ps_source, $vn_row_num));
                        }
                        continue(2);
                    }
                    ...
                    if ($vs_options_json = (string)$o_options->getValue()) {
                        if (is_null($va_options = @json_decode(caRepairJson($vs_options_json), true))) {
                            ...
                            if ($o_log) {
                                $o_log->logWarn(_t("[loadImporterFromFile:%1] Invalid json in \"options\" column at line %5  for group %2/source %3. Json was: %4.", $ps_source, $vs_group, $vs_source, $vs_options_json, $vn_row_num));
                            }
                            return null;
                        }
                    }
                    ...
                    if ($vs_refinery && ($vs_refinery_options_json = (string)$o_refinery_options->getValue())) {
                        if (!in_array($vs_refinery, $va_refineries)) {
                            ...
                            if ($o_log) {
                                $o_log->logWarn(_t("[loadImporterFromFile:%1] Invalid options for group %2/source %3", $ps_source, $vs_group, $vs_source));
                            }
                        } else {
                            ...
                            if (is_null($va_refinery_options = json_decode(caRepairJson($vs_refinery_options_json), true))) {
                                ...
                                if ($o_log) {
                                    $o_log->logError(_t("[loadImporterFromFile:%1] invalid json for refinery options at line %5 for group %2/source %3 = %4", $ps_source, $vs_group, $vs_source, $vs_refinery_options_json, $vn_row_num));
                                }
                                return null;
                            }
                        }
                    ...
                    if ($va_options && is_array($va_options) && isset($va_options['transformValuesUsingWorksheet']) && $va_options['transformValuesUsingWorksheet']) {
                        if ($o_opt_sheet = $o_excel->getSheetByName($va_options['transformValuesUsingWorksheet'])) {
                            foreach ($o_opt_sheet->getRowIterator() as $o_sheet_row) {
                                if (!$vs_original_value = trim(mb_strtolower((string)$o_opt_sheet->getCellByColumnAndRow(1, $o_sheet_row->getRowIndex())))) {
                                    continue;
                                }
                        ...
                        array_walk($va_original_values, function (&$v) {
                            $v = trim($v);
                        });
                        $va_replacement_values = preg_split("![\n\r]{1}!", (string)$o_replacement_values->getValue());
                        array_walk($va_replacement_values, function (&$v) {
                            $v = trim($v);
                        });
                    }
                ...
                case 'environment':
                    ...
                    if ($vs_options_json = (string)$o_options->getValue()) {
                        ...
                        if (is_null($va_options = @json_decode($vs_options_json, true))) {
                            ...
                            if ($o_log) {
                                $o_log->logWarn(_t("[loadImporterFromFile:environment %1] Invalid options for environment value %1. Options were: %2.", (string)$o_source->getValue(), $vs_options_json));
                            }
                        }
                    }
        ...
        if (!$va_settings['code']) {
            ...
            if ($o_log) {
                $o_log->logError(_t("[loadImporterFromFile:%1] You must set a code for your mapping!", $ps_source));
            }
            return null;
        }
        ...
        if (isset($va_settings['exporter_format'])) {
            ...
            if ($o_log) {
                $o_log->logError(_t("[loadImporterFromFile:%1] It looks like this is a mapping for the data export framework and you're trying to add it as import mapping!", $ps_source));
            }
            return null;
        }
        ...
        if (!($t_instance = Datamodel::getInstance($va_settings['table']))) {
            ...
            if ($o_log) {
                $o_log->logError(_t("[loadImporterFromFile:%1] Mapping target table %2 is invalid\n", $ps_source, $va_settings['table']));
            }
            return null;
        }
        ...
        if ($t_importer->load(array('importer_code' => $va_settings['code']))) {
            if ((!(bool)$t_importer->get('deleted'))) {
                $is_new = false;
            }
            ...
            if ($t_importer->numErrors()) {
                ...
                if ($o_log) {
                    $o_log->logError(_t("[loadImporterFromFile:%1] Could not delete existing mapping for %2: %3", $ps_source, $va_settings['code'], join("; ", $t_importer->getErrors())));
                }
                return null;
            }
        }
        ...
        if ($t_importer->numErrors()) {
            ...
            if ($o_log) {
                $o_log->logError(_t("[loadImporterFromFile:%1] Error creating mapping: %2", $ps_source, join("; ", $t_importer->getErrors())));
            }
            return null;
        }
        ...
        if ($t_importer->numErrors()) {
            ...
            if ($o_log) {
                $o_log->logError(_t("[loadImporterFromFile:%1] Error saving worksheet for future download: %2", $ps_source, join("; ", $t_importer->getErrors())));
            }
        }
        ...
        foreach ($va_mapping as $vs_group => $va_mappings_for_group) {
            ...
            if (!$vs_group_dest) {
                ...
                if ($o_log) {
                    $o_log->logWarn(_t("[loadImporterFromFile:%1] Skipped items for %2 because no common grouping could be found", $ps_source, $va_item['destination']));
                }
                continue;
            }
            ...
            if (!$t_group) {
                ...
                if ($o_log) {
                    $o_log->logError(_t("[loadImporterFromFile:%1] There was an error when adding group %2", $ps_source, $vs_group));
                }
                return null;
            }
                ...
                foreach ($va_mappings_for_source as $va_row) {
                    ...
                    if (is_array($va_row['options'])) {
                        foreach ($va_row['options'] as $vs_k => $vs_v) {
                            if ($vs_k == 'restrictToRelationshipTypes') {
                                $vs_k = 'filterToRelationshipTypes';
                            }	// "restrictToRelationshipTypes" is now "filterToRelationshipTypes" but we want to support old mappings so we translate here
     ...
     *
     */
    public static function _getGroupDestinationFromItems($pa_items)
    {
        $va_acc = null;
     ...
     *
     */
    public static function mappingExists($ps_mapping)
    {
        $t_importer = new ca_data_importers();
     ...
     *
     */
    public static function formatValuesForLog($pa_values, $ps_element)
    {
        $va_list = [];

        if (is_array($pa_values)) {
            foreach ($pa_values as $vs_element => $va_values) {
                if ($vs_element === 'locale_id') {
                    continue;
                }
    ...
    public function importDataFromSource($ps_source, $ps_mapping, $pa_options=null)
    {
        ...
        if ($limit_logging_to = caGetOption('limitLoggingTo', $pa_options, null)) {
            $limit_logging_to = array_map(function ($v) {
                return strtoupper($v);
            }, preg_split("![;,]+!", $limit_logging_to));
        }
        ...
        if ($ps_add_to_set 	= caGetOption('addToSet', $pa_options, null)) {
            ...
            if (!$pb_no_transaction) {
                $t_add_to_set->setTransaction($o_trans);
            }
        }
        ...
        //
            ...
            if (is_array($pref_label_lookup_fields_to_omit) && !sizeof($pref_label_lookup_fields_to_omit)) {
                $pref_label_lookup_fields_to_omit = null;
            }
        ...
        //
            ...
            if (is_array($va_environment_config = $t_mapping->getEnvironment())) {
                foreach ($va_environment_config as $vn_i => $va_environment_item) {
                    ...
                    if (!$o_env_reader->read($ps_source, array('basePath' => '', 'originalFilename' => caGetOption('originalFilename', $pa_options, null)))) {
                        break;
                    }
                    ...
                    switch(sizeof($va_env_tmp)) {
                        ...
                        case 2:
                            ...
                            $o_env_reader->seek(($vn_seek > 0) ? $vn_seek - 1 : $vn_seek);
                            $o_env_reader->nextRow();
        ...
        //
            ...
            //
                ...
                foreach ($va_mapping_items as $vn_item_id => $va_item) {
                    ...
                    if (is_array($va_row[$vs_key])) {
                        $va_row_with_replacements[$vs_key] = array_map(function ($v) use ($va_item) {
                            return ca_data_importers::replaceValue($v, $va_item, []);
                        }, $va_row[$vs_key]);
                    } else {
                            ...
                            foreach ($va_rule['actions'] as $va_action) {
                                ...
                                switch($vs_action_code = strtolower($va_action['action'])) {
                                    case 'set':
                                        ...
                                        $va_rule_set_values[$va_action['target']] = DisplayTemplateParser::processTemplate(
                                            $va_action['template'],
            ...
            //
                ...
                if ($vn_type_id_mapping_item_id) {
                    ...
                    $vs_idno = self::_applyConditionalOptions(
                        $vs_type,
                        $va_mapping_items,
                    ...
                    if (!self::_applyComparisonOptions(
                        $va_type,
                        $va_mapping_items,
                        $vn_type_id_mapping_item_id,
                        $o_reader,
                ...
                if (isset($va_mapping_items[$vn_type_id_mapping_item_id]['settings']['default']) && strlen($va_mapping_items[$vn_type_id_mapping_item_id]['settings']['default']) && (sizeof(array_filter($va_subject_type_list, function ($v) use ($vs_type) {
                    return ($vs_type === $v['idno']);
                })) == 0)) {
                    $vs_type = $va_mapping_items[$vn_type_id_mapping_item_id]['settings']['default'];
                }
                if (!$vs_type) {
                    $vs_type = $vs_type_mapping_setting;
                }		// fallback to mapping default if necessary
                ...
                foreach ($va_idnos_for_row as $vs_idno) {
                    ...
                    foreach ($va_preferred_label_mapping_ids as $vn_preferred_label_mapping_id => $vs_preferred_label_mapping_fld) {
                        ...
                        $vs_label_val = self::_applyConditionalOptions(
                            $vs_label_val,
                            $va_mapping_items,
                            $vn_preferred_label_mapping_id,
                            $o_reader,
                            $va_row,
                            $va_row_with_replacements,
                            ['skip' => [], 'log' => $o_log]
                        ...
                        if ($va_mapping_items[$vn_preferred_label_mapping_id]['settings']['applyRegularExpressions']) {
                            $vs_label_val = self::_processAppliedRegexes(
                                $o_reader,
                                $va_mapping_items[$vn_preferred_label_mapping_id],
                                0,
                                $va_mapping_items[$vn_preferred_label_mapping_id]['settings']['applyRegularExpressions'],
                                $vs_label_val,
                                $va_row,
                                $va_row_with_replacements
                            );
                ...
                //
                    ...
                    if ($vs_existing_record_policy != 'none') {
                        ...
                        switch($vs_existing_record_policy) {
                            ...
                            case 'merge_on_id_with_skip':
                                $va_ids = call_user_func_array(
                                    $t_subject->tableName()."::find",
                                ...
                                if (is_array($va_ids) && (sizeof($va_ids) > 0)) {
                                    ...
                                    if ($log_erp) {
                                        $o_log->logInfo(_t('[%1] Merged with existing record matched on primary key %2 for %3 by policy %4', $vs_idno, $vn_mapped_primary_key_value, $t_subject->tableName(), $vs_existing_record_policy));
                                    }
                                    break;
                                } else {
                                    if ($vs_existing_record_policy === 'merge_on_id_with_skip') {
                                        if ($log_erp) {
                                            $o_log->logInfo(_t('[%1] Skipped import because existing record could not be matched on primary key %2 for %3 by policy %4', $vs_idno, $vn_mapped_primary_key_value, $t_subject->tableName(), $vs_existing_record_policy));
                                        }
                                        $this->num_records_skipped++;
                                        continue(2);	// skip
                                    } else {
                                        if ($log_erp) {
                                            $o_log->logInfo(_t('[%1] Could not match existing record on primary key %2 for %3 by policy %4', $vs_idno, $vn_mapped_primary_key_value, $t_subject->tableName(), $vs_existing_record_policy));
                                        }
                                    }
                                }
                                break;
                            case 'skip_on_id':
                                ...
                                if (is_array($va_ids) && (sizeof($va_ids) > 0)) {
                                    if ($log_erp) {
                                        $o_log->logInfo(_t('[%1] Skipped import because of existing record matched on primary key %2 for %3 by policy %4', $vs_idno, $vn_mapped_primary_key_value, $t_subject->tableName(), $vs_existing_record_policy));
                                    }
                                    $this->num_records_skipped++;
                            ...
                            case 'overwrite_on_id':
                                ...
                                if (is_array($va_ids) && (sizeof($va_ids) > 0)) {
                                    ...
                                    // Don't stop?
                                        ...
                                        if ($log_erp) {
                                            $o_log->logInfo(_t('[%1] Overwrote existing record matched on primary key %2 for %3 by policy %4', $vs_idno, $vn_mapped_primary_key_value, $t_subject->tableName(), $vs_existing_record_policy));
                                        }
                                        $t_subject->clear();
                                        break;
                                    }
                                ...
                                if (!$vb_idno_is_template) {
                                    ...
                                    if (is_array($va_ids) && (sizeof($va_ids) > 0)) {
                                        ...
                                        if ($vs_existing_record_policy === 'merge_on_idno_with_skip') {
                                            if ($log_erp) {
                                                $o_log->logInfo(_t('[%1] Skipped import because existing record could not be matched on identifier by policy %2 using base criteria %3', $vs_idno, $vs_existing_record_policy, print_r($va_base_criteria, true)));
                                            }
                                            $this->num_records_skipped++;
                                            continue(2);	// skip
                                        } else {
                            ...
                            case 'merge_on_preferred_labels_with_replace':
                                ...
                                if (is_array($va_ids) && (sizeof($va_ids) > 0)) {
                                    ...
                                    if ($log_erp) {
                                        $o_log->logInfo(_t('[%1] Could not match existing record on label values %3 by policy %2 using base criteria %4', $vs_idno, $vs_existing_record_policy, print_r($pref_label_lookup_values, true), print_r($va_base_criteria, true)));
                                    }
                                }
                                break;
                    ...
                    if ($merge_only && !$t_subject->getPrimaryKey()) {
                        if ($log_erp) {
                            $o_log->logInfo(_t('[%1] Skipping because mergeOnly option is set and could not match existing record by policy %2.', $vs_idno, $vs_existing_record_policy));
                        }
                        continue;
                    }
                    ...
                    foreach ($va_items_by_group as $vn_group_id => $va_items) {
                        ...
                        if (! ($t_target = Datamodel::getInstance($vs_target_table, true))) {
                            ...
                            $o_log->logWarn(_t(
                                '[%1] Skipped group %2 because target %3 is invalid',
                                $vs_idno,
                                $vn_group_id,
                                $vs_target_table
                            ));
                        ...
                        foreach ($va_items as $vn_item_id => $va_item) {
                            ...
                            if ($vb_use_as_single_value = caGetOption(
                                'useAsSingleValue',
                                $va_item['settings'],
                                false
                            )
                                ...
                                $va_vals = [
                                    ca_data_importers::getValueFromSource($va_item, $o_reader, [
                                        ...
                                        'delimiter'                   => caGetOption(
                                            'delimiter',
                                            $va_item['settings'],
                                            ''
                                        ),
                                        'returnAsArray'               => false,
                                        'lookahead'                   => caGetOption(
                                            'lookahead',
                                            $va_item['settings'],
                                            0
                                        ),
                                        'filterToTypes'               => caGetOption(
                                            'filterToTypes',
                                            $va_item['settings'],
                                            null
                                        ),
                                        'filterToRelationshipTypes'   => caGetOption(
                                            'filterToRelationshipTypes',
                                            $va_item['settings'],
                                            null
                                        ),
                                        'restrictToRelationshipTypes' => caGetOption(
                                            'restrictToRelationshipTypes',
                                            $va_item['settings'],
                                            null
                                        ),
                                        'hierarchicalDelimiter'       => caGetOption(
                            ...
                            foreach ($va_vals as $vn_i => $vm_val) {
                                ...
                                if ($va_item['settings']['skipWhenEmpty'] && !is_array($va_item['settings']['skipWhenEmpty'])) {
                                    $va_item['settings']['skipWhenEmpty'] = array($va_item['settings']['skipWhenEmpty']);
                                }
                                if (isset($va_item['settings']['skipWhenEmpty']) && is_array($va_item['settings']['skipWhenEmpty'])) {
                                    foreach ($va_item['settings']['skipWhenEmpty'] as $v) {
                                        ...
                                        if (!strlen($vp = BaseRefinery::parsePlaceholder($v, $use_raw ? $va_raw_row : $va_row_with_replacements, $va_item, $vn_i, ['reader' => $o_reader, 'returnAsString' => true]))) {
                                            if ($log_skip) {
                                                $o_log->logInfo(_t('[%1] Skipped mapping for %2 because at least one value in list (%3) is empty', $vs_idno, $va_item['destination'], join('; ', $va_item['settings']['skipWhenEmpty'])));
                                            }
                                            continue(3);
                                        }
                                    }
                                }
                                if ($va_item['settings']['skipWhenAllEmpty'] && !is_array($va_item['settings']['skipWhenAllEmpty'])) {
                                    $va_item['settings']['skipWhenAllEmpty'] = array($va_item['settings']['skipWhenAllEmpty']);
                                }
                                if (isset($va_item['settings']['skipWhenAllEmpty']) && is_array($va_item['settings']['skipWhenAllEmpty'])) {
                                    ...
                                    if ($all_empty) {
                                        if ($log_skip) {
                                            $o_log->logInfo(_t('[%1] Skipped mapping for %2 because all values in list (%3) are empty', $vs_idno, $vn_row, join('; ', $va_item['settings']['skipWhenEmpty'])));
                                        }
                                        continue(3);
                                    }
                                }

                                if ($va_item['settings']['skipGroupWhenEmpty'] && !is_array($va_item['settings']['skipGroupWhenEmpty'])) {
                                    $va_item['settings']['skipGroupWhenEmpty'] = array($va_item['settings']['skipGroupWhenEmpty']);
                                }
                                if (isset($va_item['settings']['skipGroupWhenEmpty']) && is_array($va_item['settings']['skipGroupWhenEmpty'])) {
                                    foreach ($va_item['settings']['skipGroupWhenEmpty'] as $v) {
                                        ...
                                        if (!strlen($vp = BaseRefinery::parsePlaceholder($v, $use_raw ? $va_raw_row : $va_row_with_replacements, $va_item, $vn_i, ['reader' => $o_reader, 'returnAsString' => true]))) {
                                            if ($log_skip) {
                                                $o_log->logInfo(_t('[%1] Skipped group %2 because at least one value in list (%3) is empty', $vs_idno, $vn_group_id, join('; ', $va_item['settings']['skipGroupWhenEmpty'])));
                                            }
                                            continue(4);
                                        }
                                    }
                                }
                                if ($va_item['settings']['skipGroupWhenAllEmpty'] && !is_array($va_item['settings']['skipGroupWhenAllEmpty'])) {
                                    $va_item['settings']['skipGroupWhenAllEmpty'] = array($va_item['settings']['skipGroupWhenAllEmpty']);
                                }
                                if (isset($va_item['settings']['skipGroupWhenAllEmpty']) && is_array($va_item['settings']['skipGroupWhenAllEmpty'])) {
                                    ...
                                    if ($all_empty) {
                                        if ($log_skip) {
                                            $o_log->logInfo(_t('[%1] Skipped group %2 because all values in list (%3) are empty', $vs_idno, $vn_group_id, join('; ', $va_item['settings']['skipGroupWhenAllEmpty'])));
                                        }
                                        continue(3);
                                    }
                                }

                                if ($va_item['settings']['skipRowWhenEmpty'] && !is_array($va_item['settings']['skipRowWhenEmpty'])) {
                                    $va_item['settings']['skipRowWhenEmpty'] = array($va_item['settings']['skipRowWhenEmpty']);
                                }
                                if (isset($va_item['settings']['skipRowWhenEmpty']) && is_array($va_item['settings']['skipRowWhenEmpty'])) {
                                    foreach ($va_item['settings']['skipRowWhenEmpty'] as $v) {
                                        ...
                                        if (!strlen($vp = BaseRefinery::parsePlaceholder($v, $use_raw ? $va_raw_row : $va_row_with_replacements, $va_item, $vn_i, ['reader' => $o_reader, 'returnAsString' => true]))) {
                                            if ($log_skip) {
                                                $o_log->logInfo(_t('[%1] Skipped row %2 because at least one value in list (%3) is empty', $vs_idno, $vn_row, join('; ', $va_item['settings']['skipRowWhenEmpty'])));
                                            }
                                            continue(5);
                                        }
                                    }
                                }
                                if ($va_item['settings']['skipRowWhenAllEmpty'] && !is_array($va_item['settings']['skipRowWhenAllEmpty'])) {
                                    $va_item['settings']['skipRowWhenAllEmpty'] = array($va_item['settings']['skipRowWhenAllEmpty']);
                                }
                                if (isset($va_item['settings']['skipRowWhenAllEmpty']) && is_array($va_item['settings']['skipRowWhenAllEmpty'])) {
                                    ...
                                    if ($all_empty) {
                                        if ($log_skip) {
                                            $o_log->logInfo(_t('[%1] Skipped row %2 because all values in list (%3) are empty', $vs_idno, $vn_row, join('; ', $va_item['settings']['skipRowWhenAllEmpty'])));
                                        }
                                        continue(4);
                                    }
                                }
                                ...
                                if (isset($va_item['settings']['skipRowIfEmpty'])
                                    ...
                                    if ($log_skip) {
                                        $o_log->logInfo(_t(
                                            '[%1] Skipped row %2 because value for %3 in group %4 is empty',
                                            $vs_idno,
                                            $vn_row,
                                            $va_item['destination'],
                                            $vn_group_id
                                ...
                                if (isset($va_item['settings']['skipGroupIfEmpty'])
                                    ...
                                    if ($log_skip) {
                                        $o_log->logInfo(_t(
                                            '[%1] Skipped group %2 because value for %3 is empty',
                                            $vs_idno,
                                            $vn_group_id,
                                            $va_item['destination']
                                        ));
                                ...
                                if ((!isset($va_item['settings']['skipIfEmpty'])
                                    ...
                                    if ($log_skip) {
                                        $o_log->logInfo(_t(
                                            '[%1] Skipped mapping because value for %2 is empty',
                                            $vs_idno,
                                            $va_item['destination']
                                        ));
                                    }
                                ...
                                if (isset($va_item['settings']['skipIfValue'])
                                    ...
                                    if ($log_skip) {
                                        $o_log->logInfo(_t(
                                            '[%1] Skipped mapping %2 because value for %3 matches value %4',
                                            $vs_idno,
                                            $vn_row,
                                            $vs_item_terminal,
                                            $vm_val
                                ...
                                if (isset($va_item['settings']['skipIfNotValue'])
                                    ...
                                    if ($log_skip) {
                                        $o_log->logInfo(_t(
                                            '[%1] Skipped mapping %2 because value %4 for %3 is not in list of values',
                                            $vs_idno,
                                            $vn_row,
                                            $vs_item_terminal,
                                            $vm_val
                                ...
                                if (isset($va_item['settings']['skipRowIfValue'])
                                    ...
                                    if ($log_skip) {
                                        $o_log->logInfo(_t(
                                            '[%1] Skipped row %2 because value for %3 in group %4 matches value %5',
                                            $vs_idno,
                                            $vn_row,
                                            $vs_item_terminal,
                                            $vn_group_id,
                                ...
                                if (isset($va_item['settings']['skipRowIfNotValue'])
                                    ...
                                    if ($log_skip) {
                                        $o_log->logInfo(_t(
                                            '[%1] Skipped row %2 because value for %3 in group %4 is not in list of values',
                                            $vs_idno,
                                            $vn_row,
                                            $vs_item_terminal,
                                            $vn_group_id,
                                ...
                                if (isset($va_item['settings']['skipGroupIfValue'])
                                    ...
                                    if ($log_skip) {
                                        $o_log->logInfo(_t(
                                            '[%1] Skipped group %2 because value for %3 matches value %4',
                                            $vs_idno,
                                            $vn_group_id,
                                            $vs_item_terminal,
                                            $vm_val
                                ...
                                if (isset($va_item['settings']['skipGroupIfNotValue'])
                                    ...
                                    if ($log_skip) {
                                        $o_log->logInfo(_t(
                                            '[%1] Skipped group %2 because value for %3 matches is not in list of values',
                                            $vs_idno,
                                            $vn_group_id,
                                            $vs_item_terminal
                                        ));
                                ...
                                if (isset($va_item['settings']['skipIfNoReplacementValue'])
                                    ...
                                    if ($log_skip) {
                                        $o_log->logInfo(_t(
                                            '[%1] Skipped mapping %2 because there is no replacement value for value %3',
                                            $vs_idno,
                                            $va_item['destination'],
                                            $vm_val
                                        ));
                                ...
                                if ($vm_val && isset($va_item['settings']['prefix'])
                                    ...
                                    $vm_val = DisplayTemplateParser::processTemplate(
                                        $va_item['settings']['prefix'],
                                        array_replace($va_row_with_replacements, [
                                            (string) $va_item['source'] => ca_data_importers::replaceValue(
                                                $vm_val,
                                                $va_item,
                                ...
                                if (! is_array($vm_val) && ($vm_val[0] == '^')
                                    ...
                                    // Parse placeholder
                                    if (! is_null(
                                        $vm_parsed_val = BaseRefinery::parsePlaceholder(
                                            $vm_val,
                                            $va_row_with_replacements,
                                            $va_item,
                                            $vn_i,
                                            array( 'reader' => $o_reader, 'returnAsString' => true )
                                        )
                                    )
                                    ) {
                                ...
                                if ($use_raw && isset($va_item['settings']['formatWithTemplate'])
                                    ...
                                    $vm_val
                                        = DisplayTemplateParser::processTemplate(
                                            $va_item['settings']['formatWithTemplate'],
                                ...
                                if (isset($va_item['settings']['skipIfExpression'])
                                    ...
                                    try {
                                        if ($vm_ret = ExpressionParser::evaluate(
                                            $va_item['settings']['skipIfExpression'],
                                            $use_raw ? $va_raw_row : $va_row_with_replacements
                                        )
                                        ) {
                                            $o_log->logInfo(_t(
                                                '[%1] Skipped mapping because skipIfExpression %2 is true',
                                                $vs_idno,
                                                $va_item['settings']['skipIfExpression']
                                            ));
                                            continue(2);
                                        ...
                                        $o_log->logDebug(_t(
                                            "[%1] Could not evaluate expression %2: %3",
                                            $vs_idno,
                                            $va_item['settings']['skipIfExpression'],
                                            $e->getMessage()
                                        ));
                                ...
                                if (isset($va_item['settings']['applyRegularExpressions'])
                                    ...
                                    $vm_val = self::_processAppliedRegexes(
                                        $o_reader,
                                        $va_item,
                                        $vn_i,
                                        $va_item['settings']['applyRegularExpressions'],
                                        $vm_val,
                                        $va_row,
                                        $va_row_with_replacements
                                    );
                            ...
                            foreach ($va_vals as $vn_i => $vm_val) {
                                ...
                                if (isset($va_item['settings']['restrictToTypes'])
                                    ...
                                    if ($log_skip) {
                                        $o_log->logInfo(_t(
                                            '[%1] Skipped mapping %2 because of type restriction for type %3',
                                            $vs_idno,
                                            $vn_row,
                                            $vs_type
                                        ));
                                    ...
                                    try {
                                        if ($vm_ret
                                            = ExpressionParser::evaluate(
                                                $va_item['settings']['skipRowIfExpression'],
                                                $use_raw ? $va_raw_row : $va_row_with_replacements
                                            )
                                        ) {
                                            if ($log_skip) {
                                                $o_log->logInfo(_t(
                                                    '[%1] Skipped row %2 because skipRowIfExpression %3 is true',
                                                    $vs_idno,
                                                    $vn_row,
                                                    $va_item['settings']['skipRowIfExpression']
                                                ));
                                        ...
                                        $o_log->logDebug(_t(
                                            "[%1] Could not evaluate skipRowIfExpression %2: %3",
                                            $vs_idno,
                                            $va_item['settings']['skipRowIfExpression'],
                                            $e->getMessage()
                                        ));
                                ...
                                if (isset($va_item['settings']['skipGroupIfExpression'])
                                    ...
                                    try {
                                        if ($vm_ret
                                            ...
                                            if ($log_skip) {
                                                $o_log->logInfo(_t(
                                                    '[%1] Skipped group %2 because skipGroupIfExpression %3 is true',
                                                    $vs_idno,
                                                    $vn_group_id,
                                                    $va_item['settings']['skipGroupIfExpression']
                                                ));
                                        ...
                                        $o_log->logDebug(_t(
                                            '[%1] Did not skip group %2 because skipGroupIfExpression %3 is false (data was %4)',
                                            $vs_idno,
                                            $vn_group_id,
                                            $va_item['settings']['skipGroupIfExpression'],
                                            print_R($use_raw ? $va_raw_row : $va_row_with_replacements, true)
                                        ));
                                    } catch (Exception $e) {
                                        $o_log->logDebug(_t(
                                            "[%1] Could not evaluate expression %2: %3",
                            ...
                            //
                                if (($va_item['destination'] == "{$vs_subject_table}.{$vs_type_id_fld}")
                                    ...
                                    $vs_type     = $vm_val;
                                    $o_log->logDebug(_t(
                                        "Reset type to %1 using value in content tree; was %2",
                                        $vs_type,
                                        $vs_old_type
                                    ));
                                }
                                ...
                                if (! isset($va_item['settings']['errorPolicy'])
                                     || ! in_array(
                                         $vs_item_error_policy = $va_item['settings']['errorPolicy'],
                                         array( 'ignore', 'stop' )
                                     )
                                ) {
                                ...
                                if (isset($va_item['settings']['refineries'])
                                    ...
                                    foreach ($va_item['settings']['refineries'] as $vs_refinery) {
                                        ...
                                        if ($o_refinery = RefineryManager::getRefineryInstance($vs_refinery)) {
                                            $va_refined_values = $o_refinery->refine(
                                                $va_content_tree,
                                                $va_group,
                                                $va_item,
                                                $va_row_with_replacements,
                                            ...
                                            $this->logImportError(_t(
                                                '[%1] Invalid refinery %2 specified',
                                                $vs_idno,
                                                $vs_refinery
                                            ), $va_log_import_error_opts);
                                        }
                                    }
                                }
                                ...
                                if (isset($va_item['settings']['delimiter']) && $va_item['settings']['delimiter']
                                    ...
                                    if (sizeof($va_item['settings']['delimiter'])) {
                                        ...
                                        foreach ($va_val_list as $vs_list_val) {
                                            $vs_list_val = trim(ca_data_importers::replaceValue(
                                                $vs_list_val,
                                                $va_item,
                                                [ 'log' => $o_log, 'logReference' => $vs_idno ]
                                            ));
                        ...
                        if (sizeof($va_preferred_label_mapping_ids) && ($t_subject->getPreferredLabelCount() > 0) && (!$vb_was_preferred_label_match)) {
                            ...
                            if (!sizeof($pref_label_data)) {
                                ...
                                foreach ($va_preferred_label_mapping_ids as $vn_preferred_label_mapping_id => $vs_fld) {
                                    if ($va_mapping_items[$vn_preferred_label_mapping_id]['settings']['skipIfDataPresent']) {
                                        $vb_remove_labels = false;
                                        break;
                                    }
                                }
                        ...
                        if (sizeof($va_nonpreferred_label_mapping_ids) && ($t_subject->getNonPreferredLabelCount() > 0)) {
                            if (!preg_match("!^merge!", $vs_existing_record_policy)) {
                                foreach ($va_nonpreferred_label_mapping_ids as $vn_nonpreferred_label_mapping_id => $vs_fld) {
                                    if ($va_mapping_items[$vn_nonpreferred_label_mapping_id]['settings']['skipIfDataPresent']) {
                                        $vb_remove_labels = false;
                                        break;
                                    }
                                }
                        ...
                        if ($vs_table_name == $vs_subject_table) {
                                        ...
                                        switch($vs_element) {
                                            case 'preferred_labels':
                                                if (!$vb_was_preferred_label_match) {
                                                    if (
                                                        ...
                                                        &&
                                                        (sizeof(array_filter($t_subject->getSecondaryLabelDisplayFields(), function ($v) use ($va_element_content) {
                                                            return isset($va_element_content[$v]) && strlen($va_element_content[$v]);
                                                        })) == 0)
                                                    ) {
                                                        $va_element_content[$vs_disp_field] = '['.caGetBlankLabelText($vn_table_num).']';
                                                    }

                                                    if ($vb_skip_if_data_present && ($t_subject->getLabelCount(true, $vn_locale_id) > 0)) {
                                                        continue(2);
                                                    }
                                                    ...
                                                    $t_subject->replaceLabel(
                                                        $va_element_content,
                                                        $vn_locale_id,
                                                        isset($va_element_content['type_id']) ? $va_element_content['type_id'] : null,
                                                        true,
                                                        ['truncateLongLabels' => $vb_truncate_long_labels, 'displaynameFormat' => $displayname_format]
                                                    );
                                                ...
                                                break;
                                            case 'nonpreferred_labels':
                                                if ($vb_skip_if_data_present && ($t_subject->getLabelCount(false, $vn_locale_id) > 0)) {
                                                    continue(2);
                                                }
                                                $t_subject->addLabel(
                                                    $va_element_content,
                                            ...
                                            default:
                                                if ($t_subject->hasField($vs_element) && (($vs_element != $t_subject->primaryKey(true)) && ($vs_element !== $t_subject->primaryKey()) && ($vs_element !== $vs_parent_id_fld))) {
                                                    if ($vb_skip_if_data_present && strlen($t_subject->get($vs_element))) {
                                                        continue(2);
                                                    }
                                                ...
                                                if (($vs_subject_table == 'ca_representation_annotations') && ($vs_element == 'properties')) {
                                                    foreach ($va_element_content as $vs_prop => $vs_prop_val) {
                                                        if ($vb_skip_if_data_present && strlen($t_subject->getPropertyValue($vs_prop))) {
                                                            continue;
                                                        }
                                                ...
                                                if ($vb_skip_if_data_present && ($t_subject->getAttributeCountByElement($vs_element) > 0)) {
                                                    continue(2);
                                                }
                                                if (
                                                    (
                            ...
                            foreach ($va_content as $vn_i => $va_element_data) {
                                ...
                                try {
                                    ...
                                    if (isset($va_element_data['preferred_labels'][$vs_rel_label_display_fld])) {
                                        $vs_name = $va_element_data['preferred_labels'][$vs_rel_label_display_fld];
                                    } elseif (isset($va_element_data['preferred_labels'])) {
                                        $vs_name = $va_element_data['preferred_labels'];
                                    } elseif (isset($va_element_data[$vs_rel_label_display_fld][$vs_rel_label_display_fld])) {
                                        $vs_name = $va_element_data[$vs_rel_label_display_fld][$vs_rel_label_display_fld];
                                    } elseif (isset($va_element_data[$vs_rel_label_display_fld])) {
                                        $vs_name = $va_element_data[$vs_rel_label_display_fld];
                                    } elseif (isset($va_element_data) && !is_array($va_element_data) && strlen($va_element_data)) {
                                        $vs_name = $va_element_data;
                                    }
                                    ...
                                    switch($vs_table_name) {
                                        case 'ca_objects':
                                            if ($vn_rel_id = DataMigrationUtils::getObjectID($vs_name, $va_element_data['_parent_id'], $va_element_data['_type'], $vn_locale_id, $va_data_for_rel_table, array('logReference' => $vs_idno, 'forceUpdate' => true, 'dontCreate' => $vb_dont_create, 'ignoreParent' => $vb_ignore_parent, 'matchOn' => $va_match_on, 'log' => $o_log, 'transaction' => $o_trans, 'importEvent' => $o_event, 'importEventSource' => $vn_row, 'nonPreferredLabels' => $va_nonpreferred_labels))) {
                                                ...
                                                if ($rel_type_delimiter = $va_element_data['_relationship_type_delimiter']) {
                                                    $rel_types = array_map(function ($v) {
                                                        return trim($v);
                                                    }, explode($rel_type_delimiter, trim($va_element_data['_relationship_type'])));
                                                } else {
                                        ...
                                        case 'ca_object_lots':
                                            ...
                                            if ($vn_rel_id = DataMigrationUtils::getObjectLotID($vs_idno_stub, $vs_name, $va_element_data['_type'], $vn_locale_id, $va_data_for_rel_table, array('logReference' => $vs_idno, 'forceUpdate' => true, 'dontCreate' => $vb_dont_create, 'ignoreParent' => $vb_ignore_parent, 'matchOn' => $va_match_on, 'log' => $o_log, 'transaction' => $o_trans, 'importEvent' => $o_event, 'importEventSource' => $vn_row, 'nonPreferredLabels' => $va_nonpreferred_labels))) {
                                                if (!($vs_rel_type = $va_element_data['_relationship_type']) && !(is_array($va_element_data['idno_stub']) && ($vs_rel_type = $va_element_data['idno_stub']['_relationship_type']))) {
                                                    break;
                                                }
        ...
        if ($log_general) {
            $o_log->logInfo(_t('Import of %1 completed using mapping %2: %3 imported/%4 skipped/%5 errors', $ps_source, $t_mapping->get('importer_code'), $this->num_records_processed, $this->num_records_skipped, $this->num_import_errors));
        }
    ...
    private function _processChildren($parent_table, $parent_id, $children, $options=null)
    {
        ...
        foreach ($children as $child) {
            ...
            unset($vals['_table']);
            unset($vals['_type']);
            unset($vals['preferred_labels']);
            unset($vals['_children']);

            if (caIsIndexedArray($child['preferred_labels'])) {
                $child['preferred_labels'] = array_shift($child['preferred_labels']);
            }

            $id = null;
            $attrs = $vals;
            unset($attrs['_related_related']);
            if ($parent_table == $child['_table']) {	// direct child to parent (both are same table)
                ...
                if (($id = DataMigrationUtils::getIDFor($child['_table'], $child['preferred_labels'], null, $child['_type'], 1, $attrs, $options)) && ($parent = Datamodel::getInstance($parent_table, true)) && $parent->load($parent_id)) {
                    ...
                    if ($parent->numErrors() > 0) {
                        if ($log) {
                            $log->logInfo(_t('Could not create object-collection hierarchy child record relationship: %1', join("; ", $parent->getErrors())));
                        }
                        continue;
                    }
                }
            ...
            if (!$id) {
                if ($log) {
                    $log->logInfo(_t('Could not create hierarchy child record for %1', $child['_table']));
                }
                continue;
            }
            try {
                    ...
                    foreach ($vals['_related_related'] as $vs_rel_rel_table => $va_rel_rels) {
                        foreach ($va_rel_rels as $vn_i => $va_rel_rel) {
                            if (!($t_rel_instance = Datamodel::getInstance($child['_table']))) {
                                if ($log) {
                                    $log->logWarn(_t("[%1] Could not instantiate related table %2", $log_reference, $child['_table']));
                                }
                                continue;
                            }
                            if ($trans) {
                                $t_rel_instance->setTransaction($trans);
                            }
                            if ($t_rel_instance->load($id)) {
                                if ($t_rel_rel = $t_rel_instance->addRelationship($vs_rel_rel_table, $va_rel_rel['id'], $va_rel_rel['_relationship_type'])) {
                                    if ($log) {
                                        $log->logInfo(_t('[%1] Related %2 (%3) to related %4 with relationship %5', $log_reference, Datamodel::getTableProperty($vs_rel_rel_table, 'NAME_SINGULAR'), $va_rel_rel['id'], $t_rel_instance->getProperty('NAME_SINGULAR'), trim($va_rel_rel['_relationship_type'])));
                                    }
                                } else {
        ...
        if (preg_match('!^_CONSTANT_:[^:]+:(.*)$!', $pa_item['source'], $va_matches)) {
            ...
            if ($po_reader->valuesCanRepeat()) {
                ...
                foreach ($vm_value as $vs_k => $vs_v) {
                    if (is_array($vs_v)) {
                        continue;
                    }	// skip odd mappings that return arrays of arrays
     ...
     *
     */
    public static function replaceValue($pm_value, $pa_item, $pa_options=null)
    {
     ...
     */
    public static function hasReplacementValue($pm_value, $pa_item, $pa_options=null)
    {
        if (strlen($pm_value) && is_array($pa_item['settings']['original_values'])) {
            if (($vn_index = array_search(trim(mb_strtolower($pm_value)), $pa_item['settings']['original_values'])) !== false) {
                return true;
    ...
    public static function _applyComparisonOptions(?string $value, array $mapping_items, int $item_id, $o_reader, array $row, array $row_with_replacements, ?array $options=null)
    {
        ...
        if (!in_array('skipIfExpression', $skip) && isset($mapping_items[$item_id]['settings']['skipIfExpression'])
            ...
            try {
                if ($vm_ret = ExpressionParser::evaluate(
                    $mapping_items[$item_id]['settings']['skipIfExpression'],
                ...
                if ($o_log) {
                    $o_log->logDebug(_t(
                        "[%1] Could not evaluate expression %2 when getting preferred label values for merge: %3",
                        $value,
                        $mapping_items[$item_id]['skipIfExpression'],
                        $e->getMessage()
                    ));
                }
            }
    ...
    public static function getImportersAsHTMLOptions($options=null)
    {
        if (is_array($tables = caGetOption('tables', $options, null))) {
            $tables = array_filter(array_map(function ($v) {
                return (int)Datamodel::getTableNum($v);
            }, $tables), function ($v) {
                return (bool)$v;
            });
        }
        ...
        if (strlen($value)) {
            ...
            if (is_array($transforms) && sizeof($transforms)) {
                foreach ($transforms as $t) {
                    ...
                    if (!($transform = caGetOption('transform', $t, null))) {
                        continue;
                    }
                    $opts = $t;
                    unset($t['transform']);
