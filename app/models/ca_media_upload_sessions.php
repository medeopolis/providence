...
class ca_media_upload_sessions extends BaseModel
{
     ...
     *
     */
    public static function processSessions(?array $options=null): int
    {
            ...
            switch($mode) {
                case 'hierarchy':
                    ...
                    $album_rep = $t->addRepresentation(
                        $p=array_shift(array_keys($media)),
                        $rep_type,
                        $locale_id,
                        $rep_status,
                        $rep_access,
                        $is_primary,
                        [],
                        ['returnRepresentation' => true, 'original_filename' => pathinfo($p, PATHINFO_BASENAME)]
                    );
                    ...
                    foreach ($media as $path => $info) {
                        ...
                        if ($is_primary && $album_rep) {
                            ...
                            $r->addRepresentation(
                                ...
                                true,
                                [],
                                ['original_filename' => pathinfo($p, PATHINFO_BASENAME)]
                            );
     ...
     *
     */
    private static function _processContent(BaseModel $t_instance, array $config, array $data): array
    {
            ...
            if ($bundle_bits[0] === $table) {
                switch(sizeof($bundle_bits)) {
                    ...
                    case 2:	// attribute or intrinsic
                        if (!is_array($data[$info['bundle']])) {
                            $data[$info['bundle']] = [$data[$info['bundle']]];
                        }
     ...
     *
     */
    private static function _setSessionError($session, $label, $error)
    {
     ...
     *
     */
    private static function _setSessionWarning($session, $label, $warning)
    {
