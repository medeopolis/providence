...
class ca_sets extends BundlableLabelableBaseModelWithAttributes implements IBundleProvider
{
    ...
    public static $s_have_access_to_set_cache = array();
    ...
    public function rowIDsToItemIDs($pa_row_ids, $pa_options=null)
    {
        ...
        $pa_row_ids = array_filter($pa_row_ids, function ($a) {
            return (bool)$a;
        });
    ...
    public function getSets($pa_options=null)
    {
        ...
        if ($va_restrict_to_types = caGetOption(['restrict_to_types', 'restrictToTypes'], $pa_options, false)) {
            if (!is_array($va_restrict_to_types)) {
                $va_restrict_to_types = [$va_restrict_to_types];
            }
    ...
    public function getAvailableSetsForItem($pm_table_name_or_num, $pn_row_id, $pa_options=null)
    {
        ...
        if (!($va_full_set_list = $this->getSets(array_merge(array('table' => $pm_table_name_or_num), $pa_options)))) {
            return null;
        }
        if (!($va_current_set_list = $this->getSetsForItem($pm_table_name_or_num, $pn_row_id, $pa_options))) {
            $va_current_set_list = [];
        }

        $va_available_sets = array();
        foreach ($va_full_set_list as $vn_set_id => $va_set_info_by_locale) {
            if (isset($va_current_set_list[$vn_set_id]) && $va_current_set_list[$vn_set_id]) {
                continue;
            }
    ...
    public function isInSet($pm_table_name_or_num, $pn_row_id, $pm_set_code_or_id)
    {
        ...
        if (!($vn_set_id = $this->_getSetID($pm_set_code_or_id))) {
            return null;
        }
        $o_db = $this->getDb();
    ...
    {
        if (is_array($set_ids)) {
            $set_ids = array_filter($set_ids, function ($v) {
                return (bool)$v;
            });
        }
    ...
    {
        if (!($set_id = $this->getPrimaryKey())) {
            return null;
        }
        if ($user_id && (!$this->haveAccessToSet($user_id, __CA_SET_EDIT_ACCESS__))) {
            return false;
        }
    ...
    public function transferItemsTo($set, array $row_ids, int $user_id=null)
    {
        ...
        if ($user_id && (!$t_dest_set->haveAccessToSet($user_id, __CA_SET_EDIT_ACCESS__))) {
            return false;
        }
    ...
    public function removeItemByItemID($pn_item_id, $pn_user_id=null)
    {
        ...
        if (!$this->haveAccessToSet($pn_user_id, __CA_SET_EDIT_ACCESS__)) {
            return false;
        }
    ...
    public function getItemRanks($pa_options=null)
    {
        ...
        if (!$this->haveAccessToSet($pa_options['user_id'], __CA_SET_READ_ACCESS__)) {
            return false;
        }
     ...
     */
    public static function getRowIDRanksForSet($pn_set_id, $pa_options=null)
    {
        $vb_treat_row_ids_as_rids = caGetOption('treatRowIDsAsRIDs', $pa_options, false);
    ...
    public function getItems($pa_options=null)
    {
        ...
        if (!($t_rel_table = Datamodel::getInstanceByTableNum($this->get('table_num'), true))) {
            return null;
        }
...
LEFT JOIN ca_object_representations AS cor ON coxor.representation_id = cor.representation_id\n";
    ...
    public function getItemCount($pa_options=null)
    {
        ...
        if ($vn_user_id && !$this->haveAccessToSet($vn_user_id, __CA_SET_READ_ACCESS__)) {
            return 0;
        }
    ...
    public function getRepresentationTags($ps_version, $pa_options=null)
    {
        ...
        if (!$this->get('table_num') == Datamodel::getTableNum("ca_objects")) {
            return null;
        }
        if (!is_array($pa_options)) {
            $pa_options = array();
    ...
    */
    ...
    public function isSaveable($po_request, $ps_bundle_name=null)
    {
        ...
        if ($ps_bundle_name) {
            if ($po_request->user->getBundleAccessLevel($this->tableName(), $ps_bundle_name) < __CA_BUNDLE_ACCESS_EDIT__) {
                return false;
            }
        }
     ...
     * @return bool
     */
    public static function setExists($set_code, $options=null)
    {
     ...
     * @return array
     */
    public static function setContents($set_code, $options=null)
    {
        if (!$set = ca_sets::find(['set_code' => $set_code], ['returnAs' => 'firstModelInstance'])) {
            return json_encode([]);
        }
        ...
        return array_map(function ($r) {
            return ['key' => md5($r['representation_url_large']), 'url' => $r['representation_url_large'], 'media_tag' => $r['representation_tag_large'], 'media_tag_link' => $r['representation_tag_large_as_link'], 'media_tag_large_link' => $r['representation_tag_large_as_link'], 'media_tag_iconlarge' => $r['representation_tag_iconlarge'], 'caption' => $r['displayTemplate'], 'itemDescription' => $r['displayTemplateDescription']];
        }, $reps);
    }
    # ---------------------------------------------------------------
    /**
     ...
     * @return string
     */
    public static function setContentsAsJSON($set_code, $options=null)
    {
     ...
     * @return string
     */
    public static function getRowIDsForItemIDs($item_ids, $options=null)
    {
        ...
        $item_ids = array_map(function ($v) {
            return (int)$v;
        }, array_filter($item_ids, function ($v) {
            return (int)$v > 0;
        }));
        if (!sizeof($item_ids)) {
            return false;
    ...
    {
        if (!($id = caGetOption('set_id', $options, null)) && !($id = (int)$this->getPrimaryKey())) {
            return null;
        }
