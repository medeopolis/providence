...
class ca_lists extends BundlableLabelableBaseModelWithAttributes
{
    ...
    public static $s_list_id_cache = array();
    public static $s_list_code_cache = array();
    public static $s_list_item_display_cache = array();			// cache for results of getItemFromListForDisplayByItemID()
    public static $s_list_item_value_display_cache = array();		// cache for results of getItemFromListForDisplayByItemValue()
    public static $s_list_item_get_cache = array();				// cache for results of getItemFromList()
    public static $s_item_id_cache = array();						// cache for ca_lists::getItemID()
    public static $s_item_id_to_code_cache = array();				// cache for ca_lists::itemIDsToIDNOs()
    public static $s_item_id_to_value_cache = array();				// cache for ca_lists::itemIDsToItemValues()
    public static $s_code_to_item_id_cache = array();				// cache for ca_lists::IDNOsToItemIDs()
    ...
    public function editItem($pn_item_id, $ps_value, $pb_is_enabled=true, $pb_is_default=false, $pn_parent_id=null, $ps_idno=null, $ps_validation_format='', $pn_status=0, $pn_access=0, $pn_rank=null, $ps_color=null)
    {
        ...
        if ($t_item->get('list_id') != $this->getPrimaryKey()) {
            return false;
        } // don't allow editing items in other lists
        ...
        if (is_null($pn_parent_id)) {
            $pn_parent_id = $this->getRootItemIDForList($this->getPrimaryKey());
        }
    ...
    public function getItemsForList($pm_list_name_or_id, $pa_options=null)
    {
        ...
        if (!is_array($pa_check_access) && $pa_check_access) {
            $pa_check_access = [$pa_check_access];
        }
        ...
        if (!isset($pa_options['returnHierarchyLevels']) || !$pa_options['returnHierarchyLevels']) {
            ...
            while ($qr_res->nextRow()) {
                if ($pb_omit_root && !$qr_res->get('parent_id')) {
                    continue;
                }
            ...
            foreach ($va_list_items as $vn_i => $va_item) {
                ...
                if (is_array($pa_check_access) && (sizeof($pa_check_access) > 0) && in_array($va_item['access'], $pa_check_access)) {
                    continue;
                }
     ...
     */
    public static function getListID($pm_list_name_or_id, $pa_options=null)
    {
     ...
     * @return int list for the specified list, or null if the list does not exist
     */
    public static function getListCode($pm_list_name_or_id, $pa_options=null)
    {
    ...
    public static function getListAsHTMLFormElement($pm_list_name_or_id, $ps_name, $pa_attributes=null, $pa_options=null)
    {
        ...
        foreach ($va_list_items as $vn_item_id => $va_item) {
            ...
            if (is_array($pa_exclude_items) && (sizeof($pa_exclude_items) > 0) && in_array($va_item['idno'], $pa_exclude_items)) {
                continue;
            }

            if (!caGetOption('forceEnabled', $pa_options, false) && (!$va_item['is_enabled'] || (is_array($va_disabled_item_ids) && in_array($vn_item_id, $va_disabled_item_ids)))) {
        ...
        if (
            ...
            if (isset($omit_map[$omit_table]) && ($omit_map = $omit_map[$omit_table])) {
                ...
                if (is_array($omit_map)) {
                    ...
                    if (is_array($omit_map['omit'])) {
                        $omit_map = $omit_map['omit'];
                    }
        ...
        if (($max_columns = caGetOption('maxColumns', $pa_options, 1, ['castTo' => 'integer'])) < 1) {
            $max_columns = 1;
        }
        switch($render_as) {
            case 'radio_buttons':
...
";
            ...
            default:
                ...
                if ($render_as == 'multiple') {
                    $pa_attributes['multiple'] = 1;
                    unset($pa_options['value']);
    ...
    {
        if (!($vn_item_id = $this->getRootListItemID($pm_list_name_or_id))) {
            return null;
        }
    ...
    public function getListOfLists()
    {
        ...
        while ($qr_lists->nextRow()) {
            ...
            if (!$va_tmp['name']) {
                $va_tmp['name'] = $va_tmp['list_code'];
            }				// if there's no label then use the list_code as its name
     ...
     * @return array
     */
    public static function getListCodes($pa_options=null)
    {
     ...
     */
     public static function itemIDsToIDNOs($pa_ids, $pa_options=null)
     {
    ...
    /**
      ...
      */
     public static function itemIDsToItemValues($pa_ids, $pa_options=null)
     {
         if (!is_array($pa_ids) || !sizeof($pa_ids)) {
             return null;
         }
    ...
    /**
      ...
      */
     public static function IDNOsToItemIDs($pa_item_ids, $pa_options=null)
     {
         if (!is_array($pa_item_ids) || !sizeof($pa_item_ids)) {
             return null;
         }
     ...
     */
    public static function getAccessForItemID($pn_item_id)
    {
        $o_db = new Db();
