...
class ca_editor_ui_screens extends BundlableLabelableBaseModelWithAttributes
{
    ...
    public static $s_table_num_cache;			// cache for getTableNum()
     ...
     */
    public static function loadScreen($pm_ui, $pm_screen, $pa_options=null)
    {
        ...
        if (!$t_ui->isLoaded()) {
            $t_ui->load(['idno' => $pm_ui]);
        }
        if (!$t_ui->isLoaded()) {
            $t_ui = ca_editor_uis::find(['preferred_labels' => ['name' => $pm_ui]], ['returnAs' => 'firstModelInstance']);
        }
        if (!$t_ui->isLoaded()) {
            return null;
        }
        ...
        if (is_numeric($pm_screen)) {
            if ($t_screen->load(['ui_id' => $t_ui->getPrimaryKey(), 'screen_id' => $pm_screen])) {
                return $t_screen;
            }
        }
        if ($t_screen->load(['ui_id' => $t_ui->getPrimaryKey(), 'idno' => $pm_screen])) {
            return $t_screen;
        }
        if (ca_editor_ui_screens::find(['ui_id' => $t_ui->getPrimaryKey(), 'preferred_labels' => ['name' => $pm_screen]])) {
            return $t_screen;
        }
    ...
    public function addPlacement($ps_bundle_name, $ps_placement_code, $pa_settings, $pn_rank=null, $pa_options=null)
    {
        ...
        if ($this->getAppConfig()->get('enable_dependent_field_visibility')) {
            CompositeCache::flush('ca_metadata_elements_available_settings');
        }
    ...
    public function getAvailableBundles($pm_table_name_or_num=null, $pa_options=null)
    {
        ...
        if (!($t_instance = Datamodel::getInstanceByTableNum($pm_table_name_or_num, false))) {
            return null;
        }
        ...
        foreach ($va_defined_bundles as $vs_bundle => $va_info) {
            ...
            if (isset($va_info['displayOnly']) && $va_info['displayOnly']) {
                continue;
            }	// skip bundles meant for use in displays only
            ...
            switch ($va_info['type']) {
                ...
                case 'nonpreferred_label':
                    if (!$t_instance->getLabelTableInstance()) {
                        continue(2);
                    }
                ...
                case 'related_table':
                    if (preg_match("/^([a-z_]+)_(related_list|table)$/", $vs_bundle, $va_matches)) {
                        ...
                        if (!($t_rel = Datamodel::getInstanceByTableName($vs_bundle, true))) {
                            continue(2);
                        }
                    ...
                    if (in_array($vs_bundle, array('hierarchy_location', 'hierarchy_navigation'))) {
                        ...
                        switch($vs_bundle) {
                            ...
                            case 'history_tracking_chronology':
                                ...
                                $va_to_hide_when_using_defaults = array_values(array_filter(array_keys($va_additional_settings), function ($v) {
                                    return preg_match("!^(ca_|showDeaccessionInformation|deaccession_|dateMode)!", $v);
                                }));
                            ...
                            case 'ca_set_items':
                                ...
                                $t_set = new ca_sets();
                                if ($this->inTransaction()) {
                                    $t_set->setTransaction($this->getTransaction());
                                }
    ...
    public function isAvailableBundle($pm_table_name_or_num, $ps_bundle_name)
    {
        ...
        if (!is_array($va_available_bundles)) {
            return false;
        }
    ...
    public function addTypeRestriction($pn_type_id, $pa_settings=null)
    {
        ...
        $t_ui = new ca_editor_uis();
        if (!($t_instance = Datamodel::getInstanceByTableNum($this->getTableNum()))) {
            return false;
        }
    ...
    {
        if (!($vn_screen_id = (int)$this->getPrimaryKey())) {
            return null;
        }
    ...
    public function savePlacementsFromHTMLForm($po_request, $ps_form_prefix, $ps_placement_code)
    {
        if ($vs_bundles = $po_request->getParameter("{$ps_placement_code}{$ps_form_prefix}displayBundleList", pString)) {
            ...
            if ($this->inTransaction()) {
                $t_screen->setTransaction($this->getTransaction());
            }
    ...
    public function getTypeRestrictionsHTMLFormBundle($po_request, $ps_form_name, $ps_placement_code, $pa_options=null)
    {
        ...
        if (!($t_instance = Datamodel::getInstanceByTableNum($vn_table_num = $this->getTableNum()))) {
            return null;
        }
