...
class ca_relationship_types extends BundlableLabelableBaseModelWithAttributes
{
     ...
     */
    public static $s_relationship_type_id_cache = array();
     ...
     */
    public static $s_relationship_type_table_cache = array();

    /**
     * @var cached type_id to type_code entries; used by relationshipTypeIDsToTypeCodes()
     */
    public static $s_relationship_type_id_to_code_cache = array();
    ...
    protected function loadSubtypeLists()
    {
        ...
        if ($vn_table_num = $this->get('table_num')) {
            ...
            if (!isset($va_relationship_tables[$vn_table_num])) {
                return null;
            }
    ...
    public function getRelationshipTypeID($pm_table_name_or_num, $pm_type_code_or_id, $pn_locale_id=null, $pa_values=null, $pa_options=null)
    {
        ...
        if (!is_array($match_on = caGetOption('matchOn', $pa_options, []))) {
            $match_on = [];
        }
        $match_on = array_filter(array_map('strtolower', $match_on), function ($v) {
            return in_array($v, ['type_code', 'typecode', 'label', 'labels']);
        });
        if (!sizeof($match_on)) {
            $match_on = ['type_code'];
        }
    ...
    public function getRelationshipsUsingTypes()
    {
        ...
        foreach ($va_tables as $vs_table) {
            if (preg_match('!_x_!', $vs_table)) {
                ...
                if (!$t_instance || !$t_instance->hasField('type_id')) {
                    continue;
                }	// some relationships don't use types (eg. ca_users_x_roles)
    ...
    */
     ...
     {
         if (isset(ca_relationship_types::$s_relationship_type_table_cache[$ps_table1][$ps_table2])) {
             return ca_relationship_types::$s_relationship_type_table_cache[$ps_table1][$ps_table2];
         }
    ...
    /**
      ...
      * @return array A list of corresponding type_codes
      */
     public static function relationshipTypeIDsToTypeCodes($pa_ids, $pa_options=null)
     {
         ...
         if (!sizeof($va_ids)) {
             return ca_relationship_types::$s_relationship_type_id_to_code_cache[$vs_key] = $pa_ids;
         }
    ...
    # ------------------------------------------------------
     ...
     {
         $va_relationship_tables = $this->getRelationshipsUsingTypes();
         if (!sizeof($va_relationship_tables)) {
             return array();
         }
    ...
    {
        if (!($vn_source_id = $this->getPrimaryKey())) {
            return null;
        }
        if (!($vn_type_id = $this->getRelationshipTypeID($vn_table_num = $this->get('table_num'), $pm_type_code_or_id))) {
            return null;
        }

        if (!($vs_table_name = Datamodel::getTableName($vn_table_num))) {
            return null;
        }
        $qr_res = $this->getDb()->query("
				SELECT * 
