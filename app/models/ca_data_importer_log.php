<?php
/** ---------------------------------------------------------------------
 * app/models/ca_data_importer_log.php :
 * ----------------------------------------------------------------------
 * CollectiveAccess
...
BaseModel::$s_ca_models_definitions['ca_data_importer_log'] = array(
    ...
    'FIELDS' 			=> array(
        'log_id' => array(
                ...
                'IS_NULL' => false,
                'DEFAULT' => '',
                'LABEL' => 'Log id', 'DESCRIPTION' => 'Identifier for import'
        ),
        'user_id' => array(
        ...
        'importer_id' => array(
                ...
                'IS_NULL' => true,
                'DEFAULT' => '',
                'LABEL' => _t('Data importer used'), 'DESCRIPTION' => _t('Indicates which import mapping was used.')
        ),
        'log_datetime' => array(
                ...
                'IS_NULL' => false,
                'DEFAULT' => '',
                'LABEL' => _t('Import start date/time'), 'DESCRIPTION' => _t('Date and time import began.')
        ),
        'notes' => array(
                ...
                'IS_NULL' => false,
                'DEFAULT' => '',
                'LABEL' => _t('Notes'), 'DESCRIPTION' => _t('Notes regarding data import.'),
        ...
        'table_num' => array(
                ...
                'IS_NULL' => false,
                'DEFAULT' => '',
                'LABEL' => _t('Import target'), 'DESCRIPTION' => _t('The table into which data was imported.')
        ),
        'datafile' => array(
...
class ca_data_importer_log extends BaseModel
{
    ...
    /**
     * @property $opn_start_time Microtime recorded on call to ca_data_importer_log::beginItem(); used to calculate elapsed time on call to ca_data_importer_log::endItem()
     */
    ...
    /**
      *
      */
    public static function newEvent($pn_user_id, $ps_type, $ps_source, $ps_description)
    {
    ...
    {
        if (!($vn_log_id = $this->getPrimaryKey())) {
            return null;
        }
     ...
     */
    public static function getLastEventForSourceAndType($ps_source, $ps_type_code)
    {
        $o_db = new Db();

        $qr_res = $o_db->query("
			SELECT *
			FROM ca_data_importer_log
			WHERE
