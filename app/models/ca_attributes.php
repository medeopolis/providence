...
class ca_attributes extends BaseModel
{
    ...
    public static $s_attribute_cache_size = 1024;
    public static $s_get_attributes_cache = array();
    public static $s_ca_attributes_element_instance_cache = array();
    ...
    public function addAttribute($pn_table_num, $pn_row_id, $pm_element_code_or_id, $pa_values, $pa_options=null)
    {
        ...
        if (!isset($pa_values['locale_id']) || !$pa_values['locale_id']) {
            $pa_values['locale_id'] = $g_ui_locale_id;
        }
        if (isset($pa_values['locale_id'])) {
    ...
    public function editAttribute($pa_values, $pa_options=null)
    {
        ...
        foreach ($va_attr_vals as $o_attr_val) {
            ...
            if ($t_attr_val->load($o_attr_val->getValueID())) {
                $va_element = array_shift(array_filter($va_elements, function ($e) use ($vn_element_id) {
                    return $e['element_id'] == $vn_element_id;
                }));
     ...
     * @return array
     */
    public static function getAttributes($po_db, $pn_table_num, $pn_row_id, $pa_element_ids, $pa_options=null)
    {
        $pb_no_cache = (isset($pa_options['noCache']) && $pa_options['noCache']);
        if ($pb_no_cache) {
            $pa_options['resetCache'] = true;
        }
     ...
     */
    public static function getRawAttributeValuesForIDs($po_db, $pn_table_num, $pa_row_ids, $pn_element_id, $pa_options=null)
    {
        $qr_attrs = $po_db->query("
			SELECT 
				caa.attribute_id, caa.locale_id, caa.element_id element_set_id, caa.row_id, caa.value_source,
     ...
     */
    public static function getAttributeCount($po_db, $pn_table_num, $pn_row_id, $pn_element_id, $pa_options=null)
    {
     ...
     * Returns a list (indexed array) of Attribute objects.
     */
    public static function getReferencedAttributes($po_db, $pn_table_num, $pa_reference_limit_ids, $pa_options=null)
    {
        if (!is_array($pa_options)) {
            $pa_options = array();
        }
