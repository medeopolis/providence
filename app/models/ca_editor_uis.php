...
class ca_editor_uis extends BundlableLabelableBaseModelWithAttributes
{
    ...
    public static $s_available_ui_cache = [];
    public static $s_default_ui_cache = [];
    ...
    public static $s_loaded_relationship_tables = false;
    public static $s_screen_cache = [];
    public static $s_screen_bundle_cache = [];
    public static $s_placements_for_bundle_cache = [];
    ...
    public function set($pa_fields, $pm_value="", $pa_options=null)
    {
        if ($this->getPrimaryKey()) {
            if (is_array($pa_fields)) {
                ...
                if (isset($pa_fields['editor_type'])) {
                    unset($pa_fields['editor_type']);
                }
            } else {
                if ($pa_fields === 'user_id') {
                    return false;
                }
                if ($pa_fields === 'editor_type') {
                    return false;
                }
            }
        }
    ...
    public static function loadDefaultUI($pm_table_name_or_num, $po_request, $pn_type_id=null, $pa_options=null)
    {
        ...
        if ($vn_type_id && isset($va_uis_by_type[$vn_type_id])) {
            if ($t_ui = ca_editor_uis::findAsInstance(['ui_id' => $va_uis_by_type[$vn_type_id]])) {
                return $t_ui;
            }
        }
        if (isset($va_uis_by_type['__all__'])) {
            if ($t_ui = ca_editor_uis::findAsInstance(['ui_id' => $va_uis_by_type['__all__']])) {
                return $t_ui;
            }
        }
        ...
        if (!$vn_type_id || !(is_array($va_uis_by_type) && ($vn_rc = $t_ui->load($va_uis_by_type[$vn_type_id])))) {
            ...
            if (!is_array($va_ui_ids) || (sizeof($va_ui_ids) == 0)) {
                return ca_editor_uis::$s_default_ui_cache[$pm_table_name_or_num.'/'.$pn_type_id] = false;
            }
    ...
    public function getScreens($pn_type_id=null, $pa_options=null)
    {
        ...
        if (isset(self::$s_screen_cache[$vs_cache_key])) {
            return self::$s_screen_cache[$vs_cache_key];
        }
        if (!($t_instance = Datamodel::getInstanceByTableNum($this->get('editor_type')))) {
            return null;
        }
        ...
        while ($qr_res->nextRow()) {
            ...
            if ($qr_res->get('restriction_type_id')) {
                ...
                if (!isset($va_types[$qr_res->get('restriction_type_id')])) {
                    continue;
                }
        ...
        if ($vb_ids_only) {
            return self::$s_screen_cache[$vs_cache_key] = $va_ids;
        }
    ...
    public function getScreenBundlePlacements($pm_screen, $pn_type_id=null, $pa_options=null)
    {
        ...
        if (isset(self::$s_screen_bundle_cache[$vs_cache_key])) {
            return self::$s_screen_bundle_cache[$vs_cache_key];
        }
        ...
        if ($va_bundle_list = caGetOption('bundleList', $pa_options, null)) {
            if (!is_array($va_bundle_list)) {
                $va_bundle_list = [$va_bundle_list];
            }
        ...
        if (sizeof($screen_ids) > 0) {
            ...
            $va_placements = array_map(function ($v) use ($labels) {
                $v['screen_label'] = $labels[$v['screen_id']];
                return $v;
            }, $va_placements);
        }
    ...
    public function getPlacementsForBundle($ps_bundle_name, $po_request=null, $pa_options=null)
    {
        ...
        if (isset(self::$s_placements_for_bundle_cache[$vs_cache_key])) {
            return self::$s_placements_for_bundle_cache[$vs_cache_key];
        }
    ...
    public static function getUIList($pm_table=null, $pn_user_id=null, $pn_type_id=null)
    {
        ...
        if (ca_editor_uis::$s_available_ui_cache[$pm_table.'/'.$pn_user_id]) {
            return ca_editor_uis::$s_available_ui_cache[$pm_table.'/'.$pn_user_id];
        }
        $o_db = new Db();

        $va_wheres = $va_params = [];

        if (!is_array($va_type_list = caMakeTypeIDList($pn_table_num, array($pn_type_id)))) {
            $va_type_list = [];
        }
    ...
    public static function getAvailableUIs($pn_table_num, $po_request, $pn_type_id=null)
    {
        ...
        if (ca_editor_uis::$s_available_ui_cache[$pn_table_num.'/'.$pn_type_id.'/'.$pn_user_id]) {
            return ca_editor_uis::$s_available_ui_cache[$pn_table_num.'/'.$pn_type_id.'/'.$pn_user_id];
        }

        if ($pn_type_id) {
            $va_ui_list = $po_request->user->_getUIListByType($pn_table_num);
            if (!is_array($va_uis = $va_ui_list[$pn_type_id])) {
                $va_uis = [];
            }
    ...
    public function addTypeRestriction($pn_type_id, $pa_settings=null)
    {
        ...
        if ($t_instance instanceof BaseRelationshipModel) { // interstitial type restriction incoming
            ...
            if (!isset($va_rel_type_list[$pn_type_id])) {
                return false;
            }
    ...
    {
        if (!($vn_ui_id = (int)$this->getPrimaryKey())) {
            return null;
        }		// ui must be loaded
    ...
    public function getTypeRestrictionsHTMLFormBundle($po_request, $ps_form_name, $ps_placement_code, $pa_options=null)
    {
        ...
        if (!($t_instance = Datamodel::getInstanceByTableNum($vn_table_num = $this->get('editor_type')))) {
            return null;
        }
    ...
    public function createScreenFromPawtucketDetailTemplate($ps_path_to_template, $ps_screen_name, array $pa_options=null)
    {
        ...
        if (!($vs_template_content = file_get_contents($ps_path_to_template))) {
            return null;
        }
