...
class ca_tours extends BundlableLabelableBaseModelWithAttributes
{
    ...
    public static $s_stop_info_cache;
    ...
    public function getListOfTours()
    {
        ...
        while ($qr_lists->nextRow()) {
            ...
            if (!$va_tmp['name']) {
                $va_tmp['name'] = $va_tmp['tour_code'];
            }				// if there's no label then use the tour_code as its' name
    ...
    public function getStopCount()
    {
        ...
        if (ca_tours::$s_stop_info_cache[$this->getPrimaryKey()]) {
            return sizeof(ca_tours::$s_stop_info_cache[$this->getPrimaryKey()]);
        }
