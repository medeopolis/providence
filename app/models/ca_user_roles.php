...
class ca_user_roles extends BaseModel
{
    ...
    public static $s_action_list;
    public static $s_bundle_list;
    ...
    public function getTypeAccessSettings()
    {
        ...
        if (!$this->getAppConfig()->get('perform_type_access_checking')) {
            array();
        }
    ...
    public function setAccessSettingForType($ps_table, $pm_type_id_or_code, $pn_access)
    {
        ...
        if (!is_numeric($pm_type_id_or_code)) {
            if (!$t_list->itemIsInList($vs_list_code, $pm_type_id_or_code)) {
                return false;
            }
            $pm_type_id_or_code = ca_lists::getItemID($vs_list_code, $pm_type_id_or_code);
        }

        if (!$t_list->itemIDIsInList($vs_list_code, $pm_type_id_or_code)) {
            return false;
        }
    ...
    public function getSourceAccessSettings()
    {
        ...
        if (!$this->getAppConfig()->get('perform_source_access_checking')) {
            array();
        }
    ...
    {
        if (!is_array($pa_role_ids) || (sizeof($pa_role_ids) === 0)) {
            return array();
        }
    ...
    private static function _evaluateActionRequirements($pa_requirements, $pa_options=null)
    {
        if (sizeof($pa_requirements) == 0) {
            return true;
        }	// empty requirements means show the action
