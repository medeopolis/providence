...
class ca_bundle_displays extends BundlableLabelableBaseModelWithAttributes
{
    ...
    public static $s_have_access_to_display_cache = [];

    public static $s_placement_list_cache;		// cache for getPlacements()
    ...
    public function set($pa_fields, $pm_value="", $options=null)
    {
        if ($this->getPrimaryKey()) {
            if (is_array($pa_fields)) {
                if (isset($pa_fields['user_id'])) {
                    unset($pa_fields['user_id']);
                }
                if (isset($pa_fields['table_num'])) {
                    unset($pa_fields['table_num']);
                }
            } else {
                if ($pa_fields === 'user_id') {
                    return false;
                }
                if ($pa_fields === 'table_num') {
                    return false;
                }
            }
        }
                                                    ...
                                                    if ($t_element->get("list_id") > 0) {
                                                        if ($t_list->numItemsInList($t_element->get("list_id")) > 500) {
                                                            ...
                                                            while ($qr_list_items->nextHit()) {
                                                                if (!($v = trim($qr_list_items->get('ca_list_items.hierarchy.preferred_labels.name_plural', ['delimiter' => $ps_hierarchical_delimiter])))) {
                                                                    continue;
                                                                }
     ...
     */
     public function getBundleDisplays($options=null)
     {
         ...
         $pa_restrict_to_types = array_filter($pa_restrict_to_types, function ($v) {
             return ($v == '*') ? false : (bool)$v;
         });

         $pb_system_only = 									caGetOption('systemOnly', $options, false);

         if ($pm_table_name_or_num && !($vn_table_num = Datamodel::getTableNum($pm_table_name_or_num))) {
             return [];
         }
    ...
    public function getBundleDisplaysAsHTMLSelect($ps_select_name, $pa_attributes=null, $options=null)
    {
        ...
        if (!isset($options['dontIncludeSubtypesInTypeRestriction'])) {
            $options['dontIncludeSubtypesInTypeRestriction'] = true;
        }
        ...
        foreach ($va_available_displays as $vn_display_id => $info) {
            if ($ps_context && is_array($info['settings']['show_only_in']) && sizeof($info['settings']['show_only_in']) && !in_array($ps_context, $info['settings']['show_only_in'])) {
                continue;
            }
    ...
    public function getBundleDisplayTypeName($pm_table_name_or_num=null, $options=null)
    {
        ...
        if (!($vn_table_num = Datamodel::getTableNum($pm_table_name_or_num))) {
            return null;
        }
    ...
    public function haveAccessToDisplay($pn_user_id, $pn_user_access, $pn_display_id=null)
    {
        if ($pn_display_id) {
            ...
            if ($this->inTransaction()) {
                $t_disp->setTransaction($this->getTransaction());
            }
    ...
    public function getAvailableBundles($pm_table_name_or_num=null, $options=null)
    {
        ...
        foreach ($t_instance->getFormFields() as $vs_f => $info) {
            if (isset($info['DONT_USE_AS_BUNDLE']) && $info['DONT_USE_AS_BUNDLE']) {
                continue;
            }
    ...
    public function getPlacementsInDisplay($options=null)
    {
        ...
        if (!($pn_table_num = Datamodel::getTableNum($this->get('table_num')))) {
            return null;
        }
        if (!($t_instance = Datamodel::getInstance($pn_table_num, true))) {
            return null;
        }

        if (!is_array($placements = $this->getPlacements($options))) {
            $placements = [];
        }
    ...
    public function getDisplayValue($po_result, $pn_placement_id, $options=null)
    {
        ...
        if (method_exists($po_result, 'filterNonPrimaryRepresentations')) {
            $po_result->filterNonPrimaryRepresentations(!$vb_include_nonprimary_media);
        }

        if (!isset($options['convertCodesToDisplayText'])) {
            $options['convertCodesToDisplayText'] = true;
        }
        if (!isset($options['forReport'])) {
            $options['forReport'] = false;
        }
        if (!isset($options['purify'])) {
            $options['purify'] = false;
        }
        if (!isset($options['asHTML'])) {
            $options['asHTML'] = true;
        }

        if (!isset($options['maximumLength'])) {
            $options['maximumLength'] =  ($va_settings['maximum_length']) ? $va_settings['maximum_length'] : null;
        }
        if (!isset($options['filter'])) {
            $options['filter'] = caGetOption('filter', $va_settings, null);
        }
        ...
        if (!$pb_show_hierarchy && $vs_template) {
            ...
            if ($t_instance = Datamodel::getInstanceByTableName($va_bundle_bits[0], true)) {
                ...
                if ($vb_is_related) {
                    ...
                    $vs_val = $po_result->getWithTemplate(
                        $vs_template,
                        [
        ...
        if ($vb_return_info) {
            if (!$t_instance) {
                $t_instance = Datamodel::getInstanceByTableName($va_bundle_bits[0], true);
            }
    ...
    public function addTypeRestriction($pn_type_id, $pa_settings=null)
    {
        ...
        if (!($t_instance = Datamodel::getInstanceByTableNum($this->get('table_num')))) {
            return false;
        }
    ...
    {
        if (!($vn_display_id = (int)$this->getPrimaryKey())) {
            return null;
        }

        $va_params = ['display_id' => $vn_display_id];
        if ((int)$pn_type_id > 0) {
            $va_params['type_id'] = (int)$pn_type_id;
        }
    ...
    public function getTypeRestrictionsHTMLFormBundle($request, $ps_form_name, $ps_placement_code, $options=null)
    {
        ...
        if (is_array($va_type_restrictions)) {
            foreach ($va_type_restrictions as $i => $va_restriction) {
                ...
                if ($va_restriction['include_subtypes'] && !$vb_include_subtypes) {
                    $vb_include_subtypes = true;
                }
            }
        }

        if (!($t_instance = Datamodel::getInstanceByTableNum($vn_table_num = $this->get('table_num')))) {
            return null;
        }
     ...
     */
    public static function makeBundlesForResultsEditor($bundles, $settings=null)
    {
        $placements = [];
     ...
     * @return array
     */
    public static function getColumnsForResultsEditor($display_list, $options=null)
    {
        $request = caGetOption('request', $options, null);
    ...
    {
        if (!$t_subject = Datamodel::getInstanceByTableName($table, true)) {
            return null;
        }
        $request = caGetOption('request', $options, null);
            ...
            foreach ($changes as $va_change) {
                ...
                if ($t_subject->load($id)) {
                    ...
                    if ($placement_id > 0) {
                        ...
                        if (!isset($bundles['displayList'][(int)$placement_id])) {
                            continue;
                        }
                        $bundle = [
                            'placement_id' => $placement_id,
    ...
    {
        if (!($request = caGetOption('request', $options, null))) {
            return null;
        }
        if (!($t_subject = Datamodel::getInstanceByTableName($table, true))) {
            return null;
        ...
        if ($idno_context_field = $t_subject->getProperty('ID_NUMBERING_CONTEXT_FIELD')) {
            ...
            if ($context_id) {
                $t_subject->set($idno_context_field, $context_id);
            }
        }
        ...
        $vb_save_rc = $t_subject->saveBundlesForScreen(
            null,
            $request,
