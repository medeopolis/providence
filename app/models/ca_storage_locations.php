    ...
    public function saveBundlesForScreen($screen, $request, &$options)
    {
        ...
        if (($rc = parent::saveBundlesForScreen($screen, $request, $options)) && $parent_changed) {
            ...
            if ($old_parent_id === $new_parent_id) {
                return $rc;
            }	// don't track if there's no actual movement
            ...
            if (!is_array($policies = ca_movements::getDependentHistoryTrackingCurrentValuePolicies('ca_movements'))) {
                return $rc;
            }
            ...
            if ($movement_form_name && $movement_form_screen) {
                foreach ($policies as $policy_code => $policy) {
                    if (!is_array($policy_elements = caGetOption('elements', $policy, null))) {
                        continue;
                    }
                    ...
                    if (isset($policy_elements['ca_movements'])) {
                        ...
                        if (!$policy_config) {
                            continue;
                        }	// movements are not configured for storage location tracking
                        ...
                        // Check movement type
                        if (!$movement_type || !$t_movement->getTypeIDForCode($movement_type)) {
                            $movement_type = $t_movement->getDefaultTypeID();
                        }
                        ...
                        if (!is_array($sub_location_ids = $this->getHierarchyIDs($this->getPrimaryKey()))) {
                            $sub_location_ids = [];
                        }
