...
class GlobalChangeController extends ActionController
{
    ...
    public function Index()
    {
        ...
        if (!$filter_table && !isset($_REQUEST['filter_table'])) {
            $filter_table = Session::getVar('global_change_log_filter_table');
        }
        ...
        if (!$filter_change_type && !isset($_REQUEST['filter_change_type'])) {
            $filter_change_type = Session::getVar('global_change_log_filter_change_type');
        }
        ...
        if (!$filter_daterange && !isset($_REQUEST['filter_daterange'])) {
            $filter_daterange = Session::getVar('global_change_log_filter_daterange');
        }
        ...
        if ($can_filter_by_user = $this->request->user->canDoAction('can_view_change_logs')) {
            ...
            if (!in_array($filter_user_id, $user_list)) {
                $filter_user_id = null;
            }
        } else {
        ...
        if (!($page = $this->request->getParameter('page', pInteger))) {
            $page = 0;
        }
