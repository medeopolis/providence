...
class QuickSearchController extends BaseFindController
{
    ...
    public function Index($pa_options=null)
    {
        ...
        if (!$ps_search) {
            $ps_search = Session::getVar('quick_search_last_search');
        }
        ...
        foreach ($va_searches as $vs_target => $va_sorts) {
            ...
            if (!($vo_result = $this->_doSearch($vs_table, $ps_search.$search_suffix, $va_sorts[$ps_sort], $vs_type))) {
                unset($va_searches[$vs_target]);
                continue;
            }
    ...
    private function _doSearch($ps_target, $ps_search, $ps_sort, $ps_type=null)
    {
        ...
        if (!$this->request->user->canDoAction('can_search_'.(($ps_target == 'ca_tour_stops') ? 'ca_tours' : $ps_target))) {
            return '';
        }
        ...
        $search_opts = [
            ...
            'limit' => $this->opn_num_results_per_item_type, 'no_cache' => $vb_no_cache,
        ...
        switch($ps_target) {
            case 'ca_objects':
                ...
                if ($ps_type) {
                    $o_object_search->setTypeRestrictions([$ps_type], ['includeSubtypes' => false]);
                }
                return $o_object_search->search($ps_search, $search_opts);
                break;
            case 'ca_object_lots':
                $o_object_lots_search = new ObjectLotSearch();
                if ($ps_type) {
                    $o_object_lots_search->setTypeRestrictions([$ps_type], ['includeSubtypes' => false]);
                }
                return $o_object_lots_search->search($ps_search, $search_opts);
                break;
            case 'ca_entities':
                $o_entity_search = new EntitySearch();
                if ($ps_type) {
                    $o_entity_search->setTypeRestrictions([$ps_type], ['includeSubtypes' => false]);
                }
                return $o_entity_search->search($ps_search, $search_opts);
                break;
            case 'ca_places':
                $o_place_search = new PlaceSearch();
                if ($ps_type) {
                    $o_place_search->setTypeRestrictions([$ps_type], ['includeSubtypes' => false]);
                }
                return $o_place_search->search($ps_search, $search_opts);
                break;
            case 'ca_occurrences':
                $o_occurrence_search = new OccurrenceSearch();
                if ($ps_type) {
                    $o_occurrence_search->setTypeRestrictions([$ps_type], ['includeSubtypes' => false]);
                }
                return $o_occurrence_search->search($ps_search, $search_opts);
                break;
            case 'ca_collections':
                $o_collection_search = new CollectionSearch();
                if ($ps_type) {
                    $o_collection_search->setTypeRestrictions([$ps_type], ['includeSubtypes' => false]);
                }
                return $o_collection_search->search($ps_search, $search_opts);
                break;
            case 'ca_storage_locations':
                $o_storage_location_search = new StorageLocationSearch();
                if ($ps_type) {
                    $o_storage_location_search->setTypeRestrictions([$ps_type], ['includeSubtypes' => false]);
                }
            ...
            case 'ca_loans':
                ...
                if ($ps_type) {
                    $o_loan_search->setTypeRestrictions([$ps_type], ['includeSubtypes' => false]);
                }
                return $o_loan_search->search($ps_search, $search_opts);
                break;
            case 'ca_movements':
                $o_movement_search = new MovementSearch();
                if ($ps_type) {
                    $o_movement_search->setTypeRestrictions([$ps_type], ['includeSubtypes' => false]);
                }
                return $o_movement_search->search($ps_search, $search_opts);
                break;
            case 'ca_object_representations':
                $o_rep_search = new ObjectRepresentationSearch();
                if ($ps_type) {
                    $o_rep_search->setTypeRestrictions([$ps_type], ['includeSubtypes' => false]);
                }
                return $o_rep_search->search($ps_search, $search_opts);
                break;
            case 'ca_tours':
                $o_tour_search = new TourSearch();
                if ($ps_type) {
                    $o_tour_search->setTypeRestrictions([$ps_type], ['includeSubtypes' => false]);
                }
                return $o_tour_search->search($ps_search, $search_opts);
                break;
            case 'ca_tour_stops':
                $o_tour_stop_search = new TourStopSearch();
                if ($ps_type) {
                    $o_tour_stop_search->setTypeRestrictions([$ps_type], ['includeSubtypes' => false]);
                }
                return $o_tour_stop_search->search($ps_search, $search_opts);
                break;
            default:
                return null;
                break;
