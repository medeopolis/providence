...
class PawtucketController extends ActionController
{
    ...
    public function pages()
    {
        ...
        if (!$this->request->getUser()->canDoAction('can_edit_ca_site_pages')) {
            throw new ApplicationException("No access");
        }
    ...
    {
        if (!$this->request->getUser()->canDoAction('can_edit_theme_global_values')) {
            throw new ApplicationException("No access");
        }

        $o_appvars = new ApplicationVars();

        if (!is_array($va_toolbar_config = $this->request->config->getAssoc('wysiwyg_editor_toolbar'))) {
    ...
    public function saveGlobalValues()
    {
        ...
        if (caGetGlobalValuesCount() == 0) {
            throw new ApplicationException("No global values defined");
        }
