...
class ToolsController extends ActionController {
	...
	public function Settings() {
		...
		$this->view->setVar('available_settings', $settings = $o_tool->getAvailableSettings());
		
		$settings_visibility_map = [];
		
		if(is_array($valid_commands = $o_tool->getCommands())) {
			foreach($settings as $setting => $info) {
				if(is_array($info['commands'] ?? null)) {
					foreach($info['commands'] as $c) {
						$settings_visibility_map[$c][] = $setting;
					}
				} else {
					foreach($valid_commands as $c) {
						$settings_visibility_map[$c][] = $setting;
					}
				}
			}
