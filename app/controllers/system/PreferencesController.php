...
class PreferencesController extends ActionController
{
    ...
    public function EditQuickSearchPrefs()
    {
        ...
        if (!is_array($va_search_list = $this->request->user->getPreference("quicksearch_search_list"))) {
            $va_search_list = [];
        }
        $va_selected_searches = array_filter($va_search_list, "strlen");
        if (!is_array($va_selected_searches) || !sizeof($va_selected_searches)) {
            $va_selected_searches = array_keys(QuickSearch::availableSearches(['expandByType' => true]));
        }
    ...
    public function EditDuplicationPrefs()
    {
        ...
        if (!is_array($va_duplication_element_settings = $this->request->user->getPreference($vs_current_table.'_duplicate_element_settings'))) {
            $va_duplication_element_settings = [];
        }
        ...
        foreach ($va_available_bundles as $vs_bundle_name => $va_bundle_info) {
            if (Datamodel::tableExists($vs_bundle_name)) {
                continue;
            }
        ...
        switch($vs_action) {
            ...
            case 'EditDuplicationPrefs':
                ...
                if (in_array($vs_current_table, PreferencesController::$s_duplicable_tables)) {
                    $this->view->setVar('current_table', $vs_current_table);
                    foreach ($this->request->user->getValidPreferences($vs_group) as $vs_pref) {
                        if (!$this->getRequest()->getUser()->isValidPreference("{$vs_current_table}_{$vs_pref}")) {
                            continue;
                        }
            ...
            case 'EditQuickSearchPrefs':
                ...
                $va_bundle_list = array_unique(array_map(function ($v) {
                    return preg_replace("!_[\d]+$!", "", $v);
                }, explode(';', $this->request->getParameter('displayBundleList', pString))));
