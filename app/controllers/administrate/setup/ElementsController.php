...
class ElementsController extends BaseEditorController
{
    ...
    public function Save($pa_values=null)
    {
        ...
        foreach ($t_element->getFormFields() as $vs_f => $va_field_info) {
            if ((bool)$t_element->getAppConfig()->get('ca_metadata_elements_dont_allow_editing_of_codes_when_in_use') && $t_element->getPrimaryKey()) {
                continue;
            }
            if ((bool)$t_element->getAppConfig()->get('ca_metadata_elements_dont_allow_editing_of_data_types_when_in_use') && $t_element->getPrimaryKey()) {
                continue;
            }
        ...
        if ($t_element->getPrimaryKey()) {
            ...
            foreach ($va_request as $vs_key => $vs_val) {
                if (strpos($vs_key, 'element_labels_Pref') !== false) { /* label field */
                    ...
                    } elseif (!(strpos($vs_key, '_delete')===false)) { /* delete label */
