<?php
/* ----------------------------------------------------------------------
 * themes/default/views/manage/Results/ca_item_tags_paging_controls_html.php
 * ----------------------------------------------------------------------
 * CollectiveAccess
...
if ($this->getVar('num_pages') > 1) {
    ...
    if ($this->getVar('page') > 1) {
        $vs_searchNav .= "<a href='#' onclick='jQuery(\"#resultBox\").load(\"".caNavUrl($this->request, 'manage', 'Tags', 'Index', $va_previous_link_params)."\"); return false;' class='button'>&lsaquo; "._t("Previous")."</a>";
    }
