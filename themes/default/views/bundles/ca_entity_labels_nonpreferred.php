...
$read_only				= ((isset($settings['readonly']) && $settings['readonly'])  || ($this->request->user->getBundleAccessLevel('ca_entities', 'nonpreferred_labels') == __CA_BUNDLE_ACCESS_READONLY__));
		...
		<div id="{fieldNamePrefix}Label_{n}" class="labelInfo">
			...
			<table class="objectRepresentationListItem">
				<tr valign="middle">
					<td>
<?php
    $vs_label_list = $this->request->config->get('ca_entities_nonpreferred_label_type_list');
...
<?php
            if ($use_suffix_for_orgs) {
                ?>
								<td>
									<?= $t_label->htmlFormElement('suffix', null, array('name' => "{fieldNamePrefix}suffix_{n}", 'id' => "{fieldNamePrefix}suffix_{n}", "value" => "{{suffix}}", 'no_tooltips' => false, 'tooltip_namespace' => 'bundle_ca_entity_labels_nonpreferred')); ?>
								</td>
<?php
            }
        ?>
    ...
    case 'IND_SM':
        ?>
						<table>
							<tr>
								<td>
									<?= $t_label->htmlFormElement('prefix', null, array('name' => "{fieldNamePrefix}prefix_{n}", 'id' => "{fieldNamePrefix}prefix_{n}", "value" => "{{prefix}}", 'no_tooltips' => false, 'tooltip_namespace' => 'bundle_ca_entity_labels_nonpreferred')); ?>
								</td>
								<td>
									<?= $t_label->htmlFormElement('forename', null, array('name' => "{fieldNamePrefix}forename_{n}", 'id' => "{fieldNamePrefix}forename_{n}", "value" => "{{forename}}", 'no_tooltips' => false, 'tooltip_namespace' => 'bundle_ca_entity_labels_nonpreferred')); ?>
								</td>
								<td>
									<?= $t_label->htmlFormElement('middlename', null, array('name' => "{fieldNamePrefix}middlename_{n}", 'id' => "{fieldNamePrefix}middlename_{n}", "value" => "{{middlename}}", 'no_tooltips' => false, 'tooltip_namespace' => 'bundle_ca_entity_labels_nonpreferred')); ?>
								</td>
								<td>
									<?= $t_label->htmlFormElement('surname', null, array('name' => "{fieldNamePrefix}surname_{n}", 'id' => "{fieldNamePrefix}surname_{n}", "value" => "{{surname}}", 'no_tooltips' => false, 'tooltip_namespace' => 'bundle_ca_entity_labels_nonpreferred')); ?>
								</td>
								<td>
									<?= $t_label->htmlFormElement('suffix', null, array('name' => "{fieldNamePrefix}suffix_{n}", 'id' => "{fieldNamePrefix}suffix_{n}", "value" => "{{suffix}}", 'no_tooltips' => false, 'tooltip_namespace' => 'bundle_ca_entity_labels_nonpreferred')); ?>
								</td>
    ...
    default:
        ?>
						<table>
							...
							<tr>
								<td>
									<?= $t_label->htmlFormElement('other_forenames', null, array('name' => "{fieldNamePrefix}other_forenames_{n}", 'id' => "{fieldNamePrefix}other_forenames_{n}", "value" => "{{other_forenames}}", 'no_tooltips' => false, 'tooltip_namespace' => 'bundle_ca_entity_labels_nonpreferred')); ?>
								</td>
								<td colspan="4"><?= $t_label->htmlFormElement('displayname', null, array('name' => "{fieldNamePrefix}displayname_{n}", 'id' => "{fieldNamePrefix}displayname_{n}", "value" => "{{displayname}}", 'no_tooltips' => false, 'tooltip_namespace' => 'bundle_ca_entity_labels_nonpreferred', 'textAreaTagName' => 'textentry', 'readonly' => $read_only)); ?></td>
							</tr>
							<tr>
								<td colspan="5">
									<div class="formLabel">
...
<?php
    print TooltipManager::getLoadHTML('bundle_ca_entity_labels_nonpreferred');
?>
	</textarea>
...
<script type="text/javascript">
	caUI.initLabelBundle('#<?= $id_prefix; ?>NPLabels', {
		...
		templateValues: ['displayname', 'prefix', 'forename', 'other_forenames', 'middlename', 'surname', 'suffix', 'type_id', 'locale_id', 'effective_date', 'access', 'source_info'],
