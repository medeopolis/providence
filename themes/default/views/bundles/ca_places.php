...
if (!RequestHTTP::isAjax()) {
    if (caGetOption('showCount', $va_settings, false)) {
        print $count ? "({$count})" : '';
    }
...
<?php
                if (!$vb_read_only && ca_editor_uis::loadDefaultUI($t_item_rel->tableNum(), $this->request)) {
                    ?><a href="#" class="caInterstitialEditButton listRelEditButton"><?= caNavIcon(__CA_NAV_ICON_INTERSTITIAL_EDIT_BUNDLE__, "18px"); ?></a><?php
                }
...
<?php
                        print caGetRelationDisplayString($this->request, 'ca_places', array('class' => 'caEditItemButton', 'id' => "{$vs_id_prefix}_edit_related_{n}"), array('display' => '_display', 'makeLink' => true, 'prefix' => $vs_id_prefix, 'relationshipTypeDisplayPosition' => $dont_show_relationship_type));
            ?>
			</span>
			...
			<table class="caListItem">
				<tr>
					...
					<td>
						...
						<a href="<?= urldecode(caEditorUrl($this->request, 'ca_places', '{place_id}')); ?>" class="caEditItemButton" id="<?= $vs_id_prefix; ?>_edit_related_{n}"><?= caNavIcon(__CA_NAV_ICON_GO__, 1); ?></a>
					</td>
				</tr>
			</table>
<?php
...
<?php
        ...
        ?>
			<div style='width: 690px; height: <?= $va_settings['hierarchicalBrowserHeight']; ?>;'>
				
				<div id='<?= $vs_id_prefix; ?>_hierarchyBrowser{n}' style='width: 100%; height: 100%;' class='hierarchyBrowser'>
			...
			<script type='text/javascript'>
				jQuery(document).ready(function() { 
					var <?= $vs_id_prefix; ?>oHierBrowser{n} = caUI.initHierBrowser('<?= $vs_id_prefix; ?>_hierarchyBrowser{n}', {
						...
						//initItemID: <?= (int)Session::getVar('ca_places_browse_last_id'); ?>,
...
<div id="caRelationEditorPanel<?= $vs_id_prefix; ?>" class="caRelationQuickAddPanel"> 
	...
	<textarea class='caBundleDisplayTemplate' style='display: none;'>
		<?= caGetRelationDisplayString($this->request, 'ca_places', array(), array('display' => '_display', 'makeLink' => false, 'relationshipTypeDisplayPosition' => $dont_show_relationship_type)); ?>
	</textarea>
</div>	
...
<?php } ?>
		...
		caRelationBundle<?= $vs_id_prefix; ?> = caUI.initRelationBundle('#<?= $vs_id_prefix; ?>', {
			...
			autocompleteUrl: '<?= caNavUrl($this->request, 'lookup', 'Place', 'Get', $va_lookup_params); ?>',
...
<?php if ($vb_quick_add_enabled) { ?>
			...
			quickaddUrl: '<?= caNavUrl($this->request, 'editor/places', 'PlaceQuickAdd', 'Form', array('place_id' => 0, 'dont_include_subtypes_in_type_restriction' => (int)$va_settings['dont_include_subtypes_in_type_restriction'], 'prepopulate_fields' => join(";", $va_settings['prepopulateQuickaddFields']))); ?>',
<?php } ?>
			...
			partialLoadUrl: '<?= caNavUrl($this->request, '*', '*', 'loadBundleValues', array($t_subject->primaryKey() => $t_subject->getPrimaryKey(), 'placement_id' => $vn_placement_id, 'bundle' => 'ca_places')); ?>',
