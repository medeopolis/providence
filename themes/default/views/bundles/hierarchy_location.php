...
if (!($min_autocomplete_search_length = (int)$t_subject->getAppConfig()->get(["{$vs_priv_table}_autocomplete_minimum_search_length", "autocomplete_minimum_search_length"]))) {
    $min_autocomplete_search_length = 3;
}
...
$hier_browser_dims = [];
if ($hier_browser_width) {
    $hier_browser_dims[] = "width: {$hier_browser_width};";
}
if ($hier_browser_height) {
    $hier_browser_dims[] = "height: {$hier_browser_height};";
}
...
<?php
                    ...
                    if (!$vb_batch && ($pn_id > 0)) {
                        ...
                        ?>
				<div class="hierarchyCountDisplay"><?php if ($vn_items_in_hier > 0) {
				    print _t("Number of %1 in hierarchy: %2", $vs_count_label, $vn_items_in_hier);
				} ?></div>
...
<?php
                ...
                if (($pn_parent_id > 0) && !$strict_type_hierarchy) {
