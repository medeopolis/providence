...
if ($can_filter_by_user) {
    print caFormControlBox(
        ...
        '<div class="list-filter" style="margin-top: -5px; margin-left: -5px; font-weight: normal;">'._t(
            'Show %1 to %2 from %3 by %4',
            ...
            caHTMLTextInput('filter_daterange', array('size' => 12, 'value' => ($filter_daterange) ? $filter_daterange : _t('any time'), 'class' => 'dateBg')),
            caHTMLSelect('filter_user', array_merge([_t('any user') => ''], $user_list), [], ['value' => $filter_user_id, 'width' => '140px'])
        ).'</div>',
					...
					<td>
						<?php
                                            if ($va_log_entry_list['subject'] !== _t('&lt;MISSING&gt;')) {
                                                print "<a href='".caEditorUrl($this->request, $va_log_entry_list[0]['subject_table_num'], $va_log_entry_list[0]['subject_id'])."'>".$va_log_entry_list[0]['subject']."</a></span><br/>";
                                            } else {
