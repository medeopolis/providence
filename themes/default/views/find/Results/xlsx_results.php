...
if (
    ...
    &&
    !sizeof(array_filter($va_display_list, function ($v) {
        return preg_match('!^ca_object_representations.media!', $v['bundle']);
    }))
) {
...
while ($vo_result->nextHit()) {
    ...
    foreach ($va_display_list as $va_info) {
        ...
        if (is_array($va_info['settings']) && isset($va_info['settings']['format']) && ($tags = array_filter(caGetTemplateTags($va_info['settings']['format']), function ($v) {
            return preg_match("!^ca_object_representations.media.!", $v);
        }))) {
        ...
        if (
            ...
            if (!in_array($vs_version, $va_media_versions)) {
                $vs_version = $va_media_versions[0];
            }
