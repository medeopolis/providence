<?php
/* ----------------------------------------------------------------------
 * themes/default/views/find/Results/ca_entities_results_full_html.php
 * ----------------------------------------------------------------------
 * CollectiveAccess
...
<?php
    while (($vn_item_count < $vn_items_per_page) && ($vo_result->nextHit())) {
        ...
        if (!$vs_idno = $vo_result->get('ca_entities.idno')) {
            $vs_idno = "???";
        }
        ...
        ?>
			<input type='checkbox' name='add_to_set_ids' value='<?php print (int)$vn_entity_id; ?>' class="addItemToSetControl addItemToSetControlInThumbnails" />
<?php
                $va_tmp = $vo_result->getMediaTags('ca_object_representations.media', 'small');
        print caEditorLink($this->request, array_shift($va_tmp), '', 'ca_entities', $vn_entity_id, array(), array('onmouseover' => 'jQuery(".qlButtonContainerFull").css("display", "none"); jQuery("#ql_'.$vn_entity_id.'").css("display", "block");', 'onmouseout' => 'jQuery(".qlButtonContainerFull").css("display", "none");'));
        print "<div class='qlButtonContainerFull' id='ql_".$vn_entity_id."' onmouseover='jQuery(\"#ql_".$vn_entity_id."\").css(\"display\", \"block\");'><a class='qlButton' onclick='caMediaPanel.showPanel(\"".caNavUrl($this->request, 'find', 'SearchEntities', 'QuickLook', array('entity_id' => $vn_entity_id))."\"); return false;' >"._t("Quick Look")."</a></div>";

        print "</div>";
        print "<div class='objectFullText'>";

        $va_labels = $vo_result->getDisplayLabels($this->request);
        print "<div class='objectFullTextTitle'>".caEditorLink($this->request, implode("<br/>", $va_labels), '', 'ca_entities', $vn_entity_id, array())."</div>";

        // Output configured fields in display
        foreach ($va_display_list as $placement_id => $info) {
            if (in_array($info['bundle_name'], array('ca_entities.preferred_labels', 'ca_entity_labels.name'))) {
                continue;
