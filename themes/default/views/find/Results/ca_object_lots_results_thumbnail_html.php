<?php
/* ----------------------------------------------------------------------
 * themes/default/views/find/Results/ca_object_lots_results_thumbnail_html.php :
...
if (!($vs_caption_template = $this->request->config->get('ca_object_lots_results_thumbnail_caption_template'))) {
    $vs_caption_template = "^ca_object_lots.preferred_labels.name%truncate=27&ellipsis=1<br/>^ca_object_lots.idno";
}
    ...
    ?>
			<td align="center" valign="top" style="padding:2px 2px 2px 2px;">
				<div class="objectThumbnailsImageContainer" style="padding: <?php print $vn_padding_top_bottom; ?>px 0px <?php print $vn_padding_top_bottom; ?>px 0px;"> 
					<input type="checkbox" name="add_to_set_ids" value="<?php print (int)$vn_lot_id; ?>" class="addItemToSetControl addItemToSetControlInThumbnails"/>		
					<?php print caEditorLink($this->request, array_shift($va_tmp), 'qlButtonEditorLink', 'ca_object_lots', $vn_lot_id, array(), array('data-id' => $vn_lot_id)); ?>
					<?php if ($vb_has_image) { ?><div class="qlButtonContainerThumbnail" id="ql_<?php print $vn_lot_id; ?>"><a class='qlButton' data-id="<?php print $vn_lot_id; ?>"><?php print _t("Quick Look"); ?></a></div><?php } ?>
				</div>
				<div class="thumbCaption"><?php print $vs_caption; ?><br/><?php print caEditorLink($this->request, $vs_idno, '', 'ca_object_lots', $vn_lot_id); ?></div>
			</td>
<?php
                $vn_col++;
...
<script type="text/javascript">
	jQuery(document).ready(function() { 
		...
		jQuery(".qlButton").on("click", function(e) {
			...
			caMediaPanel.showPanel("<?php print caNavUrl($this->request, 'find', 'SearchObjectLots', 'QuickLook'); ?>/lot_id/" + id);
		});
	});
</script>