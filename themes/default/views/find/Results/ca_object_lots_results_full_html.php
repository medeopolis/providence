...
<?php
        ...
        print caEditorLink($this->request, array_shift($va_tmp), '', 'ca_object_lots', $vn_lot_id, array(), array('onmouseover' => 'jQuery(".qlButtonContainerFull").css("display", "none"); jQuery("#ql_'.$vn_lot_id.'").css("display", "block");', 'onmouseout' => 'jQuery(".qlButtonContainerFull").css("display", "none");'));
        print "<div class='qlButtonContainerFull' id='ql_".$vn_lot_id."' onmouseover='jQuery(\"#ql_".$vn_lot_id."\").css(\"display\", \"block\");'><a class='qlButton' onclick='caMediaPanel.showPanel(\"".caNavUrl($this->request, 'find', 'SearchObjectLots', 'QuickLook', array('lot_id' => $vn_lot_id))."\"); return false;' >"._t("Quick Look")."</a></div>";

        print "</div>";
        print "<div class='objectFullText'>";

        $va_labels = $vo_result->getDisplayLabels($this->request);
        print "<div class='objectFullTextTitle'>".caEditorLink($this->request, implode("<br/>", $va_labels), '', 'ca_object_lots', $vn_lot_id, array())."</div>";

        // Output configured fields in display
        foreach ($va_display_list as $placement_id => $info) {
            if (in_array($info['bundle_name'], array('ca_object_lots.preferred_labels', 'ca_object_lot_labels.name'))) {
                continue;
