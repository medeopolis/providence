<?php
/* ----------------------------------------------------------------------
 * themes/default/views/find/Search/widget_ca_object_representations_search_tools.php :
 * ----------------------------------------------------------------------
 * CollectiveAccess
...
<h3 class="tools"><?php print _t("History"); ?>:
	<div>
<?php
            print caFormTag($this->request, 'Index', 'caSearchHistoryForm', 'find/SearchObjectRepresentations', 'post', 'multipart/form-data', '_top', array('noCSRFToken' => true, 'disableUnsavedChangesWarning' => true));
...
if (sizeof($this->getVar("available_sets")) > 0) {
	...
	<div>
<?php
            print caFormTag($this->request, 'Index', 'caSearchSetsForm', 'find/SearchObjectRepresentations', 'post', 'multipart/form-data', '_top', array('noCSRFToken' => true, 'disableUnsavedChangesWarning' => true));
    ...
    print "</select>\n ";
    print caFormSubmitLink($this->request, _t('Search').' &rsaquo;', 'button', 'caSearchSetsForm', null, ['aria-label' => _t('Search by set')]);
    caFormSubmitLink($this->request, caNavIcon(__CA_NAV_ICON_GO__, '18px'), 'button', 'caSearchSetsForm', null, ['aria-label' => _t('Search by set')]);
    print "</form>\n";
    ?>
	</div>
	</h3>
