<?php
/** ---------------------------------------------------------------------
 * themes/default/views/mediaViewers/DivaManifest.php :
 * ----------------------------------------------------------------------
 * CollectiveAccess
...
foreach ($va_resources as $va_resource) {
    ...
    if (isset($va_resource['noPages'])) {
        ...
        // (This is used to support the "use_diva_for_image_list_length_at_least" option in media_display.conf
        //  which forces a list of image representations to be displayed as a multipage document in Diva)
        ...
        $vn_width = $va_resource['width'];
        $vn_height = $va_resource['height'];
        ...
        $vn_width = $va_resource[$vs_display_version.'_width'];
        $vn_height = $va_resource[$vs_display_version.'_height'];
    } else {
        $vs_service_url = $va_resource['url'];
        $vs_thumb_url = "{$vs_base_url}/service.php/IIIF/{$vs_identifer}:{$vn_page}/full/!512,512/0/default.jpg";
        $vn_width = $va_data['width'];
        $vn_height = $va_data['height'];
    }
