...
<script type="text/javascript">
	var caQuickAddFormHandler = caUI.initQuickAddFormHandler({
		...
		formUrl: '<?= caNavUrl($this->request, 'editor/objects', 'ObjectQuickAdd', 'Form'); ?>',
		fileUploadUrl: '<?= caNavUrl($this->request, "editor/objects", "ObjectEditor", "UploadFiles"); ?>',
		saveUrl: '<?= caNavUrl($this->request, "editor/objects", "ObjectQuickAdd", "Save"); ?>',
...
<?php
    if ($this->getVar('source') == 'ca_object_lots') {
        ?>
		<input type='hidden' name='lot_id' value='<?= (int)$this->getVar('source_id'); ?>'/>
<?php
    }
?>
